...
// manual_override wins over schedule
{
  const flag: PlatformFeatureFlagRecord = {
    ...
    schedules: [{ id: 's1', startsAt: scheduleStart, endsAt: scheduleEnd, value: true }],
  };
  ...
  assert(
    result.enabled === false && result.source === 'manual_override',
    'manual_override should win',
  );
}
...
// schedule when no override
{
  const flag: PlatformFeatureFlagRecord = {
    ...baseFlag,
    manualOverride: null,
    schedules: [{ id: 's1', startsAt: scheduleStart, endsAt: scheduleEnd, value: false }],
  };
  const active = evaluateFeatureFlag(
    flag,
    { userRole: 'user', isAuthenticated: true },
    duringSchedule,
  );
  const inactive = evaluateFeatureFlag(
    flag,
    { userRole: 'user', isAuthenticated: true },
    outsideSchedule,
  );
  ...
  assert(
    inactive.enabled === true && inactive.source === 'default',
    'outside schedule should use default',
  );
}
...
// admin_all exempt from audience block
{
  ...
  assert(
    result.enabled === false && result.source === 'default',
    'admin_all uses value not audience block',
  );
...
// registered user blocked
{
  ...
  assert(
    result.enabled === false && result.source === 'audience_blocked',
    'registered block should apply',
  );
}
