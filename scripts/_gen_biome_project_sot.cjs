...
const INPUT = process.argv[2] || path.join(process.env.TEMP || '/tmp', 'biome-full-project.json');
...
const DOC_GROUPS = [
  ...
  },
  {
    ...
    rules: ['lint/a11y/useKeyWithClickEvents', 'lint/a11y/noStaticElementInteractions'],
  },
  {
...
function renderFileTable(filesMap) {
  const rows = [...filesMap.entries()].sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]));
  const lines = [
    '| File | Occorrenze |',
    '|---|---:|',
...
function renderOccurrenceAnalysis(rule, entry, level) {
  const rows = [...entry.files.entries()].sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]));
  const lines = [];
  lines.push(
  ...
  if (entry.count <= 80) {
    ...
    for (const [f, c] of rows) {
      lines.push(`| \`${mdEscape(f)}\` | ${c} | da correggere (${c}× Livello ${level}) |`);
    }
    lines.push('');
    lines.push(
...
`);
  ...
  const title =
    group.rules.length === 1 ? RULE_META[group.rules[0]]?.title || group.rules[0] : group.id;
...
function buildRoadmap(levelStats) {
  return [
    ...
    },
    {
      ...
      level: 'B',
      cats: ['noUnusedVariables', 'noUnusedFunctionParameters', 'useOptionalChain'],
      diags:
...
`;
  ...
  const previous = fs.existsSync(auditPath) ? fs.readFileSync(auditPath, 'utf8') : '';
...
`;
  ...
  const snap = {
    ...
    levels,
    levelCats: Object.fromEntries(Object.entries(levelCats).map(([k, v]) => [k, [...v].sort()])),
    levelFiles: Object.fromEntries(Object.entries(levelFiles).map(([k, v]) => [k, v.size])),
