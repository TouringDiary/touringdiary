...
async function seedGeo() {
  ...
  for (const c of geo.continents) {
    ...
    const { data: cont, error: errC } = await supabase
      .from('continents')
      .insert({ name, slug })
      .select('id')
      .single();
  ...
  for (const n of geo.nations) {
    ...
    const continentName =
      typeof geo.continents[0] === 'string'
        ? geo.continents[0]
        : geo.continents[0].label || geo.continents[0].value; // Default
    const slug = toSlug(name);

    const { error: errN } = await supabase
      .from('nations')
      .insert({ name, slug, continent_id: getContId(continentName) })
      .select('id')
      .single();
    if (errN) {
  ...
  for (const r of regionsList) {
    ...
    const nationName =
      typeof geo.nations[0] === 'string'
        ? geo.nations[0]
        : geo.nations[0].label || geo.nations[0].value; // Default
    const slug = toSlug(name);

    const { error: errR } = await supabase
      .from('regions')
      .insert({ name, slug, nation_id: getNationId(nationName) })
      .select('id')
      .single();
    if (errR) {
