...
router.get('/:cityId/details', async (req, res) => {
  ...
  try {
    ...
    const [cityRes, poisRes, eventsRes, servicesRes, guidesRes, tourOperatorsRes, peopleRes] =
      await Promise.all([
    ...
    console.log(
      `[API-City] Caricati: ${poisRes.data?.length} POI, ${eventsRes.data?.length} eventi, ${tourOperatorsRes.data?.length ?? 0} tour operator`,
    );
