  ...
  router.post('/login', async (req, res) => {
    try {
      ...
      if (!supabaseAdmin) {
        ...
        return res
          .status(500)
          .json({ success: false, error: 'Missing SUPABASE_SERVICE_ROLE_KEY.' });
      }
      ...
      if (!userInPage1) {
        ...
        console.log(
          `[devLogin] Stato utente per ${email}: ${createError?.code === 'email_exists' ? 'Esistente' : `Creato (${createdData.user?.id})`}`,
        );
      }
