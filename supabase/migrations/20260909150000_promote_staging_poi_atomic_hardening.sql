...
-- Hardening: promote_staging_poi_to_live (auth pattern, payload validation)

-- Replaces function in 20260909143000; same atomic upsert + staging imported txn.

-- Schema-aligned: pois.city_id text, status default 'published', date_added date.
...
BEGIN
  ...
  IF p_poi ? 'date_added' AND NULLIF(trim(p_poi->>'date_added'), '') IS NOT NULL THEN
    ...
    BEGIN
      ...
      v_date_added := (p_poi->>'date_added')::timestamptz::date;
  ...
  v_cols := 'id, city_id, name, category, sub_category, description, coords_lat, coords_lng, address, visit_duration, tourism_interest, ai_reliability, image_url, is_sponsored, tier';

  v_vals := format(

    '%L, %L, %L, %L, %L, %L, %s, %s, %L, %L, %L, %L, %L, false, NULL',
    ...
    NULLIF(p_poi->>'image_url', '')

  );

  -- city_id escluso dall'UPDATE: non può essere modificato in caso di conflict

  v_updates := 'name = EXCLUDED.name, category = EXCLUDED.category, sub_category = EXCLUDED.sub_category, description = EXCLUDED.description, coords_lat = EXCLUDED.coords_lat, coords_lng = EXCLUDED.coords_lng, address = EXCLUDED.address, visit_duration = EXCLUDED.visit_duration, tourism_interest = EXCLUDED.tourism_interest, ai_reliability = EXCLUDED.ai_reliability, image_url = EXCLUDED.image_url';
  ...
  IF p_poi ? 'showcase_expiry' THEN
    ...
    ELSE
      ...
      v_vals := v_vals || format(', %L', p_poi->>'showcase_expiry');
  ...
  IF p_poi ? 'created_by' THEN

    v_cols := v_cols || ', created_by';

    v_vals := v_vals || format(', %L', NULLIF(p_poi->>'created_by', ''));

    v_updates := v_updates || ', created_by = EXCLUDED.created_by';

  END IF;



  IF p_poi ? 'updated_by' THEN

    v_cols := v_cols || ', updated_by';

    v_vals := v_vals || format(', %L', NULLIF(p_poi->>'updated_by', ''));

    v_updates := v_updates || ', updated_by = EXCLUDED.updated_by';
