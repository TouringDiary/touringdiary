-- MF2 §42.15 — RPC dual-write applicativo: media_assets + entity_image_assignments
-- Materializza il nuovo modello con versionamento primary (replace chain) e gallery per sorgente.
...
BEGIN
  ...
  -- -------------------------------------------------------------------------
  -- Autorizzazione + validazione entity/city (fail-closed)
  -- -------------------------------------------------------------------------
  IF p_entity_type = 'photo_submission' THEN
    SELECT
      ...
      CASE
        ...
        ELSE NULL
      END
    INTO v_canonical_url, v_published_at
  ...
  -- -------------------------------------------------------------------------
  -- Race protection: un lock transazionale per entity/city/role
  -- -------------------------------------------------------------------------
  ...
  v_origin_type := COALESCE(NULLIF(trim(p_origin_type), ''), 'admin');
  IF v_origin_type NOT IN ('admin', 'ai', 'wikimedia', 'community', 'placeholder') THEN
    RAISE EXCEPTION 'origin_type non valido: %', v_origin_type;
  END IF;

  -- ensure_media_asset_from_source: SECURITY DEFINER owner (postgres) — invocazione interna OK.
  ...
  -- -------------------------------------------------------------------------
  -- Lifecycle stessa sorgente (PRIMARY + GALLERY): riuso ACTIVE; block SUSPENDED/REMOVED
  -- -------------------------------------------------------------------------
  SELECT eia.id
  ...
  -- -------------------------------------------------------------------------
  -- GALLERY: più current ammessi; nuova riga solo per sorgente non già associata
  -- -------------------------------------------------------------------------
  ...
  -- -------------------------------------------------------------------------
  -- PRIMARY: una sola current — replace chain quando la sorgente cambia
  -- -------------------------------------------------------------------------
  SELECT *
  INTO v_current
  FROM public.entity_image_assignments eia
  ...
  IF FOUND THEN
    -- Sostituzione semantica: solo sorgente diversa (stessa sorgente gestita dal lifecycle sopra).
    -- Ordine obbligatorio: prima is_current=false sul precedente (indice unique primary current).
...
REVOKE ALL ON FUNCTION public.upsert_entity_image_assignment_dual_write(text, text, text, text, text, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.upsert_entity_image_assignment_dual_write(text, text, text, text, text, text, text, text) TO authenticated;

COMMENT ON FUNCTION public.upsert_entity_image_assignment_dual_write IS
  'MF2 §42.15 — dual-write applicativo: media_asset + assignment con replace chain primary, lifecycle stessa sorgente (riuso ACTIVE, block SUSPENDED/REMOVED), validazione entity/city.';
