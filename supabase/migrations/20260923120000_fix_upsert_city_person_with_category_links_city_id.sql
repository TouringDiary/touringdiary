...
BEGIN
  ...
  IF p_person ? 'id' AND NULLIF(trim(p_person->>'id'), '') IS NOT NULL THEN
    ...
    IF FOUND THEN
      ...
      IF v_existing.city_id IS DISTINCT FROM v_city_id THEN
        RAISE EXCEPTION
          'city_id del payload (%) non corrisponde alla città della persona (%); spostamento tra città non consentito.',
          v_city_id,
          v_existing.city_id;
      END IF;
    END IF;
  ...
  IF v_is_update THEN
    -- city_id immutabile in UPDATE via questa RPC; coerenza con cities.id garantita da city_people_city_id_fkey.
