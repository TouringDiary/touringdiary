...
import { fetchAdminUsersPaged, searchAdminUsersByEmail } from './ai/legacyExtraQuotaCompat';
...
function parseGrantAdminWalletCreditsResult(
  raw: Json | null | undefined,
): GrantAdminWalletCreditsResult {
  if (raw == null) {
    throw new Error('grant_admin_ai_credits: empty response payload');
  }
  ...
  // Contratto RPC: `success` è obbligatorio. Senza di esso il payload non è affidabile.
  if (typeof obj.success !== 'boolean') {
    throw new Error('grant_admin_ai_credits: missing or invalid success field');
  }

  const result: GrantAdminWalletCreditsResult = { success: obj.success };
...
export const getAiAdminData = async (): Promise<AiAdminData> => {
  const { data: modelPrices } = await supabase.from('ai_model_prices').select('*').order('model');
 ...
 *
 * ARCHITETTURA — read-modify-write non atomico:
 * 1) SELECT ai_limits 2) merge in memoria 3) UPDATE ai_limits.
 * Tra (1) e (3) un altro writer può sovrascrivere `ai_limits` → lost update.
 * Non esiste oggi una RPC di progetto per patch atomico di `ai_limits`;
 * quando verrà introdotta andrà usata al posto di questo pattern.
 */
export const updatePlanAiLimitField = async (
  versionId: string,
  field: string,
  value: PlanAiLimitFieldValue,
) => {
...
export const getEconomicsDashboardData = async (): Promise<AiEconomicsDashboardData> => {
  ...
  // TODO(architettura): caricamento full-table di `ai_global_usage` non è scalabile.
  // Sostituire con aggregazioni SQL/RPC (group by date/model/segment) quando il volume cresce.
  const { data: rawLogs, error: logsError } = await supabase.from('ai_global_usage').select(`
            request_count,
            model_type,
            date,
            user_id,
  ...
  const activeSubUserIds = new Set(
    (await supabase.from('subscriptions').select('user_id').eq('status', 'active')).data?.map(
      (s) => s.user_id,
    ) || [],
  );
  ...
  (rawLogs || []).forEach((rawLog) => {
    ...
    const cost = (log.request_count || 0) * (log.model_type ? (costMap[log.model_type] ?? 0) : 0);
    const date = log.date;
...
/**
 * Attiva una versione di pricing (disattiva le altre dello stesso piano).
 *
 * ARCHITETTURA — non atomica:
 * L'operazione è due UPDATE distinti (is_active=false su tutte le attive del piano,
 * poi is_active=true sulla target). Tra le due query può esistere una finestra senza
 * versione attiva, oppure un concurrent writer può interferire.
 * Non esiste oggi una RPC transazionale di progetto per l'attivazione; migrare lì
 * quando disponibile. Il commento storico "atomico" non riflette il comportamento reale.
 */
 ...
 */
export const updatePricingVersion = async (
  versionId: string,
  updates: AiAdminPricingVersionUpdate,
) => {
  const { data, error } = await supabase
    .from('pricing_versions')
    .update(updates)
