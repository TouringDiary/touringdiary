...
import type { PermissionCode, User, UserRole, UserStatus } from '../types/users';
import { setAuthOperationInProgress, supabase } from './supabaseClient';
...
import type { DbProfile } from '../types/domain/index';
import {
...
export const refreshUsersCache = async (): Promise<User[]> => {
  try {
    ...
    if (!data) {
      const { data: supaData, error } = await supabase.from('profiles').select('*');
...
export const devLogin = async (
  ...
  try {
    ...
    if (apiData && typeof apiData === 'object' && 'success' in apiData) {
      ...
      if (typedData.success && typedData.access_token && typedData.refresh_token) {
...
export const registerUser = async (
    ...
    try {
      ...
      if (data && typeof data === 'object' && 'success' in data) {
        ...
        return {
          success: false,
          user: null,
          error: typedData.error || 'Unknown admin registration error',
        };
      }
  ...
  try {
    ...
    if (availability.status === 'error') {
      ...
      return {
        user: null,
        error: 'Impossibile verificare la disponibilità del Nome utente. Riprova.',
      };
    }
...
export const getRolePermissions = (): Record<UserRole, PermissionCode[]> => ({
  ...ROLE_PERMISSIONS,
});
