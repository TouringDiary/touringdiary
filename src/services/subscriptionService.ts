...
export const getActiveUserSubscription = async (
  ...
  try {
    ...
    if (error) {
      console.error(
        '[SubscriptionService] getActiveUserSubscription Supabase Error:',
        error.message,
      );
      return null;
    }
...
export const getActiveSponsorSubscription = async (
  sponsorId: string,
): Promise<SubscriptionData | null> => {
...
export const getUserAiLimits = async (userId: string): Promise<ModelAwareLimits | null> => {
  ...
  if (
    activeSub &&
    activeSub.pricing_version &&
    activeSub.pricing_version.ai_limits &&
    Object.keys(activeSub.pricing_version.ai_limits).length > 0
  ) {
