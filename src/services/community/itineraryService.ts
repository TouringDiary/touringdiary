...
import type { Itinerary, ItineraryItem, PremadeItinerary } from '../../types/index';
...
// MP-03 STEP-2: collaboration/viaggio caricati on-demand (non nel parse sync bootstrap Home).

/**
 * Confine di serializzazione verso il DB (unico punto di passaggio dominio → colonna `Json`).
 *
 * Perché `JSON.parse(JSON.stringify(...))` e non `structuredClone` / mapper ad hoc:
 * - Il tipo generato `Json` di Supabase è una union ricorsiva di primitivi/array/object
 *   “puri”; le entità di dominio (classi strutturali, optional nested, Date, ecc.) non
 *   sono assegnabili a `Json` senza una normalizzazione a plain JSON.
 * - `structuredClone` preserva la forma runtime ma non produce il brand tipizzato `Json`
 *   e non elimina valori non-JSON (es. `undefined` in nested object) allo stesso modo
 *   del wire-format che Supabase persiste comunque via JSON.
 * - Un walker ricorsivo tipizzato sarebbe più verboso a parità di payload e di comportamento.
 *
 * Quindi questo helper resta il compromesso type-safe minimo: stesso payload serializzato
 * che andrebbe sul wire, con ritorno tipizzato `Json`, senza cast `as Json` ciechi.
 * Non è un workaround temporaneo: è il boundary di serializzazione intenzionale.
 */
...
const ITINERARY_DIFFICULTIES: readonly PremadeItinerary['difficulty'][] = [
  'Relax',
  'Moderato',
  'Intenso',
];
...
function isOneOf<T extends string>(value: string | null, allowed: readonly T[]): value is T {
  return value !== null && allowed.some((entry) => entry === value);
}

function toDifficulty(value: string | null): PremadeItinerary['difficulty'] {
  return isOneOf(value, ITINERARY_DIFFICULTIES) ? value : 'Moderato';
}

function toItineraryType(value: string | null): PremadeItinerary['type'] {
  return isOneOf(value, ITINERARY_TYPES) ? value : 'community';
}

function toStatus(value: string | null): PremadeItinerary['status'] {
  return isOneOf(value, ITINERARY_STATUSES) ? value : 'draft';
}
...
function buildPackedDiaryData(itinerary: Itinerary): PackedDiaryFields {
  return {
    ...
    diaryNotes:
      itinerary.diaryNotes != null ? normalizeDiaryNotesState(itinerary.diaryNotes) : null,
...
async function resolveSaveUserDraftViaggio(params: {
  ...
  viaggioCreated: boolean;
  /** Modulo già caricato (new/ensure); assente su none/existing. */
  viaggioService?: typeof import('../viaggio/viaggioService');
}> {
  ...
  if (viaggioOptions?.viaggioChoice === 'new') {
    const viaggioService = await import('../viaggio/viaggioService');
    const createdViaggio = await viaggioService.createViaggio({
      userId: realUserId,
      title: viaggioTitle,
      periodStart: itinerary.startDate ?? null,
      periodEnd: itinerary.endDate ?? null,
    });
    return { viaggioId: createdViaggio.id, viaggioCreated: true, viaggioService };
  }
  ...
  const viaggioService = await import('../viaggio/viaggioService');
  const ensured = await viaggioService.ensureViaggioForPersonalDiary({
    userId: realUserId,
  ...
  return {
    viaggioId: ensured.viaggioId,
    viaggioCreated: ensured.created,
    viaggioService,
  };
}
...
export const saveUserDraft = async (
  ...
  try {
    ...
    if (!itinerary.id) {
      throw new Error('ID Diario mancante per il salvataggio cloud.');
    }
    ...
    const isCollaborativeMemberSave = !!existingRow?.user_id && existingRow.user_id !== realUserId;

    if (isCollaborativeMemberSave) {
      const { canUserModifyResource } = await import('../collaboration/permissionService');
    ...
    const {
      viaggioId,
      viaggioCreated,
      viaggioService: resolvedViaggioService,
    } = await resolveSaveUserDraftViaggio({
      realUserId,
      itinerary,
    ...
    if (viaggioId) {
      // Riusa il modulo già caricato in resolve (new/ensure); import solo su existing.
      const viaggioService = resolvedViaggioService ?? (await import('../viaggio/viaggioService'));
      const { setActiveDiary, updateViaggio } = viaggioService;
      if (viaggioCreated) {
      ...
      try {
        ...
        console.warn(
          '[Cloud] Aggiornamento metadati Viaggio non riuscito (diario salvato):',
          metaErr,
        );
      }
    }
...
export const getAccessibleDiariesForUser = async (userId: string): Promise<Itinerary[]> => {
  const { fetchCollaborativeDiaryIdsForMember } = await import(
    '../collaboration/diaryCollaborationService'
  );
...
export const deleteUserDraft = async (
  ...
  try {
    ...
    console.log(
      `[Delete] Cancellazione itinerario ${itineraryId}. Target User: ${userId}. Auth User: ${sessionUserId}`,
    );

    if (sessionUserId && userId !== sessionUserId) {
      console.warn(
        `[Delete] ATTENZIONE: Mismatch tra utente richiesto (${userId}) e utente autenticato (${sessionUserId}). RLS potrebbe bloccare.`,
      );
    }
    ...
    if (count === 0) {
      console.warn(
        `[Delete] Nessuna riga cancellata. ID errato o RLS bloccante. (ItineraryID: ${itineraryId})`,
      );
      return false;
    }
...
export const getCommunityItinerariesAsync = async (): Promise<PremadeItinerary[]> => {
  try {
    const { data, error } = await supabase.from('itineraries').select('*').neq('type', 'personal');
...
export const getCommunityItinerariesMetadataAsync = async (): Promise<
  Partial<PremadeItinerary>[]
> => {
  try {
    const { data, error } = await supabase
      .from('itineraries')
...
export const getFilteredCommunityItinerariesAsync = async (
  filters: ItineraryFilters,
): Promise<PremadeItinerary[]> => {
  try {
...
export const savePremadeItinerary = async (itinerary: PremadeItinerary): Promise<boolean> => {
  try {
    const payload = {
      ...
      author_name: itinerary.author || 'Redazione',
      rating: itinerary.rating,
      votes: itinerary.votes,
      continent: itinerary.continent,
      nation: itinerary.nation,
      region: itinerary.region,
      zone: itinerary.zone,
      main_city: itinerary.mainCity,
...
export const toggleItineraryLike = async (
  itineraryId: string,
  userId: string,
  ...
  try {
    ...
    if (existing) {
      await supabase.from('user_interactions').delete().match({
        user_id: userId,
        target_id: itineraryId,
        target_type: 'itinerary',
        interaction_type: 'like',
    ...
    if (error) {
      ...
      if (msg.includes('ALREADY_PUBLISHED') || error.code === '23505') {
        return {
          ...
          error: 'Questo diario è già stato pubblicato.',
        };
      }
