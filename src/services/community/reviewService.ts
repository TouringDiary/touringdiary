...
import { clearCacheKey, invalidateCityCache } from '../city/cityCache';
...
function isValidCriterionStar(value: unknown): value is number {
  return (
    typeof value === 'number' && Number.isInteger(value) && value >= MIN_STARS && value <= MAX_STARS
  );
}
...
function isValidAverageRating(value: unknown): value is number {
  return (
    typeof value === 'number' && Number.isFinite(value) && value >= MIN_STARS && value <= MAX_STARS
  );
}

function collectValidCriteria(source: {
...
/** Esito reale di `saveUnifiedReview`: quale ramo di persistenza è stato eseguito. */
export type SaveUnifiedReviewResult =
  | { operation: 'insert'; review: Review }
  | { operation: 'update'; review: Review };
...
function resolveContentRating(review: SaveUnifiedReviewInput): {
  rating: number;
  criteria: ReviewCriteriaMap | null;
} {
  const criteria = sanitizeCriteriaMap(review.criteria ?? null);
  const rating = criteria ? computeReviewAverageRating(criteria) : review.rating;
 ...
 */
export const saveUnifiedReview = async (
  ...
  if (existingId) {
    // Content-only UPDATE: XP is awarded on INSERT, not on later edits.
    ...
    return { operation: 'update', review: mapDbReview(data as ReviewRow) };
  }
  ...
  await invalidateCachesForPoi(review.poiId);
  return { operation: 'insert', review: mapDbReview(data as ReviewRow) };
};
