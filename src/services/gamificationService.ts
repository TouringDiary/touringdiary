...
import type {
  DatabaseRewardCatalog,
  DatabaseUserReward,
  DatabaseXpAction,
} from '../types/database';
import type { LevelInfo, Reward, RewardCategory, UserReward, XpRule } from '../types/index';
import type { Database } from '../types/supabase';
import { randomUUID } from '../utils/runtimeId';
import { getStorageItem, setStorageItem } from './storageService';
import { supabase } from './supabaseClient';

export type { LevelInfo, Reward, RewardCategory, UserReward, XpRule };
 ...
 */
function tryParseBootstrapLevelItem(
  item:
    | {
        level?: number;
        name?: string;
...
export const claimReward = (userId: string, reward: Reward): ClaimRewardResult => {
  if (!areRewardsEnabled()) {
    console.info(
      '[GamificationService] claimReward blocked — rewards frozen (feature.gamification.rewards OFF)',
    );
