 ...
 */
export const getSponsorMessagesAsync = async (identity: {
  ...
  let query = supabase.from('sponsor_messages').select(`
            *,
 ...
 */
export const markMessagesAsReadAsync = async (
  identity: { profileId?: string | null; requestId?: string | null },
  direction: 'admin' | 'partner',
) => {
  const { profileId, requestId } = identity;
  if (!profileId && !requestId) return;
