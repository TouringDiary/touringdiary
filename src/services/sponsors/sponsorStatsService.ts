...
export const getSponsorTiers = async () => {
  ...
  type PricingVersionJoin = {
    plans?: { type?: string } | Array<{ type?: string }> | null;
  } | null;
  ...
  const types = data
    .map((d) => {
      const pv = (
        Array.isArray(d.pricing_versions) ? d.pricing_versions[0] : d.pricing_versions
      ) as PricingVersionJoin;
      const plan = (pv && Array.isArray(pv.plans) ? pv.plans[0] : pv?.plans) as {
        type?: string;
      } | null;
      return plan?.type;
    })
