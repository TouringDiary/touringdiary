...
export const getAllSponsorsForDashboard = async (): Promise<SponsorRequest[]> => {
  ...
  return (data || []).map((r) =>
    mapDbSponsorRequestToApp(r as import('../../types/database').DatabaseJoinedSponsorRequest),
  );
};
...
export const getSponsorsDashboardAsync = async (): Promise<SponsorRequest[]> => {
  try {
    ...
    if (reqResult.error)
      console.error('[SponsorDashboard] Requests fetch error:', reqResult.error.message);
    if (sponResult.error)
      console.error('[SponsorDashboard] Sponsors fetch error:', sponResult.error.message);
