...
import type {
  DatabaseJoinedSponsor,
  DatabaseJoinedSponsorRequest,
  Json,
} from '../../types/database';
import type { PartnerLog, SponsorQueryOptions, SponsorRequest } from '../../types/models/Sponsor';
import { supabase } from '../supabaseClient';
import type { SponsorSubmitFormData } from './_internalTypes';
import {
  mapDbSponsorRequestToApp,
  mapDbSponsorToRequestApp,
  resolvePlanPoiCategory,
  SPONSOR_CONTRACT_SELECT,
  SPONSOR_REQUEST_SELECT,
} from './sponsorResolvers';
...
export const getSponsorById = async (id: string): Promise<SponsorRequest | null> => {
  const { data, error } = await supabase.from('sponsor_requests').select('*').eq('id', id).single();
      ...
      ...(activeType === PLAN_TYPES.TOUR_GUIDE && {
        ...
        specialties: Array.isArray(formData.specialties)
          ? formData.specialties.filter(
              (spec): spec is string => typeof spec === 'string' && spec.trim().length > 0,
            )
          : [],
      }),
...
export const getPartnerHistoryAsync = async (identity: {
  ...
  if (reqResult.error)
    console.error('[SponsorService] Error fetching requests history:', reqResult.error.message);
  if (sponResult.error)
    console.error('[SponsorService] Error fetching sponsors history:', sponResult.error.message);

  const historyReq = (reqResult.data || []).map((r) =>
    mapDbSponsorRequestToApp(r as DatabaseJoinedSponsorRequest),
  );
  const historySpon = (sponResult.data || []).map((r) =>
    mapDbSponsorToRequestApp(r as DatabaseJoinedSponsor),
  );
 ...
 */
export const updateSponsorInternalNotes = async (
  id: string,
  notes: string,
