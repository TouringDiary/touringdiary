...
import type { DatabaseJoinedSponsor, DatabaseJoinedSponsorRequest } from '../../types/database';
...
export const SPONSOR_PUBLIC_VITRINE_SELECT = `${SPONSOR_PUBLIC_VITRINE_COLUMNS},${SPONSOR_RESOURCE_JOIN_SELECT},${SPONSOR_PRICING_VERSION_JOIN}`;

/** Lista legacy attiva: colonne vetrina + tier da pricing_versions (senza join risorse). */
export const SPONSOR_PUBLIC_LEGACY_LIST_SELECT = `${SPONSOR_PUBLIC_VITRINE_COLUMNS},${SPONSOR_PRICING_VERSION_JOIN}`;

export const SPONSOR_CONTRACT_SELECT = `*,${SPONSOR_RESOURCE_JOIN_SELECT},${SPONSOR_PRICING_VERSION_JOIN},admin_notes,admin_notes_last_updated,request_id`;
...
const normalizeSponsorStatus = (status: NullableString): SponsorLifecycleStatus => {
  ...
  return normalized && valid.includes(normalized)
    ? (normalized as SponsorLifecycleStatus)
    : 'pending';
};
...
const normalizePoiCategory = (cat: NullableString): PointOfInterest['category'] => {
  ...
  return normalized && valid.includes(normalized)
    ? (normalized as PointOfInterest['category'])
    : 'discovery';
};
...
export const mapDbSponsorToApp = (dbSponsor: SponsorMapperInput): Sponsor => {
  ...
  return {
    ...
    continent:
      toOptionalString(dbSponsor.cities?.continent) ||
      toOptionalString(dbSponsor.last_city?.continent),
    country:
      toOptionalString(dbSponsor.cities?.nation) || toOptionalString(dbSponsor.last_city?.nation),
    region:
      toOptionalString(dbSponsor.cities?.admin_region) ||
      toOptionalString(dbSponsor.last_city?.admin_region),
    touristZone:
      toOptionalString(dbSponsor.cities?.zone) || toOptionalString(dbSponsor.last_city?.zone),
    city: toOptionalString(dbSponsor.cities?.name) || toOptionalString(dbSponsor.last_city?.name),

    // DUAL-KEY SLUG RESOLUTION
    slug:
      dbSponsor.shops?.slug ||
      dbSponsor.city_guides?.slug ||
      dbSponsor.city_tour_operators?.slug ||
      undefined,
  };
};
...
export const mapResolvedSponsor = (dbRow: SponsorMapperInput): ResolvedSponsor => {
  ...
  let resolvedData: ResolvedSponsor['resolvedData'];
  ...
  if (dbRow.pois) {
    ...
    resolvedData = {
      ...
      coords: {
        lat: Number(dbRow.shops.coords_lat) || 0,
        lng: Number(dbRow.shops.coords_lng) || 0,
      },
    ...
    resolvedData = {
      ...
      coords: {
        lat: Number(dbRow.city_tour_operators.coords_lat) || 0,
        lng: Number(dbRow.city_tour_operators.coords_lng) || 0,
      },
...
export const isResolvedSponsor = (
  sponsor: Sponsor | ResolvedSponsor,
): sponsor is ResolvedSponsor => {
...
export const convertSponsorToPoi = (sponsor: Sponsor | ResolvedSponsor): PointOfInterest => {
  ...
  return {
    ...
    description:
      ...
      `Visita ${sponsor.companyName}, un partner consigliato da Touring-Diary.`,
 ...
 */
export const mapDbSponsorRequestToApp = (
  dbRequest: DatabaseJoinedSponsorRequest,
): SponsorRequest => {
