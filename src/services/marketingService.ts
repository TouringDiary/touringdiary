 ...
 */
export function getModelLimits(aiLimits: Json | null | undefined): {
  flash?: number;
  pro?: number;
} {
...
export function mergeAiLimits(
  ...
  const base =
    currentLimits && typeof currentLimits === 'object' && !Array.isArray(currentLimits)
      ? { ...currentLimits }
      : {};

  const currentModels =
    typeof base.models === 'object' && base.models !== null && !Array.isArray(base.models)
      ? { ...base.models }
      : {};
 ...
 */
export const updatePricingVersionExpiration = async (
  versionId: string,
...
export const createPricingVersion = async (
  payload: Domain.DbInsert<'pricing_versions'>,
): Promise<void> => {
  const { error } = await supabase.from('pricing_versions').insert(payload);
...
export const deletePricingVersionDraft = async (id: string): Promise<void> => {
  const { error } = await supabase.from('pricing_versions').delete().eq('id', id);
 ...
 */
export const archivePreviousActivePricingVersions = async (
  planId: string,
