...
/** Limiti AI su pricing_versions (JSON DB; campi usati da UI pricing). */
export type PricingAiLimits = {
...
export interface FormattedPricingVersion {
  ...
  price: number;
  currency: string;
  ai_limits: PricingAiLimits | null;
}

type PricingPlanJoin = { name: string; type: string };

/** Riga select pricing_versions + join plans usata da getPricingVersions. */
type PricingVersionQueryRow = {
  id: string;
  duration_days: number;
  price: number;
  currency: string;
  ...
  plans: PricingPlanJoin | PricingPlanJoin[] | null;
};

const toPricingAiLimits = (raw: Json | null): PricingAiLimits | null => {
  if (raw == null || typeof raw !== 'object' || Array.isArray(raw)) {
    return raw === null ? null : {};
  }
  return raw as PricingAiLimits;
};
...
export const getPricingVersions = async (
  campaignCode?: string,
): Promise<FormattedPricingVersion[]> => {
  ...
  try {
    ...
    let promoData: PricingVersionQueryRow[] = [];
    if (campaignId) {
      ...
      if (!campaignPricingError && campaignPricingData) {
        promoData = campaignPricingData as PricingVersionQueryRow[];
      }
    }
    ...
    const processItems = (items: PricingVersionQueryRow[]) => {
      items.forEach((item) => {
        ...
        const formatted: FormattedPricingVersion = {
          ...
          ai_limits: toPricingAiLimits(item.ai_limits),
        };
    ...
    processItems((standardData || []) as PricingVersionQueryRow[]);
 ...
 */
export const safeMergeAiLimits = (
  oldLimits: PricingAiLimits | null | undefined,
  newModels: { flash: number; pro: number },
): PricingAiLimits => {
