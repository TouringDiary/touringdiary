...
export async function getCityNameById(
  ...
  try {
    ...
    let query = supabase.from('cities_registry').select('name').eq('id', cityId);
    ...
    console.error(
      `[geoRegistryService] Unexpected exception in getCityNameById for ${cityId}:`,
      err,
    );
    return null;
  }
...
export async function getUniqueCityTypes(): Promise<string[]> {
  try {
    const { data, error } = await supabase.from('city_template_map').select('city_type');
...
export async function getActiveContinents(): Promise<ActiveContinent[]> {
  try {
    const { data, error } = await supabase.from('active_continents').select('id, name, slug');
...
export async function getActiveNations(continentId: string): Promise<ActiveNation[]> {
  ...
  try {
    ...
    if (error) {
      console.error(
        `[geoRegistryService] Error in getActiveNations for continent ${continentId}:`,
        error,
      );
      return [];
    }
    ...
    console.error(
      `[geoRegistryService] Unexpected exception in getActiveNations for continent ${continentId}:`,
      err,
    );
    return [];
  }
...
export async function getActiveRegions(nationId: string): Promise<ActiveRegion[]> {
  ...
  try {
    ...
    if (error) {
      console.error(
        `[geoRegistryService] Error in getActiveRegions for nation ${nationId}:`,
        error,
      );
      return [];
    }
    ...
    console.error(
      `[geoRegistryService] Unexpected exception in getActiveRegions for nation ${nationId}:`,
      err,
    );
    return [];
  }
...
export async function getActiveTouristZones(regionId: string): Promise<ActiveTouristZone[]> {
  ...
  try {
    ...
    if (error) {
      console.error(
        `[geoRegistryService] Error in getActiveTouristZones for region ${regionId}:`,
        error,
      );
      return [];
    }
    ...
    console.error(
      `[geoRegistryService] Unexpected exception in getActiveTouristZones for region ${regionId}:`,
      err,
    );
    return [];
  }
