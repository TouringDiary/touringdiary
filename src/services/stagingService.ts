...
export const getAllStagingPois = async ({
  ...
  aiRating = [],
  rawCategories = [],
}: Omit<StagingFilter, 'page' | 'pageSize'>): Promise<DatabasePoiStaging[]> => {
  let query = supabase
    .from('pois_staging')
...
export const getAllStagingIds = async ({
  ...
  let query = supabase.from('pois_staging').select('id').eq('city_id', cityId);
...
export const orphanCityStaging = async (cityId: string, cityName: string): Promise<void> => {
  try {
    ...
    await supabase.from('pois_staging').update({ orphan_city_tag: cityName }).eq('city_id', cityId);
