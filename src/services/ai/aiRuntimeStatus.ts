...
function resolveUserCopy(
  ...
  const body = resolveSystemMessageBody(messageKey, catalog?.defaultBody?.trim() || bootstrapBody);
...
export function getAiRuntimeStatus(ctx?: FeatureFlagEvaluationContext): AiRuntimeStatus {
  ...
  const roleFlag = evaluateCachedFeatureFlag(aiFlagKeyForContext(evaluationCtx), evaluationCtx);
