...
import type { RegionalAnalysisResult } from '../../../types/index';
...
export function generateSingleField(
  cityName: string,
  fieldType: 'coords',
): Promise<CityCoordsAiResult>;
export function generateSingleField(
  cityName: string,
  fieldType: 'hierarchy',
): Promise<CityHierarchyAiResult>;
export function generateSingleField(
  cityName: string,
  ...
  return withRetry(async () => {
    ...
    const response = await aiGateway.generateLegacy({
      ...
      config: {
        tools,
        responseMimeType:
          fieldType === 'coords' || fieldType === 'hierarchy' ? 'application/json' : undefined,
      },
    });
...
> {
  return withRetry(async () => {
    ...
    try {
      ...
      const dbPrompt = await getAiPrompt(dbKey, {
        cityName,
        existingZones: existingZones.join(', '),
      });
...
export const generateCitySuggestion = async (
  userQuery: string,
  availableCities: string[] = [],
): Promise<string> => {
  return withRetry(async () => {
    const citiesListString =
    ...
    return (
      response.text?.trim() ||
      'Non ho trovato una risposta precisa, ma ti consiglio di esplorare le nostre città in evidenza!'
    );
  });
};
