...
export const normalizeSubCategory = (subCategory: string): string => {
  ...
  const normalizationMap = getCachedSetting<Record<string, string>>(
    SETTINGS_KEYS.TAXONOMY_NORMALIZATION,
  );
...
export const getCorrectCategory = (
  subCategory: string,
  ...
  if (
    ...
    nameLower.includes('decò') ||
    nameLower.includes('dok') ||
  ...
  if (
    nameLower.includes('lido ') ||
    nameLower.startsWith('lido') ||
    nameLower.includes('beach') ||
    nameLower.includes('bagni ') ||
    nameLower.includes('cala ') ||
    nameLower.includes('baia ') ||
    nameLower.includes('spiaggia') ||
    nameLower.includes('villa comunale') ||
    nameLower.includes('parco ') ||
    nameLower.includes('giardini ') ||
    nameLower.includes('riserva naturale') ||
    nameLower.includes('oasi ')
  ) {
    return 'nature';
  }
  ...
  if (
    ...
    nameLower.includes('cattedrale') ||
    nameLower.includes('duomo') ||
    nameLower.includes('museo') ||
    nameLower.includes('scavi') ||
    nameLower.includes('parco archeologico') ||
    nameLower.includes('templi') ||
    nameLower.includes('santuario')
  ) {
    return 'monument';
  }
  ...
  const taxonomyMap = getCachedSetting<Record<string, PoiCategory>>(SETTINGS_KEYS.TAXONOMY_MAP);
...
export const resolveCategoryFromDict = (
  term: string,
  dict: Record<string, { cat: string; sub: string }>,
...
export const generateAllowedCategoriesPromptString = (): string => {
  const structure = getCachedSetting<Record<string, { value: string; label: string }[]>>(
    SETTINGS_KEYS.POI_STRUCTURE,
  );
