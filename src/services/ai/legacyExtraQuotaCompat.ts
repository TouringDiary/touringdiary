...
async function fetchWalletBonusSummary(
  ...
  for (const row of (data ?? []) as WalletBonusRow[]) {
    ...
    if (row.expires_at && (!current.earliestExpiry || row.expires_at < current.earliestExpiry)) {
...
function mapProfileRow(
  row: ProfileAdminRow,
  bonus: WalletBonusSummary | undefined,
): AdminProfileQuotaRow {
...
export async function fetchAdminUsersPaged(
  ...
  let query = supabase.from('profiles').select('id, name, email, role, created_at');
