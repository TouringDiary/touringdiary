...
import type { DatabaseCommunicationLog } from '../types/database';
...
export interface PositionConfig {
  ...
  targetId?: string;
  /** Optional viewport background image URL (onboarding visual editor). */
  bgImage?: string;
}
...
export interface UiConfig {
  ...
  bubble?: { x: number; y: number };
  arrowDirection?: BubbleArrowDirection;
  /** Legacy / desktop-fallback background image URL. */
  bgImage?: string;
}
...
function readTargetBox(value: Json | undefined): PositionConfig['targetBox'] | undefined {
  if (value === null || value === undefined || typeof value !== 'object' || Array.isArray(value)) {
    return undefined;
  }
...
function readPositionConfig(value: Json | undefined): PositionConfig | undefined {
  ...
  return {
    ...
    bgImage: typeof bgImage === 'string' ? bgImage : undefined,
  };
}
...
function parseDeviceTarget(
  target: string | null,
): NonNullable<SystemMessageTemplate['deviceTarget']> {
...
function mapCommunicationLogRow(log: DatabaseCommunicationLog): AdminMessageLog | null {
  if (log.created_at == null || log.created_at === '') {
    console.warn('[CommunicationService] Skipping communication_log without created_at', log.id);
    return null;
  }
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (apiRes.success && Array.isArray(apiRes.data)) {
            ...
            const result = apiRes.data.filter(isBootstrapMessageRow).map(mapBootstrapMessageRow);
