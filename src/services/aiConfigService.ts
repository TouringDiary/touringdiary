import type { DatabaseAiConfigInsert } from '../types/database';
...
export const getAiConfig = async (
  key: string,
): Promise<{ prompts: string[]; selected: string[]; presets: AiPreset[] } | null> => {
  try {
    const { data, error } = await supabase.from('ai_configs').select('*').eq('key', key).limit(1);
...
export const saveAiConfig = async (
  ...
  try {
    ...
    const { error } = await supabase.from('ai_configs').upsert(payload);
