...
export const runAiAnalysisService = async (
  ...
  for (let i = 0; i < targets.length; i += BATCH_SIZE) {
    ...
    if (onProgress) {
      onProgress(
        `Analisi Batch ${currentBatchNum}/${totalBatches} (${result.processed}/${result.totalRequested} elementi)...`,
      );
    }
    ...
    try {
      ...
      if (finalRatings.length > 0) {
        ...
        if (recoveredRatings.length > 0) {
          console.log(
            `[ImportAutomation] Recuperati ${recoveredRatings.length} elementi saltati dall'AI.`,
          );
        }
      } else {
      ...
      const errMsg = (e instanceof Error ? e.message : String(e)) || JSON.stringify(e);
      if (
        errMsg.includes('429') ||
