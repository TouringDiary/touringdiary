...
/** Item normalizzato da Overpass → staging import (shape runtime invariata). */
interface OsmMappedStagingItem {
  osm_id: string;
  name: string;
  raw_category: string;
...
const fetchOverpassWithRetry = async (
  query: string,
  attempt: number = 1,
  ...
  try {
    ...
    if (response.status === 429 || response.status === 504 || response.status >= 500) {
      if (attempt <= MAX_RETRIES) {
        ...
        const delay = BASE_DELAY * 1.5 ** (attempt - 1);
        // Silenced log
        await wait(delay);
...
const generateGridChunks = (
  lat: number,
  lng: number,
  radiusMeters: number,
  chunkSizeMeters: number = 5000,
): BoundingBox[] => {
  const earthRadius = 6371000;
  const latOffset = (radiusMeters / earthRadius) * (180 / Math.PI);
  const lngOffset =
    ((radiusMeters / earthRadius) * (180 / Math.PI)) / Math.cos((lat * Math.PI) / 180);
  ...
  const lngStep =
    ((chunkSizeMeters / earthRadius) * (180 / Math.PI)) / Math.cos((lat * Math.PI) / 180);
...
const mapOsmCategoryToApp = (
  tags: Record<string, string> | undefined,
  requestedCategory: string,
): { cat: PoiCategory; sub: string | null } => {
  ...
  return {
    cat: requestedCategory as PoiCategory,
    sub: amenity || tourism || leisure || shop || null,
  };
};
...
export const fetchOsmData = async (
  ...
  if (useChunking) {
    ...
    try {
      ...
      throw new Error(`Errore OSM: ${e instanceof Error ? e.message : String(e)}`);
    }
  }
  ...
  const hasNamedTags = (
    el: OsmRawElement,
  ): el is OsmRawElement & { tags: Record<string, string> & { name: string } } =>
    typeof el.tags?.name === 'string' && el.tags.name.length > 0;

  const mappedItems = distinctElements
    .filter(hasNamedTags)
    .map((el): OsmMappedStagingItem | null => {
      const tags = el.tags;
      ...
      return {
        ...
        coords_lat: finalLat,
        coords_lng: finalLon,
        address: buildAddressFromTags(tags) ?? undefined,
        processing_status: 'new',
        ai_rating: 'medium',
      };
    })
    .filter((item): item is OsmMappedStagingItem => item !== null);
