...
export const addSuitcaseItemsBulkAsync = async (
  ...
  const { data, error } = await supabase.from('suitcase_items').insert(rows).select();
...
export const updateSuitcaseItemAsync = async (
  ...
  if (dto.accepted_from_ai !== undefined) {
    payload.accepted_from_ai = dto.accepted_from_ai;
  }
  ...
  const { error } = await supabase.from('suitcase_items').update(payload).eq('id', itemId);
...
export const deleteSuitcaseItemAsync = async (itemId: string): Promise<void> => {
  const { error } = await supabase.from('suitcase_items').delete().eq('id', itemId);
