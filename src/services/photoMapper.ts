import type { DbPhotoSubmission, PhotoSubmission } from '../types/index';

/**
 * Mapper autoritativo per PhotoSubmission.
 * Centralizza il boundary tra DB e Dominio Photo.
 */
export const mapDbPhotoSubmission = (p: DbPhotoSubmission): PhotoSubmission => {
  return {
    id: p.id,
    userId: p.user_id,
    user: p.user_name,
    ...
    url: p.image_url,
    status: p.status as PhotoSubmission['status'],
    date: p.created_at ?? '',
    likes: p.likes || 0,
