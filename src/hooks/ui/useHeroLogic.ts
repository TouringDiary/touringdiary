...
import {
  ...
  getActiveNations,
  getActiveRegions,
  getActiveTouristZones,
  getUniqueCityTypes,
} from '../../services/geoRegistryService';
...
export const useHeroLogic = ({
  ...
  const matchesInspiration = (cityTypes: string[], inspirationId: string) => {
    const inspiration = geoOptions.inspirationOptions.find(
      (i) => i.id === inspirationId || i.label === inspirationId,
    );
  ...
  const uniqueZones = useMemo(
    () => Array.from(new Set(cityManifest.map((c) => c.zone))).sort(),
    [cityManifest],
  );
  ...
  useEffect(() => {
    const loadSettings = async () => {
      ...
      const uniqueTypes = Array.from(
        new Set((templates || []).map((t) => (t === 'lago' ? 'laghi_fiumi' : t))),
      ).filter((t) => TIPOLOGIA_WHITELIST.includes(t as string));

      // Load Ispirazioni
      const inspirations = await getSetting<InspirationOption[]>(
        SETTINGS_KEYS.TRAVEL_STYLES_CONFIG,
      );
      ...
      setGeoOptions((prev) => ({
        ...prev,
        continents: (continentsData || []).map((c) => ({
          label: c.name,
          value: c.id,
          slug: c.slug,
        })),
  ...
  useEffect(() => {
    ...
    const loadNations = async () => {
      ...
      setGeoOptions((prev) => ({
        ...prev,
        nations: (data || []).map((n) => ({ label: n.name, value: n.id, slug: n.slug })),
        regions: [],
        zones: [],
      }));
    };
  ...
  useEffect(() => {
    ...
    const loadRegions = async () => {
      ...
      setGeoOptions((prev) => ({
        ...prev,
        regions: (data || []).map((r) => ({ label: r.name, value: r.id, slug: r.slug })),
        zones: [],
      }));
    };
    loadRegions();
  ...
  useEffect(() => {
    ...
    const loadZones = async () => {
      ...
      setGeoOptions((prev) => ({
        ...prev,
        zones: (data || []).map((z) => ({ label: z.name, value: z.id, slug: z.slug })),
      }));
    };
    loadZones();
    return () => {
    ...
    if (cityId) {
      ...
      if (city) {
        ...
        if (!selectedZone && city.zone) {
          const matchedZone = geoOptions.zones.find(
            (z) => z.label.toLowerCase() === city.zone.toLowerCase(),
          );
          if (matchedZone) setSelectedZone(matchedZone.value);
        }
        if (!region && city.adminRegion) {
          const matchedRegion = geoOptions.regions.find(
            (r) => r.label.toLowerCase() === city.adminRegion.toLowerCase(),
          );
          if (matchedRegion) setRegion(matchedRegion.value);
        }
        if (!nation && city.nation) {
          const matchedNation = geoOptions.nations.find(
            (n) => n.label.toLowerCase() === city.nation.toLowerCase(),
          );
          if (matchedNation) setNation(matchedNation.value);
        }
        if (!continent && city.continent) {
          const matchedCont = geoOptions.continents.find(
            (c) => c.label.toLowerCase() === city.continent.toLowerCase(),
          );
  ...
  const handleAiSubmit = async (queryOverride?: string) => {
    ...
    if (userInput === 'Shortlist AI analysis') {
      ...
      const seasonLabel = selectedSeason
        ? `per la stagione ${selectedSeason}`
        : 'per questo periodo';
      const catLabels =
        activeCategories.length > 0 ? `con focus su: ${activeCategories.join(', ')}` : '';
    ...
    if (queryOverride) {
      setAiQuery(
        queryOverride === 'Shortlist AI analysis'
          ? 'Suggeriscimi la meta ideale tra queste...'
          : userInput,
      );
    }
    ...
    try {
      ...
      if (isAiEdgeError(err)) {
        ...
        setAiResponse(
          aiErrorUserMessage(err, 'Spiacenti, il nostro consulente non è disponibile al momento.'),
        );
      }
    } finally {
