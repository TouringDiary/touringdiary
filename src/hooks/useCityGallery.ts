...
import {
  getOrCreatePhotoSubmissionForUrl,
  listPhotographs,
  uploadCommunityPhoto,
} from '../services/photoService';
import type { CityDetails, PhotoSubmission, User } from '../types/index';
...
export const useCityGallery = (city: CityDetails, user: User) => {
  ...
  const loadPhotos = useCallback(async () => {
    ...
    try {
      ...
      const registeredPhotosPromises = photographicGalleryAssets.map((asset) => {
        ...
        return getOrCreatePhotoSubmissionForUrl(
          asset.url,
          city.id,
          city.name,
          'Immagine ufficiale',
      ...
      const sortedPhotos = finalPhotos.sort(
      ...
      if (err instanceof Error)
        console.error(`[useCityGallery] Error loading photos: ${err.message}`);
  ...
  const defaultTab = useMemo(
    (): 'official' | 'community' => (officialPhotos.length > 5 ? 'official' : 'community'),
  ...
  const uploadPhoto = async (
    ...
    const photosFlagUx = evaluateCachedFeatureFlag(PLATFORM_FEATURE_FLAG_KEYS.MODERATION_PHOTOS, {
      userRole: user.role,
    ...
    try {
      ...
      console.error(err);
      const errorMessage =
        err instanceof Error ? err.message : 'Errore tecnico durante il caricamento.';
