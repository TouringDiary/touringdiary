...
export const usePoiManager = (cityId: string, cityName: string) => {
    ...
    const getCategoryCountInternal = useCallback((catId: string, status?: PointOfInterest['status']) => {
    ...
    const setViewStatusWrapper = useCallback((s: PointOfInterest['status'] | 'all') => {
    ...
    const handleBulkStatusChange = useCallback((status: PointOfInterest['status'], currentUser?: User) => {
