...
export const usePoiManager = (cityId: string, cityName: string) => {
  ...
  const loadPois = useCallback(async () => {
    ...
    try {
      const result = await getPoisPaginated({
        ...
        createdDates:
          filters.filterCreatedDates.length > 0 ? filters.filterCreatedDates : undefined,
        updatedDates:
          filters.filterUpdatedDates.length > 0 ? filters.filterUpdatedDates : undefined,
        priceLevel:
          filters.filterPriceLevel && filters.filterPriceLevel.length > 0
            ? filters.filterPriceLevel
            : undefined,
    ...
    filters.filterReliability,
    filters.filterInterest,
    filters.filterCreatedDates,
    filters.filterUpdatedDates,
    filters.filterPriceLevel,
    filters.searchTerm,
    filters.sortBy,
    filters.sortDir,
  ...
  const availableDates = useMemo(() => {
    ...
    if (allCityPois.length > 0) {
      allCityPois.forEach((p) => {
        if (p.createdAt) createdSet.add(p.createdAt.split('T')[0]);
        if (p.updatedAt) updatedSet.add(p.updatedAt.split('T')[0]);
      });
    }
  ...
  const getCategoryCountInternal = useCallback(
    (catId: string, status?: PointOfInterest['status']) => {
      const currentStatus = status || filters.viewStatus;
      return allCityPois.filter((p) => {
        const catMatch = catId === 'all' || p.category === catId;
        const itemStatus: PointOfInterest['status'] = p.status ?? 'published';
  ...
  const getTotalCategoryCountInternal = useCallback(
    (catId: string) => {
  ...
  const setViewStatusWrapper = useCallback(
    (s: NonNullable<PointOfInterest['status']> | 'all') => {
  ...
  const handleBulkStatusChange = useCallback(
    (status: PointOfInterest['status'], currentUser?: User) => {
      const finder = (id: string) =>
        pois.find((p) => p.id === id) || allCityPois.find((p) => p.id === id);
  ...
  const handleBulkResetImages = useCallback(
    (currentUser?: User) => {
      const finder = (id: string) =>
        pois.find((p) => p.id === id) || allCityPois.find((p) => p.id === id);
