...
import type { PointOfInterest, SponsorRequest } from '../types/index';
...
export const useSponsorModals = () => {
  ...
  // Preview
  const openPreview = (request: SponsorRequest) =>
    setPreviewPoi(sponsorRequestToPreviewPoi(request));
  ...
  const setExtensionDate = (date: string) => {
    let days = 0;
    if (
      extensionData.currentExpirationDate &&
      extensionData.currentExpirationDate !== 'N/A' &&
      date
    ) {
