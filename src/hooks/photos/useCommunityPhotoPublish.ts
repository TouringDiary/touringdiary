...
import { updatePhotoData, uploadCommunityPhoto } from '@/services/photoService';
import { resolvePlatformUserBody } from '@/services/platformControl/resolvePlatformUserMessage';
import type { CitySummary, PhotoSubmission, User as UserType } from '@/types/index';
...
export function useCommunityPhotoPublish({
  ...
  const openPromoteModal = useCallback(
    (snap: PhotoSubmission) => {
      ...
      const cityId = resolvePhotoCityId(snap, cityManifest) || resolvePrefillCityId();
  ...
  const getPublishBlockReason = useCallback((): string | null => {
    ...
    const photos = evaluateCachedFeatureFlag(PLATFORM_FEATURE_FLAG_KEYS.MODERATION_PHOTOS, flagCtx);
