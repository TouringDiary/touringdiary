...
import { getAllPoisGlobal, getPoisByCityId } from '../services/city/poiService';
...
export const useAdminExport = () => {
  ...
  const exportGlobalPoisCsv = async () => {
    ...
    try {
      ...
      const rows = allPois.map((p) => {
        const city =
          cityMap.get(p.cityId || '') ||
          ({
            continent: 'N/D',
            nation: 'N/D',
            adminRegion: 'N/D',
            zone: 'N/D',
            name: `Sconosciuta (ID: ${p.cityId})`,
          } as CitySummary);
  ...
  const exportStagingCsv = async (
    ...
    try {
      ...
      const csvContent = [
        STAGING_HEADERS.join(DELIMITER),
