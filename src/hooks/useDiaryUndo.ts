...
/** Campi update supportati dall'undo diario (useDiaryLogic). */
type DiaryUndoUpdateField = 'timeSlotStr' | 'customIcon' | 'transportMode' | 'poi.description';
    ...
    async (action: UndoAction, inverse: boolean) => {
      ...
      try {
        ...
        const payload = action.payload as {
          field: DiaryUndoUpdateField;
          newValue: unknown;
          previousValue: unknown;
          previousItems: ItineraryItem[];
        ...
        switch (type) {
          case 'add':
            if (inverse) {
              ...
              addItem(action.payload as ItineraryItem);
            ...
            setItinerary((prev) => ({
              ...
              items: prev.items.map((item) => {
                ...
                if (field.includes('.')) {
                  ...
                  const nestedRaw = item[parent as keyof ItineraryItem];
                  const nestedBase =
                    nestedRaw !== null && typeof nestedRaw === 'object' && !Array.isArray(nestedRaw)
                      ? { ...(nestedRaw as unknown as Record<string, unknown>) }
                      : {};
                  return {
                    ...item,
                    [parent]: {
          ...
          case 'diaryNotes': {
            const { newValue, previousValue } = action.payload as {
