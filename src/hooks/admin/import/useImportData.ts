...
export const useImportData = () => {
  ...
  const [statusFilter, setStatusFilter] = useState<
    'new' | 'ready' | 'imported' | 'discarded' | 'all'
  >('new');
  ...
  const handleSetSelectedCityId = (id: string) => {
    setSelectedCityId(id);
    setPage(1);
  };
  const handleSetStatusFilter = (status: 'new' | 'ready' | 'imported' | 'discarded' | 'all') => {
  ...
  const handleSetFilterAiRating = (ratings: string[]) => {
    setFilterAiRating(ratings);
    setPage(1);
  };
  const handleSetFilterRawCategories = (cats: string[]) => {
    setFilterRawCategories(cats);
    setPage(1);
  };
