...
import { deleteTouristZone, getTouristZones, renameTouristZone } from '../../services/zoneService';
import type { CitySummary, TouristZone } from '../../types/index';
  ...
  const zoneData = useMemo(() => {
    return zones.map((z) => {
      ...
      const cleanSuggestions = suggestions.filter(
        (s) => !citiesInZone.some((c) => c.name.toLowerCase() === s.name.toLowerCase()),
      );
