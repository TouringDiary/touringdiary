...
export const useSocialTemplates = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      // Parallel Fetch
      const [tpls, msgs] = await Promise.all([getSocialTemplates(), getSystemMessagesAsync()]);
  ...
  const initDefaultViral = async () => {
    const defaultViral: SystemMessageTemplate = {
      ...
      bodyTemplate:
        'Sto organizzando il mio viaggio in Campania con Touring Diary! È gratis e fantastico. Usa il mio codice {code} per avere crediti extra.\n\nLink: {url}',
      variables: ['code', 'url'],
      deviceTarget: 'all',
    };
