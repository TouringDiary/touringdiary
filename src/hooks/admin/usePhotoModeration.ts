...
import type { CitySummary, PhotoSubmission, User } from '../../types/index';
...
export const usePhotoModeration = ({ currentUser, onUpdate }: UsePhotoModerationProps) => {
  ...
  const [filterStatus, setFilterStatus] = useState<
    'pending' | 'approved' | 'rejected' | 'city_deleted' | 'all'
  >('pending');
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const finalDesc = aiCaption
        ? `${aiCaption} (${file.name})`
        : `[${file.name}] Caricamento manuale admin`;
  ...
  const handleStatusUpdate = async (id: string, status: 'approved' | 'rejected' | 'pending') => {
    ...
    if (status === 'approved' && photo) {
      const cityMatch = manifest.find(
        (c) => photo.locationName.toLowerCase().trim() === c.name.toLowerCase().trim(),
      );

      if (!cityMatch) {
        showToast(
          `Impossibile approvare: La città "${photo.locationName}" non esiste nel database attivo.`,
          'error',
        );
        return;
      }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      showToast(`ERRORE CRITICO DB: ${e instanceof Error ? e.message : String(e)}`, 'error');
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const handleToggleOfficial = async (id: string, currentStatus: boolean) => {
    ...
    try {
      ...
      showToast(
        newStatus ? 'Foto promossa a Ufficiale!' : 'Foto riportata a Community.',
        'success',
      );
    } catch (e: unknown) {
  ...
  const saveMetadata = async () => {
    if (metadataModal) {
      try {
        ...
        setPhotos((prev) =>
          prev.map((p) => (p.id === metadataModal.photoId ? { ...p, ...updates } : p)),
        );
