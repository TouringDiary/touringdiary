...
import { getStorageItem, removeStorageItem, setStorageItem } from '../../services/storageService';
...
export const useAiTaskRunner = () => {
  ...
  // --- HYDRATION ON MOUNT ---
  useEffect(() => {
    const savedSession = getStorageItem<AiTaskRunnerSession | null>(SESSION_KEY, null);
    if (savedSession) {
      ...
      if (isFresh) {
        ...
        const restoredReports: StepReport[] = (savedSession.reports || []).map(
          (step: StepReport) => {
        ...
        if (restoredLogs.length > 0) {
          setIsRecovered(true);
          // Aggiungi log di sistema visibile (UI + accumulator per saveSession)
          const recoveryMsg = `[SYSTEM] ⚠️ Sessione precedente recuperata.`;
          logsAccumulator.current = [...logsAccumulator.current, recoveryMsg];
          setProcessLog((prev) => [...prev, recoveryMsg]);
        }
      } else {
        clearSession();
      }
  ...
  const performStep = async <T>(
    ...
    try {
      ...
      const duration = performance.now() - startTime;
      const errorMessage = (e instanceof Error ? e.message : String(e)) || 'Errore sconosciuto';
