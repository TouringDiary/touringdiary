...
import { useRegisterDiaryUndoStack } from '@/context/DiaryUndoBridgeContext';
...
import type { Itinerary, ItineraryItem, User } from '../types/index';
...
export const useDiaryLogic = ({ user, onUserUpdate, onDayDropProp }: UseDiaryLogicProps) => {
  ...
  // Identità stack: istanza hook + diario corrente (non activeTab UI).
  // Cleanup per stackId non può cancellare una registrazione più recente.
  const diaryUndoInstanceIdRef = useRef(`diary-undo-${randomUUID()}`);
  const diaryUndoStackId = `${diaryUndoInstanceIdRef.current}:${itinerary.id}`;
  useRegisterDiaryUndoStack(diaryUndoStackId, pushAction);
  ...
  const handleDateChange = useCallback(
    (type: 'startDate' | 'endDate', newValue: string) => {
    ...
    (id: string, text: string) => {
      ...
      setItinerary((prev) => ({
        ...
        items: prev.items.map((i) =>
          i.id === id ? { ...i, poi: { ...i.poi, description: text } } : i,
        ),
      }));
    },
    [itinerary.items, setItinerary, pushAction, guardCollaborativeWrite],
  ...
  const handleDiaryNotesChange = useCallback(
  ...
  const guardedUpdateDayStyle = useCallback(
    (dayIndex: number, className: string) => {
      if (guardCollaborativeWrite()) return;
  ...
  // Resource edit (Modifica → Salva Contatto): MOVE_ITEM, not memo create.

  const handleOpenMemoConfig = useCallback((resourceItem: ItineraryItem) => {
    setMemoTargetItem(resourceItem);
  }, []);

  /** Sposta guida/TO/servizio sul giorno scelto (stesso path di drag/MobileMoveModal). */
  const handleConfirmAddMemo = useCallback(
    (dayIndex: number, timeSlotStr: string) => {
      if (guardCollaborativeWrite()) return;
      if (!memoTargetItem) return;

      const targetId = memoTargetItem.id;
      const previousDayIndex = memoTargetItem.dayIndex;
      setMemoTargetItem(null);

      // Stesso giorno: chiudi senza side-effect (MOVE risorse aggiorna solo dayIndex).
      if (previousDayIndex === dayIndex) return;

      pendingMoveActionRef.current = {
        id: targetId,
        previousItems: [...itinerary.items],
      };
      onDayDropProp(dayIndex, JSON.stringify({ type: 'MOVE_ITEM', id: targetId }), timeSlotStr);

      // Conferma standard DiaryModals (toastMessage): non auto-dismiss.
      if (xpToastTimeoutRef.current) {
        clearTimeout(xpToastTimeoutRef.current);
        xpToastTimeoutRef.current = null;
      }
      setToastMessage({ title: 'Contatto spostato nel nuovo giorno', xp: 0 });
    },
    [memoTargetItem, itinerary.items, onDayDropProp, guardCollaborativeWrite],
  );
  ...
  const handleDayDrop = useCallback(
    (e: React.DragEvent, idx: number, time?: string) => {
      if (guardCollaborativeWrite()) return;
