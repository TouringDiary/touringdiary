...
const readGeoOptions = (configs: unknown): GeoOptions => {
  if (!configs || typeof configs !== 'object') return {};
  const raw = (configs as Record<string, unknown>).geo_options;
  if (!raw || typeof raw !== 'object') return {};
  const opts = raw as Record<string, unknown>;
  return {
    enableHighAccuracy:
      typeof opts.enableHighAccuracy === 'boolean' ? opts.enableHighAccuracy : undefined,
    timeout: typeof opts.timeout === 'number' ? opts.timeout : undefined,
    maximumAge: typeof opts.maximumAge === 'number' ? opts.maximumAge : undefined,
  };
};

// ✅ FIX: riceve configs dall'esterno (NO useConfig qui dentro)
export const useGpsManager = (configs: unknown) => {
  ...
  const getPositionPromise = useCallback(
    (highAccuracy: boolean): Promise<GeolocationPosition> => {
      return new Promise((resolve, reject) => {
        const geoOptions = readGeoOptions(configs);
    ...
    try {
      ...
      switch (code) {
        case 1: {
          ...
          if (perm === 'denied') {
            msg =
              "L'accesso è bloccato dal browser. Clicca sull'icona del lucchetto 🔒 o nelle impostazioni del sito per sbloccarlo.";
            isCritical = true;
          } else if (perm === 'granted') {
            // Should have been recovered above; remaining case = fix failed after grant.
            msg =
              'Permesso concesso, ma la posizione non è ancora disponibile. Riprova tra un momento.';
            isCritical = false;
          } else {
            msg =
              'Accesso negato dal Sistema Operativo. Controlla: Impostazioni di Sistema > Privacy > Localizzazione.';
            isCritical = true;
          }
          break;
        }
