...
import { fetchLevelsAsync, getCurrentLevel } from '../../services/gamificationService'; // UPDATED
import { getSetting } from '../../services/settingsService';
import { isAuthOperationInProgress, supabase } from '../../services/supabaseClient';
import { getCurrentUserProfile, refreshUsersCache } from '../../services/userService';
...
export const useAppInitialization = (viewMode: string) => {
  ...
  const [hasSeenOnboarding, setHasSeenOnboarding] = usePersistedState<boolean>(
    'has_seen_onboarding_v3',
    false,
  );
  ...
  useEffect(() => {
    const syncUserState = async (session: Session | null) => {
      if (session?.user) {
        try {
  ...
  useEffect(() => {
    ...
    const checkAutoStart = async () => {
      ...
      const onboardingEnabled =
        evaluateCachedFeatureFlag(PLATFORM_FEATURE_FLAG_KEYS.PLATFORM_ONBOARDING, {
          userRole: null,
