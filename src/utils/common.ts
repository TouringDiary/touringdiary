import { Bed, Landmark, MapPin, Music, Scan, ShoppingBag, Sun, Utensils } from 'lucide-react';
...
export const formatVisitors = (num: number | undefined | null) => {
  ...
  if (num >= 1000000)
    return `${(num / 1000000).toLocaleString('it-IT', {
...
export const getDaysArray = (start: string, end: string): Date[] => {
  if (!start || !end) return [];
  const rangeStart = parseLocalCalendarDate(start);
  const rangeEnd = parseLocalCalendarDate(end);
...
export const openMap = (lat: number, lng: number, name?: string, address?: string) => {
  // Stessa policy di open3DView / hasUsableCoords: finite, in range; solo (0,0) è placeholder.
  // Una sola coordinata a 0 (es. lat=0 equatore) resta valida — non usare if (lat && lng).
...
export const open3DView = (lat?: number, lng?: number, name?: string, address?: string) => {
  // Coordinate usabili: finite, in range geografico; (0,0) = placeholder non utilizzabile.
  // Una sola coordinata a 0 (es. lat=0 equatore) resta valida.
  const hasUsableCoords =
    typeof lat === 'number' &&
    typeof lng === 'number' &&
    Number.isFinite(lat) &&
...
export const getSubCategoryLabel = (subCat: string): string => {
  ...
  switch (lower) {
    ...
    case 'cafe':
      return 'Caffetteria';
    case 'bakery':
      return 'Forno / Panetteria';
    case 'gastronomy':
      return 'Gastronomia';
    case 'dairy':
      return 'Caseificio';
    case 'braceria':
      return 'Braceria / Steakhouse'; // NEW
    case 'fast_food':
      return 'Fast Food'; // NEW
    ...
    case 'zoo':
      return 'Zoo / Acquario';
    case 'playground':
      return 'Parco Giochi'; // NEW
    case 'stadium':
      return 'Stadio / Sport';

    // NATURA / MARE
    case 'beach_club':
      return 'Lido / Beach Club';
    case 'beach_free':
      return 'Spiaggia Libera'; // Distinct
    case 'park':
      return 'Parco Pubblico';
    case 'garden':
      return 'Giardino / Villa';
    case 'hiking':
      return 'Sentiero / Trekking';
    case 'viewpoint':
      return 'Belvedere / Panorama';
    case 'lake':
      return 'Lago';
    case 'river':
      return 'Fiume / Cascata';
    case 'reserve':
      return 'Riserva Naturale';

    // CULTURA (Monumenti)
    case 'church':
      return 'Chiesa / Luogo Sacro';
    case 'castle':
      return 'Castello / Fortezza';
    case 'museum':
      return 'Museo';
    case 'gallery':
      return "Galleria d'Arte";
    case 'square':
      return 'Piazza'; // Already present
    case 'archaeology':
      return 'Sito Archeologico';
    case 'palace':
      return 'Palazzo Storico';
    case 'monument':
      return 'Monumento / Statua';
    case 'library':
      return 'Biblioteca';

    // SHOPPING
    case 'fashion':
      return 'Moda & Abbigliamento';
    case 'crafts':
      return 'Artigianato Locale'; // Ceramiche, Presepi...
    case 'jewelry':
      return 'Gioielleria';
    case 'souvenir':
      return 'Souvenir';
    case 'market':
      return 'Mercato Rionale';
    case 'mall':
      return 'Centro Commerciale';
    case 'food_shop':
      return 'Prodotti Tipici (Vendita)'; // Diversi da ristorazione
    ...
    // DEFAULT FALLBACK
    default:
      return subCat.charAt(0).toUpperCase() + subCat.slice(1).replace(/_/g, ' ');
  }
};
