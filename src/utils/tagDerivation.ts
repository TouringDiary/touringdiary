...
export const deriveItineraryTags = (items: ItineraryItem[]): string[] => {
  ...
  items.forEach((item) => {
    ...
    const contextStr =
      `${item.poi.name} ${item.poi.description || ''} ${(item.poi.tags || []).join(' ')}`.toLowerCase();

    if (
      contextStr.includes('mare') ||
      contextStr.includes('beach') ||
      contextStr.includes('spiaggia')
    )
      tags.add('mare');
    if (
      contextStr.includes('montagna') ||
      contextStr.includes('mountain') ||
      contextStr.includes('alpi') ||
      contextStr.includes('appennino')
    )
      tags.add('montagna');
    if (contextStr.includes('lago') || contextStr.includes('lake')) tags.add('lago');
    if (
      contextStr.includes('aeroporto') ||
      contextStr.includes('volo') ||
      contextStr.includes('airport')
    )
      tags.add('volo');
    if (
      contextStr.includes('business') ||
      contextStr.includes('lavoro') ||
      contextStr.includes('fiera')
    )
      tags.add('business');
    if (
      contextStr.includes('freddo') ||
      contextStr.includes('neve') ||
      contextStr.includes('ghiaccio')
    )
      tags.add('freddo');
    if (
      contextStr.includes('trekking') ||
      contextStr.includes('cammino') ||
      contextStr.includes('sentiero')
    )
      tags.add('trekking');
  });
