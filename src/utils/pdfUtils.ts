 ...
 */
async function resolveCatalogImageUrlsForPdf(poiIds: string[]): Promise<Map<string, string>> {
...
const processImage = async (
  ...
  return new Promise((resolve) => {
    ...
    img.onerror = (event: string | Event) => {
      ...
      finish(undefined, 'processImage:img-onerror', {
        errorMessage:
          'img.onerror — immagine non caricata (rete, 404, CORS preflight, URL errato, redirect bloccato, ecc.)',
        extra: { eventType },
      });
    };
...
export const prepareItineraryForPdf = async (
  ...
  const uniqueCityIds = Array.from(
    new Set(
      sortedItems.map((i) => i.cityId).filter((id) => id && id !== 'custom' && id !== 'unknown'),
    ),
  );
      ...
      if (cityDetails) {
        ...
        if (options.includePhotos && cityDetails.details.heroImage) {
          ...
          logPdfImagePipeline({
            ...
            renderBlockedReason: heroBase64
              ? undefined
              : 'heroImageBase64 undefined dopo processImage',
          });
        }
  ...
  const catalogPoiIdsMissingImage = options.includePhotos
    ...
    : [];
  const catalogImageUrlByPoiId = await resolveCatalogImageUrlsForPdf(catalogPoiIdsMissingImage);
  ...
  for (let i = 0; i < totalItems; i++) {
    ...
    const catalogImageUrl =
      !snapshotImageUrl && item.poi?.id ? catalogImageUrlByPoiId.get(item.poi.id) : undefined;
    const resolveParams = {
