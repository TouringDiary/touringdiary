import type { TableRow as DocxTableRow, FileChild, ParagraphChild } from 'docx';
...
import type { PreparedItinerary, PreparedItineraryItem } from './pdfUtils';
...
export const generateWordDocument = async (
  ...
  const uniqueIds = Array.from(
    new Set(itinerary.items.map((i) => i.cityId).filter((id) => id && id !== 'custom')),
  );
  ...
  if (options.summary) {
    children.push(
      new Paragraph({
        ...
        children: [
          new TextRun({
            text: (itinerary.name || 'IL MIO VIAGGIO').toUpperCase(),
            bold: true,
      ...
      new Paragraph({
        ...
        children: [
          new TextRun({
            text: citiesListString,
            italics: true,
            size: STYLES.SUBTITLE_SIZE,
            color: STYLES.GRAY_COLOR,
          }),
        ],
      }),
      new Paragraph({
        ...
        children: [
          new TextRun({
            text: `${itinerary.startDate || 'Data inizio'} — ${itinerary.endDate || 'Data fine'}`,
            size: 20,
  ...
  const heroImages = itinerary.citiesInfo.map((c) => c.heroImageBase64).filter(Boolean) as string[];
  ...
  sortedDays.forEach((dayIndex, dayOrdinal) => {
    const items =
      daysMap.get(dayIndex)?.sort((a, b) => a.timeSlotStr.localeCompare(b.timeSlotStr)) || [];
    ...
    children.push(
      new Paragraph({
        ...
        border: {
          bottom: { style: BorderStyle.SINGLE, size: 6, color: STYLES.ACCENT_COLOR, space: 4 },
        },
            ...
            new TableCell({
              ...
              children: [
                ...
                new Paragraph({
                  children: [
                    new TextRun({
                      text: (item.poi.category || 'POI').toUpperCase(),
                      size: 14,
                      color: STYLES.ACCENT_COLOR,
                      bold: true,
                    }),
                ...
                new Paragraph({
                  ...
                  children: [
                    new TextRun({
                      text: item.poi.address || '',
                      italics: true,
                      size: STYLES.SMALL_SIZE,
                      color: STYLES.GRAY_COLOR,
                    }),
                  ],
                }),
                ...
                ...(item.poi.visitDuration
                  ? [
                      new Paragraph({
                        children: [
                          new TextRun({
                            text: `Durata visita: ${item.poi.visitDuration}`,
                            size: STYLES.SMALL_SIZE,
                            color: STYLES.GRAY_COLOR,
                            bold: true,
                          }),
  ...
  if (resourceItems.length > 0 && options.resources) {
    children.push(
      new Paragraph({
        ...
        border: {
          bottom: { style: BorderStyle.SINGLE, size: 6, color: STYLES.GRAY_COLOR, space: 4 },
        },
      ...
      new TableRow({
        ...
        children: [
          ...
          new TableCell({
            children: [
              new Paragraph({
                children: [new TextRun({ text: 'CONTATTO', bold: true })],
                alignment: AlignmentType.CENTER,
              }),
            ],
        ...
        new TableRow({
          children: [
            new TableCell({
              children: [
                new Paragraph({
                  children: [new TextRun({ text: res.poi.name, bold: true })],
                  alignment: AlignmentType.CENTER,
                }),
              ],
              width: { size: 40, type: WidthType.PERCENTAGE },
            ...
            new TableCell({
              children: [
                new Paragraph({
                  text: getResourceLabel(res.poi.resourceType || ''),
                  alignment: AlignmentType.CENTER,
                }),
              ],
              width: { size: 25, type: WidthType.PERCENTAGE },
            ...
            new TableCell({
              children: [
                new Paragraph({
                  text: res.poi.contactInfo?.phone || res.poi.address || '-',
                  alignment: AlignmentType.CENTER,
                }),
              ],
              width: { size: 35, type: WidthType.PERCENTAGE },
          ...
          default: new Footer({
            children: [
              new Paragraph({
                ...
                children: [
                  new TextRun({
                    text: 'Generato con Touring Diary',
                    size: 16,
                    color: STYLES.GRAY_COLOR,
                  }),
                ],
...
export const generateTextFile = (
  ...
  const uniqueCityIds = Array.from(
    new Set(
      items.map((i) => i.cityId).filter((id): id is string => Boolean(id) && id !== 'custom'),
    ),
  );
