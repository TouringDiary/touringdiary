...
export function resolveAuditActionLabel(event: PlatformControlAuditEvent): string {
  ...
  if (schedulesChanged) {
    if (schedulesAfter < schedulesBefore) {
      ...
      if (schedulesAfter === 0) {
        return schedulesBefore === 1 ? 'Programmazione eliminata' : 'Programmazioni eliminate';
      }
      return removed === 1 ? 'Programmazione eliminata' : 'Programmazioni eliminate';
    }
  ...
  const overrideChanged = JSON.stringify(overrideBefore) !== JSON.stringify(overrideAfter);
