...
import {
  type SaveUnifiedReviewResult,
  saveUnifiedReview,
} from '../services/community/reviewService';
import { fetchUserPhotoLikes, togglePhotoLikeRPC } from '../services/photoService';
...
export const InteractionProvider = ({ children }: { children?: ReactNode }) => {
  ...
  const [photoStatus, setPhotoStatus] = useState<Record<string, PhotoLikeStatus>>({});
  ...
  const hasUserLikedPhoto = useCallback(
    (photoId: string) => photoStatus[photoId]?.isLiked ?? false,
    [photoStatus],
  );
  ...
  const togglePhotoHeart = useCallback(
    async (photoId: string): Promise<number | undefined> => {
      if (isGuest) {
      ...
      const fallback = {
        ...
        count: current?.count ?? photoStatus[photoId]?.count ?? 0,
        isLoading: false,
      };
      ...
      try {
        const result = await togglePhotoLikeRPC(photoId);
  ...
  const toggleVote = useCallback(
    async (poiId: string): Promise<number | null> => {
      const isAdding = !votedPois.includes(poiId);

      const next = isAdding ? [...votedPois, poiId] : votedPois.filter((id) => id !== poiId);

      setVotedPois(next);

      setStorageItem(`voted_pois_${currentUserId}`, next);
  ...
  const toggleLike = useCallback(
    (poiId: string) => {
      setLikedPois((prev) => {
        const next = prev.includes(poiId) ? prev.filter((id) => id !== poiId) : [...prev, poiId];
  ...
  const submitReview = useCallback(
    async (
      ...
      const reviewsFlag = evaluateCachedFeatureFlag(PLATFORM_FEATURE_FLAG_KEYS.MODERATION_REVIEWS, {
        userRole: user.role,
