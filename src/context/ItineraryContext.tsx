...
export const ItineraryProvider = ({ children }: { children?: ReactNode }) => {
  ...
  const saveProject = useCallback(
    async (
      ...
      const isTempId =
        !targetId ||
        targetId.startsWith('imported-') ||
        targetId.startsWith('ai-it-') ||
        targetId.startsWith('draft_') ||
        targetId.startsWith('it-');
      ...
      if (isTempId || isGhostId || isSaveAsNewCopy) {
        console.log(
          '[Itinerary] Generating fresh UUID for project (Temp, Ghost, or SaveAs with new name detected)',
        );
        targetId = randomUUID();
      }
      ...
      let resolvedViaggioId: string | null = needsNewViaggio ? null : (itinerary.viaggioId ?? null);
          ...
          if (!isGuest) {
            try {
              ...
              if (synced?.viaggioId) {
                ...
                setItinerary((prev) =>
                  prev.id === synced.id ? { ...prev, viaggioId: synced.viaggioId } : prev,
                );
              }
            } catch (e) {
  ...
  const deleteProject = useCallback(
    async (targetId: string) => {
      ...
      if (success) {
        ...
        throw new Error(
          'Errore cancellazione: ID non trovato o permessi mancanti. Verifica la connessione.',
        );
      }
      ...
      if (itinerary.id === cleanId) {
        console.log(
          '[Itinerary] Deleted active diary. Resetting to temp draft (Viaggio non auto-promosso).',
        );
  ...
  const importPremadeItinerary = useCallback(
    async (template: PremadeItinerary, startDateOverride?: string) => {
      ...
      for (const item of template.items) {
        ...
        newItems.push({
          ...
          cityId: template.mainCity.toLowerCase(),
  ...
  const findFreeSlot = useCallback(
    (dayIndex: number) => {
      const dayItems = itinerary.items.filter(
        (i) => i.dayIndex === dayIndex && i.timeSlotStr && i.timeSlotStr.includes(':'),
      );
