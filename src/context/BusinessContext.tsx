...
import { BIZ_DASHBOARD_TABS, URL_TO_INTERNAL_TAB, useAppRouter } from '../hooks/useAppRouter';
...
export const BusinessProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const rawIdentifier = useMemo(() => {
    ...
    const isBizTab = (Object.values(BIZ_DASHBOARD_TABS) as string[]).includes(
      firstSegmentAfterDashboard,
    );
    if (isBizTab) {
    ...
    const isUserTab = Object.values(URL_TO_INTERNAL_TAB).includes(
      URL_TO_INTERNAL_TAB[firstSegmentAfterDashboard],
    );
  ...
  const activeBusiness = useMemo(() => {
    ...
    if (found) {
      console.log(
        `[V4-Resolver] Identifier "${rawIdentifier}" resolved to: ${found.companyName} (${found.id})`,
      );
    } else if (rawIdentifier && userBusinesses.length > 0) {
      console.warn(
        `[V4-Resolver] Identifier "${rawIdentifier}" could not be resolved in user business pool.`,
      );
    }
  ...
  useEffect(() => {
    ...
    if (isBizTab && !rawIdentifier && userBusinesses.length > 0) {
      ...
      console.log(
        `[V4-AutoSelect] Biz tab "${firstSegment}" detected without business slug. Injecting primary: ${bestId}`,
      );
