...
import {
  setPlatformFlagCache,
  setPlatformFlagEvaluationNow,
} from '@/domain/platformControl/platformFlagCache';
import {
...
type PlatformControlContextType = {
  ...
  evaluateFlag: (
    key: string,
    ctx?: Partial<FeatureFlagEvaluationContext>,
...
export const PlatformControlProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const armNextScheduleBoundary = useCallback(() => {
    ...
    const delay = Math.min(Math.max(nextBoundary - now, 0), SCHEDULE_TIMER_MAX_DELAY_MS);
  ...
  const getFlagDefinition = useCallback((key: string) => flagMap.get(key), [flagMap]);
  ...
  const evaluateFlag = useCallback(
    (
      key: string,
      ctxOverride?: Partial<FeatureFlagEvaluationContext>,
    ): FeatureFlagEvaluationResult | null => {
  ...
  return (
    <PlatformControlContext.Provider value={value}>{children}</PlatformControlContext.Provider>
  );
};
