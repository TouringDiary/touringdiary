...
import {
  ...
  consumeQaCityReturn,
  peekQaCityReturn,
  type QaCityReturnMemory,
} from '@/community/qaCityReturnMemory';
...
export const NavigationProvider = ({ children }: { children?: ReactNode }) => {
  ...
  /**
   * Q&A → città → Back: riapri Community Hub DOPO il cleanup pathname
   * (altrimenti closeModal dell'effect cancella il modal appena aperto).
   */
  const pendingQaCityRestoreRef = useRef<QaCityReturnMemory | null>(null);
  ...
  const openShopFromPoi = useCallback(
    (poi?: PointOfInterest) => {
  ...
  const handleAroundMeTrigger = useCallback(
    async (config: { type: 'gps' | 'manual'; cityId?: string; radius: number }) => {
  ...
  const handleMergeCities = useCallback(
    async (baseCity: CityDetails, radius: number, selectedCityIds: string[]) => {
      setIsBuildingVirtual(true);
      try {
        const virtual = await buildVirtualCity(baseCity.coords, radius, cityManifest, {
          baseCity,
          selectedCityIds,
        });
        if (isMounted.current) {
  ...
  const goHome = useCallback(() => {
    aroundMeSessionRef.current = null;
    pendingQaCityRestoreRef.current = null;
    clearQaCityReturn();
    router.goHome();
  ...
  const goBack = useCallback(() => {
    ...
    // 3. Q&A Local → città → Indietro: prima di Around Me se la memoria matcha la città attiva.
    //    Riapertura Community Hub deferred al cleanup pathname (pendingQaCityRestoreRef).
    if (router.activeCityId) {
      const peeked = peekQaCityReturn();
      if (peeked && peeked.cityId === router.activeCityId) {
        const qaReturn = consumeQaCityReturn();
        if (qaReturn) {
          pendingQaCityRestoreRef.current = qaReturn;
          router.goHome();
          return;
        }
      }
    ...
    // 4. Flusso Around Me (stesso handler del logo: goHome).
    ...
    // 5. History naturale (città fuori dal flusso Around Me / Q&A return)
    router.goBack();
  ...
  const handleNavigateGlobal = useCallback(
    (section: string, tab?: string, id?: string, extra?: NavigationGlobalExtra) => {
      if (section === 'city' && id) {
    ...
    [navigateToCity, modalContext, navigate, router, userContext?.user?.slug, openMyWorld],
  );

  const resolveCityIdFromSlug = useCallback(
  ...
  useEffect(() => {
    ...
    if (router.pathname !== lastPathnameRef.current) {
      ...
      const isInternalAdminNav =
        lastPathnameRef.current?.startsWith('/admin') && router.pathname.startsWith('/admin');
      ...
      if (isInternalAdminNav || isInternalDashboardNav) {
        ...
        // 1b. Q&A → città → Back: riapri Community Hub dopo il cleanup (non prima).
        const pendingQaRestore = pendingQaCityRestoreRef.current;
        if (pendingQaRestore) {
          pendingQaCityRestoreRef.current = null;
          modalContext.openModal('global', {
            section: 'community',
            tab: 'qa',
            qaRestore: {
              showMyPostsOnly: pendingQaRestore.showMyPostsOnly,
              listScrollTop: pendingQaRestore.listScrollTop,
            },
          });
        }
