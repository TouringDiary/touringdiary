...
import type * as Domain from './domain/index';
...
export type DatabasePoiInsert = Domain.DbInsert<'pois'> & {
  id: string;
  city_id: string;
  name: string;
  category: string;
...
export type DatabaseSponsorInsert = Domain.DbInsert<'sponsors'> & {
  contact_name: string;
  company_name: string;
  email: string;
  status: string;
...
export interface DatabaseJoinedSponsor extends DatabaseSponsor {
   ...
   */
  city_tour_operators?: Pick<
    DatabaseCityTourOperator,
    | 'name'
    | 'slug'
    | 'description'
    | 'image_url'
...
export type DatabaseUserRewardInsert = Domain.DbInsert<'user_rewards'> & {
  instance_id: string;
  user_id: string;
  reward_id: string;
  code: string;
...
export type DatabaseAiConfigInsert = Domain.DbInsert<'ai_configs'> & {
  key: string;
  prompts: string[];
  selected: string[];
};
...
// Alias per notifiche
export type DatabaseNotificationInsert = Domain.DbInsert<'notifications'> & {
  user_id: string;
  type: string;
  title: string;
  message: string;
...
export interface DatabaseCityEventWithCity extends Domain.DbCityEvent {
  cities?: Pick<Domain.DbCity, 'id' | 'name' | 'zone' | 'admin_region'> & {
    special_badge?: CitySummary['specialBadge'];
  };
}
