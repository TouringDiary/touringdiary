...
import { X, Map, Trash2, Globe, Search, ChevronDown, Check, BookOpen, Users, Zap, Lightbulb, Loader2 } from 'lucide-react';
...
import { getFilteredCommunityItinerariesAsync, getCommunityItinerariesMetadataAsync } from '../../services/communityService';
    ...
    return (
        <div className="relative">
            ...
            {isOpen && (
                <>
                    <div
                        className="fixed left-0 right-0 bottom-0 top-[var(--header-height)]"
...
export const ItinerariesExplorer = ({
    ...
    }, [activeCategory, filterContinent, filterNation, filterRegion, filterZone, filterCity, searchQuery, activeTag]);
    ...
    return (
        <div className="flex flex-col h-full min-h-0 overflow-hidden relative">
                    ...
                    <div className="flex flex-col md:flex-row gap-3 justify-between items-center animate-in fade-in slide-in-from-top-1 duration-300">
                        <div className="flex bg-slate-900 p-1 rounded-lg border border-slate-800 shadow-inner w-full md:w-auto">
                            {[
                                ...
                                <button key={tab.id} onClick={() => { setActiveCategory(tab.id as 'official' | 'community' | 'ai'); }} className={`px-4 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-wide flex items-center justify-center gap-2 transition-all whitespace-nowrap flex-1 ${activeCategory === tab.id ? 'bg-slate-800 text-white shadow-md' : 'text-slate-600 hover:text-slate-400'}`}>
                            ...
                            <div className={`absolute inset-0 backface-hidden rotate-y-180 flex flex-col ${!isListFlipped ? 'pointer-events-none opacity-0' : 'opacity-100'}`}>
                                {selectedItinerary && (
                                    <div className="h-full w-full bg-[#0f172a] flex flex-col overflow-hidden">
                                        <ItineraryReviews itinerary={selectedItinerary} onBack={() => { setIsListFlipped(false); setSelectedItinerary(null); }} user={user} />
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            {showOverwriteWarning && pendingImportData && (
                ...
                >
                    <div className="bg-slate-900 rounded-xl border border-red-500/50 p-6 w-full max-w-sm shadow-2xl animate-in zoom-in-95 pointer-events-auto" onClick={e => e.stopPropagation()}>
