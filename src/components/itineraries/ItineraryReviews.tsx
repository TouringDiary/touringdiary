import { ChevronLeft, Edit2, Loader2, MessageSquare, PenTool, Trash2 } from 'lucide-react';
...
import type { PremadeItinerary, Review, User } from '../../types/index';
...
export const ItineraryReviews = ({ itinerary, onBack, user }: Props) => {
  ...
  const refreshLocalReviews = useCallback(async () => {
    const all = await getUnifiedReviews();
    setReviews(all.filter((r) => r.itineraryId === itinerary.id && r.status === 'approved'));
  }, [itinerary.id]);

  useEffect(() => {
    void refreshLocalReviews().catch((e: unknown) => {
      console.error('Errore caricamento recensioni itinerario:', e);
    });
  }, [refreshLocalReviews]);
  ...
  const handleAddReview = async (
    ...
    const saved = await saveUnifiedReview({
      author: user.name,
      authorId: user.id,
      rating,
      text: comment,
    ...
    try {
      ...
      console.error('Errore aggiornamento elenco recensioni:', e);
      setReviews((prev) => {
        const without = prev.filter((r) => r.id !== saved.review.id);
        if (saved.review.itineraryId === itinerary.id && saved.review.status === 'approved') {
          return [saved.review, ...without];
        }
        return without;
      });
    }
  ...
  const openWriteOrEdit = async () => {
    ...
    const existing = myReview ?? (await getUserReviewForItinerary(itinerary.id, user.id));
