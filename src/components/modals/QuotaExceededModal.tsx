...
export const QuotaExceededModal: React.FC<QuotaExceededModalProps> = ({
  ...
  // evaluateFlag → null se definizione assente/non caricata: fail-closed per capability economica.
  ...
  return createPortal(
    ...
    >
      ...
      />
      <div
        className="relative bg-slate-900 border border-rose-500/30 rounded-[2.5rem] shadow-2xl w-full max-w-md p-8 text-center animate-in zoom-in-95 duration-300 pointer-events-auto"
        style={{ zIndex: Z_MODAL }}
        role="dialog"
        aria-modal="true"
        aria-labelledby="quota-exceeded-title"
        aria-describedby="quota-exceeded-desc"
      >
        <CloseButton
          onClose={onClose}
          variant="primary"
        ...
        <p id="quota-exceeded-desc" className="text-slate-400 text-sm mb-8 leading-relaxed">
          ...
          {purchaseEnabled ? (
            ...
            <div className="rounded-2xl border border-amber-500/30 bg-amber-950/20 p-4 text-left space-y-1">
              ...
              <p className="text-xs text-slate-300 leading-relaxed">
                {pausedCopy.body || 'L’acquisto di crediti AI è temporaneamente non disponibile.'}
              </p>
            </div>
          )}
