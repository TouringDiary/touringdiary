                ...
                <div className="space-y-3 flex flex-col h-full">
                    ...
                    {Object.keys(poi.affiliate || {}).length > 0 && (
                        <div className="space-y-1.5 mt-auto pt-2">
                            ...
                            {poi.contactInfo?.website && <button onClick={e => handleAffiliateClick(e, 'website', poi.contactInfo!.website!)} className="w-full bg-slate-800 hover:bg-slate-700 text-slate-300 p-2 rounded-lg font-bold text-[10px] uppercase tracking-wide flex items-center justify-between transition-all border border-slate-700 shadow-md"><span>Sito Ufficiale</span><RotateCw className="w-3.5 h-3.5" /></button>}
                        </div>
                    )}
                </div>
            </div>
