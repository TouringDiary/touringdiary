...
import type { MarketingTierConfig } from '@/types';
import { type FormattedPricingVersion, getPricingVersions } from '../../../services/dataService';
...
export const SponsorPricingSelector: React.FC<SponsorPricingSelectorProps> = ({
  onSelectionChange,
  activeGroup,
  initialPlanType,
  campaignCode,
      ...
      try {
        ...
        filteredVersions.forEach((v) => {
          ...
          if (
            !existing ||
            v.duration_days <
              filteredVersions.find((f) => f.pricing_version_id === existing)!.duration_days
          ) {
        ...
        if (initialPlanType && defaultDurations[initialPlanType]) {
          ...
          const firstType =
            relevantPlanTypes.find((t) => defaultDurations[t]) || filteredVersions[0].plan_type;
  ...
  const handleDurationChange = (planType: string, versionId: string, e: React.MouseEvent) => {
    ...
    if (
      selectedVersionId &&
      pricingVersions.find((v) => v.pricing_version_id === selectedVersionId)?.plan_type ===
        planType
    ) {
  ...
  const selectedVersion =
    pricingVersions.find((v) => v.pricing_version_id === selectedVersionId) || null;

  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-[200px] text-slate-400 gap-3">
        <Loader2 className="w-8 h-8 animate-spin text-indigo-500" />{' '}
        <span className="text-sm font-bold">Caricamento piani...</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="text-center py-10 text-red-400 bg-red-900/20 border border-red-500/30 rounded-lg">
        {error}
      </div>
    );
  }
    ...
    return (
      <div className="text-center py-10 text-amber-400 bg-amber-900/20 border border-amber-500/30 rounded-lg">
        ...
        <p className="text-xs text-slate-400 mt-2">
          Contatta l'amministrazione per maggiori dettagli.
        </p>
      </div>
    );
  }
  ...
  const groupedPlans = pricingVersions.reduce(
    (acc, current) => {
          ...
          <div className="text-right">
            ...
            <div className="flex items-baseline gap-2 justify-end mt-2">
              ...
              <span className="text-2xl font-bold text-white">
                {selectedVersion.price} {selectedVersion.currency}
              </span>
