  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-12 text-center space-y-8 bg-[#020617]">
      ...
      {loading ? (
        ...
        <div>
          <h3 className="text-3xl font-display font-bold text-white mb-3">
            {message.title || 'Richiesta Inviata!'}
          </h3>
          <div className="text-slate-400 max-w-md mx-auto text-base leading-relaxed whitespace-pre-line">
            <div
              dangerouslySetInnerHTML={{
                __html:
                  message.body ||
                  `<p class="mb-4">Grazie <strong>${contactName}</strong>!</p><p>La tua richiesta è stata ricevuta correttamente.</p>`,
              }}
            />

            {isGuest && (
              <p className="text-emerald-400 font-bold mt-4 bg-emerald-900/20 p-3 rounded-lg border border-emerald-500/30">
                Accedi con la tua email ({adminEmail}) e la password scelta per gestire il tuo
                profilo.
              </p>
            )}
          </div>
      ...
      <button
        type="button"
        onClick={onClose}
        className="bg-slate-800 p-4 rounded-xl text-white font-bold uppercase tracking-wider hover:bg-slate-700 border border-slate-700"
      >
        Chiudi
      </button>
    </div>
