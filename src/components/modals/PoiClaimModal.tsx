...
export const PoiClaimModal = ({ isOpen, onClose, poi, user }: PoiClaimModalProps) => {
  ...
  const [suggestionErrorTypes, setSuggestionErrorTypes] = useState({
    name: false,
    location: false,
    hours: false,
    photo: false,
  ...
  const handleSuggestionSubmit = async () => {
    if (!suggestionText.trim()) {
      setReportError("Descrivi cosa c'è da modificare.");
      return;
    }
    setIsReporting(true);
    setReportError(null);
  ...
  return createPortal(
    <div
      className="td-modal-overlay bg-black/90 backdrop-blur-sm p-0 md:p-4 animate-in fade-in"
    ...
    >
      ...
      />
      <div
        className="relative bg-slate-900 w-full max-w-4xl rounded-3xl border border-slate-700 shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 max-h-[90vh] pointer-events-auto"
      ...
      >
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Sei il proprietario o vuoi segnalare un errore?
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-4 p-2 bg-slate-950 border-b border-slate-800 gap-2 shrink-0">
          <button
            ...
            className={`py-3 px-2 text-[10px] md:text-xs font-bold uppercase rounded-xl transition-all flex flex-col items-center gap-1.5 ${activeTab === 'gold' ? 'bg-amber-900/20 text-amber-500 shadow-lg border border-amber-500/50' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-900'}`}
          >
            <UserCheck className="w-5 h-5" />
            <span>
              Gold <span className="opacity-70 block mt-0.5 text-[9px]">Consigliato</span>
            </span>
          </button>
          ...
          <button
            ...
            className={`py-3 px-2 text-[10px] md:text-xs font-bold uppercase rounded-xl transition-all flex flex-col items-center gap-1.5 ${activeTab === 'silver' ? 'bg-slate-800 text-white shadow-lg border border-slate-600' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-900'}`}
          >
            <Store className="w-5 h-5" />
            <span>
              Silver <span className="opacity-70 block mt-0.5 text-[9px]">Standard</span>
            </span>
          </button>
          ...
          <button
            ...
            className={`py-3 px-2 text-[10px] md:text-xs font-bold uppercase rounded-xl transition-all flex flex-col items-center gap-1.5 ${activeTab === 'shop' ? 'bg-indigo-900/20 text-indigo-400 shadow-lg border border-indigo-500/50' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-900'}`}
          >
            <ShoppingCart className="w-5 h-5" />
            <span>
              Bottega <span className="opacity-70 block mt-0.5 text-[9px]">E-Commerce</span>
            </span>
          </button>
          ...
          <button
            ...
            className={`py-3 px-2 text-[10px] md:text-xs font-bold uppercase rounded-xl transition-all flex flex-col items-center gap-1.5 ${activeTab === 'report' ? 'bg-red-900/20 text-red-400 shadow-lg border border-red-500/50' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-900'}`}
          >
            <Flag className="w-5 h-5" />
            <span>
              Segnala <span className="opacity-70 block mt-0.5">Errore</span>
            </span>
          </button>
        </div>
              ...
              (reportSuccess ? (
                <div className="text-center py-12 space-y-6">
                  ...
                  <p className="text-slate-400">
                    Grazie per il tuo contributo. Verificheremo i dati al più presto.
                  </p>
                  <button
                    type="button"
                    onClick={onClose}
                    className="bg-slate-800 px-8 py-3 rounded-xl font-bold text-white uppercase text-xs hover:bg-slate-700 transition-colors border border-slate-700"
                  >
                    Chiudi
                  </button>
                </div>
                ...
                <div className="space-y-6 animate-in fade-in">
                  <div className="bg-red-900/10 border border-red-500/20 p-4 rounded-xl flex items-start gap-3">
                    ...
                    <p className="text-xs text-red-200 leading-relaxed">
                      Usa questo modulo solo per segnalare inesattezze (orari, indirizzo, nome
                      errato). Se sei il proprietario, usa i tab Gold, Silver o Bottega per
                      rivendicare.
                    </p>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {Object.keys(suggestionErrorTypes).map((k) => (
                        <button
                          ...
                          key={k}
                          onClick={() =>
                            setSuggestionErrorTypes((p) => ({ ...p, [k]: !p[k as keyof typeof p] }))
                          }
                    ...
                    <label
                      htmlFor="fld-modals-poiclaimmodal-tsx-l235"
                      className="text-xs font-bold text-slate-500 uppercase block mt-4"
                    >
                      Dettagli Corretti
                    </label>
                    <textarea
                      id="fld-modals-poiclaimmodal-tsx-l235"
                    ...
                    {reportError && (
                      <p className="text-red-400 text-xs font-bold bg-red-900/20 p-2 rounded border border-red-500/30">
                        {reportError}
                      </p>
                    )}
