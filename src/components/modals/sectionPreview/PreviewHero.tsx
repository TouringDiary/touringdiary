import { ArrowRight, Flower, Info, Leaf, Quote, Snowflake, Sun, X } from 'lucide-react';
...
export const PreviewHero = ({ city, onExplore, className }: PreviewHeroProps) => {
  ...
  const displaySeasonalVisitors = useMemo(() => {
    ...
    if (typeof city.visitors === 'number' && city.visitors > 0) {
      return {
        spring: Math.round(city.visitors * 0.3),
        summer: Math.round(city.visitors * 0.4),
        autumn: Math.round(city.visitors * 0.2),
        winter: Math.round(city.visitors * 0.1),
      };
    }
  ...
  return (
    <div className={`relative group overflow-hidden ${className}`}>
      ...
      <div className="absolute top-4 left-4 md:left-6 z-dropdown max-w-[65%]">
        <h1 className="text-2xl md:text-5xl font-display font-bold text-white mb-1 drop-shadow-xl leading-none">
          {city.name}
        </h1>
        <p className="text-slate-300 font-serif italic text-xs md:text-base drop-shadow-md opacity-90 leading-tight">
          {subtitle}
        </p>
      </div>
        ...
        <div className="absolute top-14 right-4 md:top-auto md:bottom-4 md:right-6 z-dropdown bg-slate-900/60 backdrop-blur-md border border-white/10 rounded-xl p-2 shadow-2xl flex flex-col md:flex-row items-center gap-2 md:gap-4 animate-in slide-in-from-right-4 duration-700">
          <div className="flex flex-col md:flex-row gap-2 md:gap-4">
            <div className="flex flex-row md:flex-col items-center gap-1.5 md:gap-0.5 group/season">
              <div className="p-1 md:p-1.5 rounded-full bg-emerald-500/20 text-emerald-400 group-hover/season:scale-110 transition-transform">
                <Flower className="w-3 h-3 md:w-3.5 md:h-3.5" />
              </div>
              <span className="text-[9px] md:text-[10px] font-mono font-bold text-white leading-none">
                {formatVisitors(displaySeasonalVisitors.spring)}
              </span>
            </div>
            <div className="flex flex-row md:flex-col items-center gap-1.5 md:gap-0.5 group/season">
              <div className="p-1 md:p-1.5 rounded-full bg-amber-500/20 text-amber-400 group-hover/season:scale-110 transition-transform">
                <Sun className="w-3 h-3 md:w-3.5 md:h-3.5" />
              </div>
              <span className="text-[9px] md:text-[10px] font-mono font-bold text-white leading-none">
                {formatVisitors(displaySeasonalVisitors.summer)}
              </span>
            </div>
            <div className="flex flex-row md:flex-col items-center gap-1.5 md:gap-0.5 group/season">
              <div className="p-1 md:p-1.5 rounded-full bg-orange-500/20 text-orange-400 group-hover/season:scale-110 transition-transform">
                <Leaf className="w-3 h-3 md:w-3.5 md:h-3.5" />
              </div>
              <span className="text-[9px] md:text-[10px] font-mono font-bold text-white leading-none">
                {formatVisitors(displaySeasonalVisitors.autumn)}
              </span>
            </div>
            <div className="flex flex-row md:flex-col items-center gap-1.5 md:gap-0.5 group/season">
              <div className="p-1 md:p-1.5 rounded-full bg-cyan-500/20 text-cyan-400 group-hover/season:scale-110 transition-transform">
                <Snowflake className="w-3 h-3 md:w-3.5 md:h-3.5" />
              </div>
              <span className="text-[9px] md:text-[10px] font-mono font-bold text-white leading-none">
                {formatVisitors(displaySeasonalVisitors.winter)}
              </span>
            </div>
          </div>
        </div>
      ...
      {showDescModal && (
        <div
          className="absolute inset-0 bg-black/95 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200"
        ...
        >
          ...
          />
          <div
            className="relative bg-slate-900 border border-slate-700 p-4 rounded-2xl shadow-2xl max-w-sm"
          ...
          >
            ...
            <div className="mb-2 text-amber-500">
              <Quote className="w-6 h-6 fill-current opacity-50 transform rotate-180" />
            </div>
