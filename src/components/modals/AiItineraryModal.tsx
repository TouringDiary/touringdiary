import { AlertCircle, AlertTriangle, Crown, Lock, LogIn, MapPinOff, Sparkles } from 'lucide-react';
...
export const AiItineraryModal = ({ isOpen, onClose, defaultCity = '', user }: Props) => {
  ...
  // Pre-fill City
  useEffect(() => {
    if (isOpen && !aiSession.destination && defaultCity)
      updateAiSession({ destination: defaultCity });
  }, [isOpen, defaultCity]);
  ...
  if (isGuest) {
    return createPortal(
      <div
        className="td-modal-overlay bg-slate-950/90 backdrop-blur-xl animate-in fade-in flex justify-center p-0 md:p-4 overflow-y-auto pointer-events-auto"
      ...
      >
        ...
        >
          <CloseButton onClose={onClose} position="absolute" variant="ghost" size="sm" />
          ...
          <h3 className="text-2xl font-display font-bold text-white mb-3">
            {blockMsg.title || 'Funzionalità Esclusiva'}
          </h3>
          {/* Render HTML content from DB safely */}
          <div
            className="text-slate-300 text-sm leading-relaxed mb-8 whitespace-pre-line"
            dangerouslySetInnerHTML={{
              __html: blockMsg.body || 'Il Magic Planner AI è riservato agli utenti registrati.',
            }}
          />
          ...
          <p className="text-[10px] text-slate-500 mt-4">
            La registrazione è gratuita e richiede meno di 1 minuto.
          </p>
        </div>
      </div>,
      document.body,
  ...
  return createPortal(
    <div
      className="td-modal-overlay bg-slate-950/90 backdrop-blur-xl animate-in fade-in pointer-events-auto flex items-center justify-center p-0 md:p-4"
      style={{ zIndex: Z_OVERLAY }}
      role="presentation"
    >
      <button
      ...
      {showValidationAlert && (
        <div
          className="absolute inset-0 flex items-center justify-center p-6 bg-black/90 backdrop-blur-md animate-in fade-in pointer-events-auto"
        ...
        >
          ...
          >
            ...
            <div>
              <h3 className="text-3xl font-display font-black text-white mb-3 tracking-wide">
                Dati Mancanti
              </h3>
              <p className="text-lg text-slate-300 leading-relaxed">
                Per generare l'itinerario perfetto, abbiamo bisogno di sapere{' '}
                <strong className="text-white border-b-2 border-amber-500">dove</strong> vuoi andare
                e <strong className="text-white border-b-2 border-amber-500">quando</strong>.
              </p>
            </div>
            <button
              type="button"
              onClick={() => setShowValidationAlert(false)}
              className="w-full bg-amber-500 hover:bg-amber-400 text-black font-black text-lg py-4 rounded-2xl transition-transform active:scale-95 uppercase tracking-widest shadow-xl"
            >
              Ho Capito
            </button>
          </div>
      ...
      {showQuotaAlert && (
        ...
        >
          ...
          />
          <div
            className="bg-slate-900 border-2 border-red-500 p-8 rounded-[2rem] w-full max-w-md shadow-2xl relative animate-in zoom-in-95 text-center flex flex-col items-center gap-6 pointer-events-auto"
            style={{ zIndex: Z_MODAL_NESTED }}
          >
            ...
            <div>
              <h3 className="text-2xl font-display font-black text-white mb-2 tracking-wide">
                Crediti Smart Esauriti
              </h3>
              <p className="text-sm text-slate-300 leading-relaxed mb-4">
                L'analisi profonda dell'itinerario richiede crediti{' '}
                <strong>Gemini Pro (Smart)</strong>. Hai raggiunto il tuo limite giornaliero di{' '}
                <strong className="text-red-400">{quotaLimit}</strong> crediti.
              </p>

              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 text-xs text-slate-400">
                <p>
                  <strong>Vuoi continuare senza limiti?</strong>
                  <br />
                  Passa al piano <strong>Traveler Pro</strong> per sbloccare l'AI avanzata
                  illimitata.
                </p>
              </div>
            </div>
      ...
      {warningModal && (
        ...
        >
          ...
          />
          <div
            className="bg-slate-900 border-2 border-yellow-500 p-8 rounded-[2rem] w-full max-w-md shadow-2xl relative animate-in zoom-in-95 text-center flex flex-col items-center gap-6 pointer-events-auto"
            style={{ zIndex: Z_MODAL_NESTED }}
          >
            ...
            <div>
              <h3 className="text-2xl font-display font-black text-white mb-3 tracking-wide">
                {warningModal.title}
              </h3>
              <p className="text-base text-slate-300 leading-relaxed">{warningModal.message}</p>
            </div>
            <div className="flex gap-3 w-full">
              <button
                type="button"
                onClick={() => handleConfirmWarning(false)}
                className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold py-3 rounded-xl transition-colors uppercase tracking-wide"
              >
                Annulla
              </button>
              <button
                ...
                className="flex-1 bg-yellow-600 hover:bg-yellow-500 text-black font-bold py-3 rounded-xl transition-colors uppercase tracking-wide shadow-lg"
              >
                Procedi
              </button>
            </div>
        ...
        >
          ...
          >
            ...
            <button
              type="button"
              onClick={() => setErrorModal(null)}
              className="w-full bg-red-600 hover:bg-red-500 text-white font-bold py-3 rounded-xl transition-transform active:scale-95 uppercase tracking-widest shadow-xl"
            >
              Chiudi
            </button>
          </div>
      ...
      >
        ...
        <div className="flex justify-between items-center px-6 py-4 border-b border-slate-800 bg-[#0f172a]/80 backdrop-blur-md shrink-0 relative">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3
                className={
                  titleStyle || 'text-xl font-display font-bold text-white uppercase tracking-wider'
                }
              >
                Magic Planner AI
              </h3>
              <p className="text-slate-500 text-[9px] font-black uppercase tracking-[0.2em]">
                Touring Diary Intelligence
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar bg-slate-900 relative">
          {!aiSession.generatedPlan ? (
            <AiPlannerForm onGenerate={generatePlan} isLoading={loading} error={error} />
          ) : (
            <AiPlannerTimeline
