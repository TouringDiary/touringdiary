...
import type { CityDetails, CityService, PointOfInterest, SuggestionType, User } from '@/types';
...
export const CityServicesTab = ({
  ...
  const [activeServiceCategory, setActiveServiceCategory] = useState<string>(
    SERVICES_CATEGORIES[0]?.id || 'airport',
  );
  const [expandedServiceId, setExpandedServiceId] = useState<string | null>(null);
  const [servicesList, setServicesList] = useState<CityService[]>(city.details.services || []);
  ...
  const handleOpenSuggestion = (type: SuggestionType, prefilledName?: string) => {
    ...
    openModal('suggestion', {
      ...
      ...(activeModal ? { returnTo: activeModal, returnProps: modalProps } : {}),
    });
  };
