...
function operatorHasId(operator: CityTourOperator): boolean {
  return operator.id.length > 0;
}

function isSelectedOperator(
  operator: CityTourOperator,
  index: number,
  selectedId: string | null,
  selectedIndex: number | null,
): boolean {
  if (operatorHasId(operator)) {
    return selectedId === operator.id;
  }
  return selectedIndex === index;
}

function resolveSelectedOperator(
  list: CityTourOperator[],
  selectedId: string | null,
  selectedIndex: number | null,
): CityTourOperator | null {
  if (selectedId !== null) {
    return list.find((op) => op.id === selectedId) ?? null;
  }
  if (selectedIndex === null) return null;
  const operator = list[selectedIndex];
  if (!operator || operatorHasId(operator)) return null;
  return operator;
}

export const CityTourOperatorsTab = ({
  city,
  onAddToItinerary,
  user,
  onOpenAuth,
  ...
  const selectedOperator = resolveSelectedOperator(operatorsList, selectedId, selectedIndex);

  const filterSectionLabelStyle = useDynamicStyles('filter_section_title');

  useEffect(() => {
    const hasSelectionState = selectedId !== null || selectedIndex !== null;
    const resolved = resolveSelectedOperator(operatorsList, selectedId, selectedIndex);
    ...
    if (!hasSelectionState && isMobile) return;

    const first = operatorsList[0];
    if (!first) return;
    if (operatorHasId(first)) {
  ...
  }, [operatorsList, selectedId, selectedIndex, isMobile]);
  ...
  const renderOperatorDetail = (operator: CityTourOperator | null) => {
    ...
    const reviews: Review[] = Array.isArray(operator.reviews) ? operator.reviews : [];
    const isAdded = Boolean(operator.id) && isItemInItinerary(operator.id);
      ...
      <div className="h-full overflow-y-auto custom-scrollbar p-6 md:p-10 animate-in fade-in bg-[#020617] flex flex-col">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="flex flex-wrap gap-4 mt-6 justify-center md:justify-start">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-8 py-3 rounded-xl font-bold uppercase text-xs tracking-wide border transition-all active:scale-95 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed ${isAdded ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-cyan-600 hover:bg-cyan-500 border-cyan-500 text-white'}`}
              >
        ...
        <div className="border-t border-slate-800 pt-10 mb-8">
          <h3 className="text-2xl font-bold text-white flex items-center gap-3 mb-8">
            <MessageSquare className="w-6 h-6 text-cyan-500" /> Esperienze Clienti
          </h3>
          <div className="space-y-4">
            {reviews.length > 0 ? (
              ...
              <div className="text-center py-16 text-slate-500 italic bg-slate-900/30 rounded-3xl border border-slate-800 border-dashed">
        ...
        <div className="mt-auto pt-8 border-t border-slate-800 flex flex-col md:flex-row justify-between items-center gap-4 text-slate-500">
          <div className="text-[10px] max-w-sm flex items-start gap-2 leading-snug">
            ...
            <p>
              ...
              Touring Diary non è affiliato con questa attività se non indicato come Partner
              Ufficiale.
            </p>
          </div>
  ...
  return (
    <>
      <div
        className={`md:w-80 border-r border-slate-800 bg-[#0b0f1a] flex flex-col shrink-0 absolute md:relative inset-0 z-dropdown md:z-0 transition-transform duration-300 ${isMobile && mobileView === 'content' ? '-translate-x-full' : 'translate-x-0'}`}
      >
        <div className="h-full flex flex-col">
          <div
            className={`p-6 border-b border-slate-800 ${filterSectionLabelStyle} flex items-center gap-2`}
          >
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
            {operatorsList.length > 0 ? (
              operatorsList.map((op, index) => (
                <div
                  key={operatorHasId(op) ? op.id : index}
                  className={`w-full p-4 rounded-2xl flex items-center gap-3 transition-all ${isSelectedOperator(op, index, selectedId, selectedIndex) ? 'bg-cyan-600 text-white shadow-lg' : 'hover:bg-slate-800 text-slate-400 hover:text-white'}`}
                >
                  <button
                    type="button"
                    onClick={() => {
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <div
                        className={`text-[10px] uppercase font-bold truncate ${isSelectedOperator(op, index, selectedId, selectedIndex) ? 'text-cyan-200' : 'text-slate-600'}`}
                      >
                        {op.servicesOffered?.[0] || 'Agenzia Viaggi'}
                      </div>
                    </div>
                    <ChevronRight
                      className={`w-5 h-5 ml-auto opacity-50 shrink-0 ${isSelectedOperator(op, index, selectedId, selectedIndex) ? 'text-white' : ''}`}
                    />
                  </button>
                  {op.id ? (
                    <FavoriteBookmarkButton
                      ...
                      size="sm"
                      className={
                        isSelectedOperator(op, index, selectedId, selectedIndex)
              ...
              <div className="p-4 text-center text-slate-500 italic text-xs">
                Nessun Tour Operator in questa città.
              </div>
            )}
          </div>
        </div>
      ...
      <div
        className={`flex-1 bg-[#020617] flex flex-col min-w-0 absolute md:relative inset-0 z-floating-panel md:z-0 transition-transform duration-300 ${isMobile && mobileView !== 'content' ? 'translate-x-full' : 'translate-x-0'}`}
      >
