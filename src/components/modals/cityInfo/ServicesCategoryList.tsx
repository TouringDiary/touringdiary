...
import type { CityService, SuggestionType } from '@/types';
...
export const ServicesCategoryList = ({
  ...
  const getSpecificIcon = (service: CityService) => {
    ...
    if (activeServiceCategory === 'train') {
      ...
      if (
        nameLower.includes('circum') ||
        nameLower.includes('vesuviana') ||
        nameLower.includes('eav')
      ) {
      ...
      if (nameLower.includes('metro')) {
        return (
          <div className="w-8 h-8 bg-red-600 text-white rounded font-black flex items-center justify-center text-xl">
            M
          </div>
        );
      }
    ...
    <>
      ...
      <div className="w-full flex items-center justify-between gap-2 md:gap-4 mb-6 bg-slate-900/80 p-2 rounded-xl border border-slate-700/50 shadow-sm backdrop-blur-md">
        <div className="flex items-center gap-2 px-2 border-r border-slate-700/50 mr-1">
          ...
          <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest hidden md:inline">
            Contribuisci
          </span>
        </div>
        <div className="flex-1 flex justify-between items-center">
          <span className="text-[10px] text-slate-500 italic hidden md:block">
            Manca un servizio? Segnalalo!
          </span>
      ...
      <div className="flex items-center gap-5 mb-8 pb-4 border-b border-slate-800">
        {renderHeaderIcon()}
        <h3
          className={`text-3xl font-display font-bold uppercase tracking-wide ${activeCatInfo?.color}`}
        >
          {activeCatInfo?.label}
        </h3>
        <span className="ml-auto inline-flex items-center gap-1.5 text-xs font-normal bg-slate-800 text-slate-400 px-3 py-1 rounded-full">
          <CountBadge
            ...
            size="sm"
            variant="neutral"
            className="bg-slate-700 text-slate-300 border-0 min-w-[18px]"
          />
          Risultati
        </span>
      </div>
      ...
      {servicesList.length === 0 ? (
        <div className="text-center py-20 text-slate-500 italic bg-slate-900/30 rounded-2xl border border-slate-800 border-dashed mb-6">
          ...
          <button
            type="button"
            onClick={() => onOpenSuggestion('new_place')}
            className="text-indigo-400 underline mt-2 hover:text-white transition-colors"
          >
        ...
        <div className="space-y-4 mb-6">
          {servicesList.map((service) => {
            const hasPhone =
              service.contact &&
              (service.contact.startsWith('+') ||
                service.contact.startsWith('0') ||
                service.contact.startsWith('3') ||
                service.contact.startsWith('8') ||
                service.contact.length >= 3);
            ...
            return (
              <div
                key={service.id}
                className={`bg-slate-900 border rounded-2xl p-5 transition-all shadow-lg group ${isExpanded ? 'border-indigo-500 ring-1 ring-indigo-500/30' : 'border-slate-800 hover:border-slate-600'}`}
              >
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div className="flex items-start gap-4 min-w-0 flex-1">
                    <div
                      className={`p-3 rounded-xl bg-slate-950 border border-slate-800 ${activeCatInfo?.color}`}
                    >
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] bg-slate-800 text-slate-400 px-2 py-0.5 rounded border border-slate-700 uppercase font-bold">
                          {service.category || activeCatInfo?.label}
                        </span>
                        {!isExpanded && (
                          <span className="text-[10px] text-slate-500 italic flex items-center gap-1">
                            <Info className="w-3 h-3" /> Info disponibili
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2 w-full md:w-auto">
                    {hasPhone && (
                      <a
                        ...
                        className={`flex-1 md:flex-none flex items-center justify-center gap-2 text-white px-5 py-3 rounded-xl font-bold uppercase text-[10px] shadow-lg transition-transform active:scale-95 ${isEmergency ? 'bg-red-600 hover:bg-red-500 animate-pulse' : 'bg-emerald-600 hover:bg-emerald-500'}`}
                      >
                    ...
                    {hasUrl && service.url ? (
                      ...
                      <button
                        type="button"
                        disabled
                        className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-slate-800 text-slate-600 border border-slate-700 px-5 py-3 rounded-xl font-bold uppercase text-[10px] cursor-not-allowed opacity-70 relative overflow-hidden group/disabled"
                      >
                    ...
                    <button
                      ...
                      aria-label={
                        isExpanded ? 'Chiudi dettagli servizio' : 'Apri dettagli servizio'
                      }
                      className="min-h-11 min-w-11 shrink-0 flex items-center justify-center p-2 hover:bg-slate-800 rounded-full text-slate-500 hover:text-white transition-colors touch-manipulation"
                    >
                      {isExpanded ? (
                        <ChevronUp className="w-6 h-6 text-indigo-400" aria-hidden />
                      ) : (
