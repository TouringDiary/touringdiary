import { AlertCircle, ChevronRight, Filter, Plus, Siren } from 'lucide-react';
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
        {SERVICES_CATEGORIES.map((cat) => (
          ...
          >
            ...
            <ChevronRight
              className={`w-4 h-4 opacity-50 shrink-0 ${activeServiceCategory === cat.id ? 'text-white' : ''}`}
            />
          </button>
        ))}
      </div>
      ...
      <div className="p-4 border-t border-slate-800 bg-[#050b14] shrink-0">
        ...
        >
          <div className="flex items-center gap-3">
            <div
              className={`p-2 rounded-full ${activeServiceCategory === 'emergency' ? 'bg-white/20' : 'bg-red-500/20 group-hover:bg-white/20'} animate-pulse`}
            >
          ...
          <AlertCircle
            className={`w-5 h-5 ${activeServiceCategory === 'emergency' ? 'text-white' : 'opacity-50 group-hover:text-white'}`}
          />
        </button>
      </div>
    </div>
