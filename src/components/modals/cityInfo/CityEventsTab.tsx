...
import type { CityDetails, CityEvent, PointOfInterest } from '@/types';
...
export const CityEventsTab = ({
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
          ...
          {eventCats.map((cat) => (
            ...
            >
              ...
              <ChevronRight
                className={`w-5 h-5 opacity-50 ${activeEventFilter === cat ? 'text-white' : ''}`}
              />
            </button>
          ))}
        </div>
  ...
  const renderEventsContent = () => {
    ...
    const filteredEvents =
      activeEventFilter === 'Tutti'
        ? events
        : events.filter((e) => e.category === activeEventFilter);

    const handleAddEvent = (e: React.MouseEvent, item: CityEvent) => {
      e.stopPropagation();
      const poi: PointOfInterest = {
        ...
        imageUrl:
          item.imageUrl ||
          'https://images.unsplash.com/photo-1533174072545-e8d4aa97d848?q=80&w=400',
    ...
    return (
      <div className="h-full overflow-y-auto custom-scrollbar p-6 md:p-10 space-y-6 animate-in fade-in bg-[#020617]">
        ...
        {filteredEvents.map((item: CityEvent) => {
          ...
          return (
            <div
              key={item.id}
              className={`bg-slate-900 border rounded-2xl overflow-hidden transition-all duration-300 ${isExpanded ? 'border-rose-500 shadow-xl bg-slate-900' : 'border-slate-800 hover:border-slate-600'}`}
            >
              <div className="p-5 flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[10px] bg-rose-900/30 text-rose-300 border border-rose-500/20 px-2 py-0.5 rounded uppercase font-bold">
                      {item.category}
                    </span>
                  ...
                  <h4
                    className={`font-bold text-base md:text-lg truncate ${isExpanded ? 'text-white' : 'text-slate-200'}`}
                  >
                    {item.name}
                  </h4>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    aria-label={isExpanded ? 'Chiudi dettagli evento' : 'Apri dettagli evento'}
                    className="p-2 hover:bg-slate-800 rounded-full text-slate-500 hover:text-white transition-colors"
                  >
                    {isExpanded ? (
                      <ChevronUp className="w-6 h-6 text-rose-400" aria-hidden />
                    ) : (
        ...
        {filteredEvents.length === 0 && (
          <div className="text-center py-20 text-slate-500 italic bg-slate-900/30 rounded-2xl border border-slate-800 border-dashed">
  ...
  return (
    <>
      ...
      >
        <div className="flex-1 overflow-hidden relative">{renderEventsContent()}</div>
      </div>
    </>
  );
};
