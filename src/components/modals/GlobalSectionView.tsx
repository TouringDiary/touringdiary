...
/** Tab selezionabili via deep-link / props (sottoinsieme di CommunityTab). */
type CommunityInitialTab = 'live' | 'qa' | 'diari';
...
export const GlobalSectionView = ({
  ...
  isOpen = true,
  qaRestore,
}: GlobalSectionViewProps) => {
  const [activeTab, setActiveTab] = useState<CommunityTab>(initialTab);

  useEffect(() => {
    if (section === 'community' && initialTab) setActiveTab(initialTab);
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'live':
        return <LiveFeedTab user={user} onUserUpdate={onUserUpdate} onOpenAuth={onOpenAuth} />;
      case 'diari':
        return <ItinerariesExplorer user={user} onOpenAuth={onOpenAuth} />;
      case 'qa':
        return (
          <QaForumTab
            user={user}
      ...
      case 'partner':
        return (
          <div className="flex flex-col items-center justify-center h-64 text-slate-500 bg-slate-900/30 rounded-3xl border border-slate-800 border-dashed animate-in fade-in">
            ...
            <p className="max-w-sm text-center text-sm uppercase tracking-widest font-bold">
              In questa sezione appariranno tutti i nostri partner certificati.
            </p>
          </div>
        );
      default:
    ...
    <div className="flex flex-col md:flex-row md:items-center justify-between px-4 py-4 md:px-8 border-b border-slate-800 bg-[#0f172a] gap-4">
      ...
      <div className="flex items-center gap-3 overflow-x-auto no-scrollbar w-full md:w-auto pb-2 md:pb-0">
        {COMMUNITY_TABS.map((tab) => (
          ...
          >
            <tab.icon
              className={`w-4 h-4 ${activeTab === tab.id ? 'text-white' : 'text-slate-500'}`}
            />{' '}
            {tab.label}
          </button>
        ))}
  ...
  return (
    ...
    >
      <div
        className={`flex-1 relative min-h-0 bg-[#0b1120] ${contentLayout === 'fill' ? 'overflow-hidden flex flex-col' : 'overflow-y-auto custom-scrollbar'}`}
      >
        {renderContent()}
      </div>
    </BaseFullscreenModalShell>
