...
export const GlobalSectionView = ({ section, onClose, onUserUpdate, user, initialTab = 'live', initialSelectedPostId, onOpenAuth, isOpen = true }: GlobalSectionViewProps) => {
    ...
    return createPortal(
        <div className="td-modal-overlay bg-slate-950/95 backdrop-blur-md animate-in fade-in" onClick={onClose} style={{ zIndex: Z_OVERLAY }}>
            <div 
                className="relative bg-[#020617] w-full max-w-7xl h-full md:rounded-3xl border-0 md:border border-slate-700 shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 pointer-events-auto"
