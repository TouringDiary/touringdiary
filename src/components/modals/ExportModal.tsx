...
import { generateTextFile, generateWordDocument } from '../../utils/exportGenerators';
import { buildHeroCoverCollagePlan } from '../../utils/heroCoverCollagePlan';
import {
  logPdfImagePipeline,
  runWithPdfPipelineWarningCapture,
  summarizeProcessedImage,
} from '../../utils/pdfImagePipelineLog';
import {
  type CityVisualInfo,
  type PreparedItinerary,
  type PreparedItineraryItem,
  prepareItineraryForPdf,
} from '../../utils/pdfUtils';
    ...
    case 'timeline-item': {
      ...
      if (opts.details && block.item.poi?.description) {
        mainH += Math.min(
          ...
          PREVIEW_DESC_BASE_H +
            Math.ceil((block.item.poi.description.length || 0) / 70) * PREVIEW_DESC_LINE_H,
        );
      }
...
const packPreviewBlocksIntoPages = (
  ...
  for (const block of blocks) {
    ...
    const forceNewPage =
      (block.kind === 'day-title' && !block.continued) || block.kind === 'notes-title';
...
export const ExportModal = ({ isOpen, onClose }: ExportModalProps) => {
    ...
    try {
      ...
      logPdfImagePipeline({
        ...
        extra: {
          ...
          itemsWithProcessedImage: prepared.items.filter((item) => Boolean(item.processedImage))
            .length,
      ...
      setGenError(`Errore dati: ${e instanceof Error ? e.message : String(e)}`);
    } finally {
      setIsPreparing(false);
    }
  };
  ...
  const renderNoteInline = (node: DiaryNotesNode, key: string): React.ReactNode => {
    if (node.type !== 'text') {
      return (
        node.content?.map((child, index) => renderNoteInline(child, `${key}-${index}`)) ?? null
      );
    }
    ...
    let content: React.ReactNode =
      typeof href === 'string' && href !== node.text ? `${node.text ?? ''} (${href})` : node.text;
    ...
    if (typeof textColor === 'string') {
      content = (
        <span key={`${key}-c`} style={{ color: textColor }}>
          {content}
        </span>
      );
    }
  ...
  const renderNoteBlock = (node: DiaryNotesNode, key: string): React.ReactNode => {
    ...
    if (node.type === 'bulletList' || node.type === 'orderedList') {
      ...
      return (
        <ListTag
          key={key}
          className={`text-sm text-slate-700 mb-3 pl-5 ${node.type === 'orderedList' ? 'list-decimal' : 'list-disc'}`}
        >
          {node.content?.map((item, index) => (
            <li key={`${key}-${index}`} className="mb-1">
              {item.content?.map((child, childIndex) =>
                renderNoteBlock(child, `${key}-${index}-${childIndex}`),
              )}
            </li>
          ))}
        </ListTag>
        ...
        <div className="flex items-center justify-between">
          {logoBase64 ? (
            <img
              src={logoBase64}
              alt="TOURING Diary"
              className="h-12 max-w-[85%] object-contain object-left"
            />
          ) : (
            <h1 className="flex items-baseline gap-1.5 leading-none">
              ...
              <span className="font-handwriting text-3xl font-bold italic text-amber-500">
                Diary
              </span>
            </h1>
          )}
  ...
  const renderTimelineItemPreview = useCallback(
    (item: PreparedItineraryItem) => (
        ...
        <div className="flex min-w-0 flex-1 gap-2 pb-3">
          <div className="min-w-0" style={{ flex: PREVIEW_TIMELINE_MAIN_FLEX }}>
            ...
            <div className="text-[10px] font-bold uppercase text-amber-600">
              {item.poi.category || 'POI'}
            </div>
            <div className="text-sm font-bold text-slate-900">{item.poi.name}</div>
            {item.poi.address && (
              <div className="mt-0.5 text-[11px] italic text-slate-500">{item.poi.address}</div>
            )}
  ...
  const renderFlowBlock = useCallback(
    (block: PreviewFlowBlock, key: string) => {
      switch (block.kind) {
        case 'day-title':
          return (
            <h2
              key={key}
              className="mb-3 border-b-2 border-amber-600 pb-1.5 text-lg font-bold text-amber-600"
            >
        ...
        case 'resources-title':
          return (
            <h2
              key={key}
              className="mb-3 border-b-2 border-slate-300 pb-1.5 text-lg font-bold text-slate-800"
            >
              CONTATTI E RISORSE
            </h2>
          );
        case 'resource-row':
          return (
            <div
              key={key}
              className="grid grid-cols-3 gap-2 border-b border-slate-100 py-1.5 text-xs"
            >
          ...
          <div className="mb-auto">
            ...
            <div className="space-y-2">
              ...
              <OptionRow
                label="Note Timeline"
                icon={Edit3}
                active={options.notes}
                onClick={() => toggleOption('notes')}
                desc="Le tue annotazioni."
                disabled={!isExportOptionApplicable('notes', format)}
              />
              <OptionRow
                label="Foto Luoghi"
                icon={ImageIcon}
                active={options.photos}
                onClick={() => toggleOption('photos')}
                desc="Immagini nella timeline."
                disabled={!isExportOptionApplicable('photos', format)}
              />
              <OptionRow
                label="QR Luoghi"
                icon={QrCode}
                active={options.qrCodes}
                onClick={() => toggleOption('qrCodes')}
                desc="Link rapidi ai luoghi."
                disabled={!isExportOptionApplicable('qrCodes', format)}
              />
              <OptionRow
                label="Contatti"
                icon={Link}
                active={options.resources}
                onClick={() => toggleOption('resources')}
                desc="Link e contatti utili."
                disabled={!isExportOptionApplicable('resources', format)}
              />
              {shouldRenderExportOption('travelNotes', format) && (
                <OptionRow
                  label="Note Tab"
                  icon={Edit3}
                  active={options.travelNotes}
                  onClick={() => toggleOption('travelNotes')}
                  desc="Note del tab."
                />
              )}
            </div>
...
const OptionRow = ({ label, icon: Icon, active, onClick, desc, disabled }: OptionRowProps) => (
  ...
  >
    <div className="flex items-center gap-3">
      <div
        className={`rounded-lg p-1.5 ${active && !disabled ? 'bg-indigo-600 text-white' : 'bg-slate-950 text-slate-500'}`}
      >
        <Icon className="h-4 w-4" />
      </div>
      <div>
        <div
          className={`text-[14px] font-bold ${active && !disabled ? 'text-white' : 'text-slate-400'}`}
        >
          {label}
        </div>
    ...
    {!disabled && (
      <div
        className={`flex h-5 w-5 items-center justify-center rounded border ${active ? 'border-indigo-500 bg-indigo-600' : 'border-slate-700 bg-slate-950'}`}
      >
        {active ? (
