...
export const LevelUpModal = ({ isOpen, onClose, xp, onOpenRewards }: Props) => {
  ...
  return createPortal(
    <div
      className="td-modal-overlay pointer-events-auto flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in"
      ...
      >
        ...
        <div className="relative z-floating-panel p-8 flex flex-col items-center text-center">
          ...
          <p className="text-slate-400 text-sm mb-6">
            Hai raggiunto il <strong className="text-white">Livello {currentLevel.level}</strong>.{' '}
            <br />
            Sei ufficialmente un{' '}
            <span className={`font-bold ${currentLevel.color}`}>{currentLevel.name}</span>!
          </p>
          ...
          {rewardsEnabled ? (
            <button
              ...
              className="w-full min-h-[44px] bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-indigo-900/30 flex items-center justify-center gap-2 transition-all transform hover:scale-[1.02] active:scale-95"
            >
