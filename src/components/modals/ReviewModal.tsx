import { AlertCircle, Loader2, PenTool, Send, Star, Trophy } from 'lucide-react';
...
interface ReviewModalProps {
   ...
   */
  onSubmitSuccess?: (result: SaveUnifiedReviewResult) => void;
...
function isReviewFormDirty(
  ...
  return criteriaKeys.some((key) => (criteriaRatings[key] ?? 0) !== (initial.ratings[key] ?? 0));
}
...
export const ReviewModal = ({
  ...
  const handleSubmit = async () => {
    ...
    let saved: SaveUnifiedReviewResult;
    try {
      saved = await onSubmit(average, criteriaRatings, comment);
    } catch (err: unknown) {
  ...
  const canSubmit =
    reviewsEnabled && !isSubmitting && !Object.values(criteriaRatings).some((v) => v === 0);

  return createPortal(
    <div className={`td-modal-overlay ${overlayShell} !items-center`} style={{ zIndex: Z_OVERLAY }}>
      <button
        type="button"
        tabIndex={-1}
        aria-hidden="true"
            ...
            <div className="bg-gradient-to-r from-amber-500/10 to-indigo-500/10 border border-amber-500/20 p-3 rounded-lg flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-300">
                  ...
                  <strong className="text-emerald-400">+20 XP</strong> se scrivi una recensione
                  utile (min. 10 parole).
                </p>
              </div>
            </div>
