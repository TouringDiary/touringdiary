...
export const BuyCreditsModal: React.FC<BuyCreditsModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    const fetchPackages = async () => {
      ...
      try {
        ...
        setPackages(
          (data || []).map((row) => {
            const campaignFields = row as typeof row & {
            ...
            return {
              ...
              flash_credits: row.flash_credits,
              pro_credits: row.pro_credits,
              price_eur: row.price_eur,
              original_price_eur: campaignFields.original_price_eur ?? undefined,
              discount_badge: campaignFields.discount_badge ?? undefined,
  ...
  const handleBuy = async (pkg: Package) => {
    ...
    try {
      ...
      alert(
        message === 'LOCAL_MODE_DISABLED_FOR_PUBLIC'
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div className="bg-blue-500/5 border border-blue-500/10 p-4 rounded-2xl flex gap-3 items-center">
                  ...
                  <div>
                    <span className="font-black text-blue-400 text-[10px] uppercase tracking-widest block leading-none">
                      Flash Credits
                    </span>
                    <p className="text-[9px] text-slate-400 font-medium mt-1">
                      Risposte rapide e roadbook sintetici.
                    </p>
                  </div>
                </div>
                <div className="bg-violet-500/5 border border-violet-500/10 p-4 rounded-2xl flex gap-3 items-center">
                  ...
                  <div>
                    <span className="font-black text-violet-400 text-[10px] uppercase tracking-widest block leading-none">
                      Pro Credits
                    </span>
                    <p className="text-[9px] text-slate-400 font-medium mt-1">
                      Pianificazione avanzata e precisione.
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  {packages.map((pkg, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mb-4">
                          <span
                            className={`text-[11px] font-black uppercase tracking-[0.2em] ${typeColor}`}
                          >
                            {typeLabel}
                          </span>
                              ...
                              <>
                                <div className="flex items-center gap-2 mb-0.5">
                                  <span className="text-slate-500 line-through text-sm font-bold">
                                    €{pkg.original_price_eur?.toFixed(2)}
                                  </span>
                                  {pkg.discount_badge && (
                        ...
                        <div className="flex-1 flex flex-col justify-center mb-6">
                          ...
                          <div className="flex flex-col items-center">
                            ...
                            <span className="text-[9px] uppercase text-indigo-400 font-black tracking-[0.2em]">
                              Validità
                            </span>
                          </div>
                        </div>
        ...
        {purchaseEnabled ? (
          <div className="p-6 bg-slate-800/30 border-t border-slate-800 flex items-start gap-4">
            ...
            <p className="text-[9px] text-slate-500 leading-relaxed font-medium uppercase tracking-tight">
              I crediti acquistati vengono aggiunti al tuo saldo immediatamente dopo la conferma del
              pagamento. Validità garantita per un anno solare dall'acquisto.
            </p>
          </div>
        ) : null}
      </div>
