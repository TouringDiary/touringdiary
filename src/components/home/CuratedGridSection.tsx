import { ArrowUpRight, Calendar, Globe, Star, Sun } from 'lucide-react';
...
const MiniCityCard: React.FC<MiniCityCardProps> = ({ city, onClick, priority = false }) => {
  return (
    <button
      ...
      className="relative aspect-square w-full text-left rounded-lg overflow-hidden border border-slate-800 cursor-pointer group hover:border-amber-500/50 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/50"
    >
      <ImageWithFallback
        src={city.imageUrl}
        alt={city.name}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-2">
        <span className="text-sm md:text-base font-bold text-white block group-hover:text-amber-400 truncate">
          {city.name}
        </span>
      </div>
    </button>
...
export const CuratedGridSection = ({
  onCityClick,
  onExplore,
  cityManifest,
}: CuratedGridSectionProps) => {
  ...
  const getDisplayItems = (badge: string, count: number = 4) => {
    ...
    if (displayItems.length < count) {
      const fillers = cityManifest.filter(
        (c) => c.isFeatured && c.specialBadge !== badge && !displayItems.some((x) => x.id === c.id),
      );
      let i = 0;
  ...
  return (
    <div className="flex flex-col gap-6">
      <div
        className={`bg-slate-900/50 rounded-xl border border-slate-800 p-4 hover:border-slate-700 transition-colors w-full`}
      >
        <div className="flex items-center justify-between mb-4 pb-2 border-b border-slate-800">
          ...
          <button
            ...
            onClick={() =>
              onExplore(
                getModalList(topDestCategory.badge),
                topDestCategory.title,
                <topDestCategory.icon className={`w-5 h-5 ${topDestCategory.color}`} />,
              )
            }
            className="group/arrow inline-flex items-center justify-center min-h-11 min-w-11 p-0 hover:bg-slate-800 rounded transition-colors touch-manipulation"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-2">
          {topDestItems.map((city, cIdx) => (
            <div
              key={`${topDestCategory.badge}-${city.id}-${city.imageUrl || cIdx}`}
              className={`${cIdx >= 4 ? 'hidden md:block' : ''} ${cIdx >= 8 ? 'hidden lg:block' : ''}`}
            >
        ...
        {categories.map((cat, idx) => (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2">
              {(displayByBadge[cat.badge as keyof typeof displayByBadge] || []).map(
                (city, cIdx) => (
                  <MiniCityCard
                    key={`${cat.badge}-${city.id}-${city.imageUrl || cIdx}`}
                    city={city}
                    onClick={onCityClick}
                    priority={false}
                  />
