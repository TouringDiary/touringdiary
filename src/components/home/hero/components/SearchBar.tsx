...
import { HeroCompactInputInner, HeroCompactInputShell } from './HeroCompactInputShell';
...
export const SearchBar = ({
  ...
  return (
    <div
      className={`w-full relative ${showResults ? 'z-home-hero-popover' : ''} ${className}`}
      ref={containerRef}
    >
      {compact ? (
        <HeroCompactInputShell focused={isFocused}>
          <Search
            className={`w-3.5 h-3.5 mr-2 shrink-0 ${isFocused ? 'text-amber-500' : 'text-slate-500'}`}
          />
        ...
        <div
          className={`flex items-center transition-all h-full ${
            variant === 'minimal'
        ...
        >
          <Search
            className={`w-3.5 h-3.5 mr-3 shrink-0 ${isFocused ? 'text-amber-500' : 'text-slate-500'}`}
          />
          <input
            type="text"
            placeholder="Cerca una città"
