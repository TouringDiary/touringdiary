...
export const MultiFilterSelect = ({
  ...
  const displayText = selectedValues.length > 0 ? `${selectedValues.length} Selezionati` : 'Tutte';

  return (
    <div
      className={`relative min-w-0 group/select ${isOpen ? 'z-home-hero-popover' : ''}`}
      ref={containerRef}
    >
      <label
        className={`${labelStyle} absolute -top-1.5 left-2 bg-slate-900 px-1 z-home-hero-surface leading-none pointer-events-none`}
      >
        {label}
      </label>
      <div className="relative">
        ...
        {isOpen && (
          <div className="absolute top-full left-0 right-0 mt-1 bg-slate-900 border border-slate-700 rounded-lg shadow-2xl max-h-60 overflow-y-auto custom-scrollbar w-full min-w-[140px] animate-in fade-in zoom-in-95 duration-100 z-home-hero-popover">
            {options.map((opt) => {
