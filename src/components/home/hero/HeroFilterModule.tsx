...
interface HeroFilterModuleProps {
  ...
  resetContinent: () => void;
  handleNationChange: (value: string) => void;
  resetNation: () => void;
...
export const HeroFilterModule = (props: HeroFilterModuleProps) => {
  ...
  const stackedMode: HeroStackedMode =
    props.stackedMode ?? (isStackedLayout ? 'compact' : 'desktop');
    ...
    if (props.activeCategories.includes('equilibrato')) {
      return (
        <div className="flex items-center gap-1.5 bg-slate-900/50 px-2 py-0.5 rounded-lg border border-slate-700/50">
          <span className="text-sm cursor-help" title="Equilibrato">
            ⚖️
          </span>
        </div>
      );
    ...
    return (
      <div className="flex items-center gap-1.5 bg-slate-900/50 px-2 py-0.5 rounded-lg border border-slate-700/50">
        {visible.map((item, idx) => (
          <span key={idx} className="text-sm cursor-help" title={item.label}>
            {item.emoji}
          </span>
        ))}
  ...
  const showCitySearchPopover = props.isSearchFocused && props.manualCitySearch.trim().length > 0;
  ...
  const renderCompactTwin = () => (
    <div
      className={`h-full ${HERO_COMPACT.compactTwinStack} ${HERO_COMPACT.boxPadding} ${HERO_COMPACT.bodyGap}`}
    >
      <div
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          ...
          <h3 className={`${heroLabelStyle} drop-shadow-md truncate`}>Trova la tua meta</h3>
        </div>
        <div
  ...
  const renderFilterBody = (stacked: boolean) => (
    <>
      <div className={`flex flex-col ${stacked ? 'px-3 pt-2' : 'p-3 md:px-5 md:py-4'}`}>
        {!stacked && (
          ...
          >
            ...
            >
              <div className="lg:pointer-events-auto">{renderSummary()}</div>
            </div>
        ...
        <div
          className={`${stacked ? '' : 'mt-3.5'} ${showFullFilterContent && props.isFiltersExpanded ? 'flex' : showFullFilterContent ? 'hidden lg:flex' : 'hidden'}`}
        >
          <div className="grid grid-cols-2 md:grid-cols-3 gap-x-3 gap-y-3 lg:gap-y-4 w-full">
            <FilterSelect
              ...
              options={continents}
              onChange={props.handleContinentChange}
              onReset={props.resetContinent}
            />
            <FilterSelect
              label="Nazione"
              value={props.nation}
              options={nations}
              onChange={props.handleNationChange}
              onReset={props.resetNation}
            ...
            <FilterSelect
              ...
              onReset={props.resetRegion}
              disabled={!props.nation}
            />
            <FilterSelect
              label="Zona Turistica"
              value={props.selectedZone}
              options={props.geoOptions?.zones || []}
              onChange={props.handleZoneChange}
              onReset={props.resetZone}
              disabled={!props.region}
            />
            <FilterSelect
              label="Città"
              value={props.selectedCity}
              options={props.filteredCities.map((c) => ({ label: c.name, value: c.id }))}
              onChange={props.handleCityChange}
              onReset={props.resetCity}
              disabled={!props.region}
            />
            <FilterSelect
              label="Stagione"
              ...
              onChange={(value) => props.handleSeasonClick(value)}
              onReset={() => props.handleSeasonClick(props.selectedSeason || '')}
            />
          </div>
        </div>
      </div>
      ...
      <div
        className={`mt-auto bg-slate-950/60 backdrop-blur-md flex flex-col max-md:gap-2 md:flex-row items-stretch justify-between w-auto max-md:mx-3 lg:mx-6 xl:mx-10 max-md:mb-3 rounded-xl overflow-visible ${
          showFullFilterContent && props.isFiltersExpanded
            ? 'flex'
            : showFullFilterContent
              ? 'hidden lg:flex'
              : 'hidden'
        }`}
      >
                ...
                <div className="grid grid-cols-3 grid-rows-2 flex-1 min-w-0">
                  {TIPOLOGIA_OPTIONS.map((item) => {
                    ...
                    return (
                      ...
                      >
                        <item.icon
                          className={`w-3.5 h-3.5 transition-transform ${isActive ? 'scale-110' : 'group-hover:scale-110'}`}
                        />
                      </button>
                    );
                  })}
              ...
              <div className="flex items-center bg-slate-950/40 border border-slate-700/30 rounded-xl h-9 w-full transition-all hover:border-slate-600/50 shadow-inner">
                ...
                <div className="px-2 shrink-0 flex items-center justify-center">
                  <button
                    ...
                    aria-label="Suggerimento Shortlist AI"
                  >
              ...
              <div className="grid grid-cols-3 grid-rows-2 w-[120px] md:w-[150px] lg:w-[120px] xl:w-[180px]">
                {TIPOLOGIA_OPTIONS.map((item, idx) => (
                  <button
                    ...
                    title={item.label}
                    aria-label={item.label}
                    aria-pressed={props.activeCategories.includes(item.val)}
                  >
                    <item.icon
                      className={`w-3.5 h-3.5 transition-transform ${props.activeCategories.includes(item.val) ? 'scale-110' : 'group-hover:scale-110'}`}
                    />
                  </button>
                ))}
              </div>
  ...
  return (
    <div
      ...
      className={`min-w-0 relative group shadow-2xl flex flex-col min-h-0 bg-slate-900 border border-slate-800 rounded-2xl transition-all duration-300 ease-in-out overflow-visible ${
