...
export const HeroAiModule = (props: HeroAiModuleProps) => {
    ...
    const showCompactAi = isMobileCompact && !props.isAiExpanded;
    const showFullAiContent = props.isAiExpanded || !isMobileCompact;

    const renderHeader = (rowClass: string) => (
        <div
            className={`flex items-center justify-between cursor-pointer lg:cursor-default relative z-home-hero-surface shrink-0 ${rowClass}`}
            onClick={() => props.setIsAiExpanded(!props.isAiExpanded)}
        >
            <div className="flex items-center gap-2 min-w-0">
                <div className={`bg-purple-500 rounded-full shadow-[0_0_10px_rgba(168,85,247,0.5)] shrink-0 ${isMobileCompact ? 'w-1 h-5' : 'w-1.5 h-8'}`} />
                <h3 className={`${aiTitleStyle} truncate`}>Il Tuo Consulente</h3>
            </div>
            <div className="flex items-center gap-1.5 shrink-0">
                {!isMobileCompact && <Bot className="w-6 h-6 text-purple-500 animate-pulse" />}
                <div
                    className="lg:hidden p-1.5 bg-slate-800/50 rounded-full border border-white/10 text-white backdrop-blur-sm"
                    onClick={(e) => {
                        e.stopPropagation();
                        props.setIsAiExpanded(!props.isAiExpanded);
                    }}
                >
    ...
    return (
        <div
            id="tour-ai-button"
            className={`max-md:col-span-1 md:col-span-12 lg:col-span-5 min-w-0 relative rounded-2xl border border-slate-800 bg-slate-900 flex flex-col shadow-2xl transition-all duration-300 ease-in-out overflow-hidden ${
                isMobileCompact
                    ? `h-auto ${HERO_COMPACT.boxPadding}`
        ...
        >
            ...
            {showCompactAi ? (
                <div className={`relative z-home-hero-surface ${HERO_COMPACT.compactTwinStack} ${HERO_COMPACT.bodyGap}`}>
                    {renderHeader(HERO_COMPACT.headerRowAi)}
                    <div
                ...
                <>
                    {renderHeader(isMobileCompact ? 'h-8 mb-1' : 'h-8 mb-2')}

                    <div className={`relative z-home-hero-surface flex-1 flex flex-col min-h-0 justify-between ${isMobileCompact ? 'mt-2' : 'mt-4'} ${
