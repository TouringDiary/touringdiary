import { Bot, ChevronDown, ChevronUp, Loader2, MessageCircle, Send } from 'lucide-react';
...
import { HeroCompactInputInner, HeroCompactInputShell } from './components/HeroCompactInputShell';
...
export const HeroAiModule = (props: HeroAiModuleProps) => {
  ...
  const aiRuntimeBannerEl = aiBlocked ? (
    <AiRuntimeBanner status={props.aiRuntimeStatus} variant="inline" className="w-full" />
  ) : null;

  /** Inline slot for same-row placement (desktop / tablet compact). */
  const aiRuntimeBannerInline = aiRuntimeBannerEl ? (
    <div className="min-w-0 flex-1 overflow-hidden" onClick={(e) => e.stopPropagation()}>
      {aiRuntimeBannerEl}
    </div>
  ) : null;
  ...
  const renderCompactTwin = () => (
    <div
      className={`relative z-home-hero-surface h-full ${HERO_COMPACT.compactTwinStack} ${HERO_COMPACT.bodyGap}`}
    >
      ...
      >
        {props.isAiLoading ? (
          <div className="flex items-center gap-2 min-w-0 w-full text-slate-400">
            ...
            <span
              className={`${HERO_COMPACT.fieldText} text-purple-300 uppercase tracking-widest font-mono animate-pulse`}
            >
              Elaborazione...
            </span>
          </div>
  ...
  const renderAiBody = () => (
    <div
      className={`relative z-home-hero-surface flex-1 flex flex-col min-h-0 justify-between ${
        isStackedLayout ? 'px-3 pt-2 pb-3' : 'mt-4'
      } ${
        showFullAiContent && props.isAiExpanded
          ? 'flex'
          : showFullAiContent
            ? 'hidden lg:flex'
            : 'hidden'
    ...
    >
      {props.isAiLoading ? (
        <div className="flex-1 flex flex-col items-center justify-center text-slate-400 gap-3 bg-slate-950/50 rounded-xl border border-slate-800/50 min-h-[8rem]">
          ...
          <span className="text-xs font-mono animate-pulse text-purple-300 uppercase tracking-widest">
            Elaborazione...
          </span>
        </div>
      ) : props.aiResponse ? (
        <div className="flex-1 flex flex-col h-full">
          ...
          <button
            ...
            className="w-full py-3 bg-slate-800 hover:bg-slate-700 text-white rounded-xl text-xs font-bold uppercase tracking-wider transition-colors border border-slate-700 flex items-center justify-center gap-2 shadow-lg"
          >
        ...
        <>
          <div className="flex-1 flex flex-col justify-start mb-3 overflow-hidden min-h-[6rem]">
            ...
            <div
              onClick={() => {
                if (!props.isAiLoading && !aiBlocked) props.handleAiSubmit(props.typingText);
              }}
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative z-home-hero-surface flex flex-col min-h-0 ${stackedMode === 'compact' ? 'h-full' : ''}`}
      >
        {isStackedLayout ? (
          renderStacked()
        ) : (
