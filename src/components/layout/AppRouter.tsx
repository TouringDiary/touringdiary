...
const isPlanType = (value: string | undefined): value is PlanType =>
  typeof value === 'string' && Object.hasOwn(PLAN_TYPES, value);

// --- LAZY IMPORTS ---
const CityDetailContent = React.lazy(() =>
  import('../city/CityDetailContent').then((module) => ({ default: module.CityDetailContent })),
);
...
const CheckoutSuccessPage = React.lazy(() =>
  import('../features/checkout/CheckoutSuccessPage').then((module) => ({
  ...
  if (activeShopId) {
    ...
    const aroundMeTerritory =
      virtualCity?.virtualMode === 'around_me' || virtualCity?.id === 'around-me-virtual'
        ? virtualCity
        : null;
    const contextCityId = isAroundMeShop ? 'around-me-virtual' : activeCityId || 'napoli';
    const city = isAroundMeShop ? null : cityManifest.find((c) => c.id === contextCityId);
