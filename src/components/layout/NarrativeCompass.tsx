...
import type { JourneyPhase } from '@/types/index';
...
export const NarrativeCompass = ({ activeCityId }: NarrativeCompassProps) => {
  ...
  return (
    <div
      className={`w-full h-full flex flex-col items-center justify-center pointer-events-none select-none ${containerStyle}`}
    >
      ...
      <div className="md:hidden relative w-full h-full flex flex-col items-center justify-center gap-0.5 animate-in fade-in slide-in-from-top-2 duration-500">
        <span className={`${mobileTextStyle} drop-shadow-md leading-none`}>{currentPhase}</span>
            ...
            return (
              <div
                ...
                className={`
                                    ...
                                    ${mobileDotStyle}
                                    ${isActive ? 'w-6 h-1.5 opacity-100' : 'w-1.5 h-1.5 opacity-40'}
                                `}
              />
            );
          })}
