...
export const NewsTicker = ({ overrideSpeed, overrideItems, isVisible = true }: NewsTickerProps) => {
  ...
  return (
    ...
    >
      <div
        className={`${labelStyle} bg-amber-600 px-3 h-full flex items-center justify-center z-dropdown shadow-lg flex-shrink-0 relative border-r border-amber-700 max-md:!text-[10px]`}
      >
        NEWS
      </div>
      ...
      {maintenanceOn ? (
        <div className="flex-shrink-0 h-full flex items-center gap-2 px-3 border-r border-orange-500/40 bg-orange-950/50 max-w-[55%] sm:max-w-[40%]">
          ...
          <span
            className={`${textStyle} text-orange-100 truncate max-md:!text-[10px]`}
            ...
            <div className="flex items-center h-full">
              {newsItems.map((item, i) => {
                ...
                return (
                  <span
                    key={uniqueKey}
                    className={`mx-10 ${textStyle} flex items-center gap-2.5 max-md:!text-[10px]`}
                  >
