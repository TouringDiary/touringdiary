...
import {
  type CollaborationWorkspaceTarget,
  openCollaborationWorkspaceFlow,
} from '@/hooks/useOpenCollaborationWorkspace';
import { openMyWorldFlow } from '@/hooks/useOpenMyWorld';
import { useCityData } from '../../hooks/useCityData';
import type { ItineraryItem, PointOfInterest, User } from '../../types/index';
...
const ModalManagerClassic = () => {
  ...
  const { confirmAddToItinerary, resolveConflict, resolveDuplicate } =
  ...
  // 3. HANDLERS ADATTATI
  const resumeAfterIdentitySetup = useCallback(
    (u: User) => {
      ...
      if (pendingReturnTo === COLLABORATION_RETURN_TO) {
        closeModal();
        const resumeProps = pendingReturnProps;
        ...
        if (resumeProps?.intent === 'workspace') {
          openCollaborationWorkspaceFlow(u, openModal, {
            ...
            initialSection:
              resumeProps.initialSection as CollaborationWorkspaceTarget['initialSection'],
          });
          return;
        }
  ...
  const handleToggleItinerary = (poi: PointOfInterest) => {
    ...
    if (exists) {
      ...
      if (items.length === 1) removeItem(items[0].id);
        ...
        openModal('removeSelection', {
          items,
          onRemoveSingle: async (id: string) => {
            await removeItem(id);
            closeModal();
          },
          onRemoveAll: async () => {
            await handleRemoveAll(items);
          },
        });
    } else {
      openModal('add', { poi });
  ...
  const activeCitySummary = activeCityId ? cityManifest.find((c) => c.id === activeCityId) : null;
  ...
  const sharedProps: ModalManagerExternalProps = useMemo(
    () => ({
      ...
      onSetItineraryDates: (s, e) =>
        setItinerary((prev) => ({ ...prev, startDate: s, endDate: e })),
