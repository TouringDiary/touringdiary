...
const TravelDiary = React.lazy(() =>
  import('@/components/features/diary/TravelDiary').then((module) => ({
    default: module.TravelDiary,
  })),
);
...
export const Sidebar = ({
  ...
  useEffect(() => {
    ...
    fetchActiveSponsorsResolvedAsync(cityId).then((all) => {
      if (cancelled) return;
  ...
  useEffect(() => {
    setSponsorIndex((prev) => {
      if (goldSponsors.length === 0) return 0;
      return prev >= goldSponsors.length ? 0 : prev;
    });
  }, [goldSponsors.length]);
  ...
  useEffect(() => {
    if (goldSponsors.length > 1) {
      const interval = setInterval(
        () => setSponsorIndex((prev) => (prev + 1) % goldSponsors.length),
        10000,
      );
  ...
  const sidebarSponsor = useMemo(
    () => (goldSponsors.length === 0 ? null : goldSponsors[sponsorIndex % goldSponsors.length]),
    [goldSponsors, sponsorIndex],
  );
  const activeCity = useMemo(
    () => (activeCityId ? cityManifest.find((c) => c.id === activeCityId) : null),
    [activeCityId, cityManifest],
  );
      ...
      switch (activeContext) {
        case 'sea':
          sorted = sorted.filter(
            (c) =>
              c.tags?.includes('Mare') || c.zone.includes('Costiera') || c.zone.includes('Isole'),
          );
          break;
        case 'mountain':
          sorted = sorted.filter(
            (c) => c.tags?.includes('Entroterra') || c.zone.includes('Irpinia'),
          );
          break;
        case 'lakes':
          sorted = sorted.filter(
            (c) => c.tags?.includes('Natura') && c.zone.includes('Entroterra'),
          );
          break;
        case 'rivers':
          sorted = sorted.filter((c) => c.tags?.includes('Natura'));
          break;
        case 'villages':
          sorted = sorted.filter((c) => c.tags?.includes('Storia') && !c.isFeatured);
          break;
        case 'discovery':
          sorted = sorted.filter(
            (c) =>
              c.specialBadge === 'event' ||
              c.specialBadge === 'trend' ||
              c.tags?.includes('Novità'),
          );
          break;
        case 'summer':
          sorted = sorted.filter((c) => c.specialBadge === 'season' || c.tags?.includes('Mare'));
          break;
        case 'spring':
          sorted = sorted.filter((c) => c.tags?.includes('Natura') || c.tags?.includes('Fiori'));
          break;
        case 'winter':
          sorted = sorted.filter((c) => c.tags?.includes('Luci') || c.tags?.includes('Mistero'));
          break;
        case 'autumn':
          sorted = sorted.filter((c) => c.tags?.includes('Vino') || c.tags?.includes('Entroterra'));
          break;
      }
    }
    if (rankingSource === 'ai')
  ...
  const sponsorPoi = useMemo(
    () => (sidebarSponsor ? convertSponsorToPoi(sidebarSponsor) : null),
    [sidebarSponsor],
  );
  const isSponsorInItinerary = sponsorPoi
    ? itinerary.items.some((i) => i.poi.id === sponsorPoi.id)
    : false;
    ...
    return (
      <div className="fixed top-[var(--header-height)] left-0 right-0 bottom-16 z-dropdown bg-slate-950 p-4 flex flex-col overflow-hidden animate-in slide-in-from-bottom-5">
        <div className="flex justify-between items-center pb-4 border-b border-slate-800 mb-6 shrink-0">
          ...
          <button
            type="button"
            onClick={() => setMobileShowWeather?.(false)}
        ...
        <div className="flex-1 overflow-y-auto">
          {activeCityId && activeCity ? (
            ...
            <div className="text-center text-slate-500 py-20 italic">
              Seleziona una città per il meteo.
            </div>
          )}
        </div>
      </div>
      ...
      <div className="h-full min-h-0 flex flex-col overflow-hidden bg-slate-950">
        ...
        {sponsorPoi && (
          <div className="px-3 py-2 shrink-0 bg-slate-950 border-t-2 border-slate-800 shadow-[0_-15px_30px_rgba(0,0,0,0.8)]">
            <div className="bg-slate-900 rounded-2xl border border-amber-500/60 p-2 flex items-center gap-3 relative overflow-hidden group shadow-2xl">
              <button
                type="button"
                aria-label={`Apri dettagli ${sponsorPoi.name}`}
                onClick={() => openModal('poiDetail', { poi: sponsorPoi })}
              ...
              />
              <div className="w-10 h-10 rounded-xl overflow-hidden shrink-0 border border-slate-800 pointer-events-none relative z-[1]">
                <img
                  src={sponsorPoi.imageUrl}
                  className="w-full h-full object-cover"
                  alt={sponsorPoi.name}
                />
              </div>
              <div className="flex-1 min-w-0 pointer-events-none relative z-[1]">
                ...
                <div className="mb-0.5">
                  <StarRating value={sponsorPoi.rating} size="w-2 h-2" showValue={false} />
                </div>
                <h4 className="text-white font-bold text-sm truncate leading-none mb-1">
                  {sponsorPoi.name}
                </h4>
              </div>
              <button
                ...
                aria-label={
                  isSponsorInItinerary
                    ? 'Partner già in itinerario'
                    : "Aggiungi partner all'itinerario"
                }
                className={`relative z-[1] pointer-events-auto p-2 rounded-xl transition-all shadow-lg border flex items-center justify-center w-9 h-9 ${isSponsorInItinerary ? 'bg-emerald-600 border-emerald-500 text-white' : 'bg-amber-600 border-amber-500 text-white active:scale-95'}`}
              >
                {isSponsorInItinerary ? (
  ...
  return (
    <div
      ...
      className="hidden lg:flex flex-col h-full overflow-hidden px-3 pb-4 gap-3 relative"
    >
      ...
      >
        ...
        <div className="flex-shrink-0 h-auto">
          {activeCityId && activeCity ? (
            ...
            <div
              id="tour-sidebar-rankings"
              className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden shadow-sm flex flex-col h-[13rem] w-full"
            >
              <div className="flex border-b border-slate-800 bg-slate-950/50 flex-shrink-0">
                {(
                  [
                    { id: 'mix', label: 'Mix', icon: Star },
                  ...
                  <button
                    type="button"
                    key={opt.id}
                    onClick={() => setRankingSource(opt.id)}
                    className={`flex-1 py-2 text-[9px] font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 transition-colors ${rankingSource === opt.id ? 'bg-slate-800 text-amber-500 border-b-2 border-amber-500' : 'text-amber-500 hover:text-amber-400'}`}
                  >
                    <opt.icon
                      className={`w-3.5 h-3.5 ${rankingSource === opt.id ? 'text-amber-400' : 'text-slate-600'}`}
                    />{' '}
                    {opt.label}
                  </button>
                ))}
              ...
              <div className="flex flex-1 min-h-0">
                <div className="w-[55%] flex flex-col border-r border-slate-800 bg-slate-900">
                  <div className="px-3 py-1 border-b border-slate-800/50 flex justify-center items-center bg-slate-950/20 relative flex-shrink-0">
                    <span className="text-[9px] font-bold text-amber-500 uppercase tracking-widest text-center">
                      {externalZoneFilter ? 'TOP 5 ZONA' : 'TOP 5'}
                    </span>
                    <button
                      type="button"
                      onClick={onOpenFullRankings}
                      aria-label="Apri classifiche complete"
                      className="group/arrow p-1 hover:bg-slate-800 rounded absolute right-1 top-0.5"
                    >
                      <ArrowUpRight className="w-3.5 h-3.5 text-slate-600 group-hover/arrow:text-amber-500" />
                    </button>
                  </div>
                  <div className="flex-1 overflow-y-auto custom-scrollbar">
                    {topCities.map((c, i) => (
                      <button
                        ...
                        className="w-full text-left grid grid-cols-[1.2rem_1fr_auto_auto] gap-2 items-center px-3 py-1 hover:bg-slate-800/50 transition-colors cursor-pointer group border-b border-slate-800/30 last:border-0 h-7"
                      >
                        <span
                          className={`text-[9px] font-black ${i < 3 ? 'text-amber-500' : 'text-slate-600'}`}
                        >
                          #{i + 1}
                        </span>
                        <span
                          className="text-[9px] font-bold text-slate-200 truncate min-w-0"
                          title={c.name}
                        >
                          {c.name}
                        </span>
                        <span className="text-[8px] font-bold text-slate-500 font-mono text-right w-[22px]">
                ...
                <div className="w-[45%] flex flex-col bg-slate-900 h-full">
                  <div className="flex-[1.2] flex flex-col min-h-0">
                    <div className="text-[9px] uppercase font-bold text-amber-500 text-center py-1 bg-slate-950/30 flex-shrink-0 tracking-widest border-b border-slate-800/30">
                      <span>{hoverInspiration || 'ISPIRAZIONE'}</span>
                    </div>
                    <div className="grid grid-cols-3 grid-rows-2 flex-1 border-slate-800">
                      {[
                        { id: 'sea', label: 'Mare', icon: Waves, color: 'text-cyan-400' },
                        { id: 'mountain', label: 'Monti', icon: Mountain, color: 'text-stone-400' },
                        { id: 'lakes', label: 'Laghi', icon: Droplets, color: 'text-blue-400' },
                        { id: 'rivers', label: 'Fiumi', icon: Wind, color: 'text-sky-300' },
                        { id: 'villages', label: 'Borghi', icon: Castle, color: 'text-amber-600' },
                        {
                          id: 'discovery',
                          label: 'Novità',
                          icon: Sparkles,
                        ...
                        <button
                          key={f.id}
                          type="button"
                          onClick={() => setActiveContext(activeContext === f.id ? null : f.id)}
                          onMouseEnter={() => setHoverInspiration(f.label)}
                          onMouseLeave={() => setHoverInspiration(null)}
                          aria-label={f.label}
                          aria-pressed={activeContext === f.id}
                          className={`flex items-center justify-center min-h-0 w-full h-full border-r border-b border-slate-800 ${activeContext === f.id ? 'bg-slate-800' : 'bg-slate-900 hover:bg-slate-800 text-slate-500 hover:text-slate-300'}`}
                        >
                          <f.icon
                            className={`w-3.5 h-3.5 ${activeContext === f.id ? f.color : ''}`}
                          />
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="flex-1 flex flex-col min-h-0 border-t border-slate-800">
                    <div className="text-[9px] uppercase font-bold text-amber-500 text-center py-1 bg-slate-950/30 flex-shrink-0 tracking-widest border-b border-slate-800/30">
                      <span>{hoverSeason || 'STAGIONE'}</span>
                    </div>
                    <div className="grid grid-cols-2 grid-rows-2 flex-1">
                      {[
                        { id: 'summer', label: 'Estate', icon: Sun, color: 'text-amber-400' },
                        { id: 'spring', label: 'Primav.', icon: Flower, color: 'text-emerald-400' },
                        { id: 'winter', label: 'Inverno', icon: Snowflake, color: 'text-cyan-400' },
                        { id: 'autumn', label: 'Autunno', icon: Leaf, color: 'text-orange-500' },
                      ].map((f) => (
                        <button
                          key={f.id}
                          type="button"
                          onClick={() => setActiveContext(activeContext === f.id ? null : f.id)}
                          onMouseEnter={() => setHoverSeason(f.label)}
      ...
      >
        {isCompanionPortaled ? (
                ...
                <div className="px-3 pb-4">
                  <div className="relative h-20 overflow-hidden">
                    {sponsorPoi && sidebarSponsor ? (
                      <div className="bg-slate-900 rounded-xl border border-amber-500 hover:border-amber-400 transition-all overflow-hidden relative group h-20 shadow-lg animate-in fade-in">
                        <button
                          type="button"
                          aria-label={`Apri dettagli ${sponsorPoi.name}`}
                          onClick={() => openModal('poiDetail', { poi: sponsorPoi })}
                          className="absolute inset-0 z-0 cursor-pointer border-0 bg-transparent p-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 focus-visible:ring-inset"
                        />
                        <ImageWithFallback
                          src={sponsorPoi.imageUrl}
                          alt=""
                        ...
                        />
                        <div
                          className="absolute inset-0 bg-gradient-to-r from-slate-950 via-slate-950/50 to-transparent pointer-events-none"
                          aria-hidden
                        />
                        <div className="absolute top-1.5 right-1.5 z-local-overlay pointer-events-none">
                          <span className="bg-gradient-to-r from-amber-200 via-yellow-400 to-amber-500 text-black text-[8px] font-bold px-1.5 py-0.5 rounded shadow-lg uppercase tracking-normal flex items-center gap-0.5 border border-yellow-100">
                            <Award className="w-2.5 h-2.5" aria-hidden /> SPONSOR
                          </span>
                        </div>
                        <div className="absolute top-1.5 left-3 z-local-overlay pr-16 max-w-full pointer-events-none">
                          <div className="mb-0.5">
                            <StarRating
                              value={sponsorPoi.rating}
                              size="w-2.5 h-2.5"
                              showValue={false}
                            />
                          </div>
                        ...
                        <div className="absolute bottom-2 right-2 z-dropdown flex items-center gap-1 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 lg:group-focus-within:opacity-100 transition-opacity">
                          <button
                            type="button"
                            className="hidden lg:flex p-1.5 rounded-lg bg-black/60 hover:bg-indigo-600 text-white cursor-grab active:cursor-grabbing border border-white/20 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400"
                            draggable="true"
                            onDragStart={(e) => {
                              e.stopPropagation();
                          ...
                          <button
                            ...
                            className={`rounded-lg border transition-colors flex items-center justify-center min-h-11 min-w-11 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 ${isSponsorInItinerary ? 'bg-emerald-600 border-emerald-500 text-white' : 'bg-amber-600 hover:bg-amber-500 border-amber-500 text-white'}`}
                          >
                            {isSponsorInItinerary ? (
                      ...
                      <AdPlaceholder className="h-16" onClick={onOpenSponsor} />
                    )}
                  </div>
                </div>
              </div>,
          ...
          <>
            ...
            <div
              id="tour-sponsor-box"
              className="mt-auto flex-shrink-0 relative overflow-hidden h-20"
            >
              {sponsorPoi && sidebarSponsor ? (
                <div className="bg-slate-900 rounded-xl border border-amber-500 hover:border-amber-400 transition-all overflow-hidden relative group h-20 shadow-lg animate-in fade-in">
                  <button
                  ...
                  <div className="absolute top-1.5 left-3 z-local-overlay pr-16 max-w-full pointer-events-none">
                    <div className="mb-0.5">
                      <StarRating value={sponsorPoi.rating} size="w-2.5 h-2.5" showValue={false} />
                    </div>
                    <h4 className="text-white font-bold text-sm leading-tight truncate">
                      {sponsorPoi.name}
                    </h4>
