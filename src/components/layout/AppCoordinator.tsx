...
const RemoveItemModal = React.lazy(() =>
  import('../modals/RemoveItemModal').then((module) => ({ default: module.RemoveItemModal })),
);
...
export const AppCoordinator = () => {
  ...
  const renderLayout = () => {
    ...
    return <MainLayout helpFlash={helpFlash} onCompleteOnboarding={handleCompleteOnboarding} />;
  };
  ...
  return (
    <FocusModeProvider>
      ...
      {activeModal === 'removeSelection' &&
        modalProps?.items &&
        modalProps?.onRemoveSingle &&
        modalProps?.onRemoveAll && (
