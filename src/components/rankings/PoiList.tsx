...
import type { PointOfInterest, RankedItemMixin } from '../../types/index';
    ...
    <div className="space-y-3 p-6">
      {pois.map((poi, idx) => (
        <button
          ...
          key={poi.id}
          onClick={() => onClick(poi)}
          aria-label={`Apri ${poi.name}`}
          className="flex items-center gap-4 w-full text-left bg-slate-900/50 hover:bg-slate-900 p-3 rounded-xl border border-slate-800 transition-colors group cursor-pointer hover:border-indigo-500/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/50"
        >
          <div className="w-10 text-center font-black text-slate-500 text-lg group-hover:text-white transition-colors">
            #{idx + 1}
          </div>
          ...
          <div className="flex-1 min-w-0">
            <h4 className="text-white font-bold truncate group-hover:text-indigo-400 transition-colors text-base">
              {poi.name}
            </h4>
