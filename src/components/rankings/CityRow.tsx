import { Crown, Maximize2, Medal } from 'lucide-react';
import type React from 'react';
import type { CityAlgo } from '../../hooks/useRankingsLogic';
import type { CitySummary, RankedItemMixin } from '../../types/index';
import { formatVisitors } from '../../utils/common';
import { ImageWithFallback } from '../common/ImageWithFallback';
import { StarRating } from '../common/StarRating';

interface CityRowProps {
  /** coords opzionale: ranking RPC può omettere geo senza invalidare la riga. */
  item: Omit<CitySummary, 'coords'> & RankedItemMixin & { coords?: CitySummary['coords'] };
  originalRank: number;
  type: CityAlgo;
  onNavigate: () => void;
...
export const CityRow: React.FC<CityRowProps> = ({
  ...
  if (originalRank === 1)
    rankIcon = <Crown className="w-5 h-5 text-yellow-400 fill-yellow-400/20" />;
  ...
  return (
    <tr
      className={`transition-colors cursor-pointer group border-b border-slate-800/50 ${rowBg} ${borderClass}`}
      onClick={onNavigate}
    >
      ...
      <td className="px-4 py-3">
        <div className="font-bold text-white text-base group-hover:text-indigo-400 transition-colors">
          {item.name}
        </div>
      </td>
      <td className="px-4 py-3">
      ...
      <td className="px-4 py-3">
        {item.specialBadge ? (
          <span
            className={`text-[9px] font-black uppercase px-2 py-0.5 rounded border tracking-wider whitespace-nowrap
                        ${
                          item.specialBadge === 'event'
                            ? 'bg-rose-900/20 text-rose-400 border-rose-500/30'
                            : item.specialBadge === 'trend'
                              ? 'bg-blue-900/20 text-blue-400 border-blue-500/30'
                              : item.specialBadge === 'season'
                                ? 'bg-emerald-900/20 text-emerald-400 border-emerald-500/30'
                                : item.specialBadge === 'destination'
      ...
      <td className="px-4 py-3 text-right">
        <div className="flex justify-end items-center gap-1.5">
          <span
            className={`text-sm font-bold ${type === 'ai' ? 'text-emerald-400' : 'text-white'}`}
          >
          ...
          <StarRating
            ...
            size="w-3 h-3"
            showValue={false}
            activeColor={type === 'ai' ? 'text-emerald-500' : 'text-amber-400'}
            fillColor={type === 'ai' ? 'fill-emerald-500' : 'fill-amber-400'}
          />
        </div>
      </td>
    </tr>
