...
export const PhotoAcquireDialog: React.FC<Props> = ({
  ...
  return createPortal(
    <div
      ...
      role="presentation"
    >
      {/* Sibling backdrop dismiss (never onClick on td-modal-overlay). */}
      <button
        type="button"
        tabIndex={-1}
        aria-label="Chiudi"
        ...
        onClick={onClose}
      />
      {/* Dialog above backdrop (Foundation bottom sheet). */}
      <div
        className={`relative ${containerShell} w-full max-w-sm outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/50 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900`}
      ...
      >
        ...
        <div className={`${bodyShell} space-y-3 pb-6`}>
          <button
            ...
            onClick={onCapture}
            disabled={!canCapture}
            title={canCapture ? 'Apri la fotocamera' : 'Disponibile su smartphone e tablet'}
          ...
          >
            ...
            <span className="min-w-0">
              <span className="block text-sm font-bold text-white">Scegli dalla galleria</span>
              <span className="block text-[10px] text-slate-500 mt-0.5">
