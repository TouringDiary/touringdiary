...
export const PhotoRow: React.FC<PhotoRowProps> = ({ photo, manifest, isSuperAdmin, actions }) => {
  ...
  return (
    <tr
      className={`hover:bg-slate-800/30 transition-colors group text-xs ${isOrphan ? 'bg-red-950/10' : ''}`}
    >
      <td className="px-4 py-3 text-center align-middle w-20">
        <button
          type="button"
          className="w-12 h-9 bg-slate-800 rounded mx-auto overflow-hidden border border-slate-700 cursor-pointer hover:border-indigo-500 transition-all shadow-sm group-hover:scale-110 relative block p-0"
          title={displayUrl}
      ...
      <td className="px-4 py-3 align-middle">
        <div className="flex flex-col">
          <span
            className={`text-xs font-bold mb-0.5 flex items-center gap-1.5 ${isOrphan ? 'text-red-400 line-through decoration-red-500/50' : 'text-white'}`}
          >
            {photo.locationName}
            {displayUrl && (
              <a
          ...
          {cityInfo ? (
            <div className="flex gap-1 mb-1 opacity-70">
              <span className="text-[8px] bg-slate-800 text-slate-400 px-1.5 rounded uppercase font-mono">
                {cityInfo.zone}
              </span>
            </div>
          ) : isOrphan ? (
            ...
            <div className="flex gap-1 mb-1">
              <span className="text-[8px] bg-slate-800 text-slate-500 px-1.5 rounded uppercase font-mono italic">
                Zona N/D
              </span>
            </div>
          )}
          ...
          <div className="flex items-center gap-2 text-[10px] text-slate-500">
            ...
            <span className="font-mono truncate max-w-[100px] opacity-60" title={photo.url}>
              {filename}
            </span>
          </div>
          {photo.description && (
        ...
        <div className="flex items-center justify-end gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
          {photo.status === 'pending' && (
            <>
              <button
                type="button"
                onClick={() => actions.onStatusUpdate(photo.id, 'approved')}
                className="p-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg shadow-lg transition-transform active:scale-95"
                title="Approva"
              >
              ...
              <button
                type="button"
                onClick={() => actions.onStatusUpdate(photo.id, 'rejected')}
                className="p-1.5 bg-slate-800 hover:bg-red-600 text-slate-400 hover:text-white rounded-lg transition-colors border border-slate-700 hover:border-red-500"
                title="Rifiuta"
              >
          ...
          {photo.status === 'city_deleted' && (
            <button
              type="button"
              onClick={() => actions.onStatusUpdate(photo.id, 'approved')}
              className="p-1.5 bg-slate-800 hover:bg-emerald-600 text-slate-400 hover:text-white rounded-lg transition-colors border border-slate-700 hover:border-emerald-500 flex items-center gap-1"
              title="Ripristina (Se città esiste)"
            >
          ...
          {photo.status !== 'pending' && photo.status !== 'city_deleted' && (
            <button
              type="button"
              onClick={() => actions.onStatusUpdate(photo.id, 'pending')}
              className="p-1.5 hover:bg-blue-900/30 text-slate-500 hover:text-blue-400 rounded transition-colors"
              title="Ripristina"
            >
          ...
          <button
            type="button"
            onClick={() => actions.onOpenMetadata(photo)}
            className="p-1.5 hover:bg-indigo-900/30 text-slate-500 hover:text-indigo-400 rounded transition-colors"
            title="Modifica Dati"
          >
          ...
          {isSuperAdmin && (
            <button
              type="button"
              onClick={() => actions.onDeleteRequest(photo)}
              className="p-1.5 hover:bg-red-900/20 text-slate-600 hover:text-red-500 rounded transition-colors"
