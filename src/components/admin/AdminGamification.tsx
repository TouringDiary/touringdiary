...
import { Z_ADMIN_MODAL, Z_ADMIN_MODAL_NESTED, Z_ADMIN_MODAL_TOP } from '@/constants/zIndex';
  ...
  return (
    ...
    >
      ...
      <div className="p-4 flex justify-between items-start">
        ...
        <span
          className={`text-[9px] font-black px-2 py-1 rounded-full uppercase tracking-wide border bg-emerald-900/20 text-emerald-500 border-emerald-500/30`}
        >
      ...
      <div className="px-4 flex-1">
        ...
        <p className="text-xs text-slate-500 line-clamp-3 leading-relaxed">
          {reward.description || 'Descrizione premio...'}
        </p>
      </div>
...
export const AdminGamification = () => {
  ...
  const handleSaveRule = async () => {
    ...
    try {
      ...
      console.error(error);
      alert(
        `Errore salvataggio regola: ${(error instanceof Error ? error.message : String(error)) || 'Sconosciuto'}`,
      );
    } finally {
      setIsSaving(false);
    }
  ...
  const handleSaveReward = async () => {
    ...
    try {
      ...
      if (
        ...
        alert(
          "ERRORE PERMESSI (42501): Il database ha bloccato la modifica.\n\nEsegui lo script SQL di sblocco permessi ('Fix Permessi Catalogo') nella chat per risolvere.",
        );
      } else {
        alert(
          `Errore salvataggio premio: ${(error instanceof Error ? error.message : String(error)) || 'Sconosciuto'}`,
        );
      }
    } finally {
      setIsSaving(false);
  ...
  const executeDeleteReward = async () => {
    ...
    try {
      ...
      if (
        ...
        alert(
          `Errore eliminazione: ${(error instanceof Error ? error.message : String(error)) || 'Sconosciuto'}`,
        );
      }
    } finally {
      setIsSaving(false);
    ...
    <div className="space-y-6 h-full flex flex-col animate-in fade-in relative">
      ...
      <div className="flex bg-slate-900 p-1.5 rounded-xl border border-slate-800 w-fit shrink-0">
        <button
          ...
          className={`px-6 py-2.5 rounded-lg text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeTab === 'rules' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}
        >
        ...
        <button
          ...
          className={`px-6 py-2.5 rounded-lg text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeTab === 'rewards' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}
        >
        ...
        {activeTab === 'rules' && (
          <div className="p-6">
            ...
            <div className="mt-6 p-5 bg-slate-950 border border-slate-800 rounded-2xl flex gap-4 text-sm text-slate-400 shadow-inner">
              ...
              <div>
                <h4 className="font-black text-amber-400 mb-2 uppercase tracking-wide text-base">
                  Come funzionano le Regole?
                </h4>
                <p className="leading-relaxed">
                  Il database salva i valori (etichetta e punti XP). Tuttavia, l'azione che scatena
                  l'assegnazione dei punti (es. "utente carica foto") deve essere programmata nel
                  codice dell'app dallo sviluppatore usando la{' '}
                  <code className="bg-slate-800 px-1 py-0.5 rounded text-indigo-400 font-mono">
                    Key Sistema
                  </code>
                  . <br />
                  <strong className="text-slate-300">
                    Creare una regola qui non la rende "magicamente" attiva nell'app se non c'è il
                    codice che la chiama.
                  </strong>
                </p>
              </div>
            </div>
          ...
          <div className="p-6 space-y-10">
            <div className="flex justify-between items-center mb-6 border-b border-slate-800 pb-4">
              ...
              <button
                ...
                className="bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-lg font-bold text-xs uppercase flex items-center gap-2 shadow-lg transition-all"
              >
              ...
              return (
                <div key={level} className="relative">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                      <h4
                        className={`text-lg font-black uppercase tracking-wider ${levelInfo.color}`}
                      >
                  ...
                  {rewardsList.length === 0 ? (
                    <div className="text-slate-600 text-xs italic ml-14 mb-4">
                      Nessun premio configurato per questo livello.
                    </div>
                  ) : (
                        ...
                        <div key={reward.id} className="group relative">
                          ...
                          <div className="absolute top-2 right-2 flex gap-1 z-dropdown opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              ...
                              className="p-1.5 bg-indigo-600 text-white rounded shadow hover:bg-indigo-500"
                            >
                            ...
                            <button
                              ...
                              className="p-1.5 bg-red-600 text-white rounded shadow hover:bg-red-500"
                            >
              ...
              >
                <div className="flex justify-between items-center p-5 border-b border-slate-800 bg-[#0f172a]">
                  <h3 className="text-lg font-bold text-white flex items-center gap-2">
                    <Zap className="w-5 h-5 text-amber-500" /> Modifica Regola XP
                  </h3>
                  <button
                    type="button"
                ...
                <div className="p-6 space-y-4">
                  <div>
                    <label
                      htmlFor="fld-admin-admingamification-tsx-l529"
                      className="text-xs font-bold text-slate-500 uppercase block mb-1"
                    >
                      Key Sistema
                    </label>
                    <input
                      id="fld-admin-admingamification-tsx-l529"
                      value={editingRule.key}
                      onChange={(e) => setEditingRule({ ...editingRule, key: e.target.value })}
                      className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-white font-mono text-sm outline-none focus:border-indigo-500"
                      disabled={!editingRule.key.startsWith('custom')}
                    />
                  </div>
                  <div>
                    <label
                      htmlFor="fld-admin-admingamification-tsx-l540"
                      className="text-xs font-bold text-slate-500 uppercase block mb-1"
                    >
                      Etichetta
                    </label>
                    <input
                      id="fld-admin-admingamification-tsx-l540"
                      value={editingRule.label}
                      onChange={(e) => setEditingRule({ ...editingRule, label: e.target.value })}
                      className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-white font-bold outline-none focus:border-indigo-500"
                    />
                  </div>
                  <div>
                    <label
                      htmlFor="fld-admin-admingamification-tsx-l550"
                      className="text-xs font-bold text-slate-500 uppercase block mb-1"
                    >
                      XP Assegnati
                    </label>
                    <input
                      id="fld-admin-admingamification-tsx-l550"
                      type="number"
                      value={editingRule.xp}
                      onChange={(e) =>
                        setEditingRule({ ...editingRule, xp: parseInt(e.target.value, 10) })
                      }
                      className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-amber-500 font-black text-lg outline-none focus:border-indigo-500"
                    />
                  </div>
                  <button
                    type="button"
                    ...
                    disabled={isSaving}
                    className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3.5 rounded-xl shadow-lg mt-2 flex items-center justify-center gap-2 transition-all active:scale-95"
                  >
                    {isSaving ? (
              ...
              >
                ...
                <div className="w-full md:w-[60%] p-6 md:p-8 overflow-y-auto custom-scrollbar border-r border-slate-800 bg-[#0f172a]">
                  <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-bold text-white flex items-center gap-2">
                      <Gift className="w-5 h-5 text-emerald-500" /> Editor Premio
                    </h3>
                  </div>
                  ...
                  <div className="space-y-5">
                    <div>
                      <label
                        htmlFor="fld-admin-admingamification-tsx-l603"
                        className="text-[10px] font-bold text-slate-500 uppercase block mb-1"
                      >
                        Titolo
                      </label>
                      <input
                        id="fld-admin-admingamification-tsx-l603"
                        value={editingReward.title}
                        onChange={(e) =>
                          setEditingReward({ ...editingReward, title: e.target.value })
                        }
                        className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-white font-bold outline-none focus:border-emerald-500"
                      />
                    </div>
                    <div>
                      <label
                        htmlFor="fld-admin-admingamification-tsx-l615"
                        className="text-[10px] font-bold text-slate-500 uppercase block mb-1"
                      >
                        Descrizione
                      </label>
                      <textarea
                        id="fld-admin-admingamification-tsx-l615"
                        value={editingReward.description}
                        onChange={(e) =>
                          setEditingReward({ ...editingReward, description: e.target.value })
                        }
                        className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-white text-sm outline-none resize-none h-24 focus:border-emerald-500"
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label
                          htmlFor="fld-admin-admingamification-tsx-l629"
                          className="text-[10px] font-bold text-slate-500 uppercase block mb-1"
                        >
                          Livello Richiesto
                        </label>
                        <select
                          id="fld-admin-admingamification-tsx-l629"
                          value={editingReward.requiredLevel}
                          onChange={(e) =>
                            setEditingReward({
                              ...editingReward,
                              requiredLevel: parseInt(e.target.value, 10),
                            })
                          }
                          className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-white outline-none focus:border-emerald-500 cursor-pointer"
                        >
                          {LEVELS.map((l) => (
                      ...
                      <div>
                        <label
                          htmlFor="fld-admin-admingamification-tsx-l650"
                          className="text-[10px] font-bold text-slate-500 uppercase block mb-1"
                        >
                          Categoria
                        </label>
                        <select
                          id="fld-admin-admingamification-tsx-l650"
                    ...
                    <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                      ...
                      <div className="flex gap-2">
                        <button
                          type="button"
                          onClick={() => setEditingReward({ ...editingReward, type: 'internal' })}
                          className={`flex-1 py-2 text-xs font-bold rounded-lg border transition-all ${editingReward.type === 'internal' ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-slate-900 text-slate-400 border-slate-700'}`}
                        >
                          Interno
                        </button>
                        <button
                          type="button"
                          onClick={() => setEditingReward({ ...editingReward, type: 'partner' })}
                          className={`flex-1 py-2 text-xs font-bold rounded-lg border transition-all ${editingReward.type === 'partner' ? 'bg-amber-600 text-white border-amber-500' : 'bg-slate-900 text-slate-400 border-slate-700'}`}
                        >
                          Partner Esterno
                        </button>
                      </div>
                    ...
                    <div className="flex items-center gap-3 pt-2">
                      ...
                      <span className="text-sm text-slate-300 font-bold">
                        Attivo e visibile nel catalogo
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="w-full md:w-[40%] bg-black/50 flex flex-col relative justify-between border-l border-slate-800">
                  <button
                    ...
                    className="absolute top-4 right-4 p-2 bg-red-600 hover:bg-red-700 text-white rounded-full transition-colors shadow-lg z-admin-modal"
                  >
                    <X className="w-5 h-5" />
                  </button>
                  ...
                  <div className="p-6 border-t border-slate-800 bg-slate-900">
                    <button
                      ...
                      disabled={isSaving}
                      className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-4 rounded-xl shadow-lg flex items-center justify-center gap-2 transition-all active:scale-95 disabled:opacity-50"
                    >
                      {isSaving ? (
