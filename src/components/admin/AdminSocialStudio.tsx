...
import type { SocialTemplate } from '../../types/index';
...
export const AdminSocialStudio = () => {
  ...
  const handleSelectTemplateWrapper = (t: SocialTemplate) => {
  ...
  const handleConfirmDeleteWrapper = async () => {
    ...
    const wasActiveTemplate = idToDelete != null && templateLogic.activeTemplate?.id === idToDelete;
    await templateLogic.confirmDelete();
    // confirmDelete può azzerare activeTemplate: decidere il reset PRIMA della mutazione
        ...
        createPortal(
          ...
          >
            ...
            >
              ...
              <div>
                <h3 className="text-2xl font-bold text-white mb-1">Salvato!</h3>
                <p className="text-slate-400 text-sm">Operazione completata.</p>
              </div>
              <button
                type="button"
                onClick={templateLogic.closeModals}
                className="mt-2 w-full py-3 bg-slate-800 hover:bg-slate-700 text-white rounded-xl font-bold uppercase text-xs transition-colors border border-slate-700 hover:border-slate-600"
              >
                Chiudi
              </button>
            </div>
            ...
            activeTab === 'templates' ? (
              <div className="flex gap-2">
                <button
                  ...
                  className="px-4 py-2 bg-slate-800 text-white rounded-lg font-bold text-xs uppercase flex items-center gap-2 hover:bg-slate-700 transition-colors"
                >
                  <Plus className="w-4 h-4" /> Nuovo
                </button>
                <button
                  type="button"
                  onClick={handleSaveGraphicsWrapper}
                  disabled={templateLogic.isSaving}
                  className="px-6 py-2 bg-emerald-600 text-white rounded-lg font-bold text-xs uppercase flex items-center gap-2 shadow-lg hover:bg-emerald-500 transition-colors"
                >
                  {templateLogic.isSaving ? (
        ...
        <div className="flex-1 flex flex-col lg:flex-row gap-6 overflow-hidden">
          ...
          <div className="w-full lg:w-80 flex flex-col gap-6 overflow-y-auto custom-scrollbar pr-2">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-lg">
              ...
              <div className="space-y-2">
                {templateLogic.templates.map((t) => (
                  <div
                    key={t.id}
                    className={`relative p-3 rounded-xl border transition-all flex justify-between items-center group ${templateLogic.activeTemplate?.id === t.id ? 'bg-indigo-900/30 border-indigo-500' : 'bg-slate-950 border-slate-800 hover:border-slate-600'}`}
                  >
                    <button
                      type="button"
                      onClick={() => handleSelectTemplateWrapper(t)}
                      ...
                      aria-label={`Seleziona template ${t.name}`}
                    />
                    <span className="relative z-[1] pointer-events-none text-sm font-bold text-white truncate max-w-[150px]">
                      {t.name}
                    </span>
                    <button
                      type="button"
                      onClick={(e) => {
                        e.stopPropagation();
                        templateLogic.requestDelete(t.id, t.name);
                      }}
                      className="relative z-[1] pointer-events-auto text-slate-600 hover:text-red-500 p-1.5 hover:bg-slate-900 rounded transition-colors"
                    >
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                  </div>
                ))}
                {templateLogic.templates.length === 0 && (
                  <div className="text-slate-600 text-xs italic text-center">Nessun template.</div>
                )}
              </div>
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-lg">
              <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3 flex items-center gap-2">
                <Palette className="w-4 h-4" /> Stile Testi
              </h4>
              <div className="mb-4 pb-4 border-b border-slate-800">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label
                      htmlFor="fld-admin-adminsocialstudio-tsx-l217"
                      className="text-[9px] text-slate-500 block"
                    >
                      Colore
                    </label>
                    <input
                      id="fld-admin-adminsocialstudio-tsx-l217"
                      type="color"
                      value={canvasLogic.editorLayout.userName.color}
                      onChange={(e) =>
                        canvasLogic.updateTextStyle('userName', 'color', e.target.value)
                      }
                  ...
                  <div>
                    <label
                      htmlFor="fld-admin-adminsocialstudio-tsx-l228"
                      className="text-[9px] text-slate-500 block"
                    >
                      Size
                    </label>
                    <input
                      id="fld-admin-adminsocialstudio-tsx-l228"
                      type="number"
                      value={canvasLogic.editorLayout.userName.fontSize}
                      onChange={(e) =>
                        canvasLogic.updateTextStyle(
                          'userName',
                          'fontSize',
              ...
              <div>
                <label className="text-[10px] font-bold text-amber-400 uppercase block mb-2">
                  Codice Referral
                </label>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label
                      htmlFor="fld-admin-adminsocialstudio-tsx-l250"
                      className="text-[9px] text-slate-500 block"
                    >
                      Colore
                    </label>
                    <input
                      id="fld-admin-adminsocialstudio-tsx-l250"
                      type="color"
                      value={canvasLogic.editorLayout.referralCode.color}
                      onChange={(e) =>
                        canvasLogic.updateTextStyle('referralCode', 'color', e.target.value)
                      }
                      className="w-full h-8 rounded bg-transparent cursor-pointer"
                    />
                  </div>
                  <div>
                    <label
                      htmlFor="fld-admin-adminsocialstudio-tsx-l261"
                      className="text-[9px] text-slate-500 block"
                    >
                      Size
                    </label>
                    <input
                      id="fld-admin-adminsocialstudio-tsx-l261"
                      type="number"
                      value={canvasLogic.editorLayout.referralCode.fontSize}
                      onChange={(e) =>
                        canvasLogic.updateTextStyle(
                          'referralCode',
                          'fontSize',
          ...
          <div className="flex-1 bg-slate-950 rounded-3xl border border-slate-800 flex flex-col items-center justify-center p-8 relative shadow-inner overflow-hidden">
            ...
            <p className="text-slate-500 text-xs mt-4 flex items-center gap-2">
              <Type className="w-4 h-4" /> Trascina i testi per posizionarli
            </p>
          </div>
        </div>
      )}
      ...
      {activeTab === 'viral' && (
        <div className="flex-1 overflow-hidden flex flex-col">
          <div className="p-6 bg-emerald-900/10 border-b border-emerald-500/20 text-center shrink-0 mb-4 rounded-xl border">
            <h3 className="text-emerald-400 font-bold uppercase tracking-widest text-sm mb-1">
              Messaggio Condivisione Social
            </h3>
            <p className="text-slate-400 text-xs">
              Modifica il testo precompilato che appare quando gli utenti cliccano "Condividi su
              WhatsApp/Facebook".
            </p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar relative">
            {templateLogic.viralTemplates.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center h-full gap-4 text-slate-500">
                ...
                <button
                  type="button"
                  onClick={templateLogic.initDefaultViral}
                  className="px-6 py-3 bg-emerald-600 text-white rounded-xl font-bold uppercase text-xs shadow-lg flex items-center gap-2 hover:bg-emerald-500 transition-colors"
                >
