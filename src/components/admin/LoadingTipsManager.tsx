...
import {
  deleteLoadingTipAsync,
  getLoadingTipsAsync,
  saveLoadingTipAsync,
} from '../../services/contentService';
import type { LoadingTip } from '../../types/index';
...
interface TipRowProps {
  ...
  onUpdate: (id: string, field: keyof LoadingTip, value: LoadingTip[keyof LoadingTip]) => void;
...
const TipRow: React.FC<TipRowProps> = ({
  tip,
  savingId,
  onUpdate,
  onReorder,
  ...
  return (
    <div
      className={`p-3 rounded-2xl border flex items-center gap-4 transition-all group ${tip.active ? 'bg-slate-900 border-slate-700 shadow-sm' : 'bg-slate-900/50 border-slate-800 opacity-60'}`}
    >
      {/* ORDER INPUT */}
      <div className="flex flex-col items-center justify-center w-20 shrink-0 border-r border-slate-800 pr-4 mr-2">
        <label
          htmlFor="fld-admin-loadingtipsmanager-tsx-l58"
          className="text-[10px] font-black text-slate-500 uppercase mb-1 tracking-widest"
        >
          ORDINE
        </label>
        <input
          id="fld-admin-loadingtipsmanager-tsx-l58"
          type="number"
          value={localOrder}
...
export const LoadingTipsManager = () => {
  ...
  const handleAdd = async () => {
    ...
    const maxOrder =
      relevantTips.length > 0 ? Math.max(...relevantTips.map((t) => t.order || 0)) : 0;
  ...
  const handleInlineUpdate = (
    id: string,
    field: keyof LoadingTip,
    value: LoadingTip[keyof LoadingTip],
  ) => {
  ...
  const handleReorder = async (tipId: string, newOrderStr: string) => {
    ...
    if (Number.isNaN(newOrder) || newOrder < 1) return;
    ...
    <div className="flex flex-col h-full space-y-6 relative">
            ...
            <div className="flex flex-col items-center text-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-300 leading-relaxed">
                  Stai per cancellare: <br />
                  <strong className="text-white">"{deleteTarget.text}"</strong>.<br />
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 shrink-0 shadow-lg flex flex-col md:flex-row gap-4 items-end">
        <div className="flex-1 w-full">
          <label
            htmlFor="fld-admin-loadingtipsmanager-tsx-l334"
            className="text-[10px] text-slate-500 uppercase font-bold block mb-1"
          >
            Nuovo Testo
          </label>
          <input
            id="fld-admin-loadingtipsmanager-tsx-l334"
            value={newTipText}
        ...
        <div className="w-full md:w-48">
          <label
            htmlFor="fld-admin-loadingtipsmanager-tsx-l346"
            className="text-[10px] text-slate-500 uppercase font-bold block mb-1"
          >
            Tipologia
          </label>
          <select
            id="fld-admin-loadingtipsmanager-tsx-l346"
            value={newTipType}
            onChange={(e) => {
              const value = e.target.value;
              if (value === 'tip' || value === 'status') setNewTipType(value);
            }}
