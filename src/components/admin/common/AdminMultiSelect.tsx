import { CheckSquare, ChevronDown, Filter, Square, X } from 'lucide-react';
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      >
        ...
        {badgeCount > 0 && <CountBadge count={badgeCount} size="xs" variant="white" shape="pill" />}
        <ChevronDown
          className={`w-3 h-3 transition-transform ${isOpen ? 'rotate-180' : ''} opacity-50`}
        />
      </button>
          ...
          <div className="p-3 border-b border-slate-800 bg-slate-950/50 flex justify-between items-center">
            ...
            {badgeCount > 0 && (
              <button
                type="button"
                onClick={() => onChange([])}
                className="text-[9px] text-red-400 hover:text-white flex items-center gap-1"
              >
            ...
            {options.map((opt) => {
              ...
              return (
                ...
                >
                  ...
                  {opt.badge && (
                    <span className="text-[9px] bg-slate-950 px-1.5 rounded text-slate-500 border border-slate-800">
                      {opt.badge}
                    </span>
                  )}
                </button>
