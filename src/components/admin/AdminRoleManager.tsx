import { CheckCircle, CheckSquare, ShieldCheck, Square } from 'lucide-react';
...
import type { PermissionCode, User, UserRole } from '../../types/users';
...
export const AdminRoleManager = ({ currentUser }: AdminRoleManagerProps) => {
  const [permissionsMatrix, setPermissionsMatrix] = useState<Record<UserRole, PermissionCode[]>>(
    getRolePermissions(),
  );
  const [lastAction, setLastAction] = useState<{ msg: string; type: 'info' | 'success' } | null>(
    null,
  );
  ...
  const togglePermission = (role: UserRole, code: PermissionCode) => {
    ...
    const newPerms = isEnabled ? current.filter((c) => c !== code) : [...current, code];
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400">
              Le modifiche ai permessi sono <strong>salvate automaticamente</strong>.
            </p>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-slate-950/80 text-slate-400 text-xs uppercase tracking-wider font-bold sticky top-0 z-floating-panel backdrop-blur-sm border-b border-slate-800">
                ...
                {ROLES.map((role) => (
                  <th key={role} className="px-2 py-3 text-center w-24">
                    {getRoleLabel(role)}
                  </th>
                ))}
              </tr>
              ...
              {ALL_PERMISSIONS.map((code) => (
                <tr key={code} className="hover:bg-slate-800/40 transition-colors">
                  <td className="px-4 py-3">
                    ...
                    <div
                      className="text-[10px] text-slate-500 leading-tight truncate max-w-[300px]"
                      title={getPermissionsDescription(code)}
                    >
                      {getPermissionsDescription(code)}
                    </div>
                  </td>
                  {ROLES.map((role) => {
                    ...
                    const isLocked =
                      isSelfLockoutProtection || isSuperAdminProtected || isModeratorProtected;
                    ...
                    return (
                      <td key={`${role}-${code}`} className="px-2 py-3 text-center">
                        <button
                          ...
                          title={
                            isLocked
                              ? 'Modifica bloccata per sicurezza'
                              : 'Clicca per attivare/disattivare'
                          }
                        >
                          {hasPerm ? (
                            <CheckSquare
                              className={`w-5 h-5 ${role === 'admin_all' ? 'text-purple-500' : 'text-emerald-500'}`}
                            />
                          ) : (
