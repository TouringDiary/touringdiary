...
export const MarketingManager = () => {
  ...
  const handlePricingAction = async (
    action: 'create_draft' | 'activate' | 'delete_draft' | 'rollback' | 'cancel',
    id: string,
  ) => {
    const now = new Date().toISOString();
    try {
      switch (action) {
        case 'create_draft': {
          const baseVersion =
            allVersions.find((v) => v.plan_id === id && v.valid_until === null) ||
    ...
    return (
      <div className="text-center py-40 flex flex-col items-center gap-4">
        ...
        <p className="text-slate-500 font-bold tracking-widest text-xs uppercase">
          Sincronizzazione Pricing Engine...
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in pb-20 relative">
      ...
      <div className="flex bg-slate-900/80 p-1.5 rounded-2xl border border-slate-800 backdrop-blur-xl shadow-2xl overflow-x-auto shrink-0">
        {NAV_TABS.map((tab) => (
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-[10px] font-black transition-all flex items-center gap-2 shrink-0 ${activeTab === tab.id ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/40' : 'text-slate-500 hover:text-white'}`}
          >
      ...
      <div className="space-y-12">
        {activeTab === 'pricing' && (
          <PricingPlansPanel plansData={plansData} onAction={handlePricingAction} />
        )}
        {activeTab === 'ai_limits' && (
          <AiLimitsPanel versions={allVersions} onSave={handleUpdateAiLimits} />
        )}
        {activeTab === 'history' && (
          <PricingHistoryPanel
