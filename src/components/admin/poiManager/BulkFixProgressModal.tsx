import { Activity, CheckCircle, PauseCircle, X } from 'lucide-react';
...
interface BulkStats {
  ...
  byCategory: Record<
    string,
    { total: number; processed: number; success: number; waiting: number }
  >;
}
...
export const BulkFixProgressModal = ({
  isOpen,
  onClose,
  stats,
  isProcessing,
  ...
  return createPortal(
    ...
    <div
      className="td-modal-overlay bg-black/95 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in"
      ...
      <div className="bg-slate-900 w-full max-w-4xl rounded-2xl border border-slate-700 shadow-2xl flex flex-col overflow-hidden max-h-[90vh]">
        ...
        <div className="p-6 border-b border-slate-800 bg-[#0f172a] flex justify-between items-center shrink-0">
          <div className="flex items-center gap-4">
            <div
              className={`p-3 rounded-xl ${isProcessing ? 'bg-indigo-600 animate-pulse' : 'bg-emerald-600'}`}
            >
              {isProcessing ? (
        ...
        <div className="px-6 pt-6 pb-2 shrink-0">
          <div className="flex justify-between text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">
            ...
            <span className={isProcessing ? 'text-indigo-400' : 'text-emerald-400'}>
              {progressPercent}%
            </span>
          </div>
          <div className="h-4 w-full bg-slate-800 rounded-full overflow-hidden border border-slate-700">
            ...
            >
              {isProcessing && (
                <div className="w-full h-full bg-[url('https://www.transparenttextures.com/patterns/diagonal-stripes.png')] opacity-20 animate-slide"></div>
              )}
            </div>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-800/50 text-sm">
              {Object.entries(stats.byCategory).map(([catKey, catStats]) => {
                ...
                const catProgress =
                  catStats.total > 0 ? (catStats.processed / catStats.total) * 100 : 0;
                ...
                return (
                  <tr key={catKey} className="hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="py-3 text-right font-mono text-slate-500">{catStats.waiting}</td>
                  </tr>
                );
              })}
            </tbody>
