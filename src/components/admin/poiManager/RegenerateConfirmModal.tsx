        ...
        <div className="flex flex-col items-center text-center gap-4 mb-6">
          ...
          <div>
            <h3 className="text-xl font-bold text-white mb-2 font-display">
              Rigenerazione Totale POI
            </h3>
            <p className="text-sm text-slate-300 leading-relaxed">
              Questa operazione <strong className="text-red-400">ELIMINERÀ DEFINITIVAMENTE</strong>{' '}
              i POI esistenti per <strong className="text-white">{cityName}</strong> e chiederà
              all'IA di rigenerarli da zero.
            </p>
          </div>
        </div>

        <div className="space-y-3 mb-6">
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest text-center mb-2">
            Seleziona cosa cancellare:
          </p>
          ...
          {[
            ...
            <button
              ...
              className={`w-full flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all text-left ${selectedStatuses.has(opt.id) ? 'bg-red-900/20 border-red-500/50' : 'bg-slate-950 border-slate-800 hover:border-slate-700'}`}
            >
              <span className={`text-xs font-bold uppercase ${opt.color}`}>{opt.label}</span>
              {selectedStatuses.has(opt.id) ? (
