...
interface PoiToolbarProps {
  state: {
    viewStatus: NonNullable<PointOfInterest['status']> | 'all';
    activeCategory: PoiToolbarCategory;
    searchTerm: string;
  ...
  counts: {
    getCategoryCount: (id: string, status?: PointOfInterest['status']) => number;
    getTotalCategoryCount: (id: string) => number;
  };
  actions: {
    setViewStatus: (s: NonNullable<PointOfInterest['status']> | 'all') => void;
    setCategory: (c: PoiToolbarCategory) => void;
...
export type PoiToolbarCategory =
  | 'all'
  | 'monument'
  | 'food'
  | 'hotel'
...
export const PoiToolbar: React.FC<PoiToolbarProps> = ({
  state,
  counts,
  actions,
  cityName,
  ...
  const [selectedDiscCats, setSelectedDiscoveryCats] = useState<PoiToolbarCategory[]>(['monument']);
  ...
  const areAllSelected =
    state.pois.length > 0 && state.pois.every((p) => state.selectedIds.has(p.id));

  const handleRegeneratePois = async (_selectedStatuses: string[]) => {
  ...
  const toggleDiscCat = (catId: PoiToolbarCategory) => {
    ...
    <div className="space-y-6">
      {showRegenModal && (
        <RegenerateConfirmModal
          ...
          onConfirm={handleRegeneratePois}
          cityName={cityName || ''}
        />
      )}
          ...
          <div className="flex items-center gap-2 bg-indigo-900/10 px-4 py-2 rounded-xl border border-indigo-500/30">
            ...
            <div>
              ...
              <span className="text-xs text-white font-bold">
                {state.isBulkProcessing ? state.genStatus : 'Gestione Intelligente'}
              </span>
            </div>
          </div>
      ...
      {isDiscoveryOpen && (
        <div className="bg-emerald-900/10 border border-emerald-500/30 rounded-2xl p-4 animate-in fade-in slide-in-from-top-2 relative">
            ...
            <div className="flex-1 w-full">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                {CATEGORY_TABS.filter((c) => c.id !== 'all').map((cat) => (
                  ...
                  >
                    {selectedDiscCats.includes(cat.id) ? (
          ...
          <div className="mt-3 pt-3 border-t border-emerald-500/20 text-[10px] text-emerald-200/70 italic flex gap-2 items-center">
            <Info className="w-3 h-3" /> I risultati verranno aggiunti come "Bozza" (Draft). Usa poi
            "Bonifica Pro" per validarli.
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-900 p-4 rounded-2xl border border-slate-800">
        ...
        <div className="flex gap-2 w-full md:w-auto overflow-x-auto items-center">
          ...
          >
            <Plus className="w-4 h-4" /> Nuovo POI
          </button>
        </div>
      </div>
      ...
      <div className="overflow-x-auto custom-scrollbar bg-slate-900 p-2 rounded-xl border border-slate-800 shadow-lg">
        <div className="flex gap-2 min-w-max">
          {CATEGORY_TABS.map((cat) => {
            ...
            const count = counts.getCategoryCount(
              cat.id,
            ...
            return (
              ...
              >
                ...
                <CountBadge
                  count={count}
                  size="xs"
                  variant={isActive ? 'neutral-active' : 'neutral'}
                  shape="pill"
                />
              </button>
            );
      ...
      <div className="flex flex-col gap-3 bg-slate-900 p-3 rounded-xl border border-slate-800">
        ...
        <div className="relative w-full group">
          ...
          <input
            type="text"
            placeholder="Cerca nome, descrizione..."
            value={state.searchTerm}
            onChange={(e) => actions.setSearch(e.target.value)}
            className="w-full bg-slate-900 border border-slate-700 rounded-lg pl-9 pr-3 py-2 text-[12px] text-white focus:border-indigo-500 outline-none placeholder:text-slate-600"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {showLoadAllButton && (
            <button
              ...
              className="bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-2 rounded-lg text-[10px] font-black uppercase tracking-wide shadow-lg flex items-center gap-2 transition-transform active:scale-95 whitespace-nowrap border border-indigo-400"
            >
          ...
          <AdminMultiSelect
            ...
            options={state.availableDates.created.map((d) => ({
              value: d,
          ...
          <AdminMultiSelect
            ...
            options={state.availableDates.updated.map((d) => ({
              value: d,
              label: formatLabelDate(d),
            }))}
          ...
          >
            ...
            {state.sortDir === 'desc' ? 'Recenti' : 'Vecchi'}
          </button>
        </div>
      </div>
    </div>
