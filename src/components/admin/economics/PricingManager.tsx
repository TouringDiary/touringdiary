...
/** Riga listino admin (Row pricing_versions + join plans usata dalla UI). */
type PricingManagerVersion = Omit<
  Database['public']['Tables']['pricing_versions']['Row'],
  'ai_limits'
> & {
  ai_limits?: {
...
const planDisplayName = (plans: PricingManagerVersion['plans']): string | undefined => {
  const plan = Array.isArray(plans) ? plans[0] : plans;
  return plan?.name;
};

export const PricingManager = () => {
  const [versions, setVersions] = useState<PricingManagerVersion[]>([]);
  ...
  }>({ isOpen: false, versionId: '', type: 'activate' });
  const [editingDraft, setEditingDraft] = useState<PricingManagerVersion | null>(null);
  ...
  const loadVersions = async () => {
    ...
    try {
      ...
      setVersions((data ?? []) as PricingManagerVersion[]);
    } catch (err) {
  ...
  const formatPlanName = (name: string) => {
    if (name === PLAN_TYPES.PRO_USER) return 'Pro User';
    if (name === PLAN_TYPES.LOCAL_ACTIVITY) return 'Silver';
    if (name === PLAN_TYPES.REGIONAL_ACTIVITY) return 'Gold';
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      await updatePricingVersion(editingDraft.id, {
        ...
        ai_limits: editingDraft.ai_limits ?? undefined,
      });
    ...
    <div className="space-y-8 pb-20">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-black text-white uppercase tracking-tight">
                Pricing Engine Control (SSOT)
              </h2>
              <p className="text-[10px] text-slate-500 font-bold uppercase mt-0.5">
                Architettura Immutabile & Governance Economica
              </p>
            </div>
          </div>
        ...
        <div className="xl:col-span-2 space-y-6">
          <div className="flex items-center justify-between px-2">
            ...
            <span className="text-[10px] font-bold text-slate-500">
              {activeVersions.length} Piani Attivi
            </span>
          </div>
          ...
          <div className="space-y-4">
            {activeVersions.map((v) => (
              <div
                key={v.id}
                className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl relative overflow-hidden group"
              >
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h4 className="text-lg font-black text-white uppercase">
                          {formatPlanName(planDisplayName(v.plans) ?? '')}
                        </h4>
                        <span className="px-2 py-0.5 bg-slate-800 text-slate-500 rounded text-[8px] font-black uppercase">
                          v{v.id.slice(0, 4)}
                        </span>
                      </div>
                      <p className="text-xs text-slate-500 font-bold uppercase tracking-tighter">
                        Durata: {v.duration_days} Giorni • Prezzo: €{v.price.toFixed(2)}
                      </p>
                    </div>
                  </div>
                  <button
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="p-3 bg-slate-950/50 rounded-2xl border border-slate-800">
                    ...
                    <span className="text-sm font-black text-white">
                      {v.ai_limits?.soft_daily_limit || 0}
                    </span>
                  </div>
                  <div className="p-3 bg-slate-950/50 rounded-2xl border border-slate-800">
                    <span className="text-[9px] font-black text-slate-500 uppercase block mb-1">
                    ...
                    <span
                      className={`text-[10px] font-black ${v.ai_limits?.burst_allowed ? 'text-emerald-400' : 'text-slate-600'}`}
                    >
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            {draftVersions.map((v) => (
              <div
                key={v.id}
                className="bg-slate-900 border border-amber-500/20 rounded-3xl p-5 shadow-lg group hover:border-amber-500/40 transition-all"
              >
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <p className="text-[9px] text-slate-500 font-bold uppercase mt-0.5">
                      Creato il {new Date(v.created_at).toLocaleDateString()}
                    </p>
                  </div>
                  <span
                    className={`px-2 py-0.5 rounded text-[8px] font-black uppercase ${v.valid_until ? 'bg-slate-800 text-slate-600' : 'bg-amber-500/10 text-amber-500'}`}
                  >
                ...
                <div className="flex gap-2">
                  ...
                  {!v.valid_until && (
                    <button
                      type="button"
                      onClick={() =>
                        setShowConfirmModal({ isOpen: true, versionId: v.id, type: 'activate' })
                      }
                  ...
                  {v.valid_until && (
                    <button
                      type="button"
                      onClick={() =>
                        setShowConfirmModal({ isOpen: true, versionId: v.id, type: 'rollback' })
                      }
          ...
          <div className="bg-indigo-600/5 border border-indigo-500/20 rounded-3xl p-6">
            ...
            <p className="text-[10px] text-slate-500 leading-relaxed font-medium">
              Ogni modifica ai limiti o ai prezzi richiede la creazione di una nuova versione. Le
              versioni attive non possono essere modificate (Immutabilità) per garantire la
              consistenza dello storico transazioni.
            </p>
          </div>
        </div>
      </div>
      ...
      {editingDraft &&
        createPortal(
          ...
          <div
            className="td-modal-overlay bg-slate-950/90 backdrop-blur-md animate-in fade-in p-4"
            ...
            >
              ...
              <div className="p-8 border-b border-slate-800 bg-gradient-to-r from-indigo-900/20 to-slate-900">
                <h2 className="text-2xl font-black text-white tracking-tight uppercase">
                  Configura Bozza Pricing
                </h2>
                <p className="text-xs text-slate-500 font-bold uppercase mt-1">
                  {formatPlanName(planDisplayName(editingDraft.plans) ?? '')} • v
                  {editingDraft.id.slice(0, 4)}
                </p>
              </div>
              ...
              <div className="p-8 space-y-6 flex-1 overflow-y-auto custom-scrollbar">
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label
                      htmlFor="fld-admin-economics-pricingmanager-tsx-l384"
                      className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1"
                    >
                      Prezzo (€)
                    </label>
                    <input
                      id="fld-admin-economics-pricingmanager-tsx-l384"
                      type="number"
                      step="0.01"
                      value={editingDraft.price}
                      onChange={(e) =>
                        setEditingDraft({ ...editingDraft, price: parseFloat(e.target.value) })
                      }
                  ...
                  <div className="space-y-2">
                    <label
                      htmlFor="fld-admin-economics-pricingmanager-tsx-l398"
                      className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1"
                    >
                      Durata (Giorni)
                    </label>
                    <input
                      id="fld-admin-economics-pricingmanager-tsx-l398"
                      type="number"
                      value={editingDraft.duration_days}
                      onChange={(e) =>
                        setEditingDraft({
                          ...editingDraft,
                          duration_days: parseInt(e.target.value, 10),
                        })
                      }
                      className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 text-white font-bold outline-none focus:border-indigo-500 transition-colors"
                    />
                ...
                <div className="space-y-4">
                  <h3 className="text-[10px] font-black text-indigo-400 uppercase tracking-widest flex items-center gap-2">
                    <Sparkles size={12} /> Limiti AI
                  </h3>
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label
                        htmlFor="fld-admin-economics-pricingmanager-tsx-l421"
                        className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1"
                      >
                        Flash Credits
                      </label>
                      <div className="relative">
                        ...
                        <input
                          id="fld-admin-economics-pricingmanager-tsx-l421"
                          type="number"
                          value={editingDraft.ai_limits?.models?.flash || 0}
                          onChange={(e) =>
                            setEditingDraft({
                              ...editingDraft,
                              ai_limits: {
                                ...(editingDraft.ai_limits ?? {}),
                                models: {
                                  ...(editingDraft.ai_limits?.models ?? {}),
                                  flash: parseInt(e.target.value, 10),
                                },
                              },
                            })
                          }
                    ...
                    <div className="space-y-2">
                      <label
                        htmlFor="fld-admin-economics-pricingmanager-tsx-l446"
                        className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1"
                      >
                        Pro Credits
                      </label>
                      <div className="relative">
                        ...
                        <input
                          id="fld-admin-economics-pricingmanager-tsx-l446"
                          type="number"
                          value={editingDraft.ai_limits?.models?.pro || 0}
                          onChange={(e) =>
                            setEditingDraft({
                              ...editingDraft,
                              ai_limits: {
                                ...
                                models: {
                                  ...(editingDraft.ai_limits?.models ?? {}),
                                  pro: parseInt(e.target.value, 10),
                                },
                              },
                            })
                          }
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label
                        htmlFor="fld-admin-economics-pricingmanager-tsx-l473"
                        className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1"
                      >
                        Soft Daily Limit
                      </label>
                      <input
                        id="fld-admin-economics-pricingmanager-tsx-l473"
                        type="number"
                        value={editingDraft.ai_limits?.soft_daily_limit || 0}
                        onChange={(e) =>
                          setEditingDraft({
                            ...editingDraft,
                            ai_limits: {
                              ...(editingDraft.ai_limits ?? {}),
                              soft_daily_limit: parseInt(e.target.value, 10),
                            },
                          })
                        }
                        className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 text-white font-bold outline-none focus:border-indigo-500 transition-colors"
                    ...
                    <div className="space-y-2 flex flex-col justify-end">
                      <label className="flex items-center gap-3 cursor-pointer group mb-2">
                        <div
                          className={`w-12 h-6 rounded-full relative transition-all ${editingDraft.ai_limits?.burst_allowed ? 'bg-indigo-600 shadow-[0_0_10px_rgba(79,70,229,0.4)]' : 'bg-slate-800'}`}
                        >
                          <div
                            className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${editingDraft.ai_limits?.burst_allowed ? 'left-7' : 'left-1'}`}
                          />
                        </div>
                        <input
                          type="checkbox"
                          ...
                          onChange={(e) =>
                            setEditingDraft({
                              ...
                              ai_limits: {
                                ...(editingDraft.ai_limits ?? {}),
                                burst_allowed: e.target.checked,
                              },
                            })
                          }
                        />
      ...
      <DeleteConfirmationModal
        ...
        message={
          showConfirmModal.type === 'rollback'
