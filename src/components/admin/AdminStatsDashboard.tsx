import { BarChart, LayoutDashboard, Loader2, MapPin, PieChart, UserCheck } from 'lucide-react';
...
interface CityStatRow {
  ...
  contracts: number;
  guidesCount: number;
  categories: CityStatCategories;
}

export const AdminStatsDashboard = () => {
  const [activeTab, setActiveTab] = useState<
    'overview' | 'matrix' | 'guides' | 'financial' | 'affiliations'
  >('overview');
  const [liveManifest, setLiveManifest] = useState<CitySummary[]>([]);
  ...
  const stats = useMemo(() => {
    ...
    const pipeline = safeSponsors.filter(
      (s) => s.status === 'waiting_payment' || s.status === 'pending',
    );

    console.log(
      `[DashboardAnalytics] Approved: ${approvedAndActive.length} | Expired: ${expired.length} | Pipeline: ${pipeline.length}`,
    );
    ...
    const cityStats: Record<string, CityStatRow> = {};
    safeLiveManifest.forEach((c) => {
      cityStats[c.id] = {
        ...
        contracts: 0,
        guidesCount: 0,
        categories: { GUI: 0, FOO: 0, HOT: 0, SHO: 0, LEI: 0, NAT: 0, MON: 0, DIS: 0 },
      };
    });
    ...
    approvedAndActive.forEach((s) => {
      ...
      cs.contracts += 1;
      if (s.type === PLAN_TYPES.TOUR_GUIDE) {
        cs.categories.GUI++;
        cs.guidesCount++;
      } else if (s.poiCategory === 'food') cs.categories.FOO++;
      else if (s.poiCategory === 'hotel') cs.categories.HOT++;
      else if (s.type === PLAN_TYPES.DIGITAL_SHOWCASE || s.poiCategory === 'shop')
        cs.categories.SHO++;
    ...
    const activeCitiesCount = Object.values(cityStats).filter((c) => c.contracts > 0).length;
    const uncoveredCities = Object.values(cityStats).filter((c) => c.contracts === 0);
    ...
    return {
      totalRevenue,
      totalPipeline,
      totalContracts: approvedAndActive.length,
      activeCitiesCount,
      totalCities: safeLiveManifest.length,
      uncoveredCities,
      catRevenue,
      zoneRevenue,
      guideList,
      rejectedCount,
      matrix: Object.values(cityStats).sort((a, b) => b.revenue - a.revenue),
    };
  ...
  if (loading)
    return (
      <div className="h-full flex flex-col items-center justify-center gap-4 text-slate-500">
        ...
        <p className="font-bold uppercase tracking-widest text-[11px]">Caricamento Dati Cloud...</p>
      </div>
    );

  const totalGuides = (stats.guideList ?? []).length;
  const citiesWithoutGuides =
    (liveManifest ?? []).length - (stats.matrix ?? []).filter((c) => c.guidesCount > 0).length;
      ...
      <div className="flex justify-between items-center bg-slate-900 p-1.5 rounded-xl border border-slate-800 shrink-0">
        <div className="flex gap-1">
          <button
            ...
            className={`px-6 py-2.5 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'overview' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}
          >
            Overview
          </button>
          <button
            ...
            className={`px-6 py-2.5 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'matrix' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}
          >
          ...
          <button
            ...
            className={`px-6 py-2.5 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'guides' ? 'bg-fuchsia-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}
          >
            Focus Guide
          </button>
          <button
            ...
            className={`px-6 py-2.5 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'financial' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}
          >
            Financial
          </button>
          <button
            ...
            className={`px-6 py-2.5 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'affiliations' ? 'bg-purple-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}
          >
            Affiliazioni
          </button>
        </div>
          ...
          <div className="space-y-6 animate-in fade-in">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-slate-900/50 border border-indigo-500/30 p-6 rounded-3xl shadow-lg flex items-center gap-5">
                ...
                <div>
                  <h3 className="text-4xl font-display font-black text-white">{totalGuides}</h3>
                  <p className="text-xs font-bold text-indigo-400 uppercase tracking-widest">
                    Guide Totali Certificate
                  </p>
                </div>
              </div>
            ...
            <div className="flex justify-end gap-4 text-[10px] font-black uppercase tracking-widest mb-2">
              <span className="text-emerald-500 flex items-center gap-1">
                <div className="w-2 h-2 rounded-full bg-emerald-500"></div> OTTIMA (3+)
              </span>
              <span className="text-amber-500 flex items-center gap-1">
                <div className="w-2 h-2 rounded-full bg-amber-500"></div> MINIMA (1-2)
              </span>
              <span className="text-red-500 flex items-center gap-1">
                <div className="w-2 h-2 rounded-full bg-red-500"></div> ASSENTE (0)
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {(stats.matrix ?? []).map((c) => {
                ...
                const statusColor =
                  count >= 3 ? 'text-emerald-500' : count > 0 ? 'text-amber-500' : 'text-red-500';
                const dotColor =
                  count >= 3 ? 'bg-emerald-500' : count > 0 ? 'bg-amber-500' : 'bg-red-500';

                return (
                  <div
                    key={c.id}
                    className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex items-center justify-between shadow-sm hover:border-slate-600 transition-colors"
                  >
                    <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div
                        className={`w-2 h-2 rounded-full ${dotColor} shadow-[0_0_8px_rgba(255,255,255,0.2)]`}
                      ></div>
                    </div>
                  </div>
                );
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl">
                <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-3">
                  <PieChart className="w-6 h-6 text-emerald-500" /> Ricavi per Categoria
                </h3>
                        ...
                        <div className="flex justify-between items-end mb-1">
                          ...
                          <div className="text-sm font-mono font-bold text-white">
                            ...
                            <span className="text-slate-600 text-[10px]">
                              ({Math.round((val / (stats.totalRevenue || 1)) * 100) || 0}%)
                            </span>
                          </div>
                        </div>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl">
                <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-3">
                  <BarChart className="w-6 h-6 text-blue-500" /> Ricavi per Zona
                </h3>
                <div className="space-y-3">
                  {Object.entries(stats.zoneRevenue ?? {})
                    .sort((a, b) => b[1] - a[1])
                    .map(([zone, val]: [string, number]) => (
                      <div
                        key={zone}
                        className="flex justify-between items-center p-3 bg-slate-950/50 rounded-xl border border-slate-800 hover:border-slate-700 transition-colors"
                      >
                  ...
                  {Object.keys(stats.zoneRevenue ?? {}).length === 0 && (
                    <div className="text-center text-slate-500 italic py-10">
                      Nessun dato di zona disponibile.
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="space-y-8 animate-in fade-in">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="bg-slate-900 border border-emerald-500/30 p-8 rounded-3xl shadow-xl relative group">
                ...
                <h3 className="text-4xl font-display font-bold text-white leading-none">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl">
              <div className="p-6 border-b border-slate-800 bg-[#0f172a] flex justify-between items-center">
                <h3 className="text-xl font-bold text-white flex items-center gap-3">
                  <MapPin className="w-6 h-6 text-indigo-500" /> Matrice Territoriale
                </h3>
                <div className="text-xs text-slate-400 uppercase font-bold tracking-widest">
                  Totale Città: {stats.totalCities}
                </div>
              </div>
