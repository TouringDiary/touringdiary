...
export const EditorMedia = () => {
  ...
  const executeRegenerateMetadata = async () => {
    if (!guardAiAction()) {
    ...
    setGenerating(true);
    try {
      // Solo metadati da generateCitySection('general'). Hero/gallery invariate.
      ...
      const newLog = `[${new Date().toISOString()}] ✅ Fine: Aggiornamento metadati Media (Hero e galleria invariate)`;
      ...
      alert(
        'Metadati Media aggiornati.\nHero e galleria non modificate: nessun generatore Hero automatico. Usa Upload o Photo Inspector.',
      );
    } catch (e: unknown) {
      console.error(e);
      const msg =
        e instanceof Error ? e.message : 'Errore tecnico durante l’aggiornamento metadati.';
      alert(`Errore aggiornamento metadati: ${msg}`);
    } finally {
      setGenerating(false);
    }
  };
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in relative">
      ...
      <DeleteConfirmationModal
        ...
        onConfirm={executeRegenerateMetadata}
      ...
      <div className="flex justify-stretch sm:justify-end border-b border-slate-800 pb-4">
        ...
        >
          ...
          {aiBlocked ? 'AI DISABILITATA' : 'AGGIORNA METADATI MEDIA'}
        </button>
      </div>
      ...
      <div className="bg-slate-900 p-4 md:p-8 rounded-2xl border border-slate-800 shadow-xl">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
          ...
          <button
            type="button"
            className="aspect-square min-h-11 rounded-xl border-2 border-dashed border-slate-700 flex flex-col items-center justify-center text-slate-500 hover:text-white cursor-pointer transition-colors bg-slate-950/50 hover:bg-slate-900 hover:border-indigo-500"
