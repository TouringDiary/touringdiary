import { AlertTriangle, CheckCircle, ShieldAlert, Terminal, Timer } from 'lucide-react';
...
const parseLogsToReport = (logs: string[]) => {
  ...
  logs.forEach((log) => {
    ...
    if (endMatch) {
      ...
      report.push({
        step: 'Forzatura Manuale Contenuti',
        status: 'success',
        duration: 'N/D',
        durationMs: 0,
      });
...
export const TabLogs = () => {
  ...
  const hasManualOverride = useMemo(() => {
    return (
      city?.details?.generationLogs?.some((log) => log.includes('[MANUAL_OVERRIDE_HAS_CONTENT]')) ||
      false
    );
  }, [city?.details?.generationLogs]);
    ...
    <div className="bg-[#0f172a] rounded-2xl border border-slate-800 shadow-2xl overflow-hidden h-[600px] flex flex-col animate-in fade-in">
      <div className="p-4 border-b border-slate-800 bg-[#0f172a] flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-3 bg-slate-900/50 px-4 py-2 rounded-xl border border-slate-800">
          <ShieldAlert
            className={`w-5 h-5 ${hasManualOverride ? 'text-emerald-500' : 'text-slate-500'}`}
          />
          <div>
            ...
            <p className="text-[10px] text-slate-500 mt-0.5">
              Usa se i log si sono persi ma le pagine sono piene.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        <div className="flex-1 bg-black p-4 overflow-y-auto font-mono text-[10px] text-slate-400 space-y-1 md:border-r border-slate-800 min-w-[300px] custom-scrollbar">
          <h4 className="text-slate-500 font-bold uppercase mb-2 border-b border-slate-900 pb-1 sticky top-0 bg-black">
            Console Output
          </h4>
          {city.details.generationLogs && city.details.generationLogs.length > 0 ? (
            city.details.generationLogs.map((log, i) => (
              <div
                key={i}
                className={`${log.includes('ERRORE') ? 'text-red-400' : log.includes('✅') ? 'text-emerald-400' : log.includes('FATTO') || log.includes('COMPLETATO') || log.includes('RECAP') ? 'text-green-300 font-bold' : 'text-slate-300'} border-b border-slate-900/50 pb-0.5 break-all`}
              >
                {log}
              </div>
            ))
          ...
          <table className="w-full text-left border-collapse">
              ...
              {reportData.map((rep, idx) => (
                <tr key={idx} className="hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="p-3 text-center">
                    {rep.status === 'success' ? (
                      <span className="text-emerald-500 flex justify-center">
            ...
            <tfoot>
              <tr className="bg-slate-950/80 font-bold text-xs border-t-2 border-slate-800">
                <td className="p-4 text-right text-slate-300 uppercase tracking-widest" colSpan={2}>
                  TEMPO TOTALE GENERAZIONE:
                </td>
