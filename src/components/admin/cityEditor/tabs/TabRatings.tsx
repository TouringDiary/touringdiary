...
type RatingKey = keyof CityDetails['details']['ratings'];

const RATING_LABELS: Record<RatingKey, string> = {
  cultura: 'Cultura Generale',
  monumenti: 'Monumenti',
  musei_arte: 'Musei & Arte',
  tradizione: 'Storia & Tradizione',
  architettura: 'Architettura',
  natura: 'Natura',
  mare_spiagge: 'Mare & Spiagge',
  paesaggi: 'Paesaggi',
  clima: 'Clima',
  sostenibilita: 'Sostenibilità',
  gusto: 'Gusto',
  cucina: 'Cucina Locale',
  vita_notturna: 'Vita Notturna',
  caffe_bar: 'Caffè & Bar',
  mercati: 'Mercati',
  viaggiatore: 'Vibe Viaggiatore',
  mobilita: 'Mobilità',
...
const RATING_CATEGORIES: { title: string; color: string; keys: RatingKey[] }[] = [
  {
    title: 'Cultura & Storia',
    color: 'emerald',
    keys: ['cultura', 'monumenti', 'musei_arte', 'tradizione', 'architettura'],
  },
  {
    title: 'Natura & Paesaggio',
    color: 'emerald',
    keys: ['natura', 'mare_spiagge', 'paesaggi', 'clima', 'sostenibilita'],
  },
  {
    title: 'Gusto & Vita',
    color: 'emerald',
    keys: ['gusto', 'cucina', 'vita_notturna', 'caffe_bar', 'mercati'],
  },
  {
    title: 'Viaggio & Servizi',
    color: 'emerald',
    keys: ['viaggiatore', 'mobilita', 'accoglienza', 'costo', 'sicurezza'],
  },
];
...
export const TabRatings = () => {
  ...
  const executeRegeneratePage = async () => {
    ...
    try {
      ...
      newCity.details.generationLogs = appendGenerationLogs(newCity.details.generationLogs, [
        newLog,
      ]);
      ...
      alert(
        `Errore durante la generazione: ${error instanceof Error ? error.message : String(error)}`,
      );
    } finally {
      setGenerating(false);
    }
  ...
  const updateRatingValue = (category: RatingKey, value: number) => {
    ...
    <div className="bg-slate-900 p-4 md:p-8 rounded-2xl md:rounded-3xl border border-slate-800 shadow-2xl animate-in fade-in">
      ...
      <div className="flex justify-between items-center mb-6 md:mb-8 border-b border-slate-800 pb-4">
        <h3 className="text-lg md:text-2xl font-bold text-white flex items-center gap-2">
          <BarChart3 className="w-5 h-5 md:w-7 md:h-7 text-emerald-500" /> Valutazioni
        </h3>
        <div className="flex items-center gap-3">
          <div className="bg-slate-950 px-3 py-1.5 rounded-lg border border-slate-800 flex flex-col items-end">
            ...
            <span
              className={`text-lg md:text-2xl font-black leading-none ${liveRating !== city.rating ? 'text-amber-500 animate-pulse' : 'text-white'}`}
            >
          ...
          <button
            type="button"
            onClick={() => triggerPreview('ratings', 'Punteggi')}
            className="bg-slate-800 hover:bg-slate-700 text-white px-3 py-1.5 rounded-lg font-bold flex items-center gap-2 text-xs md:text-sm"
          >
            <Eye className="w-3.5 h-3.5" /> <span className="hidden md:inline">Anteprima</span>
          </button>
          <button
            type="button"
      ...
      <div className="mb-8 p-4 bg-slate-950/50 border border-purple-500/20 rounded-xl">
        <label
          htmlFor="fld-admin-cityeditor-tabs-tabratings-tsx-l217"
        ...
        />
        {ratingInstructions && (
          <div className="mt-2 text-xs text-purple-200 italic border-l-2 border-purple-500 pl-2">
            Strategia: "{ratingInstructions}"
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
        {RATING_CATEGORIES.map((cat, idx) => (
          <div key={idx} className="bg-slate-950/50 p-5 md:p-6 rounded-2xl border border-slate-800">
            <h4 className="text-emerald-500 font-bold uppercase tracking-widest text-xs md:text-sm mb-4 border-b border-emerald-900/30 pb-2">
              {cat.title}
            </h4>
            <div className="space-y-4">
              {cat.keys.map((key) => (
                <div key={key}>
                  ...
                  <input
                    id="fld-admin-cityeditor-tabs-tabratings-tsx-l217"
                    type="range"
                    min="0"
                    max="100"
      ...
      <div className="mt-4 text-center text-slate-500 text-[10px] italic">
        * La media generale (rating) verrà aggiornata e sincronizzata automaticamente nel database
