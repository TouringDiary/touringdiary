...
export const ServiceOperators = () => {
    ...
    const handleImport = async (item: Record<string, unknown>) => {
                    ...
                    <div key={op.id || `op-item-${idx}`} className="bg-slate-900 p-3 md:p-4 rounded-xl border border-slate-800 text-sm space-y-2 relative group flex gap-2">
                        ...
                        <div className="flex-1">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <input value={op.phone || ''} onChange={e => handleUpdate(op.id, 'phone', e.target.value)} className="bg-slate-900 border border-slate-700 rounded px-2 py-1.5 text-xs text-slate-300 w-full" placeholder="Telefono"/>
                                <input value={op.website || ''} onChange={e => handleUpdate(op.id, 'website', e.target.value)} className="bg-slate-900 border border-slate-700 rounded px-2 py-1.5 text-xs text-slate-300 w-full" placeholder="Sito Web"/>
                            </div>
                        </div>
                    </div>
                ))}
