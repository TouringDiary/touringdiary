import { Bot, Database, Loader2, RefreshCw, Sparkles, X } from 'lucide-react';
      ...
      <ProcessLogModal
        isOpen={
          ...
          (stepReports.length > 0 &&
            !showConfirmRegen &&
            stepReports.some((s) => s.status === 'success' || s.status === 'error'))
        }
      ...
      <div className="flex flex-col xl:flex-row justify-between items-center gap-4 bg-slate-900/50 p-4 rounded-2xl border border-indigo-500/20 shadow-lg">
        <div className="flex flex-col md:flex-row items-center gap-4 w-full xl:w-auto">
          ...
          <p className="text-[10px] text-slate-400 max-w-lg leading-tight hidden md:block">
            L'IA analizza guide, eventi e servizi, unisce i dati duplicati e riempie le informazioni
            mancanti.
          </p>
        </div>
        ...
        <div className="flex gap-2 w-full xl:w-auto justify-end">
          ...
          <button
            ...
            className="bg-slate-800 hover:bg-blue-600 text-slate-400 hover:text-white px-3 py-2.5 rounded-lg text-[10px] font-bold uppercase flex items-center gap-2 border border-slate-700 transition-colors"
          >
