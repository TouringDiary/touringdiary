...
const PatronSaintModal = lazy(() =>
  import('../modals/PatronSaintModal').then((m) => ({ default: m.PatronSaintModal })),
);

// --- COMPONENTI UI LOCALI ---

type PreviewCityDetailsOverrides = Partial<
  Pick<CityDetails['details'], 'guides' | 'events' | 'services' | 'tourOperators'>
>;
...
const AdminToast = ({
  ...
  <div
    className={`fixed top-6 left-4 right-4 md:left-auto md:right-6 z-toast px-6 py-4 rounded-xl shadow-2xl flex items-center gap-3 animate-in slide-in-from-top-4 border ${type === 'success' ? 'bg-emerald-600 border-emerald-400' : 'bg-red-600 border-red-400'} text-white md:max-w-md`}
  >
    {type === 'success' ? (
    ...
    <div className="font-bold text-sm min-w-0 flex-1">{message}</div>
    <button
      type="button"
      onClick={onClose}
      className="ml-4 hover:bg-white/20 p-1 min-h-11 min-w-11 flex items-center justify-center rounded-full shrink-0"
    >
      <X className="w-4 h-4" />
    </button>
  </div>
...
const EditorOrchestrator = ({
  ...
  onBack: () => void;
  currentUser: User;
  initialTab?: AdminCityEditorTab;
}) => {
  const cityEditor = useCityEditor();
  ...
  const {
    ...
    clearPreviewRequest,
  } = useAdminCityEditorLogic(cityEditor, { initialTab });
  ...
  const previewCity = useMemo(() => {
    ...
    if ('items' in previewRequest && previewRequest.items && previewRequest.items.length > 0) {
  ...
  const historyPreviewContent =
    previewRequest.type === 'snippet' ? city.details.historySnippet : city.details.historyFull;
    ...
    <div className="bg-slate-950 min-h-screen pb-20 animate-in fade-in relative">
      ...
      {previewRequest.type === 'ratings' && (
        <div className="fixed inset-0 z-admin-modal flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm animate-in fade-in">
          <div className="relative bg-slate-900 w-full max-w-4xl rounded-2xl border border-slate-700 shadow-2xl overflow-hidden animate-in zoom-in-95 flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center p-4 border-b border-slate-800 bg-[#0f172a] shrink-0">
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                <BarChart3 className="w-5 h-5 text-emerald-500" /> Anteprima Valutazioni
              </h3>
              <button
                type="button"
                onClick={clearPreviewRequest}
                className="p-2 min-h-11 min-w-11 flex items-center justify-center hover:bg-slate-800 rounded-full text-slate-400 hover:text-white transition-colors"
                aria-label="Chiudi anteprima valutazioni"
              >
                <X className="w-5 h-5" aria-hidden />
              </button>
            </div>
            <div className="p-6 md:p-8 bg-[#020617] overflow-y-auto min-h-0">
            ...
            <div className="p-4 border-t border-slate-800 bg-slate-950 text-center shrink-0">
              <button
                type="button"
                onClick={clearPreviewRequest}
                className="bg-slate-800 hover:bg-slate-700 text-white px-6 py-2 rounded-lg font-bold text-xs uppercase transition-colors"
              >
                Chiudi Anteprima
              </button>
            </div>
      ...
      {isCityInfoPreviewTab(previewRequest.type) && previewCity && (
        <CityInfoModal
          ...
          onAddToItinerary={() =>
            alert("Funzione 'Aggiungi al diario' disabilitata in anteprima admin.")
          }
          user={currentUser}
          onOpenAuth={() => {}}
        />
      ...
      <div className="border-b border-slate-800 p-4 md:p-6 bg-slate-900/95 backdrop-blur-md flex flex-col md:flex-row justify-between items-start md:items-center sticky top-0 z-local-sticky shadow-lg gap-4">
        <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-start">
          <button
            type="button"
            onClick={handleBackClick}
            className="p-3 bg-slate-800 hover:bg-slate-700 rounded-xl text-slate-400 hover:text-white transition-colors border border-slate-700"
          >
          ...
          <div className="text-right md:text-left">
            <div className="flex items-center gap-2">
              ...
              {isDirty && (
                <span className="text-[10px] bg-amber-500/20 text-amber-500 border border-amber-500/30 px-2 py-0.5 rounded font-bold uppercase animate-pulse">
                  Modificato
                </span>
              )}
            </div>
            <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mt-1 block">
              {city.zone}
            </span>
          </div>
        </div>
        <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3 w-full md:w-auto">
          <button
            ...
            disabled={isSaving}
            className="w-full md:w-auto md:flex-none min-h-11 bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-3 rounded-xl font-bold text-xs uppercase tracking-widest shadow-lg transition-all border border-indigo-500 flex items-center justify-center gap-2 whitespace-nowrap"
          >
          ...
          <button
            type="button"
            onClick={() => handleSaveRequest('published')}
            disabled={isSaving}
            className="w-full md:w-auto md:flex-none min-h-11 bg-emerald-600 hover:bg-emerald-500 text-white px-8 py-3 rounded-xl font-black text-xs uppercase tracking-widest shadow-xl transition-all border border-emerald-500 flex items-center justify-center gap-2 whitespace-nowrap disabled:opacity-50"
          >
            {isSaving ? (
              <Loader2 className="w-4 h-4 animate-spin" />
            ) : (
          ...
          <div className="flex bg-slate-900 p-1.5 rounded-xl border border-slate-800 inline-flex shadow-inner min-w-max gap-1">
            {EDITOR_TABS.map((tab) => (
              <button
                ...
                className={`min-h-11 px-4 md:px-6 py-2.5 rounded-lg text-xs font-black uppercase tracking-widest flex items-center gap-2 transition-all ${activeTab === tab.id ? 'bg-slate-800 text-white shadow-sm border border-slate-700' : 'text-slate-500 hover:text-slate-300'}`}
              >
...
export const AdminCityEditor = ({
  ...
  onBack: () => void;
  currentUser: User;
  /** Tab iniziale (es. deep-link matita Santo Patrono → Storia / culture). */
  initialTab?: AdminCityEditorTab;
}) => {
  return (
    <CityEditorProvider cityId={cityId}>
      <EditorOrchestrator onBack={onBack} currentUser={currentUser} initialTab={initialTab} />
    </CityEditorProvider>
  );
};
