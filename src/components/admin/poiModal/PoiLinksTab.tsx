import { ExternalLink, Eye, Globe, Search, Sparkles, Ticket, Utensils } from 'lucide-react';
import type React from 'react';
import type { AffiliateLinks } from '../../../types/index';
...
const LinkRow = ({ label, value, onChange, brandBg, icon, onSmartSearch }: LinkRowProps) => (
  <div className="flex flex-col gap-2 mb-4 p-3 rounded-xl bg-slate-950 border border-slate-800 shadow-sm group hover:border-slate-600">
    <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-3">
      <div
        className={`h-10 md:h-11 flex items-center px-3 gap-2 font-bold text-xs uppercase rounded-lg text-white shadow-md w-full md:w-40 shrink-0 ${brandBg}`}
      >
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-800 mb-6 flex items-start gap-3">
        ...
        <p className="text-xs text-slate-300 leading-relaxed">
          I link di affiliazione vengono generati automaticamente. Inserisci solo ID o link diretti
          se necessario.
        </p>
      </div>
      ...
      <LinkRow
        ...
        onSmartSearch={() => performSmartSearch('site official')}
      />
      <LinkRow
        label="Booking.com"
        value={formData.affiliate?.booking || ''}
        onChange={(v: string) => updateAffiliate('booking', v)}
        brandBg="bg-[#003580]"
        icon={<Ticket className="w-4 h-4" />}
        onSmartSearch={() => performSmartSearch('booking.com')}
      />
      <LinkRow
        label="TripAdvisor"
        value={formData.affiliate?.tripadvisor || ''}
        onChange={(v: string) => updateAffiliate('tripadvisor', v)}
        brandBg="bg-[#00AA6C]"
        icon={<Eye className="w-4 h-4" />}
        onSmartSearch={() => performSmartSearch('tripadvisor')}
      />
      <LinkRow
        label="TheFork"
        value={formData.affiliate?.thefork || ''}
        onChange={(v: string) => updateAffiliate('thefork', v)}
        brandBg="bg-[#58902d]"
        icon={<Utensils className="w-4 h-4" />}
        onSmartSearch={() => performSmartSearch('thefork')}
      />
      <LinkRow
        label="GetYourGuide"
        value={formData.affiliate?.getyourguide || ''}
        onChange={(v: string) => updateAffiliate('getyourguide', v)}
        brandBg="bg-[#FF5533]"
        icon={<Ticket className="w-4 h-4" />}
        onSmartSearch={() => performSmartSearch('getyourguide')}
      />
    </div>
  );
};
