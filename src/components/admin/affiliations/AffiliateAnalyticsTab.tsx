...
export const AffiliateAnalyticsTab = ({
  ...
  return (
    <div
      className={`space-y-8 animate-in fade-in duration-500 ${isRefreshing ? 'opacity-70' : ''}`}
    >
      {/* Filtri Temporali */}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-8 shadow-2xl">
          ...
          <div className="space-y-4">
            ...
            {stats.topPartners.length === 0 && (
              <p className="text-slate-600 text-xs italic">Nessun dato disponibile</p>
            )}
          </div>
        </div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-[2.5rem] p-8 shadow-2xl text-white">
            ...
            <h4 className="text-xl font-black truncate mb-1">
              {stats.topProducts[0]?.id || 'Nessun Prodotto'}
            </h4>
            <p className="text-sm font-bold opacity-90">
              {stats.topProducts[0]?.count || 0} Click Generati
            </p>
          </div>
        </div>
      </div>
