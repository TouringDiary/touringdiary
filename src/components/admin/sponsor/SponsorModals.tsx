import { AlertTriangle, CalendarPlus, Info, Sparkles, XCircle } from 'lucide-react';
...
export const SponsorModals: React.FC<SponsorModalsProps> = ({
  ...
  const { activationData, rejectData, cancelData, previewPoi, partnerCrmIdentity, extensionData } =
    state;
    ...
    <>
      ...
      >
        <div className="space-y-4 pt-4 text-left w-full">
          {logic.activation.isShop && (
            <div className="bg-amber-900/20 border border-amber-500/30 p-3 rounded-xl flex items-start gap-3">
              ...
              <div className="text-xs text-amber-200">
                <strong>Bottega / Shop:</strong> Il tempo del contratto è "congelato". La
                sponsorizzazione inizierà ufficialmente solo quando la bottega avrà caricato il suo
                primo prodotto attivo nel catalogo.
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label
                htmlFor="fld-admin-sponsor-sponsormodals-tsx-l117"
                className="text-xs font-bold text-slate-400 block mb-1 uppercase"
              >
                Importo Incassato (€)
              </label>
              <input
                id="fld-admin-sponsor-sponsormodals-tsx-l117"
                type="number"
            ...
            <div>
              <label
                htmlFor="fld-admin-sponsor-sponsormodals-tsx-l129"
                className="text-xs font-bold text-slate-400 block mb-1 uppercase"
              >
                Numero Fattura
              </label>
              <input
                id="fld-admin-sponsor-sponsormodals-tsx-l129"
                type="text"
      ...
      >
        <div className="space-y-4 pt-4 text-left w-full">
          <div>
            <label
              htmlFor="fld-admin-sponsor-sponsormodals-tsx-l164"
              className="text-xs font-bold text-slate-400 block mb-1 uppercase"
            >
              Motivazione Rapida (Obbligatoria)
            </label>
            <select
              id="fld-admin-sponsor-sponsormodals-tsx-l164"
            ...
            >
              ...
              <option value="Dati incompleti o non verificabili">
                Dati incompleti o non verificabili
              </option>
              <option value="Attività fuori target per Touring Diary">
                Attività fuori target per Touring Diary
              </option>
              <option value="Violazione delle policy commerciali">
                Violazione delle policy commerciali
              </option>
          ...
          <div>
            <label
              htmlFor="fld-admin-sponsor-sponsormodals-tsx-l188"
              className="text-xs font-bold text-slate-400 block mb-1 uppercase"
            >
              Note Aggiuntive (Interne)
            </label>
            <textarea
              id="fld-admin-sponsor-sponsormodals-tsx-l188"
      ...
      >
        <div className="pt-4 text-left w-full">
          <label
            htmlFor="fld-admin-sponsor-sponsormodals-tsx-l215"
            className="text-xs font-bold text-slate-400 block mb-1 uppercase"
          >
            Motivazione Annullamento
          </label>
          <textarea
            id="fld-admin-sponsor-sponsormodals-tsx-l215"
      ...
      >
        <div className="mt-4 space-y-4 text-left w-full">
          {extensionData?.mode === 'mass' && logic.extension.criticalPartnersCount > 0 && (
            <div className="p-4 bg-rose-900/30 border border-rose-500/30 rounded-xl">
              ...
              <p className="text-xs text-rose-200/70 mb-4">
                Rilevati{' '}
                <strong className="text-white">{logic.extension.criticalPartnersCount}</strong>{' '}
                sponsor con rating molto basso. Vuoi escluderli dall'estensione gratuita?
              </p>
              <button
                type="button"
          ...
          <div>
            <label
              htmlFor="fld-sponsor-extension-days"
              className="text-xs font-bold text-slate-400 block mb-1 uppercase"
            >
          ...
          <div>
            <label
              htmlFor="fld-admin-sponsor-sponsormodals-tsx-l300"
              className="text-xs font-bold text-slate-400 block mb-1 uppercase"
            >
              Motivazione (obbligatoria)
            </label>
            <textarea
              id="fld-admin-sponsor-sponsormodals-tsx-l300"
