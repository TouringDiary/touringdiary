...
export const SponsorModals = ({ state, actions, onConfirmActivation, onConfirmReject, onConfirmCancel, onConfirmExtension, onDataChange, sponsorsList, user }: SponsorModalsProps) => {
    ...
    // ESC Key Handler
    useEffect(() => {
        if (!activationData && !rejectData && !cancelData && !extensionData.isOpen && !previewPoi && !partnerCrmVat) return;
        const handleKeyDown = (e: KeyboardEvent) => {
            if (e.key === 'Escape') {
                ...
                if (previewPoi) actions.closePreview();
                if (partnerCrmVat) actions.closeCrm();
            }
        };
        document.addEventListener('keydown', handleKeyDown);
        return () => document.removeEventListener('keydown', handleKeyDown);
    }, [activationData, rejectData, cancelData, extensionData, previewPoi, partnerCrmVat, actions]);
                ...
                <div className="fixed inset-0 z-[600] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-slate-900 rounded-xl border border-emerald-500/50 p-6 w-full max-w-md shadow-2xl animate-in zoom-in-95">
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Data Inizio {isActivationShop ? '(Sospesa)' : ''}</label>
                                <input 
                                    type="date" 
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Durata</label>
                                ...
                                <div>
                                    <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Importo (€)</label>
                            ...
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Numero Fattura</label>
                            ...
                            <div className="flex gap-2 pt-2">
                                <button onClick={actions.closeActivation} className="flex-1 py-2 bg-slate-800 text-slate-300 rounded hover:bg-slate-700 font-bold text-xs uppercase transition-colors">Annulla</button>
                                <button onClick={handleActivationClick} className="flex-1 py-2 bg-emerald-600 text-white rounded hover:bg-emerald-500 font-bold text-xs uppercase transition-colors">Conferma {isActivationShop ? '& Abilita' : '& Attiva'}</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {rejectData && (
                <div className="fixed inset-0 z-[600] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-slate-900 rounded-xl border border-red-500/50 p-6 w-full max-w-md shadow-2xl animate-in zoom-in-95">
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Motivazione (per il cliente)</label>
                                <select value={rejectData.reason} onChange={e => actions.updateReject({ reason: e.target.value })} className="w-full bg-slate-950 border border-slate-700 rounded p-2 text-white mb-2 text-sm">
                            ...
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Note Interne (Admin)</label>
                                <textarea value={rejectData.notes} onChange={e => actions.updateReject({ notes: e.target.value })} className="w-full bg-slate-950 border border-slate-700 rounded p-2 text-white h-24 text-sm" placeholder="Note visibili solo agli admin..."></textarea>
                            </div>
                            <div className="flex gap-2 pt-2">
                                <button onClick={actions.closeReject} className="flex-1 py-2 bg-slate-800 text-slate-300 rounded hover:bg-slate-700 font-bold text-xs uppercase transition-colors">Annulla</button>
                                <button onClick={onConfirmReject} disabled={!rejectData.reason} className="flex-1 py-2 bg-red-600 text-white rounded hover:bg-red-500 font-bold disabled:opacity-50 text-xs uppercase transition-colors">Conferma Rifiuto</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {cancelData && (
                <div className="fixed inset-0 z-[600] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-slate-900 rounded-xl border border-red-500/50 p-6 w-full max-w-md shadow-2xl animate-in zoom-in-95">
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Motivazione (Obbligatoria)</label>
                                <textarea 
                            ...
                            <div className="flex gap-2 pt-2">
                                <button onClick={actions.closeCancel} className="flex-1 py-2 bg-slate-800 text-slate-300 rounded hover:bg-slate-700 font-bold text-xs uppercase transition-colors">Annulla</button>
                                <button onClick={onConfirmCancel} disabled={!cancelData.reason.trim()} className="flex-1 py-2 bg-red-600 text-white rounded hover:bg-red-500 font-bold disabled:opacity-50 text-xs uppercase transition-colors">Conferma Chiusura</button>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="bg-slate-900 rounded-xl border border-indigo-500/50 p-6 w-full max-w-sm shadow-2xl animate-in zoom-in-95">
                        ...
                        {extensionData.mode === 'single' ? (
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Giorni +/-</label>
                                    ...
                                    <div>
                                        <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Seleziona Data</label>
                                ...
                                <div className="flex gap-2 pt-6">
                                    <button onClick={actions.closeExtension} className="flex-1 py-2 bg-slate-800 text-slate-300 rounded hover:bg-slate-700 font-bold text-xs uppercase transition-colors">Annulla</button>
                                    <button onClick={() => onConfirmExtension()} className="flex-1 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-500 font-bold text-xs uppercase transition-colors">
                                        Applica Modifica
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Giorni da aggiungere</label>
                                ...
                                <div className="flex flex-col gap-2 pt-4">
                                    {criticalPartnersCount > 0 ? (
                                        <>
                                            <button onClick={() => onConfirmExtension(true)} className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg font-bold flex items-center justify-center gap-2 shadow-lg transition-colors text-xs uppercase">
                                                <CheckCircle className="w-4 h-4"/> Escludi Critici (Sicuro)
                                            </button>
                                            <button onClick={() => onConfirmExtension(false)} className="w-full py-2 bg-slate-800 hover:bg-amber-900/30 text-slate-300 hover:text-amber-400 rounded-lg text-xs font-bold border border-slate-700 hover:border-amber-500/30 uppercase transition-colors">
                                                Estendi a TUTTI (Inclusi Critici)
                                            </button>
                                        </>
                                    ) : (
                                        <button onClick={() => onConfirmExtension(false)} className="w-full py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-500 font-bold shadow-lg text-xs uppercase transition-colors">
                                            Applica a Tutti
                                        </button>
                                    )}
                                    <button onClick={actions.closeExtension} className="w-full py-2 text-slate-500 hover:text-white text-xs font-bold uppercase mt-1 transition-colors">Annulla</button>
                                </div>
                            </div>
                        )}
                    </div>
