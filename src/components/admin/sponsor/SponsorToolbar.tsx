...
import type { SponsorSortConfig } from '../../../types/index';
...
interface SponsorToolbarProps {
  ...
  onToggleBelowRatingThreshold?: () => void;
  showBelowRatingFilter?: boolean;
...
export const SponsorToolbar = ({
  ...
  onlyUnread,
  onToggleUnread,
  onlyBelowRatingThreshold = false,
  onToggleBelowRatingThreshold,
  showBelowRatingFilter = false,
  allowMassSelection = false,
  sortConfig,
  onSortChange,
}: SponsorToolbarProps) => {
  ...
  return (
    <div className="flex flex-col md:flex-row w-full xl:w-auto gap-3 items-start md:items-center animate-in fade-in">
      {/* SELECT ALL — estensione massiva (admin_all + admin_limited su Sponsor Attivi) */}
      {allowMassSelection && (
        <div className="flex items-center gap-2 bg-slate-900 border border-slate-700 rounded-lg p-2">
          <button
            ...
            title="Seleziona sponsor per estensione massiva"
          >
            {areAllSelected ? (
      ...
      {/* BULK DELETE — solo admin_all (DL-027 / DL-034) */}
      ...
      <div className="flex gap-2">
        <button
          type="button"
          onClick={onExport}
          className="px-3 py-2 bg-slate-900 hover:bg-slate-800 text-slate-300 hover:text-white rounded-lg transition-colors border border-slate-800 hover:border-slate-600 flex items-center gap-2 text-xs font-bold uppercase shadow-sm"
        >
        ...
        {showMassExtension && (
          <button
            ...
            title={
              massExtensionDisabled ? 'Seleziona sponsor con le checkbox' : 'Estensione massiva'
            }
          ...
          >
            ...
            <span className="hidden lg:inline">Estensione</span>
          </button>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-2 w-full md:w-auto">
        ...
        <button
          ...
          className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-3 py-2 rounded-lg border transition-all text-xs font-bold uppercase shadow-sm ${onlyUnread ? 'bg-rose-600 border-rose-500 text-white shadow-lg' : 'bg-slate-900 border-slate-800 text-slate-500 hover:text-white'}`}
        >
          {onlyUnread ? (
        ...
        {showBelowRatingFilter && onToggleBelowRatingThreshold && (
          <button
            type="button"
            onClick={onToggleBelowRatingThreshold}
            className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-3 py-2 rounded-lg border transition-all text-xs font-bold uppercase shadow-sm ${onlyBelowRatingThreshold ? 'bg-amber-600 border-amber-500 text-white shadow-lg' : 'bg-slate-900 border-slate-800 text-slate-500 hover:text-white'}`}
          >
            <Star className="w-3.5 h-3.5" />
            <span className="hidden md:inline">
              {onlyBelowRatingThreshold ? 'SOTTO SOGLIA' : 'TUTTI I RATING'}
            </span>
          </button>
        )}
        ...
        <div className="flex items-center gap-2 bg-slate-900 p-1.5 rounded-lg border border-slate-800 flex-1 md:flex-none justify-between md:justify-start">
          <select
            ...
            className="bg-transparent text-xs text-white focus:outline-none w-full md:w-24 font-bold cursor-pointer pl-1"
          >
