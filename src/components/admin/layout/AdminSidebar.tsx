...
export const AdminSidebar = ({
    ...
    return (
        <>
            {isOpen && (
                <div
                    className="fixed inset-0 bg-black/80 z-dropdown backdrop-blur-sm md:hidden animate-in fade-in"
                    onClick={onClose}
                ></div>
            )}

            <div className={`
                fixed inset-y-0 left-0 z-dropdown w-64 bg-[#0b0f1a] border-r border-slate-800 flex flex-col h-full shadow-2xl transition-transform duration-300 ease-in-out
            ...
            `}>
                ...
                <nav className="flex-1 p-2 overflow-y-auto custom-scrollbar">
                    ...
                    <NavGroup title="Business">
                        ...
                        <NavItem id="affiliations" label="Affiliazioni & Override" icon={ShoppingBag} active={activeView === 'affiliations' && !editingCityId} />
                        <NavItem id="marketing" label="Listini & Marketing" icon={TrendingUp} active={activeView === 'marketing' && !editingCityId} />
                        <NavItem id="ai_control" label="AI Control Center" icon={Zap} active={activeView === 'ai_control' && !editingCityId} />
                        <NavItem id="social_studio" label="Social Studio" icon={Brush} active={activeView === 'social_studio' && !editingCityId} />
                        <NavItem id="ai_economics" label="Control Tower" icon={Euro} active={activeView === 'ai_economics' && !editingCityId} />
                    </NavGroup>
