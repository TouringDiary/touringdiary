...
export const AdminPoiModal = ({ isOpen, onClose, onSave, poi, cityName }: AdminPoiModalProps) => {
  ...
  const handleSaveClick = () => {
    ...
    if (error) {
      ...
      if (error.includes('Sottocategoria') || error.includes('apertura')) setActiveTab('info');
      return;
    }
  ...
  return (
    ...
    <div
      className="td-modal-overlay bg-slate-950/90 backdrop-blur-sm flex items-center justify-center p-4 pt-24 md:p-6"
    ...
    >
      <button
        ...
        className="absolute inset-0 h-full w-full cursor-default border-0 bg-slate-950/90 backdrop-blur-sm p-0"
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar bg-[#020617]">
          {isMissingAsset && (
            <div className="mb-6 bg-red-900/20 border-2 border-red-500 rounded-xl p-4 flex items-start gap-4 animate-pulse">
              ...
              <div>
                <h4 className="text-red-400 font-bold uppercase text-sm mb-1">Asset Mancante</h4>
                <p className="text-slate-300 text-xs">
                  Manca foto o placeholder per "{formData.category}". Pubblicazione bloccata.
                </p>
              </div>
            </div>
          )}

          {activeTab === 'info' && <PoiInfoTab formData={formData} updateField={updateField} />}
        ...
        <div className="p-4 border-t border-slate-800 bg-[#0f172a] flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex gap-6 text-[10px] text-slate-500 font-mono self-start md:self-center">
            <div className="flex items-center gap-2" title="Data Creazione">
              ...
              <span className="text-slate-400 font-bold">{formData.createdBy || 'Sistema'}</span>{' '}
              <span className="opacity-60">| {formatMetaDate(formData.createdAt)}</span>
            </div>
            <div className="flex items-center gap-2" title="Ultima Modifica">
              <History className="w-3 h-3 text-amber-500" />{' '}
              <span className="text-slate-400 font-bold">{formData.updatedBy || 'Sistema'}</span>{' '}
              <span className="opacity-60">| {formatMetaDate(formData.updatedAt)}</span>
            </div>
          </div>
          <div className="flex gap-2 w-full md:w-auto">
            <button
              ...
              className="bg-emerald-600 hover:bg-emerald-500 text-white px-8 py-3 rounded-xl font-bold shadow-xl transition-all flex items-center justify-center gap-2 hover:scale-105 active:scale-95 w-full md:w-auto"
            >
