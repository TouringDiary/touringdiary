...
import { deleteUser, getAllUsers, refreshUsersCache, updateUser } from '../../services/userService';
...
export const AdminUserManager = ({ currentUser }: AdminUserManagerProps) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      showToast(
        (e instanceof Error ? e.message : String(e)) || "Impossibile eliminare l'utente.",
        'error',
      );

      if ((e instanceof Error ? e.message : String(e)).includes('RLS')) {
  ...
  return (
    <div className="h-full flex flex-col relative">
      {toast && (
        <div
          className={`absolute top-4 left-1/2 -translate-x-1/2 px-6 py-3 rounded-full shadow-2xl z-admin-modal text-sm font-bold flex items-center gap-2 border ${toast.type === 'success' ? 'bg-emerald-600 border-emerald-400 text-white' : 'bg-red-600 border-red-400 text-white'}`}
        >
          {toast.type === 'success' ? (
