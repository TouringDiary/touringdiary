import { CheckCircle, CreditCard, Plus, Zap } from 'lucide-react';
import type React from 'react';
import type { Json } from '@/types/database';
import { getModelLimits } from '../../../services/marketingService';

type PricingAction = 'create_draft' | 'activate' | 'delete_draft' | 'rollback' | 'cancel';

interface PricingPlanVersionView {
  id: string;
  plan_id: string;
...
interface PricingPlanGroupView {
  planName: string;
  versions: PricingPlanVersionView[];
}

interface PricingPlansPanelProps {
  plansData: PricingPlanGroupView[];
  onAction: (action: PricingAction, id: string, data?: unknown) => Promise<void>;
}

export const PricingPlansPanel: React.FC<PricingPlansPanelProps> = ({ plansData, onAction }) => {
  const getStatusInfo = (ver: PricingPlanVersionView) => {
    const now = new Date();
    const start = ver.valid_from ? new Date(ver.valid_from) : null;
    const end = ver.valid_until ? new Date(ver.valid_until) : null;

    if (!start && !end)
      return { label: 'DRAFT', color: 'bg-amber-500/20 text-amber-500 border-amber-500/30' };
    if (start && start <= now && (!end || end >= now))
      return { label: 'ACTIVE', color: 'bg-emerald-500/20 text-emerald-500 border-emerald-500/30' };
    if (end && end < now)
      return { label: 'EXPIRED', color: 'bg-slate-700/50 text-slate-500 border-slate-700' };
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 animate-in fade-in duration-500">
      {plansData.map((group) => (
        <div
          key={group.planName}
          className="bg-slate-900/40 border border-slate-800 rounded-3xl overflow-hidden shadow-xl backdrop-blur-sm"
        >
          ...
          <div className="divide-y divide-slate-800/50">
            {group.versions
              .filter((v) => !v.valid_until || new Date(v.valid_until) >= new Date())
              .map((ver) => {
                      ...
                      <div className="space-y-4 flex-1">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-slate-500 font-mono text-sm uppercase">
                            / {ver.duration_days} giorni
                          </span>
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] font-black border ${status.color}`}
                          >
                            {status.label}
                          </span>
                        </div>
