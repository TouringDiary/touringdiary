...
export const PromoManagerModal = ({
  isOpen,
  onClose,
  onAdd,
  promoTypes,
  onDeleteRequest,
}: PromoManagerModalProps) => {
      ...
      <div className="bg-slate-900 border border-indigo-500/50 p-6 rounded-2xl w-full max-w-sm shadow-2xl relative animate-in zoom-in-95 flex flex-col max-h-[80vh]">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-lg font-bold text-white flex items-center gap-2">
            <Tag className="w-5 h-5 text-indigo-500" /> Gestione Promo
          </h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="space-y-2 mb-6 shrink-0">
          <label
            htmlFor="fld-admin-marketing-promomanagermodal-tsx-l47"
            className="text-[10px] font-bold text-slate-500 uppercase block ml-1"
          >
            Nuova Etichetta
          </label>
          <div className="flex gap-2">
            <input
              id="fld-admin-marketing-promomanagermodal-tsx-l47"
              autoFocus
              value={newPromoName}
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 border-t border-slate-800 pt-4">
          <label className="text-[10px] font-bold text-slate-500 uppercase block mb-3 ml-1">
            Campagne Attive ({promoTypes.length})
          </label>
          {promoTypes.length === 0 ? (
            ...
            <div className="space-y-2">
              {promoTypes.map((promo) => (
                <div
                  key={promo.id}
                  className="flex items-center justify-between bg-slate-950/50 p-3 rounded-lg border border-slate-800 group hover:border-slate-700 transition-colors"
                >
        ...
        <div className="mt-4 pt-4 border-t border-slate-800 shrink-0">
          <button
            type="button"
            onClick={onClose}
            className="w-full py-3 bg-slate-800 text-slate-300 font-bold rounded-xl hover:bg-slate-700 transition-colors text-sm uppercase"
          >
            Chiudi
          </button>
        </div>
