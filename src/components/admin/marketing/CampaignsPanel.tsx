...
import { getCampaigns } from '@/services/aiAdminService';

type CampaignRow = NonNullable<Awaited<ReturnType<typeof getCampaigns>>>[number];
...
export const CampaignsPanel: React.FC<CampaignsPanelProps> = ({ campaigns, onAdd, onDelete }) => {
  ...
  // Confirmation Modal
  const [confirmModal, setConfirmModal] = useState<{ isOpen: boolean; name: string }>({
    isOpen: false,
    name: '',
  });
        ...
        {campaigns.length === 0 ? (
          ...
          campaigns.map((camp) => {
            ...
            return (
              <div
                key={camp.id}
                className="bg-slate-900 border border-slate-800 rounded-3xl p-6 flex items-center justify-between group hover:border-slate-700 transition-all"
              >
                <div className="flex items-center gap-6">
                  <div
                    className={`p-4 rounded-2xl transition-all ${isActive ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 'bg-slate-800 text-slate-500 border border-slate-700'}`}
                  >
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      {isActive ? (
                        <span className="text-[8px] font-black bg-emerald-500 text-white px-1.5 py-0.5 rounded uppercase">
                          Live
                        </span>
                      ) : (
                        <span className="text-[8px] font-black bg-slate-700 text-slate-400 px-1.5 py-0.5 rounded uppercase">
                          Offline
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-4 text-[10px] font-mono text-slate-500">
                      <div className="flex items-center gap-1">
                        ...
                        <span>
                          {camp.start_date
                            ? format(new Date(camp.start_date), 'dd MMM yy', { locale: it })
                            : 'Immediato'}
                        </span>
                      </div>
                      <span>→</span>
                      <div className="flex items-center gap-1">
                        <span>
                          {camp.end_date
                            ? format(new Date(camp.end_date), 'dd MMM yy', { locale: it })
                            : 'Senza scadenza'}
                        </span>
                      </div>
                    </div>
            ...
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 font-bold uppercase mt-0.5">
                      Seleziona o crea un codice promo
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-8 space-y-6">
                <div className="space-y-3">
                  <label
                    htmlFor="fld-admin-marketing-campaignspanel-tsx-l214"
                    className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1"
                  >
                    Seleziona Campagna Esistente
                  </label>
                  <select
                    id="fld-admin-marketing-campaignspanel-tsx-l214"
                  ...
                  {loadingCampaigns && (
                    <p className="text-[10px] text-indigo-400 animate-pulse uppercase font-black ml-2">
                      Caricamento...
                    </p>
                  )}
                </div>
                ...
                <div className="space-y-3">
                  <label
                    htmlFor="fld-admin-marketing-campaignspanel-tsx-l247"
                    className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1"
                  >
                    Nuova Campagna (Nome/Codice)
                  </label>
                  <div className="relative">
                    <Sparkles className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-indigo-400" />
                    <input
                      id="fld-admin-marketing-campaignspanel-tsx-l247"
                      type="text"
                      value={customName}
                      onChange={(e) => {
