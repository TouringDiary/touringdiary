import { BarChart2, Mail, Search } from 'lucide-react';
...
type CitySponsorSortKey = keyof CitySponsorStats;
type SponsorPresenceFilter = 'all' | 'with' | 'without';

const SPONSOR_PRESENCE_FILTERS: { id: SponsorPresenceFilter; label: string }[] = [
  { id: 'all', label: 'Tutte' },
...
export const SponsorDashboardOverview = ({ requests, onNavigate }: DashboardProps) => {
  const [cities, setCities] = useState<CitySponsorStats[]>([]);
  const [sortKey, setSortKey] = useState<CitySponsorSortKey>('pending');
  const [sortDir, setSortDir] = useState<'asc' | 'desc'>('desc');
  const [filterText, setFilterText] = useState('');
  const [sponsorFilter, setSponsorFilter] = useState<SponsorPresenceFilter>('all');

  useEffect(() => {
    getFullManifestAsync().then((manifest) => {
      const stats = manifest.map((city) => {
        const cityReqs = requests.filter((r) => r.cityId === city.id);
        const countUnread = (list: SponsorRequest[]) =>
          list.reduce((sum, r) => sum + (r.partnerLogs?.filter((l) => l.isUnread).length || 0), 0);
        ...
        return {
          ...
          activeUnread: countUnread(
            cityReqs.filter((r) => r.status === 'approved' && !r.isExpired),
          ),
  ...
  const handleSort = (key: CitySponsorSortKey) => {
  ...
  const StatusCell = ({
    ...
    <div
      className={`w-full h-full flex items-center justify-center gap-1 py-1 px-2 rounded-lg transition-colors group/cell ${count > 0 ? 'hover:bg-white/5' : 'opacity-30'}`}
    >
      <button
        type="button"
        onClick={() => onNavigate(cityId, status, false)}
        className={`flex-1 flex items-center justify-center h-8 rounded hover:bg-white/10 text-lg font-bold ${count > 0 ? colorText : 'text-slate-600'}`}
      >
        {count}
      </button>
      <button
        type="button"
        onClick={() => onNavigate(cityId, status, true)}
        disabled={unread === 0}
        className={`relative w-8 h-8 flex items-center justify-center rounded hover:bg-white/10 ${unread > 0 ? 'cursor-pointer' : 'opacity-0'}`}
      >
        <Mail
          className={`w-4 h-4 ${unread > 0 ? 'text-rose-500 animate-pulse' : 'text-slate-700'}`}
        />
        {unread > 0 && (
          <CountBadge
            count={unread}
            ...
            variant="rose"
            position="overlay-tr"
            className="top-0 right-0 translate-x-0 translate-y-0"
          />
        )}
      </button>
    </div>
      ...
      <div className="p-4 border-b border-slate-800 bg-slate-950/50 flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-6">
          <h3 className="font-bold text-white flex items-center gap-2 whitespace-nowrap">
            <BarChart2 className="w-5 h-5 text-indigo-500" /> Partner per Città
          </h3>

          {/* Filtro Sponsor Toggle */}
          <div className="flex bg-slate-900 border border-slate-700 p-1 rounded-xl">
            {SPONSOR_PRESENCE_FILTERS.map((f) => (
              <button
                type="button"
                key={f.id}
                onClick={() => setSponsorFilter(f.id)}
        ...
        <div className="relative w-full md:w-64">
          ...
          <input
            ...
            className="w-full bg-slate-900 border border-slate-700 rounded-lg pl-9 pr-4 py-2 text-xs text-white focus:outline-none"
          />
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse min-w-[1100px]">
          <thead>
            <tr className="bg-slate-950/50 text-slate-500 text-[10px] uppercase font-bold">
              ...
              <th
                className="p-4 border-b border-slate-800 text-center w-28"
            ...
            {sortedAndFilteredStats.map((stat) => (
              <tr key={stat.id} className="hover:bg-slate-800/30 transition-colors">
                <td className="p-4">
                  ...
                  <div className="text-[10px] text-slate-500 uppercase">{stat.zone}</div>
                </td>
                <td className="p-2 text-center bg-slate-900/50">
                  <StatusCell
                    count={stat.pending}
                    unread={stat.pendingUnread}
                    status="pending"
                    cityId={stat.id}
                ...
                <td className="p-2 text-center bg-slate-900/50">
                  <StatusCell
                    count={stat.cancelled}
                    unread={stat.cancelledUnread}
                    status="cancelled"
                    cityId={stat.id}
                ...
                <td
                  className={`p-4 text-center font-mono font-bold border-l border-slate-800 ${stat.total > 0 ? 'text-white' : 'text-slate-600'}`}
                >
                  {stat.total}
                </td>
              </tr>
