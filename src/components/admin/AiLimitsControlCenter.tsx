...
export const AiLimitsControlCenter = () => {
  ...
  const [planLimitDrafts, setPlanLimitDrafts] = useState<
    Record<string, { soft: string; flash: string; pro: string }>
  >({});
  ...
  const syncFormDrafts = useCallback((adminData: aiAdmin.AiAdminData) => {
    ...
    (['anon_flash_budget_eur', 'anon_pro_budget_eur', 'anon_guest_budget_ratio'] as const).forEach(
      (key) => {
        const val = adminData.globalSettings.find((s) => s.key === key)?.value ?? '0.0';
        budgets[key] = key === 'anon_guest_budget_ratio' ? (parseFloat(val) * 100).toFixed(0) : val;
      },
    );
  ...
  const emergencyStop =
    data?.globalSettings.find((s) => s.key === 'ai_emergency_stop')?.value === 'true';
  const aiEnabled = data?.globalSettings.find((s) => s.key === 'ai_enabled')?.value !== 'false';

  const emergencyActions = (
    <div className="flex flex-wrap items-center gap-3">
      <div
        className={`p-1 rounded-2xl border transition-all flex items-center gap-3 pr-4 shadow-lg ${!aiEnabled ? 'bg-amber-950/40 border-amber-500/50' : 'bg-slate-900 border-slate-700'}`}
      >
        <button
          type="button"
        ...
        >
          {isSaving === 'ai_enabled' ? (
            ...
            <Toggle className="w-6 h-6" />
          )}
        </button>
        <div>
          <p
            className={`text-[10px] font-black uppercase tracking-tighter ${!aiEnabled ? 'text-amber-400' : 'text-slate-500'}`}
          >
            AI Runtime
          </p>
      ...
      <div
        className={`p-1 rounded-2xl border transition-all flex items-center gap-3 pr-4 shadow-lg ${emergencyStop ? 'bg-red-950/40 border-red-500/50' : 'bg-slate-900 border-slate-700'}`}
      >
        <button
          type="button"
        ...
        <div>
          <p
            className={`text-[10px] font-black uppercase tracking-tighter ${emergencyStop ? 'text-red-400' : 'text-slate-500'}`}
          >
            Platform Status
          </p>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        ...
        >
          <div className="grid grid-cols-2 gap-4">
            {(['flash', 'pro'] as const).map((model) => (
              <div
                key={model}
                className="p-4 bg-slate-950/50 rounded-2xl border border-slate-800 flex flex-col gap-3"
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setModelCostDrafts((prev) => ({ ...prev, [model]: e.target.value }))
                    }
        ...
        >
          <div className="space-y-4">
            {(
              ['anon_flash_budget_eur', 'anon_pro_budget_eur', 'anon_guest_budget_ratio'] as const
            ).map((key) => {
                ...
                >
                  <div>
                    <span className="text-[10px] uppercase font-black tracking-widest text-emerald-400">
                      {key === 'anon_flash_budget_eur'
                        ? 'Budget Google Flash'
                        : key === 'anon_pro_budget_eur'
                          ? 'Budget Google Pro'
                          : 'Guest Budget Ratio (%)'}
                    </span>
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setBudgetDrafts((prev) => ({ ...prev, [key]: e.target.value }))
                        }
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead>
              <tr className="bg-slate-850">
                <th className={`p-4 border-b border-slate-800 ${tableHeadClass}`}>
                  Piano & Durata
                </th>
                <th className={`p-4 border-b border-slate-800 text-center ${tableHeadClass}`}>
                  3. Soft Daily Limit
                </th>
                <th className={`p-4 border-b border-slate-800 text-center ${tableHeadClass}`}>
                  4. Flash Monthly
                ...
                <th className={`p-4 border-b border-slate-800 text-right ${tableHeadClass}`}>
                  Azioni
                </th>
              </tr>
            </thead>
                  ...
                  <tr key={ver.id} className="hover:bg-slate-800/40 transition-colors">
                    <td className={`p-4 ${tableCellClass}`}>
                      ...
                      <div className="text-[10px] text-slate-500 font-mono italic">
                        {ver.duration_days} giorni - {ver.price}
                        {ver.currency}
                      </div>
                    </td>
                    ...
                    <td className="p-4 text-center">
                      <button
                        type="button"
                        onClick={() =>
                          handleUpdatePlanLimit(ver.id, 'burst_allowed', !limits.burst_allowed)
                        }
                    ...
                    <td className="p-4 text-right">
                      <button
                        ...
                        onClick={() => {
                          ...
                          handleUpdatePlanLimit(ver.id, 'models', {
                            flash: draft.flash,
                            pro: draft.pro,
                          });
                        }}
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="w-full md:w-1/3 bg-slate-950/50 p-6 rounded-3xl border border-slate-800 space-y-4">
            ...
            <div className="relative space-y-2 max-h-[350px] overflow-y-auto custom-scrollbar pr-1">
              ...
              {userList.length === 0 && (
                <p className="text-center text-[10px] text-slate-600 italic py-10">
            ...
            <div className="flex-1 bg-slate-900/50 p-8 rounded-3xl border border-indigo-500/30 animate-in zoom-in-95 duration-300">
              <div className="flex items-center justify-between mb-8">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span
                      className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-tight ${selectedUser.role === 'admin_all' ? 'bg-purple-900/40 text-purple-400' : 'bg-slate-800 text-slate-400'}`}
                    >
                      {selectedUser.role}
                    </span>
                  </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
                <div className="space-y-2">
                  <label
                    htmlFor="fld-admin-ailimitscontrolcenter-tsx-l683"
                    className="text-[10px] font-black uppercase text-slate-500 tracking-widest pl-1"
                  >
                    Flash da aggiungere (BONUS)
                  </label>
                  <input
                    id="fld-admin-ailimitscontrolcenter-tsx-l683"
                    type="number"
                    min={0}
                ...
                <div className="space-y-2">
                  <label
                    htmlFor="fld-admin-ailimitscontrolcenter-tsx-l695"
                    className="text-[10px] font-black uppercase text-slate-500 tracking-widest pl-1"
                  >
                    Pro da aggiungere (BONUS)
                  </label>
                  <input
                    id="fld-admin-ailimitscontrolcenter-tsx-l695"
                    type="number"
                    min={0}
                    value={newProCredits}
                ...
                <div className="space-y-2 sm:col-span-2">
                  <label
                    htmlFor="fld-admin-ailimitscontrolcenter-tsx-l707"
                    className="text-[10px] font-black uppercase text-slate-500 tracking-widest pl-1"
                  >
                    Scadenza bonus (opzionale, default 365g)
                  </label>
                  <input
                    id="fld-admin-ailimitscontrolcenter-tsx-l707"
                    type="datetime-local"
