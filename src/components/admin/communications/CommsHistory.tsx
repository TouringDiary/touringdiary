import { AlertTriangle, Bell, CheckCircle, Loader2, Mail } from 'lucide-react';
import type { AdminMessageLog } from '../../../services/communicationService';
        ...
        <tbody className="divide-y divide-slate-800/50 text-sm">
          {logs.map((log) => (
            <tr key={log.id} className="hover:bg-slate-800/30 transition-colors group">
              ...
              <td className="p-4">
                <span
                  className={`px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-wide flex items-center gap-1.5 w-fit ${
                    log.type === 'email'
              ...
              <td className="p-4 text-center">
                {log.status === 'sent' ? (
                  <span className="text-emerald-500 flex justify-center bg-emerald-900/10 p-1.5 rounded-full mx-auto w-fit">
