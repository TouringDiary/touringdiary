...
export const CommsTemplates = ({
  ...
  const isOnboardingMode =
    (templates.length > 0 &&
      templates.every((t) => t.type === 'onboarding' || t.key.startsWith('onboarding_'))) ||
    !!forcedTarget;
  ...
  const [visualEditorTemplate, setVisualEditorTemplate] = useState<SystemMessageTemplate | null>(
    null,
  );
  ...
  const handleSaveVisualConfig = async (config: UiConfig) => {
        ...
        <div className="max-w-3xl mx-auto space-y-6 bg-slate-900 p-8 rounded-3xl border border-slate-800 shadow-2xl animate-in zoom-in-95">
          <div className="flex justify-between items-center mb-6 border-b border-slate-800 pb-4">
            <h3 className="text-xl font-bold text-white flex items-center gap-2">
              ...
              {isNew ? 'Crea Nuovo Messaggio' : 'Modifica Messaggio'}
            </h3>
            <button
              type="button"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label
                htmlFor="fld-admin-communications-commstemplates-tsx-l253"
                className="text-[10px] font-bold text-slate-500 uppercase block mb-1.5 ml-1"
              >
                Chiave Univoca (ID Sistema)
              </label>
              <input
                id="fld-admin-communications-commstemplates-tsx-l253"
            ...
            <div>
              <label
                htmlFor="fld-admin-communications-commstemplates-tsx-l265"
                className="text-[10px] font-bold text-slate-500 uppercase block mb-1.5 ml-1"
              >
                Etichetta Admin
              </label>
              <input
                id="fld-admin-communications-commstemplates-tsx-l265"
            ...
            <div className="bg-slate-950 border border-slate-800 p-4 rounded-xl">
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                  onClick={() =>
                    setEditingTemplate({ ...editingTemplate, deviceTarget: 'desktop' })
                  }
          ...
          {forcedTarget && (
            <div
              className={`p-3 rounded-xl border flex items-center gap-2 text-xs font-bold uppercase ${
                forcedTarget === 'mobile'
          ...
          <div>
            <label
              htmlFor="fld-admin-communications-commstemplates-tsx-l332"
              className="text-[10px] font-bold text-slate-500 uppercase block mb-1.5 ml-1"
            >
              Titolo (Oggetto)
            </label>
            <input
              id="fld-admin-communications-commstemplates-tsx-l332"
              ...
              placeholder={
                activeTab === 'internal' ? 'Titolo del modale...' : 'Oggetto della notifica...'
              }
            />
          </div>
          ...
          <div>
            <label
              htmlFor="fld-admin-communications-commstemplates-tsx-l348"
              className="text-[10px] font-bold text-slate-500 uppercase block mb-1.5 ml-1"
            >
              Contenuto Messaggio
            </label>
            <textarea
              id="fld-admin-communications-commstemplates-tsx-l348"
          ...
          <div className="flex justify-end gap-3 pt-6 border-t border-slate-800">
            <button
              type="button"
              onClick={() => setEditingTemplate(null)}
              className="px-6 py-3 text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl font-bold uppercase text-xs transition-colors"
            >
              Annulla
            </button>
            <button
              ...
              onClick={handleSave}
              disabled={isSaving}
              className="bg-emerald-600 hover:bg-emerald-500 text-white px-8 py-3 rounded-xl font-black uppercase text-xs shadow-lg flex items-center gap-2 transition-all active:scale-95 disabled:opacity-50"
            >
              {isSaving ? (
  ...
  if (previewTemplate) {
    return (
      <div className="fixed inset-0 z-admin-modal bg-black/90 backdrop-blur-sm flex items-center justify-center p-4">
        <button
          type="button"
          tabIndex={-1}
          aria-hidden="true"
        ...
        />
        <div className="bg-slate-900 w-full max-w-md p-6 rounded-3xl border border-slate-700 shadow-2xl relative z-local-overlay animate-in zoom-in-95">
          <CloseButton
            onClose={() => setPreviewTemplate(null)}
            variant="primary"
            position="absolute"
            className="z-local-overlay"
          />
          <div className="mb-6 text-center">
            ...
            <h3 className="text-xl font-bold text-white mb-2">
              {previewTemplate.titleTemplate || previewTemplate.label}
            </h3>
          ...
          <button
            type="button"
            onClick={() => setPreviewTemplate(null)}
            className="w-full py-3 bg-slate-800 hover:bg-slate-700 text-white rounded-xl font-bold uppercase text-xs"
          >
            Chiudi Anteprima
          </button>
        </div>
          ...
          <div className="bg-slate-900 border border-emerald-500/50 p-8 rounded-3xl shadow-2xl flex flex-col items-center gap-4 animate-in zoom-in-95 max-w-sm w-full text-center">
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm">
                Le modifiche sono state salvate nel database.
              </p>
            </div>
            <button
              type="button"
      ...
      {!isOnboardingMode ? (
        <div className="px-8 pt-6 pb-2 shrink-0 flex justify-between items-center">
          <div className="flex gap-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'external' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-900 text-slate-500 hover:text-white border border-slate-800'}`}
            >
              Notifiche Esterne
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'internal' ? 'bg-amber-600 text-white shadow-lg' : 'bg-slate-900 text-slate-500 hover:text-white border border-slate-800'}`}
            >
              Testi Interni
            </button>
          </div>
          <button
            ...
            className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-xs font-black uppercase tracking-widest shadow-lg flex items-center gap-2 transition-all"
          >
        ...
        <div className="px-8 pt-6 pb-2 shrink-0 flex justify-end">
          <button
            ...
            className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-xs font-black uppercase tracking-widest shadow-lg flex items-center gap-2 transition-all"
          >
            <Plus className="w-4 h-4" /> Nuovo Testo
          </button>
        </div>
      )}
      ...
      <div className="p-8 overflow-y-auto custom-scrollbar flex-1">
        <div className="grid grid-cols-1 gap-4">
          {filteredTemplates.map((tpl, idx) => (
            <div
              key={tpl.key}
              className="bg-slate-900 border border-slate-800 p-5 rounded-2xl flex flex-col md:flex-row gap-6 items-start md:items-center hover:border-indigo-500/30 transition-colors group relative"
            >
              {isOnboardingMode && (
                <div className="flex flex-col gap-1 items-center bg-slate-950 p-1 rounded-lg border border-slate-800 shrink-0">
                  ...
                  <button
                    ...
                    disabled={isSaving || idx === filteredTemplates.length - 1}
                    className="p-1 hover:bg-slate-800 rounded text-slate-500 hover:text-white disabled:opacity-30"
                  >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[10px] font-mono bg-slate-950 px-2 py-1 rounded border border-slate-700 text-slate-400">
                    {tpl.key}
                  </span>
                  {tpl.deviceTarget === 'desktop' && (
                    <span className="text-[9px] bg-blue-900/30 text-blue-400 px-2 py-0.5 rounded border border-blue-500/30 font-bold uppercase flex items-center gap-1">
                      <Monitor className="w-3 h-3" /> Desktop
                    </span>
                  )}
                  {tpl.deviceTarget === 'mobile' && (
                    <span className="text-[9px] bg-purple-900/30 text-purple-400 px-2 py-0.5 rounded border border-purple-500/30 font-bold uppercase flex items-center gap-1">
                ...
                {tpl.titleTemplate && (
                  <p className="text-xs font-bold text-indigo-400 mb-1">
                    Titolo: "{tpl.titleTemplate}"
                  </p>
                )}
                <p className="text-sm text-slate-400 font-mono bg-slate-950/50 p-3 rounded-lg border border-slate-800 line-clamp-2">
                  "{tpl.bodyTemplate}"
                </p>
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0 w-full md:w-auto">
                ...
                {isOnboardingMode && (
                  <button
                    type="button"
                    onClick={() => setVisualEditorTemplate(tpl)}
                    className="flex-1 md:flex-none px-4 py-2.5 bg-amber-600 hover:bg-amber-500 text-white rounded-xl font-bold text-xs uppercase flex items-center justify-center gap-2 shadow-lg transition-all active:scale-95 whitespace-nowrap"
                    title="Posiziona Mascotte"
                  >
                    <Move className="w-4 h-4" /> Editor Visivo
                  </button>
                )}
                {!isOnboardingMode && (
                  <button
                    ...
                    className="flex-1 md:flex-none px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold text-xs uppercase flex items-center justify-center gap-2 shadow-lg transition-all active:scale-95 whitespace-nowrap"
                  >
                    <ArrowRight className="w-4 h-4" /> Usa Questo
                  </button>
                )}
                <button
                  type="button"
                  onClick={() => setEditingTemplate(tpl)}
                  className="flex-1 md:flex-none px-4 py-2.5 bg-transparent hover:bg-slate-800 text-slate-400 hover:text-white rounded-xl font-bold text-xs uppercase flex items-center justify-center gap-2 border border-slate-700 transition-all whitespace-nowrap"
                >
                  <PenTool className="w-4 h-4" /> Modifica
                </button>
