...
type Channel = 'notification' | 'email' | 'system_alert';

const CHANNEL_OPTIONS: { id: Channel; label: string; icon: LucideIcon }[] = [
...
export const CommsComposer = ({ onSent, showToast, initialDraft }: CommsComposerProps) => {
  ...
  // Sincronizza subject/body se il parent aggiorna initialDraft dopo il mount
  // (solo quando i campi locali coincidono ancora con il draft precedente / sono vuoti).
  ...
  useEffect(() => {
    ...
    if (prev === initialDraft) return;

    setSubject((current) => {
      if (!prev || current === (prev.subject || '')) return initialDraft.subject || '';
      return current;
    });
    setBody((current) => {
      if (!prev || current === (prev.body || '')) return initialDraft.body || '';
      return current;
    });
  }, [initialDraft]);

  // Caricamento asincrono utenti solo quando serve.
  // showToast è incluso nelle deps: se il parent lo ricrea a ogni render può
  // ri-triggerare l'effect; accettabile rispetto a dipendenze incomplete.
  useEffect(() => {
    if (target !== 'single_user') return;
    ...
    const loadUsers = async () => {
      ...
      try {
        ...
        if (!cancelled) setUsersList(freshUsers);
      } catch {
        if (!cancelled) showToast('Errore caricamento lista utenti', 'error');
      } finally {
  ...
  // Evita stato sporco di selezione/ricerca quando si esce da "Utente singolo".
  useEffect(() => {
    if (target === 'single_user') return;
    setSelectedUserId('');
    setUserSearch('');
    ...
    <div className="flex h-full relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col xl:flex-row h-full gap-6 p-6 overflow-y-auto custom-scrollbar pb-24">
        ...
        <div className="flex-1 flex flex-col gap-6 min-w-0">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <fieldset className="m-0 min-w-0 border-0 p-0">
                <legend className="mb-2 block text-[10px] font-bold uppercase text-slate-500">
                  Canale
                </legend>
                <div className="flex rounded-xl border border-slate-800 bg-slate-950 p-1">
                  {CHANNEL_OPTIONS.map((opt) => (
                    <button
                      ...
                      className={`flex flex-1 flex-col items-center gap-1 rounded-lg py-2.5 text-[10px] font-bold uppercase transition-all ${channel === opt.id ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:bg-slate-800 hover:text-white'}`}
                    >
              ...
              <div>
                <label
                  htmlFor="comms-target"
                  className="text-[10px] font-bold text-slate-500 uppercase block mb-2"
                >
                  Destinatari
                </label>
                <select
            ...
            {target === 'single_user' && (
              <div className="mt-4 pt-4 border-t border-slate-800 animate-in slide-in-from-top-2">
                <label
                  htmlFor="comms-user-search"
                  className="text-[10px] font-bold text-indigo-400 uppercase block mb-2"
                >
                  Seleziona Utente Specifico{' '}
                  {isLoadingUsers && <Loader2 className="inline w-3 h-3 animate-spin ml-2" />}
                </label>
                <div className="relative">
                  <Search className="absolute left-3 top-3 w-4 h-4 text-slate-500" />
                  <input
                    id="comms-user-search"
                    type="text"
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg flex-1 flex flex-col min-h-[400px]">
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="p-2 hover:bg-slate-800 rounded-lg text-slate-400 hover:text-amber-400 transition-colors relative"
                >
                    ...
                    <div className="absolute top-full right-0 mt-2 p-2 bg-slate-800 border border-slate-700 rounded-xl shadow-xl z-dropdown grid grid-cols-6 gap-1 w-48">
                      {EMOJIS.map((em) => (
                        <button
                          ...
                          key={em}
                          onClick={() => addEmoji(em)}
                          className="hover:bg-slate-700 p-1 rounded text-lg"
                        >
                          {em}
                        </button>
                      ))}
                ...
                <button
                  ...
                  className={`p-2 hover:bg-slate-800 rounded-lg transition-colors ${logoUrl ? 'text-emerald-400' : 'text-slate-400 hover:text-white'}`}
                  title="Carica Logo"
                >
                  {isUploadingLogo ? (
            ...
            <div className="space-y-4 flex-1 flex flex-col">
              ...
              <fieldset className="m-0 min-w-0 border-0 p-0">
                <legend className="mb-2 block text-[10px] font-bold uppercase text-slate-500">
                  Tipologia (Influisce sullo stile)
                </legend>
                  ...
                  {(Object.keys(MSG_TYPE_CONFIG) as MsgType[]).map((type) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-2 rounded-lg border px-4 py-2 text-xs font-bold uppercase transition-all ${isActive ? `${cfg.border} bg-slate-800 text-white` : 'border-slate-800 bg-slate-900 text-slate-500 hover:border-slate-600'}`}
                      >
        ...
        <div className="w-full xl:w-[400px] shrink-0 flex flex-col gap-4">
          <div
            className={`bg-slate-900 border rounded-2xl p-5 h-full shadow-2xl relative overflow-hidden transition-colors ${MSG_TYPE_CONFIG[msgType].border.replace('border-', 'border-opacity-30 border-')}`}
          >
            {/* Dynamic Top Bar Color based on Type */}
            <div
              className={`absolute top-0 left-0 w-full h-1.5 ${MSG_TYPE_CONFIG[msgType].color}`}
            ></div>
            ...
            {channel === 'notification' || channel === 'system_alert' ? (
              ...
              <div className="bg-black border-[6px] border-slate-800 rounded-[2.5rem] p-4 h-[600px] relative shadow-2xl mx-auto max-w-[300px] overflow-hidden">
                ...
                <div className="relative z-floating-panel mt-12 mx-1 bg-white/20 backdrop-blur-md border border-white/30 rounded-2xl p-3 shadow-lg animate-in slide-in-from-top-4 duration-700">
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-1.5">
                      {logoUrl ? (
                        <img
                          src={logoUrl}
                          className="w-5 h-5 rounded-md object-cover bg-black"
                          alt="Logo"
                        />
                      ) : (
                      ...
                      <span className="text-[10px] font-black uppercase text-white/90 tracking-wide">
                        TOURING DIARY
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-1 mb-1">
                    <div
                      className={`w-1.5 h-1.5 rounded-full ${MSG_TYPE_CONFIG[msgType].color}`}
                    ></div>
                    <span className="text-[8px] font-bold text-white/80 uppercase">
                      {MSG_TYPE_CONFIG[msgType].label}
                    </span>
                  </div>
                  <div className="font-bold text-white text-sm mb-0.5 leading-tight">
                    {subject || 'Nessun Titolo'}
                  </div>
                  <div className="text-xs text-white/90 leading-snug line-clamp-4">
              ...
              <div className="bg-white rounded-xl h-[600px] border border-slate-200 overflow-hidden flex flex-col shadow-inner relative">
                ...
                <div className="p-6 flex-1 overflow-y-auto relative">
                  ...
                  <div
                    className={`text-xs font-bold uppercase tracking-widest mb-2 px-2 py-1 w-fit rounded ${MSG_TYPE_CONFIG[msgType].color.replace('bg-', 'text-')} bg-opacity-10`}
                  >
                    {MSG_TYPE_CONFIG[msgType].label}
                  </div>

                  <h2 className="text-xl font-bold text-slate-900 mb-4 font-display leading-tight">
                    {subject || 'Nessun Titolo'}
                  </h2>
                  ...
                  <div className="mt-8 pt-8 border-t border-slate-100 text-center">
                    <button
                      type="button"
                      className={`text-white px-6 py-3 rounded-lg text-xs font-bold uppercase tracking-widest shadow-lg ${MSG_TYPE_CONFIG[msgType].color}`}
                    >
                      Scopri di più
                    </button>
                    <p className="text-[10px] text-slate-400 mt-6">
                      © 2025 Touring Diary. Tutti i diritti riservati.
                    </p>
                  </div>
                </div>
              </div>
