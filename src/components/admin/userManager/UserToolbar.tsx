import { FileSpreadsheet, FlaskConical, Plus, RefreshCw, Search } from 'lucide-react';

type EnvFilter = 'all' | 'prod' | 'test';

const ENV_FILTER_VALUES: readonly EnvFilter[] = ['all', 'prod', 'test'];

const isEnvFilter = (value: string): value is EnvFilter =>
  (ENV_FILTER_VALUES as readonly string[]).includes(value);
      ...
      <div className="flex gap-2 flex-wrap md:flex-nowrap justify-start md:justify-end">
        <div className="relative">
          ...
          <select
            value={envFilter}
            onChange={(e) => {
              if (isEnvFilter(e.target.value)) onEnvFilterChange(e.target.value);
            }}
        ...
        <button
          type="button"
          onClick={onExport}
          className="bg-slate-900 hover:bg-slate-800 text-slate-300 hover:text-white px-3 py-2 rounded-lg border border-slate-700 transition-colors"
          title="Scarica CSV"
        >
        ...
        <button
          type="button"
          onClick={onCreate}
          className="bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-colors shadow-lg shadow-emerald-900/20 whitespace-nowrap border border-emerald-500"
        >
        ...
        <button
          type="button"
          onClick={onRefresh}
          className="bg-slate-900 hover:bg-slate-800 text-slate-300 hover:text-white p-2 rounded-lg border border-slate-700 transition-colors"
          title="Aggiorna"
        >
