...
import {
  getSubscriptionsWithPlans,
  type SubscriptionJoined,
              ...
              return (
                <tr key={sub.id} className="border-b border-slate-800 hover:bg-slate-800/50">
                  <td className="p-3 font-mono text-xs text-slate-500">
                    {sub.user_id?.substring(0, 8) || 'Sponsor'}
                  </td>
                  ...
                  <td className="p-3 text-emerald-400 font-mono">
                    {sub.price_paid} {sub.currency_paid}
                  </td>
                  <td className="p-3">
                    <span
                      className={`px-2 py-1 rounded text-xs font-bold ${sub.status === 'ACTIVE' ? 'bg-emerald-900/50 text-emerald-400' : 'bg-slate-800 text-slate-500'}`}
                    >
                      {sub.status}
                    </span>
                  </td>
