...
export const UserTable = ({
  ...
  const getRoleBadge = (role: UserRole) => {
    switch (role) {
      case 'admin_all':
        return (
          <span className="flex items-center justify-center w-6 h-6 rounded bg-purple-900/50 text-purple-400 border border-purple-500/30">
            <Shield className="w-3.5 h-3.5" />
          </span>
        );
      case 'admin_limited':
        return (
          <span className="flex items-center justify-center w-6 h-6 rounded bg-indigo-900/50 text-indigo-400 border border-indigo-500/30">
            <Shield className="w-3.5 h-3.5" />
          </span>
        );
      case 'business':
        return (
          <span className="flex items-center justify-center w-6 h-6 rounded bg-blue-900/50 text-blue-400 border border-blue-500/30">
            <Briefcase className="w-3.5 h-3.5" />
          </span>
        );
      default:
        return (
          <span className="flex items-center justify-center w-6 h-6 rounded bg-slate-800 text-slate-400 border border-slate-700">
        ...
        <div className="flex items-center gap-3 px-2">
          ...
          <div>
            ...
            <div className="text-xs text-slate-400">
              Crediti consumati: <strong className="text-white">{siteTotals.credits}</strong>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <table className="w-full text-left text-sm text-slate-400">
          <thead className="sticky top-0 bg-slate-900/50 backdrop-blur-sm z-floating-panel">
            <tr>
              <th className={`p-3 ${tableHeadClass}`} onClick={() => onSort('name')}>
                Utente <SortIcon colKey="name" />
              </th>
              <th className={`p-3 ${tableHeadClass}`} onClick={() => onSort('registrationDate')}>
                Registrato <SortIcon colKey="registrationDate" />
              </th>
                ...
                <tr key={user.id} className="border-t border-slate-800 hover:bg-slate-800/50">
                  ...
                  <td className={`p-3 ${tableCellClass}`}>
                    ...
                    <div className="text-slate-500">
                      Ultimo accesso:{' '}
                      {user.lastAccess ? new Date(user.lastAccess).toLocaleDateString() : 'Mai'}
                    </div>
                  </td>
