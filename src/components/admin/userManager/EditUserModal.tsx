...
import { getRoleLabel, resetUserReferralStatus, updateUser } from '../../../services/userService';
...
export const EditUserModal = ({
  ...
  const handleUpdateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setEditError(msg || 'Errore durante il salvataggio.');
    } finally {
  ...
  const executeResetReferral = async () => {
    ...
    try {
      ...
      setEditingUser((prev) => ({ ...prev, referredBy: undefined }));
  ...
  return createPortal(
    ...
    <div
      className="td-modal-overlay flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm animate-in fade-in fixed inset-0"
    ...
    >
      ...
      />
      <div
        className="bg-slate-900 w-full max-w-lg rounded-2xl border border-indigo-500/50 shadow-2xl p-6 relative animate-in zoom-in-95 max-h-[90vh] overflow-y-auto custom-scrollbar"
        ...
        <form onSubmit={handleUpdateUser} className="space-y-4">
          ...
          <div className="p-3 bg-slate-950 border border-slate-800 rounded-lg flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-[10px] text-slate-500">
                  Visibile nel login rapido sviluppatori
                </span>
              </div>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  setEditingUser({ ...editingUser, isTestAccount: e.target.checked })
                }
          ...
          <div className="p-3 bg-slate-950 border border-slate-800 rounded-lg">
            ...
            {editingUser.referredBy ? (
              ...
              <span className="text-[10px] text-slate-500 italic">
                Nessun codice riscattato. Utente "pulito".
              </span>
            )}
          </div>
          ...
          <div className="pt-2 border-t border-slate-800 mt-2">
            <h4 className="text-xs font-bold text-slate-400 uppercase mb-3 flex items-center gap-2">
              <Briefcase className="w-3.5 h-3.5" /> Dati Business (Opzionali)
            </h4>
          ...
          {editError && (
            <div className="p-3 bg-red-900/20 border border-red-500/50 rounded-lg flex flex-col gap-2 text-xs text-red-200">
              ...
              {(editError.includes('RLS Policy') ||
                editError.includes('Permessi insufficienti')) && (
                <button
                  type="button"
          ...
          <div className="pt-4 flex gap-3">
            ...
            <button
              ...
              className="flex-1 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl shadow-lg flex items-center justify-center gap-2 transition-all active:scale-95"
            >
              {isSavingEdit ? (
