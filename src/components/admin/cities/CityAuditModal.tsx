...
import {
  getCorrectCategory,
  resolveCategoryFromDict,
} from '../../../services/ai/utils/taxonomyUtils';
import { getPoisByCityId, saveSinglePoi } from '../../../services/cityService';
...
export const CityAuditModal = ({ isOpen, onClose, cityId, cityName }: Props) => {
  ...
  const handleImport = async () => {
    ...
    for (const item of toImport) {
      ...
      const dbMatch =
        resolveCategoryFromDict(item.name, taxonomyDict) ||
        resolveCategoryFromDict(item.category, taxonomyDict);
      ...
      if (dbMatch) {
        ...
        if (
          finalCategory === 'monument' &&
          ['museum', 'church', 'square'].includes(item.category)
        ) {
      ...
      const newPoi: PointOfInterest = {
        ...
        name: item.name,
        category: (finalCategory || 'discovery') as PoiCategory,
        subCategory: (finalSubCategory || undefined) as PoiSubCategory | undefined,
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 font-medium">
                Gap Analysis & Enrichment (Fase 4)
              </p>
            </div>
          </div>
          <button
            ...
            <div className="h-full flex flex-col items-center justify-center gap-6 text-center p-8">
              ...
              <div className="space-y-2">
                <h4 className="text-2xl font-bold text-white">
                  Analisi Micro-Territoriale in Corso...
                </h4>
                <p className="text-slate-400 max-w-md mx-auto">
                  L'AI sta identificando i "Must-See" e li sta confrontando geometricamente con il
                  database esistente.
                </p>
              </div>
            ...
            <div className="h-full flex flex-col">
              <div className="grid grid-cols-12 flex-1 min-h-0 bg-[#020617]">
                ...
                <div className="col-span-12 overflow-y-auto custom-scrollbar p-2 space-y-1">
                  {results.map((item, idx) => {
                    ...
                    const dbMatch = isMatch
                      ? existingPois.find((p) => p.id === item.matchedDbId)
                      : null;

                    // Anteprima della categoria che verrà assegnata (Smart Guess)
                    const previewCatMatch =
                      resolveCategoryFromDict(item.name, taxonomyDict) ||
                      resolveCategoryFromDict(item.category, taxonomyDict);
                    const previewLabel = previewCatMatch
                      ? `DB: ${previewCatMatch.cat}/${previewCatMatch.sub}`
                      : `AI: ${item.category}`;

                    return (
                      <div
                        key={idx}
                        className={`grid grid-cols-12 gap-4 p-3 rounded-lg items-center border transition-all ${isMatch ? 'bg-slate-900/30 border-slate-800 opacity-60' : isSelected ? 'bg-indigo-900/20 border-indigo-500/50' : 'bg-slate-900 border-slate-800'}`}
                      >
                        ...
                        <div className="col-span-4">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <span
                              className={`text-[9px] px-1.5 rounded ${previewCatMatch ? 'bg-emerald-900/30 text-emerald-400 border border-emerald-500/30' : 'bg-slate-800 text-slate-300'}`}
                            >
                              {previewLabel}
                            </span>
                        ...
                        <div className="col-span-2 flex justify-center">
                          ...
                          {item.matchStatus === 'geo_match' && (
                            <span
                              className="bg-emerald-900/20 text-emerald-400 border border-emerald-500/20 px-2 py-1 rounded text-[10px] font-bold uppercase flex items-center gap-1"
                              title={`Distanza: ${item.matchedDistance}m`}
                            >
                        ...
                        <div className="col-span-5 pl-4 border-l border-slate-800 border-dashed">
                          {dbMatch ? (
                            ...
                            <div className="text-slate-600 text-xs italic flex items-center gap-2">
                              <ArrowRight className="w-3 h-3 opacity-50" /> Nessuna corrispondenza
                              trovata
                            </div>
                          )}
                        </div>
              ...
              <div className="p-4 border-t border-slate-800 bg-slate-950 flex justify-between items-center shrink-0">
                <div className="text-xs text-slate-400">
                  Selezionati: <strong className="text-white">{selectedIndices.size}</strong> nuovi
                  POI
                </div>
                <button
                  type="button"
            ...
            <div className="h-full flex flex-col items-center justify-center text-center p-8 gap-6">
              ...
              <div>
                ...
                <p className="text-slate-300 text-lg">
                  Importati con successo <strong className="text-emerald-400">{importCount}</strong>{' '}
