import { Bot, Loader2, Map, Microscope, Pencil, Save, ScanSearch, X } from 'lucide-react';
  ...
  <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl mt-8">
    <div className="flex flex-col md:flex-row gap-6 justify-between items-start md:items-center text-[10px] uppercase font-bold text-slate-400">
      ...
      <div className="flex flex-wrap gap-4 pt-4 md:pt-0 border-t md:border-t-0 border-slate-800 w-full md:w-auto">
        ...
        <span className="flex items-center gap-1.5" title="Analisi specifica per singola zona">
          <Microscope className="w-3.5 h-3.5 text-indigo-400" /> + Città AI - Zona Turistica
        </span>
    ...
    <div className="fixed inset-0 z-admin-modal flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm animate-in fade-in">
      <div className="bg-slate-900 border border-indigo-500/50 p-6 rounded-2xl w-full max-w-sm shadow-2xl relative animate-in zoom-in-95">
        ...
        <p className="text-xs text-slate-400 mb-4">
          Questa azione aggiornerà il nome della zona e <strong>tutte le città collegate</strong>{' '}
          nel database.
        </p>
        ...
        <div className="space-y-4">
          <div>
            <label
              htmlFor="fld-admin-cities-strategicmaptab-tsx-l99"
              className="text-[10px] font-bold text-slate-500 uppercase block mb-1"
            >
              Nuovo Nome
            </label>
            <input
              id="fld-admin-cities-strategicmaptab-tsx-l99"
              ref={inputRef}
              value={newName}
...
export const StrategicMapTab = ({
  allCities,
  onSelectCity,
  onMagicGenerate,
  onDataChange,
}: StrategicMapTabProps) => {
  ...
  const [deleteSuggestionTarget, setDeleteSuggestionTarget] = useState<{
    name: string;
    zoneName: string;
  } | null>(null);
  ...
  const handleDeleteSuggestionConfirm = async () => {
    ...
    try {
      await removeZoneSuggestion(
        deleteSuggestionTarget.zoneName,
        deleteSuggestionTarget.name,
        geoFilter.region,
      );
      await loadZones();
  ...
  const handleDeleteSuggestionRequest = (item: { name: string; zoneName: string }) => {
    setDeleteSuggestionTarget(item);
  };
        ...
        {!isLoading && zoneData.length === 0 && (
          <div className="text-center py-20 border-2 border-dashed border-slate-800 rounded-3xl bg-slate-900/30">
            ...
            <p className="text-slate-500 font-bold uppercase text-xs tracking-widest">
              Nessuna zona trovata in {geoFilter.region}.
            </p>
            <button
              type="button"
              onClick={handleRegionalAnalysis}
              className="mt-4 text-indigo-400 hover:text-white underline text-xs"
            >
              Avvia Analisi AI per creare zone
            </button>
          </div>
        )}
