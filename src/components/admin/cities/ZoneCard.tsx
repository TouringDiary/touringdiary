    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden hover:border-indigo-500/50 transition-all shadow-lg group flex flex-col h-[450px]">
      ...
      <div className="p-3 bg-slate-950 border-b border-slate-800 flex justify-between items-start group/header shrink-0">
        ...
        <div className="flex gap-1 opacity-100 md:opacity-0 md:group-hover/header:opacity-100 transition-opacity">
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              onAnalyzeZone(zoneName);
            }}
            className="p-1.5 bg-indigo-900/30 hover:bg-indigo-600 text-indigo-400 hover:text-white rounded-lg transition-colors border border-indigo-500/30"
            title="Analisi AI Zona"
          >
          ...
          <button
            ...
            className="p-1.5 bg-slate-800 hover:bg-amber-600 text-slate-400 hover:text-white rounded-lg transition-colors border border-slate-700 hover:border-amber-500/30"
            title="Rinomina Zona"
          >
          ...
          <button
            ...
            className="p-1.5 bg-red-900/30 hover:bg-red-600 text-red-400 hover:text-white rounded-lg transition-colors border border-red-500/30"
            title="Elimina Zona"
          >
      ...
      <div className="flex bg-slate-900 p-1 border-b border-slate-800 overflow-x-auto no-scrollbar">
        <button
          ...
          className={`flex-1 py-1 text-[9px] font-bold uppercase rounded transition-all whitespace-nowrap ${activeTab === 'all' ? 'bg-indigo-600 text-white shadow' : 'text-slate-500 hover:text-white'}`}
        >
          Tutte ({counts.all})
        </button>
        <button
          ...
          className={`flex-1 py-1 text-[9px] font-bold uppercase rounded transition-all whitespace-nowrap ${activeTab === 'published' ? 'bg-emerald-600 text-white shadow' : 'text-slate-500 hover:text-emerald-400'}`}
        >
          Online ({counts.published})
        </button>
        <button
          type="button"
          onClick={() => setActiveTab('draft')}
          className={`flex-1 py-1 text-[9px] font-bold uppercase rounded transition-all whitespace-nowrap ${activeTab === 'draft' ? 'bg-amber-600 text-white shadow' : 'text-slate-500 hover:text-amber-400'}`}
        >
          Bozza ({counts.draft})
        </button>
        <button
          ...
          className={`flex-1 py-1 text-[9px] font-bold uppercase rounded transition-all whitespace-nowrap ${activeTab === 'missing' ? 'bg-red-600 text-white shadow' : 'text-slate-500 hover:text-red-400'}`}
        >
      ...
      <div className="flex justify-between items-center px-3 py-1.5 bg-slate-900 border-b border-slate-800/50">
        ...
        <div className="flex bg-slate-950 p-0.5 rounded-lg border border-slate-800">
          <button
            ...
            className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase transition-all ${sortKey === 'status' ? 'bg-slate-700 text-white' : 'text-slate-500 hover:text-slate-300'}`}
          >
            Stato
          </button>
          <button
            type="button"
            onClick={() => setSortKey('visitors')}
            className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase transition-all ${sortKey === 'visitors' ? 'bg-slate-700 text-white' : 'text-slate-500 hover:text-slate-300'}`}
          >
            Visitatori
          </button>
          <button
            ...
            className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase transition-all ${sortKey === 'name' ? 'bg-slate-700 text-white' : 'text-slate-500 hover:text-slate-300'}`}
          >
            A-Z
          </button>
        </div>
            ...
            if (item.type === 'real') {
              ...
              mainButton = (
                ...
                >
                  ...
                  <span className="text-[8px] font-black uppercase tracking-tight">CERCA POI</span>
                </button>
              );
            } else {
              const m = item.data;
              ...
              mainButton = (
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    onMagicGenerate(cityName, 10, undefined, undefined, adminRegion);
                  }}
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 overflow-hidden">
                  <div
                    className={`text-sm font-bold truncate leading-tight ${item.type === 'missing' ? 'text-red-300' : 'text-slate-200 group-hover/row:text-white'}`}
                  >
                    {cityName}
                  </div>
                  <div className="flex items-center gap-1 mt-0.5">
                    <div
                      className={`w-1.5 h-1.5 rounded-full ${item.type === 'real' ? (statusText === 'ONLINE' ? 'bg-emerald-500' : 'bg-amber-500') : 'bg-red-500'}`}
                    ></div>
                    <span
                      className={`text-[8px] font-bold uppercase tracking-wide truncate ${statusColor}`}
                    >
                      {statusText}
                    </span>
                  </div>
                ...
                <div className="text-right">
                  <span
                    className={`text-[10px] font-mono font-bold ${cityVisitors > 0 ? 'text-indigo-400' : 'text-slate-600'}`}
                  >
                    {formatVisitors(cityVisitors)}
                  </span>
                </div>

                <div className="h-7 w-full">{mainButton}</div>
