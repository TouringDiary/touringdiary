...
import type { StepReport } from '../../../hooks/useCityGenerator';
  ...
  return (
    <div className="fixed inset-0 z-admin-modal bg-black/95 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-300">
      <div className="bg-slate-900 w-full max-w-[95vw] h-[90vh] rounded-2xl border border-slate-700 shadow-2xl flex flex-col overflow-hidden ring-1 ring-white/10">
        {/* HEADER */}
        <div
          className={`p-4 border-b ${isRecovered ? 'border-amber-500/50 bg-amber-950/20' : isProcessing ? 'border-blue-500/30 bg-blue-950/10' : 'border-emerald-500/30 bg-emerald-950/10'} flex justify-between items-center shrink-0 transition-colors duration-500`}
        >
          <div className="flex items-center gap-4">
            <div
              className={`p-2 rounded-lg ${isRecovered ? 'bg-amber-500/20 text-amber-500' : isProcessing ? 'bg-blue-500/20 text-blue-500 animate-pulse' : 'bg-emerald-500/20 text-emerald-500'}`}
            >
              {isRecovered ? (
        ...
        {isRecovered && (
          <div className="bg-amber-500/10 border-b border-amber-500/20 p-2 px-4 flex items-center justify-between text-xs text-amber-200">
            <div className="flex items-center gap-2">
              ...
              <span>
                È stata rilevata una sessione precedente interrotta. I log mostrano lo stato al
            ...
            {onClearSession && (
              <button
                ...
                className="text-[10px] font-bold uppercase bg-amber-900/30 hover:bg-amber-900/50 px-3 py-1 rounded border border-amber-500/30 flex items-center gap-1 transition-colors"
              >
          ...
          <div className="flex-[1.4] bg-[#050505] p-0 flex flex-col border-r border-slate-800 min-w-[300px]">
            ...
            <div className="flex-1 overflow-y-auto p-4 font-mono text-[11px] space-y-1.5 scroll-smooth">
              {logs.map((log, i) => (
                <div
                  key={i}
                  className={`pb-1 border-b border-white/5 break-all ${log.includes('ERRORE') ? 'text-red-400 bg-red-900/10 p-1 rounded' : log.includes('✅') ? 'text-emerald-400' : log.includes('⏳') ? 'text-blue-400' : 'text-slate-300'}`}
                >
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-800/50 text-xs">
                  {reports.map((rep, idx) => (
                    <tr
                      key={idx}
                      className={`transition-all duration-500 ${rep.status === 'processing' ? 'bg-blue-900/10' : rep.status === 'success' ? 'bg-emerald-900/5' : rep.status === 'error' ? 'bg-red-900/10' : ''}`}
                    >
                      <td
                        className="p-3 text-slate-300 font-medium flex items-center gap-2 border-l-2 border-transparent"
                        style={{
                          borderLeftColor:
                      ...
                      <td className="p-3 text-center">
                        {rep.status === 'success' && (
                          <span className="text-emerald-500 bg-emerald-900/20 px-2 py-0.5 rounded text-[10px] font-bold uppercase">
                            Fatto
                          </span>
                        )}
                        {rep.status === 'error' && (
                          <span
                            className="text-red-500 bg-red-900/20 px-2 py-0.5 rounded text-[10px] font-bold uppercase"
                            title={rep.details}
                          >
                            Errore
                          </span>
                        ...
                        {rep.status === 'pending' && (
                          <span className="text-slate-600 text-[10px] uppercase font-bold opacity-50">
                            In Coda
                          </span>
                        )}
                        {rep.status === 'processing' && (
                          <span className="text-blue-500 text-[9px] uppercase font-bold animate-pulse">
                            Eseguo...
                          </span>
                        )}
                      </td>
                      <td className="p-3 text-center font-mono text-white">
                        {rep.status === 'success' ? (
                          rep.details ? (
                            <span className="text-[10px] font-bold text-slate-300 bg-slate-800 px-2 py-1 rounded">
                              {rep.details}
                            </span>
                          ) : rep.itemsCount > 0 ? (
                            <span className="font-bold">{rep.itemsCount}</span>
                          ) : (
                            '-'
                          )
                        ) : (
                  ...
                  <tr className="bg-slate-950/50 font-bold border-t-2 border-slate-800">
                    <td
                      className="p-3 text-right text-slate-300 uppercase tracking-widest"
                      colSpan={3}
                    >
                      TEMPO TOTALE:
                    </td>
        ...
        {/* FOOTER ACTIONS */}
        <div
          className={`p-4 border-t ${isRecovered ? 'border-amber-500/30 bg-amber-950/10' : isProcessing ? 'border-blue-500/30 bg-blue-950/10' : 'border-slate-800 bg-slate-900'} text-center shrink-0 transition-colors duration-500`}
        >
          {isProcessing ? (
            <div className="flex flex-col items-center justify-center gap-3 py-2">
              ...
              <p className="text-[10px] text-slate-500">
                I log verranno salvati automaticamente nel database a fine processo.
              </p>
            </div>
          ) : (
            <div className="flex justify-between items-center animate-in slide-in-from-bottom-4">
              ...
              >
                ...
                {isRecovered ? 'Chiudi Recupero' : 'Chiudi Report & Aggiorna'}
              </button>
            </div>
          )}
        </div>
