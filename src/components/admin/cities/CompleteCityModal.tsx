import { AlertTriangle, ShieldCheck, User, Wand2 } from 'lucide-react';
      ...
      <div className="bg-slate-900 border-2 border-indigo-500/50 p-8 rounded-[2rem] w-full max-w-lg shadow-2xl relative animate-in zoom-in-95">
        ...
        <div className="space-y-6 bg-slate-950/50 p-6 rounded-2xl border border-slate-800">
          ...
          <div className="flex items-start gap-3">
            <div
              className={`p-2 rounded-lg transition-colors ${runPoiDeepScan ? 'bg-emerald-900/30 text-emerald-400' : 'bg-slate-800 text-slate-500'}`}
            >
            ...
            <div className="flex-1 text-left">
              <div className="flex justify-between items-center mb-1">
                <span
                  className={`text-sm font-bold transition-colors ${runPoiDeepScan ? 'text-white' : 'text-slate-400'}`}
                >
                  Bonifica POI (Pro)
                </span>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    checked={runPoiDeepScan}
                    onChange={(e) => setRunPoiDeepScan(e.target.checked)}
              ...
              <p className="text-[10px] text-slate-500 leading-relaxed">
                Esegue una verifica profonda (Gemini Pro + Search) su tutti i POI in bozza
                recuperati da OSM.
                <br />
