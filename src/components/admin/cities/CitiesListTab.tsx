...
import { GeoCascadingFilters, type GeoSelection } from './GeoCascadingFilters';
import { ProcessLogModal } from './ProcessLogModal';

interface CitiesListTabProps {
  list: ReturnType<typeof useCityList>;
    ...
    <div className="flex flex-col md:flex-row gap-6 justify-between items-start md:items-center text-[10px] uppercase font-bold text-slate-400">
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <span className="flex items-center gap-1.5 text-amber-500">
          <div className="w-2 h-2 rounded-full bg-amber-500 shadow-[0_0_5px_#f59e0b]"></div> Bozza
          (Con Contenuti)
        </span>
        <span className="flex items-center gap-1.5 text-red-400">
          <div className="w-2 h-2 rounded-full bg-red-500 shadow-[0_0_5px_#ef4444]"></div> Mancante
          (Scheletro)
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-4 pt-4 md:pt-0 border-t md:border-t-0 border-slate-800 w-full md:w-auto">
        <span className="flex items-center gap-1.5" title="Generazione Nuova Città">
          <Bot className="w-3.5 h-3.5 text-purple-400" /> + Città AI - Corpo
        </span>
        <span className="flex items-center gap-1.5" title="Arricchimento Dati">
          <ScanSearch className="w-3.5 h-3.5 text-indigo-400" /> Cerca POI
        </span>
        <span className="flex items-center gap-1.5" title="Gestione Manuale">
...
export const CitiesListTab = ({ list, onEdit, currentUser }: CitiesListTabProps) => {
  ...
  const filteredData = useMemo(() => {
    ...
    // 4. Sorting (sortKey è già keyof CitySummary da useCityList)
    data.sort((a, b) => {
      const rawA = a[list.sortKey];
      const rawB = b[list.sortKey];

      if (list.sortKey === 'createdAt' || list.sortKey === 'updatedAt') {
        const valA = rawA ? new Date(String(rawA)).getTime() : 0;
        const valB = rawB ? new Date(String(rawB)).getTime() : 0;
        return list.sortDir === 'asc' ? valA - valB : valB - valA;
      }

      if (typeof rawA === 'number' && typeof rawB === 'number') {
        return list.sortDir === 'asc' ? rawA - rawB : rawB - rawA;
      }
  ...
  const handleCompleteCity = async (config: { peopleCount: number; runPoiDeepScan: boolean }) => {
    ...
    const cityObj = list.effectiveCities.find((c) => c.id === cityId);
  ...
  const handleBadgeChange = async (cityId: string, newBadge: string) => {
    try {
      ...
      showToast(`Errore: ${e instanceof Error ? e.message : String(e)}`, 'error');
    }
  };
  ...
  const formatDate = (isoStr?: string) => {
    ...
    if (Number.isNaN(d.getTime()))
      return <span className="text-slate-700 font-mono text-[9px]">Err</span>;
  ...
  const counts = {
    ...
    draft: list.effectiveCities.filter(
      (c: CitySummary) => c.status !== 'published' && c.hasGeneratedContent,
    ).length,
    missing: list.effectiveCities.filter(
      (c: CitySummary) => c.status !== 'published' && !c.hasGeneratedContent,
    ).length,
  };

  const selectedCityForCompletion =
    list.selectedIds.size === 1
      ? list.effectiveCities.find((c) => c.id === Array.from(list.selectedIds)[0])
      : null;
    ...
    <div className="space-y-4 flex flex-col h-full overflow-hidden">
      ...
      {showAiModal && (
        <CityGeneratorModal
          ...
          onGenerate={handleMagicAdd}
          isGenerating={generator.isProcessing}
        />
      )}
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3">
          <div className="flex items-center gap-3 w-full lg:w-auto">
            <div className="relative w-full lg:w-72 group">
              ...
              <input
                ...
                className="w-full bg-slate-900 border border-slate-700 rounded-lg pl-9 pr-3 py-2 text-[12px] text-white focus:border-amber-500 outline-none"
              />
            </div>
          </div>
          <div className="flex items-center gap-2 overflow-x-auto w-full lg:w-auto">
            ...
            <button
              type="button"
              onClick={() => onEdit('new')}
              className="bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-lg font-bold shadow-lg flex items-center gap-2 text-[11px] uppercase tracking-wider transition-all border border-emerald-500"
            >
              <Plus className="w-4 h-4" /> + Città Manuale
            </button>
            <button
              type="button"
              onClick={handleFixStats}
              className="bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white px-3 py-2 rounded-lg font-bold shadow-md flex items-center gap-2 text-[10px] uppercase tracking-wide transition-all border border-slate-700"
            >
              <Target className="w-3.5 h-3.5" /> Fix Stats
            </button>
            <button
              type="button"
      ...
      <div className="flex bg-slate-900 p-1 border border-slate-800 rounded-xl overflow-x-auto no-scrollbar shrink-0">
        <button
          ...
          className={`flex-1 py-2 text-[10px] font-bold uppercase rounded-lg transition-all whitespace-nowrap flex items-center justify-center gap-2 ${listTab === 'all' ? 'bg-indigo-600 text-white shadow' : 'text-slate-500 hover:text-white'}`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2 text-[10px] font-bold uppercase rounded-lg transition-all whitespace-nowrap flex items-center justify-center gap-2 ${listTab === 'published' ? 'bg-emerald-600 text-white shadow' : 'text-slate-500 hover:text-emerald-400'}`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2 text-[10px] font-bold uppercase rounded-lg transition-all whitespace-nowrap flex items-center justify-center gap-2 ${listTab === 'draft' ? 'bg-amber-600 text-white shadow' : 'text-slate-500 hover:text-amber-400'}`}
        >
          <Pencil className="w-3 h-3" /> Bozza ({counts.draft})
        </button>
        <button
          type="button"
          onClick={() => setListTab('missing')}
          className={`flex-1 py-2 text-[10px] font-bold uppercase rounded-lg transition-all whitespace-nowrap flex items-center justify-center gap-2 ${listTab === 'missing' ? 'bg-red-600 text-white shadow' : 'text-slate-500 hover:text-red-400'}`}
        >
          ...
          <table className="w-full text-left border-collapse table-auto">
            <thead>
              <tr className="bg-slate-950 text-slate-400 text-[10px] uppercase tracking-widest font-black sticky top-0 z-dropdown border-b border-slate-800">
                ...
                <th className="p-3 text-center border-r border-slate-800/50 w-24 bg-slate-950/80 text-amber-500">
                  POSIZIONE HOME
                </th>
                <th className="p-3 text-left border-r border-slate-800/50 w-40 bg-slate-950/80">
                  BADGE HOME
                </th>
                <Th id="status" label="Stato" center />
                <Th id="createdAt" label="Data Creazione" center />
                <Th id="updatedAt" label="Ultimo Update" center />
                <th className="p-3 text-right pr-4 sticky right-0 bg-[#0f172a] shadow-[-10px_0_15px_rgba(0,0,0,0.5)]">
                  Azioni
                </th>
              </tr>
            </thead>
              ...
              {paginatedData.map((city: CitySummary) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="p-3 text-[10px] text-slate-400 border-r border-slate-800/30">
                      {city.continent || 'Europa'}
                    </td>
                    <td className="p-3 text-[10px] text-slate-400 border-r border-slate-800/30">
                      {city.nation}
                    </td>
                    ...
                    <td className="p-3 text-[10px] font-bold text-indigo-300 border-r border-slate-800/30">
                      {city.zone}
                    </td>
                    <td className="p-3 font-black text-white text-[12px] group-hover:text-amber-400 transition-colors border-r border-slate-800/30">
                      {city.name}
                    </td>
                      ...
                      <div className="relative">
                        <select
                          ...
                          className={`w-full bg-slate-950 border border-slate-700 rounded-lg py-1.5 pl-2 pr-1 text-[10px] font-black uppercase focus:border-amber-500 outline-none cursor-pointer ${
                            city.specialBadge === 'event'
                              ? 'text-rose-400'
                              : city.specialBadge === 'trend'
                                ? 'text-blue-400'
                                : city.specialBadge === 'season'
                                  ? 'text-emerald-400'
                                  : city.specialBadge === 'editor'
                                    ? 'text-purple-400'
                                    : city.specialBadge === 'destination'
                                      ? 'text-indigo-400'
                                      : 'text-slate-500'
                          }`}
                        >
                          {BADGE_OPTIONS.map((opt) => (
                            <option key={opt.label} value={opt.value} className={opt.color}>
                              {opt.label}
                            </option>
                          ))}
                        </select>
                    ...
                    <td className="p-3 text-center border-r border-slate-800/30">
                      {city.status === 'published' ? (
                        <span className="text-[8px] bg-emerald-900/30 text-emerald-500 px-1.5 py-0.5 rounded-full font-black uppercase">
                          Online
                        </span>
                      ) : isMissing ? (
                        <span
                          className="text-[8px] bg-red-900/30 text-red-500 px-1.5 py-0.5 rounded-full font-black uppercase"
                          title="Scheletro vuoto"
                        >
                          Mancante
                        </span>
                      ) : (
                        <span className="text-[8px] bg-amber-900/30 text-amber-500 px-1.5 py-0.5 rounded-full font-black uppercase">
                          Bozza
                        </span>
                      )}
                    </td>
                    <td className="p-3 text-center border-r border-slate-800/30 text-slate-500">
                      {formatDate(city.createdAt)}
                    </td>
                    <td className="p-3 text-center border-r border-slate-800/30 bg-slate-900/30 text-white font-bold">
                    ...
                    <td className="p-3 text-right pr-2 sticky right-0 bg-[#0f172a] shadow-[-10px_0_15px_rgba(0,0,0,0.5)] group-hover:bg-[#1e293b]">
                      <div className="flex gap-1.5 justify-end items-center">
                        <button
                          ...
                          className="bg-slate-800 hover:bg-indigo-600 text-white px-2 py-0.5 rounded-xl text-[10px] font-black border border-slate-700 uppercase transition-all shadow-md"
                        >
                          Edit
                        </button>
                      </div>
