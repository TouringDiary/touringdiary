...
export const OnboardingVisualEditor = ({ isOpen, onClose, template, onSavePosition }: Props) => {
  ...
  const [draggingTarget, setDraggingTarget] = useState<'mascot' | 'bubble' | 'targetBox' | null>(
    null,
  );
  ...
  useEffect(() => {
    if (isOpen && template.uiConfig) {
      ...
      const dConfig =
        template.uiConfig.desktop || template.uiConfig.mascot
          ? {
              mascot:
                template.uiConfig.desktop?.mascot || template.uiConfig.mascot || defaults.mascot,
              bubble:
                template.uiConfig.desktop?.bubble || template.uiConfig.bubble || defaults.bubble,
              arrow:
                template.uiConfig.desktop?.arrowDirection ||
                template.uiConfig.arrowDirection ||
                'bottom',
      ...
      const mConfig = template.uiConfig.mobile
        ? {
            ...
            targetBox: template.uiConfig.mobile.targetBox || {
              ...defaults.targetBox,
              w: 80,
              h: 40,
            },
      ...
      const dBg = template.uiConfig.desktop?.bgImage || template.uiConfig.bgImage;
      const mBg = template.uiConfig.mobile?.bgImage;
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    else if (draggingTarget === 'targetBox')
      setActiveConfig({ targetBox: { ...activeConfig.targetBox, x: clampedX, y: clampedY } });
  };
  ...
  const toggleTargetActive = () => {
    setActiveConfig({
      targetBox: { ...activeConfig.targetBox, active: !activeConfig.targetBox.active },
    });
  };
  ...
  return (
    <div className="fixed inset-0 z-admin-modal bg-black flex flex-col animate-in fade-in select-none">
      ...
      >
        ...
        >
          ...
          {activeConfig.targetBox.active && (
            ...
            >
              <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-indigo-600 text-white text-[8px] font-black uppercase px-2 py-0.5 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                Target Area
              </div>
            </div>
          )}
          ...
          {/* GUIDES & CONNECTOR */}
          <div
            className="absolute inset-0 pointer-events-none opacity-10 z-floating-panel"
            style={{
          ...
          <svg
            aria-hidden="true"
            className="absolute inset-0 w-full h-full pointer-events-none z-floating-panel opacity-50"
          >
            <line
              x1={`${activeConfig.mascot.x}%`}
              y1={`${activeConfig.mascot.y}%`}
              x2={`${activeConfig.bubble.x}%`}
              y2={`${activeConfig.bubble.y}%`}
              stroke="#6366f1"
              strokeWidth="2"
              strokeDasharray="5,5"
            />
          ...
          >
            <div
              className={`absolute w-4 h-4 md:w-6 md:h-6 bg-white border-l border-t border-slate-100 ${getBubbleArrowClass(activeConfig.arrow)}`}
            ></div>
            ...
            <h3
              className={
                titleStyle ||
                'font-display font-black text-indigo-600 text-2xl leading-none tracking-tight mb-2'
              }
            >
            ...
            <p
              className={`${textStyle || 'text-slate-600 leading-relaxed font-medium'} mb-4 whitespace-pre-line`}
            >
            ...
            <div className="flex justify-between items-center pt-3 border-t border-slate-100">
              ...
              <div
                className={`bg-indigo-600 px-4 py-2 md:px-6 md:py-3 rounded-xl md:rounded-2xl shadow-lg uppercase tracking-wide flex items-center justify-center ${btnStyle || 'text-[10px] md:text-xs font-bold text-white'}`}
              >
                AVANTI
              </div>
            </div>
      ...
      {/* HEADER TOOLS */}
      <div
        className={`absolute top-0 left-0 right-0 z-admin-modal transition-transform duration-300 ${hideUI ? '-translate-y-full' : 'translate-y-0'}`}
      >
        <div className="bg-slate-900/95 backdrop-blur-md border-b border-slate-800 p-2 md:p-3 shadow-2xl flex flex-col gap-3 overflow-x-auto">
          <div className="flex w-full items-center justify-between gap-2 px-2 flex-wrap md:flex-nowrap">
            <div className="flex items-center gap-3">
              <div className="flex bg-slate-950 p-1 rounded-lg border border-slate-800">
                <button
                  ...
                  className={`p-2 rounded ${viewport === 'desktop' ? 'bg-indigo-600 text-white' : 'text-slate-500'}`}
                >
                ...
                <button
                  ...
                  className={`p-2 rounded ${viewport === 'mobile' ? 'bg-indigo-600 text-white' : 'text-slate-500'}`}
                >
              ...
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 rounded-lg border transition-colors text-[10px] font-bold uppercase tracking-wide ${currentBg ? 'bg-amber-600 border-amber-500 text-white' : 'bg-slate-800 border-slate-600 text-white hover:bg-slate-700'}`}
              >
                {isUploading ? (
                ...
                {currentBg ? 'Cambia' : 'Carica Sfondo'}
              </button>
              <input
                type="file"
                ref={bgInputRef}
            ...
            <div className="flex gap-2 ml-auto">
              <button
                ...
                className="p-2 bg-slate-800 text-slate-400 hover:text-white rounded-lg border border-slate-700 hidden md:block"
                title="Nascondi UI"
              >
              ...
              <button
                ...
                onClick={onClose}
                disabled={isUploading}
                className="px-4 py-2 bg-slate-800 text-slate-300 rounded-xl font-bold text-xs uppercase hover:text-white transition-colors border border-slate-700"
              >
                Annulla
              </button>
              <button
                ...
                onClick={handleSave}
                disabled={isUploading}
                className="px-6 py-2 bg-emerald-600 text-white rounded-xl font-bold text-xs uppercase shadow-lg hover:bg-emerald-500 flex items-center gap-2 transition-all active:scale-95 border border-emerald-500 disabled:opacity-50"
              >
          ...
          <div className="w-full flex flex-col md:flex-row items-center gap-4 bg-slate-950 p-2 rounded-xl border border-slate-800">
            <div className="flex items-center gap-2 flex-1 w-full">
              <label
                htmlFor="onboarding-visual-editor-target-id"
                className="text-[9px] font-black text-slate-500 uppercase tracking-widest pl-2 hidden md:block"
              >
                Target ID:
              </label>
              <div className="relative group flex-1">
                ...
                <input
                  id="onboarding-visual-editor-target-id"
                  type="text"
                  value={activeConfig.targetId || ''}
                  onChange={handleTargetIdChange}
                  placeholder="Inserisci ID HTML Elemento..."
                  className="w-full bg-slate-900 border border-slate-700 rounded-lg pl-9 pr-8 py-1.5 text-xs text-white focus:border-indigo-500 outline-none"
                />
            ...
            <div className="flex items-center gap-2 justify-between w-full md:w-auto">
              <button
                type="button"
                onClick={toggleTargetActive}
                className={`p-1.5 rounded-lg text-[10px] font-bold uppercase flex items-center gap-1.5 ${activeConfig.targetBox.active ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white bg-slate-900 border border-slate-700'}`}
              >
              ...
              {activeConfig.targetBox.active && (
                <div className="flex items-center gap-1">
                  <input
                    type="number"
                    value={activeConfig.targetBox.w}
                    onChange={(e) => handleTargetSizeChange('w', parseInt(e.target.value, 10))}
                    className="w-10 bg-slate-900 border border-slate-700 rounded text-center text-white text-[10px] py-1"
                  />
                  <span className="text-slate-600 text-[10px]">x</span>
                  <input
                    type="number"
                    value={activeConfig.targetBox.h}
                    onChange={(e) => handleTargetSizeChange('h', parseInt(e.target.value, 10))}
                    className="w-10 bg-slate-900 border border-slate-700 rounded text-center text-white text-[10px] py-1"
                  />
                </div>
              )}
              ...
              <div className="grid grid-cols-3 gap-1 bg-slate-900 p-1 rounded border border-slate-800">
                ...
                <div className="w-6 h-6 flex items-center justify-center">
                  <Grid className="w-3 h-3 text-slate-700" />
                </div>
      ...
      {hideUI && (
        <button
          ...
          className="absolute top-4 right-4 z-admin-modal bg-indigo-600 text-white px-4 py-2 rounded-full shadow-lg font-bold text-xs uppercase animate-pulse border-2 border-white/20"
        >
          Mostra Strumenti
        </button>
      )}
