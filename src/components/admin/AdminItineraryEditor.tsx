...
import {
  ...
  savePremadeItinerary,
} from '../../services/communityService';
import type { PointOfInterest, PremadeItinerary } from '../../types/index';
...
export const AdminItineraryEditor = ({ itineraryId, onBack }: Props) => {
  ...
  const [validationModal, setValidationModal] = useState<{
    ...
    loading: boolean;
    report: string | null;
  }>({ isOpen: false, loading: false, report: null });
  ...
  const handleSave = async () => {
    ...
    if (formData.status === 'published') {
      if (editorItems.length === 0) {
        alert(
          'Non puoi pubblicare un itinerario vuoto! Aggiungi almeno una tappa nella Timeline o salvalo come Bozza.',
        );
        return;
      }
      if (formData.tags.length === 0) {
        alert(
          "OBBLIGATORIO: Devi inserire almeno un TAG per pubblicare l'itinerario (es. 'Arte', 'Mare').",
        );
        return;
      }
    }
  ...
  const requestPublicStatus = async () => {
    ...
    try {
      ...
      setValidationModal({
        isOpen: true,
        loading: false,
        report: response.text || 'Nessun report generato.',
      });
    } catch (e: unknown) {
      ...
      setValidationModal({
        isOpen: true,
        loading: false,
        report: `⚠️ Errore di connessione AI: ${e instanceof Error ? e.message : String(e)}. Verifica manuale consigliata.`,
      });
    }
  };
  ...
  const updateField = (
    field: keyof PremadeItinerary,
    value: PremadeItinerary[keyof PremadeItinerary],
  ) => {
  ...
  const addItemToDay = (poi: PointOfInterest, dayIndex: number) => {
    ...
    setEditorItems((prev) =>
      [...prev, newItem].sort((a, b) => a.timeSlotStr.localeCompare(b.timeSlotStr)),
    );
  };

  const updateItem = (id: string, field: keyof EditorItem, value: EditorItem[keyof EditorItem]) => {
    setEditorItems((prev) =>
  ...
  const isPublishingInvalid =
    formData.status === 'published' && (editorItems.length === 0 || formData.tags.length === 0);
        ...
        <div className="flex gap-3 items-center">
          ...
          <div className="flex bg-slate-800 p-1 rounded-lg border border-slate-700">
            <button
              ...
              className={`px-4 py-1.5 rounded-md text-xs font-bold uppercase transition-all ${activeTab === 'general' ? 'bg-slate-700 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}
            >
              Generali
            </button>
            <button
              ...
              className={`px-4 py-1.5 rounded-md text-xs font-bold uppercase transition-all ${activeTab === 'timeline' ? 'bg-slate-700 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}
            >
              Timeline
            </button>
          </div>
              ...
              <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    <div>
                      <label
                        htmlFor="fld-admin-adminitineraryeditor-tsx-l431"
                        className="text-xs font-bold text-slate-500 uppercase block mb-1"
                      >
                        Titolo Itinerario
                      </label>
                      <input
                        id="fld-admin-adminitineraryeditor-tsx-l431"
                        value={formData.title}
                        onChange={(e) => updateField('title', e.target.value)}
                        className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white font-bold"
                        placeholder="Es. Napoli in 3 Giorni"
                      />
                    </div>
                    <div>
                      <label
                        htmlFor="fld-admin-adminitineraryeditor-tsx-l442"
                        className="text-xs font-bold text-slate-500 uppercase block mb-1"
                      >
                        Città Principale
                      </label>
                      <input
                        id="fld-admin-adminitineraryeditor-tsx-l442"
                        value={formData.mainCity}
                        onChange={(e) => updateField('mainCity', e.target.value)}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label
                          htmlFor="fld-admin-adminitineraryeditor-tsx-l454"
                          className="text-xs font-bold text-slate-500 uppercase block mb-1"
                        >
                          Durata (Giorni)
                        </label>
                        <input
                          id="fld-admin-adminitineraryeditor-tsx-l454"
                          type="number"
                          min="1"
                          max="10"
                          value={formData.durationDays}
                          onChange={(e) =>
                            updateField('durationDays', parseInt(e.target.value, 10))
                          }
                          className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white"
                        />
                      </div>
                      <div>
                        <label
                          htmlFor="fld-admin-adminitineraryeditor-tsx-l469"
                          className="text-xs font-bold text-slate-500 uppercase block mb-1"
                        >
                          Difficoltà
                        </label>
                        <select
                          id="fld-admin-adminitineraryeditor-tsx-l469"
                  ...
                  <div className="space-y-4">
                    <div>
                      <label
                        htmlFor="fld-admin-adminitineraryeditor-tsx-l486"
                        className="text-xs font-bold text-slate-500 uppercase block mb-1"
                      >
                        Tipologia
                      </label>
                      <select
                        id="fld-admin-adminitineraryeditor-tsx-l486"
                        value={formData.type}
                    ...
                    <div>
                      <label
                        htmlFor="fld-admin-adminitineraryeditor-tsx-l500"
                        className="text-xs font-bold text-slate-500 uppercase block mb-1 flex justify-between"
                      >
                        Tag (Separati da virgola)
                        {isPublishingInvalid && (
                          <span className="text-red-500 animate-pulse text-[10px]">RICHIESTO</span>
                        )}
                      </label>
                      <input
                        id="fld-admin-adminitineraryeditor-tsx-l500"
                    ...
                    <div>
                      <label
                        htmlFor="fld-admin-adminitineraryeditor-tsx-l549"
                        className="text-xs font-bold text-slate-500 uppercase block mb-1"
                      >
                        Autore (Opzionale)
                      </label>
                      <input
                        id="fld-admin-adminitineraryeditor-tsx-l549"
                ...
                <div className="mt-6">
                  <label
                    htmlFor="fld-admin-adminitineraryeditor-tsx-l561"
                    className="text-xs font-bold text-slate-500 uppercase block mb-1"
                  >
                    Descrizione Completa
                  </label>
                  <textarea
                    id="fld-admin-adminitineraryeditor-tsx-l561"
                    rows={4}
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <img
                        src={poi.imageUrl}
                        alt={poi.name}
                        className="w-10 h-10 rounded object-cover bg-slate-700 shrink-0"
                      />
                      ...
                      {dayItems.length === 0 ? (
                        ...
                        <div className="space-y-3">
                          {dayItems.map((item) => (
                            <div
                              key={item.id}
                              className="bg-slate-800 border border-slate-700 rounded-xl p-3 flex gap-4 items-start group"
                            >
                              <div className="w-20 pt-1">
                                <input
                                  ...
                                  onChange={(e) =>
                                    updateItem(item.id, 'timeSlotStr', e.target.value)
                                  }
              ...
              <>
                ...
                <div className="mt-6 flex justify-end gap-3">
                  <button
                    type="button"
                    onClick={cancelPublish}
                    className="px-4 py-2 rounded-lg bg-slate-800 text-slate-300 hover:text-white hover:bg-slate-700 transition-colors text-xs font-bold uppercase"
                  >
                    Correggi
                  </button>
                  <button
                    ...
                    className="px-6 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-500 transition-colors text-xs font-bold uppercase shadow-lg flex items-center gap-2"
                  >
