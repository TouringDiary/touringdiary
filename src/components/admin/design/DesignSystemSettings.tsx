...
import {
  rebuildDesignSystemCache,
  updateDesignSystemRule,
...
const DesignSystemSettings: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const rulesMap = Object.entries(source).reduce<Record<string, StyleRule>>(
      (acc, [key, value]) => {
  ...
  const currentTab = TABS.length > 0 ? (TABS.includes(activeTab) ? activeTab : TABS[0]) : '';
  ...
  if (isConfigLoading) {
    return (
      <div className="p-8 flex items-center justify-center gap-3">
        ...
        <span className="text-slate-400">Caricamento Design System...</span>
      </div>
    );
  }
      ...
      <div style={{ paddingBottom: '80px' }}>
        <div className="p-8 text-center bg-slate-800 rounded-lg border border-yellow-500/50">
          ...
          <p className="mt-2 text-slate-300">
            La cache del design system è vuota o non è stata generata.
          </p>
          <p className="mt-1 text-sm text-slate-400">
            Premi il pulsante qui sotto per processare le regole di stile e costruire la cache.
          </p>
          {rebuildError && (
              ...
              {groupedRules.map(({ key, desktop, mobile }) => (
                <tr key={key} className="hover:bg-slate-800/50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap bg-slate-900">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="bg-slate-800 rounded-lg border border-slate-700">
                        <div className="flex items-center gap-2 mb-2 text-xs text-slate-400 px-3 pt-2">
                          <Monitor size={14} /> Desktop
                        </div>
                        <ComponentPreviewHost
                          rule={desktop}
                          componentKey={desktop.component_key}
                          isLarge={false}
                          isMobile={false}
                        />
                      </div>
                      {mobile && (
                        <div className="bg-slate-800 rounded-lg border border-slate-700">
                          ...
                          <ComponentPreviewHost
                            rule={mobile}
                            componentKey={mobile.component_key}
                            isLarge={false}
                            isMobile={true}
                          />
                        </div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button
                      type="button"
                      onClick={() => setSelectedBaseKey(key)}
