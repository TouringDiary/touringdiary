import { Aperture, Loader2, PenTool, Wand2 } from 'lucide-react';
...
const STYLES = {
  city_real: {
    label: 'Città / Card (Reale)',
    icon: Aperture,
    dbKey: 'safe_art_city_real',
    defaultPrompt: '...',
    previewUrl: '...',
  },
  // ...
...
export const SafeArtPanel = ({
  ...
  const generateImage = async () => {
    ...
    try {
      ...
      const generatedBase64 = dataUrl?.includes('base64,') ? dataUrl.split('base64,')[1] : '';

      if (generatedBase64) {
        const file = dataURLtoFile(
          `data:image/png;base64,${generatedBase64}`,
          `ai_gen_${Date.now()}.png`,
        );
      ...
      onError(`Errore generazione: ${e instanceof Error ? e.message : String(e)}`);
    } finally {
  ...
  return (
    <div className="flex flex-col gap-5 animate-in fade-in bg-slate-950 p-5 rounded-2xl border border-indigo-500/30 shadow-xl">
      ...
      <button
        ...
        className="w-full bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white py-4 rounded-xl font-bold text-xs uppercase tracking-widest shadow-xl shadow-indigo-900/20 transition-all flex items-center justify-center gap-2 mt-2 border border-indigo-400/20 hover:scale-[1.02] active:scale-95"
      >
        {isGenerating ? (
