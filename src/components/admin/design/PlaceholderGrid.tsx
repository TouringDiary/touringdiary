import { Crop, Layers, Trash2, Upload } from 'lucide-react';
...
export const PlaceholderGrid = ({
  title = 'Placeholder di Categoria',
  description = 'Usati automaticamente se manca la foto del POI',
      ...
      <div className="overflow-y-auto custom-scrollbar pr-2 flex-1">
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 pb-4">
          {categories.map((cat) => (
            <div
              key={cat.id}
              className="relative aspect-video w-full rounded-xl overflow-hidden border border-slate-700 group bg-black/20 shrink-0"
            >
              <ImageWithFallback
                src={placeholders[cat.id]}
                alt={cat.label}
              ...
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 backdrop-blur-sm">
                <button
                  ...
                  className="p-2 bg-indigo-600 rounded-lg text-white hover:bg-indigo-500 shadow-lg transition-transform hover:scale-110"
                  title="Carica"
                >
                ...
                {placeholders[cat.id] && (
                  <button
                    type="button"
                    onClick={() => onEditClick(placeholders[cat.id], cat.id)}
                    className="p-2 bg-slate-700 rounded-lg text-white hover:bg-slate-600 shadow-lg transition-transform hover:scale-110"
                    title="Modifica"
                  >
                ...
                {placeholders[cat.id] && onDeleteClick && (
                  <button
                    ...
                    className="p-2 bg-red-700 rounded-lg text-white hover:bg-red-600 shadow-lg transition-transform hover:scale-110"
                    title="Elimina"
                  >
