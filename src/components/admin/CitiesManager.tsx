...
export const CitiesManager = ({ onEdit, currentUser }: CitiesManagerProps) => {
  ...
  const handleMagicAddRequest = async (name: string, poiCount: number) => {
    const existing = list.effectiveCities.find(
      (c: CitySummary) => c.name.toLowerCase() === name.toLowerCase(),
    );
    ...
    if (existing) {
      ...
      if (displayStatus === 'Online') {
        alert(
          `La città "${existing.name}" è già ONLINE. Non puoi rigenerarla da qui. Usa la modifica manuale.`,
        );
        return;
      }
  ...
  const confirmMergeGeneration = () => {
    ...
    startGeneration(
      ...
      conflictData.poiCount,
      currentUser,
      conflictData.existingCity.id,
    );
    ...
    <div className="space-y-6 animate-in fade-in flex flex-col h-full max-w-full overflow-hidden relative">
      ...
      {showAiModal && (
        <CityGeneratorModal
          ...
          onGenerate={handleMagicAddRequest}
      ...
      <div className="flex-1 min-h-0 relative">
        {activeTab === 'map' ? (
          ...
          <CitiesListTab list={list} onEdit={onEdit} currentUser={currentUser} />
        ) : (
          <ObservatoryLayout />
        )}
      </div>
