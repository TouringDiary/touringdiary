import { Camera, ChevronRight, Loader2, UserPlus, Users } from 'lucide-react';
...
type AdminTab = 'categories' | 'person_suggestions' | 'photo_suggestions';
...
function useDialogFocusTrap(active: boolean, dialogRef: RefObject<HTMLDivElement | null>): void {
  useEffect(() => {
    if (!active) return;
...
export type AdminFamousPeopleEmbeddedSection = 'person_suggestions' | 'photo_suggestions';

type AdminFamousPeopleManagerProps = {
  /** Quando impostato, renderizza solo la sezione indicata (hub Segnalazioni MF2). */
  embeddedSection?: AdminFamousPeopleEmbeddedSection;
};

export const AdminFamousPeopleManager = ({
  embeddedSection,
}: AdminFamousPeopleManagerProps = {}) => {
  ...
  const [tab, setTab] = useState<AdminTab>(embeddedSection ?? 'categories');
  ...
  const [isLoading, setIsLoading] = useState(!!embeddedSection);
  ...
  const loadData = useCallback(async (): Promise<{
    ...
    setIsLoading(true);
    try {
      if (embeddedSection === 'person_suggestions') {
        const persons = await listFamousPersonSuggestionsForAdmin();
        setPersonSuggestions(persons);
        return { personSuggestions: persons, photoSuggestions: [] };
      }
      const photos = await listFamousPersonPhotoSuggestionsForAdmin();
      setPhotoSuggestions(photos);
      return { personSuggestions: [], photoSuggestions: photos };
    } catch {
  ...
  const anyActive = selectedPerson !== null || selectedPhoto !== null;
  ...
  const activeTab = embeddedSection ?? tab;
      ...
      {!embeddedSection ? (
        <>
          <AdminPageHeader
            ...
            subtitle="Categorie personaggio — suggerimenti e segnalazioni abuso sono in Segnalazioni"
            accent="indigo"
          />
