...
export const ImportDashboard = () => {
  ...
  const handleApplyFilters = (newFilters: {
    ...
    if (newFilters.interest) {
      if (
        newFilters.interest === 'all' ||
        (Array.isArray(newFilters.interest) && newFilters.interest.includes('all'))
      ) {
        dataLogic.setFilterAiRating([]);
      } else {
        dataLogic.setFilterAiRating(
          Array.isArray(newFilters.interest) ? newFilters.interest : [newFilters.interest],
        );
      }
    }
  ...
  const activeFiltersCount =
    (dataLogic.filterAiRating.length > 0 ? 1 : 0) +
    (dataLogic.filterRawCategories.length > 0 ? 1 : 0);

  return (
    <div className="flex flex-col h-full space-y-4 animate-in fade-in">
      {/* MODALI & DRAWER */}
      {showGuideModal && (
        <AdminGuideModal guideKey="admin_manual_cities" onClose={() => setShowGuideModal(false)} />
      )}
        ...
        actions={
          <>
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-xs font-bold uppercase tracking-widest shadow-lg transition-all border border-blue-500"
            >
            ...
            <button
              type="button"
              onClick={() => dataLogic.refreshData()}
              disabled={!dataLogic.selectedCityId}
              className="p-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg border border-slate-700 disabled:opacity-50 transition-colors"
            ...
            <button
              type="button"
              onClick={() => activeCity && actionLogic.handleRecoverOrphans(activeCity.name)}
              disabled={!dataLogic.selectedCityId || dataLogic.isLoading}
              className="flex items-center gap-2 px-3 py-2 bg-indigo-950/30 hover:bg-indigo-900/50 text-indigo-400 hover:text-indigo-300 rounded-lg border border-indigo-900/30 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
              title="Recupera elementi staging rimasti orfani"
            >
      ...
      {dataLogic.selectedCityId ? (
        <>
          <div className="flex flex-col md:flex-row gap-4 items-start">
            ...
            <button
              ...
              className="bg-slate-800 hover:bg-emerald-900/30 text-slate-300 hover:text-emerald-400 px-4 py-3 rounded-xl text-[10px] font-bold uppercase flex items-center justify-center gap-2 border border-slate-700 transition-colors w-full md:w-auto h-full"
            >
              {isExporting ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-slate-500 bg-slate-900/30 border border-slate-800 border-dashed rounded-xl m-4">
          ...
          <p className="font-bold uppercase tracking-widest text-sm">
            Seleziona una città per iniziare
          </p>
        </div>
      )}
    </div>
