...
type ImportFilterStatus = 'all' | 'new' | 'ready' | 'imported' | 'discarded';
type ImportStatKey = Exclude<ImportFilterStatus, 'all'>;
...
const FILTER_KEYS = [
  ...
  'imported',
  'discarded',
] as const satisfies readonly ImportFilterStatus[];

interface ImportStatsBarProps {
  stats: Record<ImportStatKey, number>;
  grandTotal: number;
  currentFilter: ImportFilterStatus;
  onFilterChange: (status: ImportFilterStatus) => void;
}
...
export const ImportStatsBar = ({
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
      {FILTER_KEYS.map((statKey) => (
        <button
          type="button"
          key={statKey}
          onClick={() => onFilterChange(statKey)}
          className={`cursor-pointer p-3 rounded-xl border flex items-center justify-between transition-all text-left w-full ${currentFilter === statKey ? 'bg-indigo-900/30 border-indigo-500 shadow-md' : 'bg-slate-900 border-slate-800 hover:border-slate-700'}`}
        >
          <span
            className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 ${currentFilter === statKey ? 'text-indigo-300' : 'text-slate-500'}`}
          >
            {statKey === 'all' && <Layers className="w-3.5 h-3.5" />} {statKey.toUpperCase()}
          </span>
          <span
            className={`text-xl font-black ${currentFilter === statKey ? 'text-white' : 'text-slate-500'}`}
          >
            {statKey === 'all' ? grandTotal : stats[statKey]}
          </span>
        </button>
      ))}
    </div>
