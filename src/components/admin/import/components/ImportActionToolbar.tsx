    ...
    <div className="flex flex-wrap gap-2 items-center w-full justify-end">
      ...
      {selectedCount > 0 ? (
        <>
          ...
          <button
            type="button"
            onClick={() => onBulkStatus('ready')}
            disabled={isBulkUpdating}
            className="bg-emerald-600 hover:bg-emerald-500 text-white px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase flex items-center gap-2 shadow-lg transition-colors disabled:opacity-50 min-w-max"
          >
            {isBulkUpdating ? (
              <Loader2 className="w-3.5 h-3.5 animate-spin" />
            ) : (
          ...
          <div className="flex bg-slate-950 p-1 rounded-lg border border-slate-800 shrink-0">
            ...
            <button
              ...
              disabled={isBulkUpdating}
              className="p-2 text-slate-500 hover:text-red-500 hover:bg-red-900/20 rounded transition-colors disabled:opacity-50"
        ...
        <span className="text-[10px] text-slate-500 italic ml-auto md:ml-0">
