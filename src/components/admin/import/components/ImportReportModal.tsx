  ...
  return (
    <div className="fixed inset-0 z-admin-modal flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl p-6 relative flex flex-col items-center text-center">
        <div
          className={`w-20 h-20 rounded-full flex items-center justify-center border-4 shadow-xl mb-4 ${report.quotaExceeded ? 'bg-amber-500/20 border-amber-500' : 'bg-emerald-500/20 border-emerald-500'}`}
        >
        ...
        <div className="grid grid-cols-2 gap-4 w-full mb-6">
          <div className="bg-slate-950 p-3 rounded-xl border border-slate-800">
            ...
            <div className="text-xl font-black text-white">
              {report.processed}{' '}
              <span className="text-sm text-slate-600">/ {report.totalRequested}</span>
            </div>
          </div>
        ...
        <button
          type="button"
          onClick={onClose}
          className="w-full py-3 bg-slate-800 hover:bg-slate-700 text-white rounded-xl font-bold uppercase text-xs transition-colors"
        >
          Chiudi Report
        </button>
      </div>
