import { AlertTriangle, CheckCircle, Megaphone } from 'lucide-react';
import { useEffect, useState } from 'react';
import { CloseButton } from '@/components/ui/controls/CloseButton';
import {
  type AdminMessageLog,
  deleteSystemMessageAsync,
  getCommunicationLogsAsync,
...
export const AdminCommunications = () => {
  const [activeTab, setActiveTab] = useState<
    'compose' | 'history' | 'templates' | 'onboarding_desktop' | 'onboarding_mobile'
  >('compose');
  ...
  const [composerDraft, setComposerDraft] = useState<{ subject: string; body: string } | null>(
    null,
  );
    ...
    <div className="space-y-6 h-full flex flex-col animate-in fade-in relative">
      ...
      <div className="flex bg-slate-900 p-1.5 rounded-xl border border-slate-800 w-fit shrink-0 mb-4 shadow-lg overflow-x-auto gap-1">
        <button
          ...
          className={`px-4 py-2.5 rounded-lg text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === 'compose' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-white hover:bg-slate-800'}`}
        >
          Nuovo Messaggio
        </button>
        <button
          ...
          className={`px-4 py-2.5 rounded-lg text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === 'history' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-white hover:bg-slate-800'}`}
        >
          STORICO
        </button>
        <button
          ...
          className={`px-4 py-2.5 rounded-lg text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === 'templates' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-white hover:bg-slate-800'}`}
        >
          Standard
        </button>
        ...
        <button
          type="button"
          onClick={() => setActiveTab('onboarding_desktop')}
          className={`px-4 py-2.5 rounded-lg text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === 'onboarding_desktop' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-white hover:bg-slate-800'}`}
        >
          GUIDA DESKTOP
        </button>
        <button
          type="button"
          onClick={() => setActiveTab('onboarding_mobile')}
          className={`px-4 py-2.5 rounded-lg text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === 'onboarding_mobile' ? 'bg-purple-600 text-white shadow-lg' : 'text-slate-500 hover:text-white hover:bg-slate-800'}`}
        >
          GUIDA MOBILE
        </button>
      </div>
      ...
      <div className="flex-1 bg-slate-900 rounded-3xl border border-slate-800 shadow-2xl overflow-hidden flex flex-col min-h-0 relative">
        {activeTab === 'compose' && (
          <CommsComposer onSent={refreshData} showToast={showToast} initialDraft={composerDraft} />
        )}
        {activeTab === 'history' && <CommsHistory logs={logs} />}
        {activeTab === 'templates' && (
          <CommsTemplates
            templates={templates.filter(
              (t) => !t.key.startsWith('onboarding') && t.key !== 'social_share_global',
            )}
          ...
          <div className="flex flex-col h-full">
            <div className="p-6 bg-blue-900/10 border-b border-blue-500/20 text-center">
              <h3 className="text-blue-400 font-bold uppercase tracking-widest text-sm mb-1">
                Editor Guida Desktop
              </h3>
              <p className="text-slate-400 text-xs">
                Questa lista viene mostrata SOLO su PC/Tablet.
              </p>
            </div>
            <CommsTemplates
        ...
        {activeTab === 'onboarding_mobile' && (
          <div className="flex flex-col h-full">
            <div className="p-6 bg-purple-900/10 border-b border-purple-500/20 text-center">
              <h3 className="text-purple-400 font-bold uppercase tracking-widest text-sm mb-1">
                Editor Guida Mobile
              </h3>
              <p className="text-slate-400 text-xs">
                Questa lista viene mostrata SOLO su Smartphone.
              </p>
            </div>
            <CommsTemplates
