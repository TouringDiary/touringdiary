...
export const AdminPageAiAssistant = ({ section }: Props) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'model', text: response.text || 'Non ho capito, puoi ripetere?' },
      ]);
    } catch (error: unknown) {
      setMessages((prev) => [
        ...prev,
        {
          role: 'model',
          text: `Errore AI: ${error instanceof Error ? error.message : String(error)}`,
        },
      ]);
    } finally {
      setLoading(false);
        ...
        <div className="absolute top-14 left-0 z-admin-modal bg-slate-900 border border-slate-700 rounded-xl shadow-2xl w-80 md:w-96 h-[400px] flex flex-col overflow-hidden animate-in zoom-in-95 origin-top-left">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-950" ref={scrollRef}>
            {messages.map((m, i) => (
              ...
              >
                <div
                  className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${m.role === 'user' ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-slate-800 text-slate-200 rounded-tl-none border border-slate-700'}`}
                >
                  <p>{m.text}</p>
                </div>
              </div>
          ...
          <div className="p-3 bg-slate-900 border-t border-slate-800">
            <div className="flex gap-2 relative">
              <input
                ...
                placeholder="Chiedi all'AI Pro..."
                className="flex-1 bg-slate-950 border border-slate-700 rounded-xl pl-4 pr-12 py-3 text-sm text-white focus:border-indigo-500 outline-none"
              />
              <button
                type="button"
                onClick={handleSend}
                disabled={!input.trim() || loading}
                className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg shadow-lg"
              >
