          ...
          <table className="w-full text-left min-w-[760px]">
            ...
            <tbody>
              {filtered.map((event) => {
                ...
                const actorName = event.actorId ? actorNames[event.actorId] : undefined;
                return (
                  <tr key={event.id} className="border-b border-slate-800/80">
                    ...
                    <td className={`p-2 ${ty.tableCell}`}>
                      {actorName ? (
                        <span title={event.actorId ?? undefined}>{actorName}</span>
                      ) : event.actorId ? (
                        <span className={ty.helper} title={event.actorId}>
                    ...
                    {canWrite ? (
                      <td className="p-2">
                        <button
                          ...
                          disabled={isDeleting}
                          onClick={() => setPendingDelete({ kind: 'single', event })}
      ...
      >
        <label className="mt-3 flex items-start gap-2 text-left text-xs text-slate-300 cursor-pointer">
          <input
            ...
            className="mt-0.5"
          />
          <span>Confermo di voler cancellare in modo permanente l’intero Storico Audit.</span>
        </label>
      </DeleteConfirmationModal>
    </div>
  );
