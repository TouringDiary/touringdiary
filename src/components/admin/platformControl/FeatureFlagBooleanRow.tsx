    ...
    return (
      ...
      >
        <div className="min-w-0 flex-1">
          ...
          {adminHelp ? (
            <p className={`${ty.helper} mt-0.5 line-clamp-2 leading-snug`}>{adminHelp}</p>
          ) : null}
        </div>
        <button
          type="button"
  ...
  return (
    <article
      className={`h-full flex flex-col rounded-2xl border p-3 sm:p-4 gap-2.5 transition-colors ${
        effectiveOn ? 'border-emerald-500/25 bg-slate-950/80' : 'border-slate-800 bg-slate-950/50'
      }`}
    >
      <div className="flex items-start justify-between gap-3 min-w-0">
        <div className="min-w-0 flex-1 space-y-1">
          ...
          <p className={`${ty.cardDescription} whitespace-pre-line`}>{adminHelp}</p>
        </div>
        <button
          type="button"
          role="switch"
