...
const KpiCard = ({ label, value, subValue, trend, prefix = '€' }: KpiCardProps) => (
  <div className="p-4 bg-slate-950/50 rounded-2xl border border-slate-800 flex flex-col gap-1">
    ...
    <div className="flex items-baseline gap-1">
      ...
      {trend !== undefined && (
        <div
          className={`flex items-center text-[10px] font-bold ${trend >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}
        >
          {trend >= 0 ? (
  ...
  return (
    <div className="space-y-8 animate-in fade-in pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        <SectionCard title="Ricavi Sponsor" icon={Euro} subtitle="Entrate mensili ricorrenti (MRR)">
          <div className="space-y-4">
            ...
            <KpiCard
              label="Incasso ult. 30gg"
              value={data.mrr}
              subValue="Totale reale da subscriptions"
            />
          </div>
        </SectionCard>

        <SectionCard
          title="Costi AI Reali"
          icon={Zap}
          subtitle="Basati su consumi effettivi ai_global_usage"
        >
          <div className="grid grid-cols-1 gap-3">
            <KpiCard
              label="Costo Ultimi 30gg"
              value={data.costs.thirtyDays.total}
              trend={monthTrend}
            />
            <div className="flex gap-2">
              <div className="flex-1 p-3 bg-slate-950/30 rounded-xl border border-slate-800">
                ...
                <p className="text-sm font-bold text-indigo-400">
                  €{data.costs.thirtyDays.flash.toFixed(2)}
                </p>
              </div>
              <div className="flex-1 p-3 bg-slate-950/30 rounded-xl border border-slate-800">
                <p className="text-[10px] font-black text-slate-500 uppercase">Pro</p>
                <p className="text-sm font-bold text-rose-400">
                  €{data.costs.thirtyDays.pro.toFixed(2)}
                </p>
              </div>
            </div>
          </div>
        ...
        <SectionCard
          title="Margine Netto (Stima)"
          icon={BarChart3}
          subtitle="Revenue - Infrastruttura AI (30gg)"
        >
          <div className="space-y-4">
            <div className="p-4 bg-slate-950/50 rounded-2xl border border-slate-800">
              ...
              <p
                className={`text-4xl font-black mt-1 ${realMargin >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}
              >
                €{Math.abs(realMargin).toLocaleString('it-IT', { minimumFractionDigits: 2 })}
              </p>
              <div className="mt-2 flex items-center gap-2">
                <span
                  className={`px-2 py-0.5 rounded-full text-[10px] font-black ${realMarginPercent > 30 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'}`}
                >
        ...
        <SectionCard
          title="Performance Oggi"
          icon={ArrowUpRight}
          subtitle="Dati real-time ultima finestra 24h"
        >
          <div className="space-y-4">
            <KpiCard
              label="Costo Oggi"
              value={data.costs.today.flash + data.costs.today.pro}
              subValue="Aggiornato in tempo reale"
            />
            <div className="grid grid-cols-2 gap-2">
              <div className="text-center p-2 bg-slate-950/30 rounded-xl border border-slate-800">
                ...
                <p className="text-xs font-bold text-white">
                  €{data.costs.today.flash.toFixed(2)} / €{data.costs.today.pro.toFixed(2)}
                </p>
              </div>
              <div className="text-center p-2 bg-slate-950/30 rounded-xl border border-slate-800">
                <p className="text-[8px] text-slate-500 font-black uppercase">7 Giorni</p>
                <p className="text-xs font-bold text-white">
                  €{data.costs.sevenDays.total.toFixed(2)}
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        <SectionCard
          title="Breakdown per Target"
          icon={Users}
          subtitle="Distribuzione costi AI (Ultimi 30 giorni)"
        >
          <div className="space-y-4 mt-2">
            {COST_TARGET_BREAKDOWN.map((item) => {
              const val = data.costs.thirtyDays[item.key];
              const percent =
                data.costs.thirtyDays.total > 0 ? (val / data.costs.thirtyDays.total) * 100 : 0;
              return (
                <div key={item.key} className="space-y-1">
                  <div className="flex justify-between text-[10px] font-black uppercase tracking-tight">
                    <span className="text-slate-400">{item.label}</span>
                    <span className="text-white">
                      €{val.toFixed(2)} ({percent.toFixed(0)}%)
                    </span>
                  </div>
        ...
        <SectionCard
          ...
          icon={Leaf}
          subtitle="Distribuzione costi cumulativa storica"
        >
          <div className="grid grid-cols-2 gap-4 h-full content-center">
            {SEASON_BREAKDOWN.map((season) => (
              <div
                key={season.key}
                className="p-4 bg-slate-950/40 rounded-2xl border border-slate-800 flex items-center gap-3"
              >
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-white">
                    €{(data.trends.seasonal[season.key] || 0).toFixed(0)}
                  </p>
                </div>
              </div>
            ))}
          ...
          {Object.values(data.trends.seasonal).every((v) => v === 0) && (
            <p className="text-[9px] text-center text-slate-600 italic mt-4">
              In attesa di dati storici stagionali...
            </p>
          )}
        </SectionCard>
        ...
        <SectionCard
          ...
          icon={Calculator}
          subtitle="Simulatore locale (nessuna scrittura sul DB)"
        >
          <div className="space-y-5">
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between">
                <label
                  htmlFor="fld-admin-aieconomicsdashboard-tsx-l346"
                  className="text-[10px] font-black uppercase text-slate-400"
                >
                  Variazione Traffico
                </label>
                <span
                  className={`text-[10px] font-bold ${simTraffic >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}
                >
                  {simTraffic >= 0 ? '+' : ''}
                  {simTraffic}%
                </span>
              ...
              <input
                id="fld-admin-aieconomicsdashboard-tsx-l346"
                type="range"
                min="-90"
                max="500"
                step="10"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label
                  htmlFor="fld-admin-aieconomicsdashboard-tsx-l370"
                  className="text-[10px] font-black uppercase text-slate-400"
                >
                  Costo Flash (€)
                </label>
                <input
                  id="fld-admin-aieconomicsdashboard-tsx-l370"
                  type="number"
                  step="0.0001"
                  value={simFlashCost}
              ...
              <div className="space-y-1.5">
                <label
                  htmlFor="fld-admin-aieconomicsdashboard-tsx-l383"
                  className="text-[10px] font-black uppercase text-slate-400"
                >
                  Costo Pro (€)
                </label>
                <input
                  id="fld-admin-aieconomicsdashboard-tsx-l383"
                  type="number"
                  step="0.0001"
                  value={simProCost}
            ...
            <div className="p-4 bg-indigo-600/10 border border-indigo-500/20 rounded-2xl flex flex-col gap-2">
              ...
              <div className="flex justify-between items-center">
                ...
                <span
                  className={`text-sm font-black ${simulatorReady ? (estMargin >= 0 ? 'text-emerald-400' : 'text-rose-400') : 'text-slate-500'}`}
                >
                  {simulatorReady
      ...
      <div className="bg-slate-900/50 border border-slate-800 p-8 rounded-[40px] flex gap-8 items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-6">
          <GuideItem
            icon={Euro}
            text="Modificando i costi unitari modelli, influisci istantaneamente sulla spesa calcolata per ogni singola richiesta AI."
          />
          <GuideItem
            icon={Users}
            text="Il budget anonimi limita l'esposizione economica sui guest, mentre i limiti piani controllano l'infrastruttura inclusa negli abbonamenti."
          />
          <GuideItem
            icon={ShieldAlert}
            text="L'Emergency Stop è l'ultima linea di difesa: blocca ogni chiamata AI se i costi dovessero superare i parametri di sicurezza."
          />
        </div>
      </div>
    </div>
...
const GuideItem = ({ icon: Icon, text }: GuideItemProps) => (
  <div className="flex gap-3">
    ...
    <p className="text-[11px] leading-relaxed text-slate-400 italic">{text}</p>
  </div>
);
