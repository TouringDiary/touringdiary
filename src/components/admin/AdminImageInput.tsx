...
export const AdminImageInput = ({
  ...
  const handleLicenseChange = (newLicense: 'own' | 'cc' | 'public' | 'copyright') => {
    ...
    if (
      (newLicense === 'cc' || newLicense === 'copyright') &&
      (imageCredit === 'Opera Propria' || !imageCredit)
    )
      newCredit = '';
  ...
  const analyzeCopyright = async () => {
    ...
    try {
      ...
      let prompt =
        'Analizza questa immagine (o URL) per scopi di copyright in una guida turistica. Cerca watermark, loghi di agenzie stock.';
      prompt +=
        " Rispondi JSON: { status: 'safe'|'caution'|'danger', message: 'breve spiegazione' }";
    ...
    <div className="bg-slate-900 border border-slate-700 rounded-xl overflow-hidden mb-6">
      <div className="flex border-b border-slate-700">
        <button
          type="button"
          onClick={() => setMode('url')}
          className={`flex-1 py-3 text-xs font-bold uppercase flex items-center justify-center gap-2 ${mode === 'url' ? 'bg-slate-800 text-white' : 'text-slate-500 hover:text-slate-300'}`}
        >
        ...
        <button
          ...
          className={`flex-1 py-3 text-xs font-bold uppercase flex items-center justify-center gap-2 ${mode === 'upload' ? 'bg-slate-800 text-white' : 'text-slate-500 hover:text-slate-300'}`}
        >
      ...
      <div className="p-4 space-y-4">
        {mode === 'url' ? (
            ...
            <input
              ...
              onChange={(e) => {
                ...
                updateData({
                  ...
                  image_status: e.target.value ? 'real' : 'missing',
                });
              }}
              placeholder="https://..."
              className="flex-1 bg-slate-950 border border-slate-700 rounded-lg p-3 text-white focus:border-indigo-500 outline-none text-sm"
            />
            {imageUrl && (
              <button
                type="button"
                onClick={handleResetToPlaceholder}
                className="p-3 bg-slate-800 hover:bg-red-900/30 text-slate-400 hover:text-red-400 rounded-lg border border-slate-700 transition-colors"
                title="Rimuovi e usa Placeholder"
              >
                <X className="w-4 h-4" />
              </button>
            )}
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white px-4 py-2 rounded-lg text-xs font-bold uppercase flex items-center gap-2 shadow-lg"
            >
              {isUploading ? (
              ...
              {isUploading
                ? 'Caricamento Cloud...'
                : `Seleziona Foto (${qualityMode === 'high' ? 'HD' : 'SD'})`}
            </button>
            <input
              ref={fileInputRef}
              type="file"
        ...
        {localPreview ? (
          <div className="flex gap-4 items-start bg-slate-950/50 p-3 rounded-xl border border-slate-800">
            <div className="w-24 h-24 rounded-lg overflow-hidden shrink-0 border border-slate-700 relative bg-black group">
              ...
              {dimensions && (
                <div className="absolute bottom-0 left-0 right-0 bg-black/70 text-white text-[9px] font-mono text-center py-0.5 opacity-100">
            ...
            <div className="flex-1 space-y-2">
              <div className="flex justify-between items-start">
                <div className="flex flex-col">
                  <h4 className="text-xs font-bold text-slate-300 uppercase">
                    {localPreview.startsWith('data:') ? 'File Locale (In Upload...)' : 'Cloud URL'}
                  </h4>
                  {dimensions && (
                    <span className="text-[10px] text-slate-500 font-mono mt-0.5 flex items-center gap-1">
                      <Maximize className="w-3 h-3" /> {dimensions.w} x {dimensions.h} px
                    </span>
                  )}
                </div>
                <button
                  type="button"
                  onClick={analyzeCopyright}
                  disabled={analyzing || aiBlocked}
                  title={aiBlocked ? blockMessage : undefined}
                  className="text-[10px] bg-purple-600/20 text-purple-300 border border-purple-500/50 px-2 py-1 rounded flex items-center gap-1 hover:bg-purple-600/40 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {analyzing ? (
              ...
              {aiAdvice && (
                <div
                  className={`text-xs p-2 rounded border flex gap-2 ${aiAdvice.status === 'safe' ? 'bg-emerald-900/20 border-emerald-500/50 text-emerald-300' : aiAdvice.status === 'danger' ? 'bg-red-900/20 border-red-500/50 text-red-300' : 'bg-amber-900/20 border-amber-500/50 text-amber-300'}`}
                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-slate-800">
          <div>
            <label
              htmlFor="fld-admin-adminimageinput-tsx-l397"
              className="text-[10px] font-bold text-slate-500 uppercase block mb-1"
            >
              Licenza / Fonte
            </label>
            <select
              id="fld-admin-adminimageinput-tsx-l397"
              value={imageLicense}
              onChange={(e) => {
          ...
          <div>
            <label
              htmlFor="fld-admin-adminimageinput-tsx-l417"
              className={`text-[10px] font-bold uppercase block mb-1 flex items-center justify-between ${isCreditRequired ? 'text-amber-500' : 'text-slate-500'}`}
            >
              Attribuzione{' '}
              {isBlocking && (
                <span className="text-[9px] bg-red-500 text-white px-1.5 rounded animate-pulse">
                  RICHIESTO
                </span>
              )}
            </label>
            <input
              id="fld-admin-adminimageinput-tsx-l417"
              value={imageCredit}
              onChange={(e) => updateData({ imageCredit: e.target.value })}
              placeholder="Es. Foto di Mario Rossi"
              className={`w-full bg-slate-900 border rounded-lg p-2 text-white text-sm focus:outline-none ${isBlocking ? 'border-red-500 focus:border-red-500' : 'border-slate-700 focus:border-indigo-500'}`}
            />
          </div>
        </div>
      </div>
