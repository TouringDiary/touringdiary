  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (config) {
        ...
        setDescription(
          config.description ||
            'Pianifica il tuo viaggio, scopri luoghi unici e ottieni sconti esclusivi.',
        );
      }
      setIsLoading(false);
    };
      ...
      <div className="space-y-6">
        <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-xl">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label
                htmlFor="fld-admin-social-socialpreviewconfig-tsx-l87"
                className="text-[10px] font-bold text-slate-500 uppercase block mb-1"
              >
                Titolo Sito
              </label>
              <input
                id="fld-admin-social-socialpreviewconfig-tsx-l87"
                value={title}
            ...
            <button
              ...
              onClick={handleSave}
              disabled={isSaving}
              className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-bold text-xs uppercase tracking-widest shadow-lg flex items-center justify-center gap-2 mt-4"
            >
              {isSaving ? (
