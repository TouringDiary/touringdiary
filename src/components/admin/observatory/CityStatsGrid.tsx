...
import type { CityQualityStats } from '../../../types/index';
...
export const CityStatsGrid = ({
  ...
  const sortedData = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      filtered = data.filter(
        (d) =>
          d.city_name.toLowerCase().includes(lower) || d.zone_name.toLowerCase().includes(lower),
      );
    }
    ...
    if (cityStatusFilter !== 'all') {
      ...
      if (cityStatusFilter === 'published')
        filtered = filtered.filter((d) => d.city_status === 'published');
      else if (cityStatusFilter === 'draft')
        filtered = filtered.filter(
          (d) => d.city_status === 'draft' || d.city_status === 'needs_check',
        );
      else if (cityStatusFilter === 'missing')
        filtered = filtered.filter((d) => d.city_status === 'missing' || !d.city_status);
    }
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      'download',
      `observatory_report_${new Date().toISOString().slice(0, 10)}.csv`,
    );
  ...
  const StatusBadge = ({ status }: { status: string }) => {
    if (status === 'published')
      return (
        <span className="bg-emerald-900/30 text-emerald-400 border border-emerald-500/30 px-2 py-0.5 rounded text-[9px] font-black uppercase">
          Online
        </span>
      );
    if (status === 'draft')
      return (
        <span className="bg-amber-900/30 text-amber-400 border border-amber-500/30 px-2 py-0.5 rounded text-[9px] font-black uppercase">
          Bozza
        </span>
      );
    return (
      <span className="bg-slate-800 text-slate-400 border border-slate-700 px-2 py-0.5 rounded text-[9px] font-black uppercase">
        Missing
      </span>
    );
  };
          ...
          <tbody className="divide-y divide-slate-800/50 text-[10px] text-slate-300 font-mono">
            {sortedData.map((row) => (
              <tr key={row.city_id} className="hover:bg-slate-800/30 transition-colors group">
                ...
                <td className="px-3 py-2 border-r border-slate-800/30 truncate max-w-[150px]">
                  {row.zone_name}
                </td>
                <td className="px-3 py-2 font-bold text-white border-r border-slate-800/30">
                  {row.city_name}
                </td>
                ...
                {/* TOTAL POI */}
                <td className="px-3 py-2 text-center border-r border-slate-800/30 font-bold text-white">
                  {row.total_pois}
                </td>
                  ...
                  <div className="flex flex-col gap-1 w-20">
                    ...
                    <div className="h-1 w-full bg-slate-800 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${row.photo_coverage}%` }}
                      ></div>
                    </div>
                  </div>
                </td>
                ...
                {/* POI LEVELS */}
                <td className="px-3 py-2 text-center border-r border-slate-800/30 text-emerald-400 font-bold">
                  {row.poi_top}
                </td>
                <td className="px-3 py-2 text-center border-r border-slate-800/30 text-amber-400">
                  {row.poi_medium}
                </td>
                <td className="px-3 py-2 text-center border-r border-slate-800/30 text-slate-500">
                  {row.poi_low}
                </td>
                ...
                <td className="px-3 py-2 text-center border-r border-slate-800/30 text-rose-400 font-bold">
                  {row.svc_emergency}
                </td>
                <td className="px-3 py-2 text-center border-r border-slate-800/30 text-emerald-400 font-bold">
                  {row.svc_pharmacy}
                ...
                {/* SPONSOR */}
                <td className="px-3 py-2 text-center border-r border-slate-800/30 text-slate-400 font-bold">
                  {row.sponsor_silver}
                </td>
                <td className="px-3 py-2 text-center border-r border-slate-800/30 text-amber-500 font-bold">
                  {row.sponsor_gold}
                </td>
