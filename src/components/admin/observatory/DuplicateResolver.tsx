...
import { useDuplicateFinder } from '../../../hooks/admin/useDuplicateFinder';
...
const PoiCardMini = ({
  ...
  <div
    className={`p-4 rounded-xl border-2 flex flex-col gap-3 h-full relative ${colorClass} bg-slate-900/50`}
  >
    ...
    <div className="flex gap-4">
      ...
      <div className="min-w-0 flex-1">
        <h4 className="font-bold text-white text-sm truncate" title={poi.name}>
          {poi.name}
        </h4>
        <div className="text-[10px] text-slate-400 mt-1 flex flex-col gap-0.5">
          <span className="flex items-center gap-1">
            <MapPin className="w-3 h-3" /> {poi.address || 'No Indirizzo'}
          </span>
          <span className="font-mono opacity-70">
            {poi.coords
              ? `${poi.coords.lat.toFixed(5)}, ${poi.coords.lng.toFixed(5)}`
              : 'GPS assente'}
          </span>
        </div>
      </div>
    ...
    <div className="flex gap-2 mt-auto">
      <span className="px-2 py-0.5 bg-slate-800 rounded text-[9px] font-bold uppercase text-slate-400">
        {poi.category}
      </span>
      <span className="px-2 py-0.5 bg-slate-800 rounded text-[9px] font-bold uppercase text-slate-400">
        {poi.status || 'pub'}
  ...
  const filteredManifest = useMemo(() => {
    ...
    if (cityStatusFilter !== 'all') {
      if (cityStatusFilter === 'published') list = list.filter((c) => c.status === 'published');
        ...
        list = list.filter((c) => c.status === 'draft' || c.status === 'needs_check');
      else if (cityStatusFilter === 'missing')
        list = list.filter((c) => !c.hasGeneratedContent && c.status !== 'published');
    }
    ...
    <div className="h-full flex flex-col animate-in fade-in">
      ...
      <div className="flex flex-col md:flex-row gap-4 bg-slate-900 p-4 rounded-xl border border-slate-800 shadow-lg shrink-0 mb-6">
        <div className="flex-1">
          <label
            htmlFor="fld-admin-observatory-duplicateresolver-tsx-l130"
            className="text-[10px] font-bold text-slate-500 uppercase block mb-1"
          >
            Seleziona Città ({filteredManifest.length})
          </label>
          <select
            id="fld-admin-observatory-duplicateresolver-tsx-l130"
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="bg-slate-950 p-4 border-b border-slate-800 flex justify-between items-center shrink-0">
              <div>
                <h3 className="text-white font-bold flex items-center gap-2">
                  <AlertTriangle className="w-5 h-5 text-amber-500" /> Possibile Duplicato (
                  {duplicates.length} rimanenti)
                </h3>
                <p className="text-xs text-slate-400 mt-1">
                  Score Similarità:{' '}
                  <span className="text-amber-400 font-bold">{Math.round(currentPair.score)}%</span>{' '}
                  • {currentPair.reasons.join(', ')}
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="flex-1 flex flex-col lg:flex-row p-6 gap-8 items-center justify-center overflow-y-auto">
              ...
              <div className="w-full lg:w-[45%]">
                <PoiCardMini
                  poi={currentPair.poiA}
                  label="POI A (Originale?)"
            ...
            <div className="p-4 bg-slate-950 border-t border-slate-800 text-center text-[10px] text-slate-500">
              <p>
                Il "Merge" sposterà foto e recensioni sull'elemento mantenuto e cancellerà l'altro.
                I campi vuoti verranno riempiti.
              </p>
            </div>
          </div>
        )}
