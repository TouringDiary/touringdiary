...
import type { AnomalyRecord, PointOfInterest } from '../../../types/index';
...
export const AnomalyInspector = ({
  ...
  const cityById = useMemo(() => new Map(cityManifest.map((c) => [c.id, c])), [cityManifest]);

  const anomalyById = useMemo(() => new Map(anomalies.map((a) => [a.id, a])), [anomalies]);

  // Ricarica quando cambia il filtro città (ottimizza la fetch POI) o dopo un salvataggio.
  // reloadToken è in deps per forzare il re-run; la closure di un run non vede token nuovi —
  // il cleanup (`cancelled`) invalida la richiesta precedente.
  useEffect(() => {
    let cancelled = false;
    // Lettura intenzionale: forza il re-fetch su refresh manuale / post-save
    void reloadToken;

    const loadData = async () => {
      setLoading(true);
      try {
        // getAnomalies() senza limit: paginazione completa in observatoryService
        const rawAnomalies = await getAnomalies();
        if (cancelled) return;
        setAnomalies(rawAnomalies);

        if (rawAnomalies.length > 0) {
          const anomalyIds = new Set(rawAnomalies.map((a) => a.id));
          let relevantPois: PointOfInterest[] = [];

          if (geoFilter.city) {
            const cityPois = await getPoisByCityId(geoFilter.city);
            if (cancelled) return;
            relevantPois = cityPois.filter((p) => anomalyIds.has(p.id));
          } else {
            const uniqueCityIds = Array.from(new Set(rawAnomalies.map((a) => a.city_id)));
            const poisData = await getPoisByCityIds(uniqueCityIds);
            if (cancelled) return;
            relevantPois = poisData.filter((p) => anomalyIds.has(p.id));
          }
  ...
  const handleSave = async (updatedPoi: PointOfInterest) => {
    if (!updatedPoi.cityId) {
      alert('Errore critico: ID Città mancante.');
      return;
    }
    setIsSaving(true);
    try {
  ...
  const AnomalyBadge = ({ type }: { type: string }) => {
    switch (type) {
      case 'no_gps':
        return (
          <span className="text-[9px] font-black uppercase bg-red-900/30 text-red-400 px-2 py-0.5 rounded border border-red-500/30 flex items-center gap-1">
            <MapPinOff className="w-3 h-3" /> No GPS
          </span>
        );
      case 'missing_image':
        return (
          <span className="text-[9px] font-black uppercase bg-orange-900/30 text-orange-400 px-2 py-0.5 rounded border border-orange-500/30 flex items-center gap-1">
            <ImageOff className="w-3 h-3" /> No Foto
          </span>
        );
      case 'low_reliability':
        return (
          <span className="text-[9px] font-black uppercase bg-indigo-900/30 text-indigo-400 px-2 py-0.5 rounded border border-indigo-500/30 flex items-center gap-1">
            <ShieldAlert className="w-3 h-3" /> Low Trust
          </span>
        );
      case 'short_desc':
        return (
          <span className="text-[9px] font-black uppercase bg-yellow-900/30 text-yellow-400 px-2 py-0.5 rounded border border-yellow-500/30 flex items-center gap-1">
            Testo Breve
          </span>
        );
      default:
        return (
          <span className="text-[9px] font-black uppercase bg-slate-800 text-slate-400 px-2 py-0.5 rounded border border-slate-700 flex items-center gap-1">
  ...
  const activeFilterLabel = geoFilter.city
    ? `Città: ${getCityNameForPoi({ cityId: geoFilter.city })}`
    : geoFilter.zone
      ? `Zona: ${geoFilter.zone}`
      : `Filtri Attivi`;
    ...
    <div className="flex flex-col h-full space-y-6">
      ...
      {isEditorOpen && editingPoi && (
        <AdminPoiModal
          ...
          onSave={handleSave}
          poi={editingPoi}
          cityName={getCityNameForPoi(editingPoi)}
        />
      )}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium">
              {isSaving ? 'Salvataggio...' : `Rilevati ${anomalies.length} problemi totali`}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 items-center w-full xl:w-auto">
          ...
          <div className="flex bg-slate-950 p-1 rounded-lg border border-slate-800 overflow-x-auto max-w-full">
            <button
              ...
              className={`px-3 py-1.5 rounded text-[10px] font-bold uppercase transition-all whitespace-nowrap ${filterType === 'all' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:text-white'}`}
            >
              Tutti
            </button>
            <button
              type="button"
              onClick={() => setFilterType('no_gps')}
              className={`px-3 py-1.5 rounded text-[10px] font-bold uppercase transition-all whitespace-nowrap flex items-center gap-1 ${filterType === 'no_gps' ? 'bg-red-600 text-white' : 'text-slate-500 hover:text-white'}`}
            >
              <MapPinOff className="w-3 h-3" /> No GPS
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded text-[10px] font-bold uppercase transition-all whitespace-nowrap flex items-center gap-1 ${filterType === 'no_image' ? 'bg-orange-600 text-white' : 'text-slate-500 hover:text-white'}`}
            >
              <ImageOff className="w-3 h-3" /> No Foto
            </button>
            <button
              type="button"
              onClick={() => setFilterType('suspicious_hours')}
              className={`px-3 py-1.5 rounded text-[10px] font-bold uppercase transition-all whitespace-nowrap flex items-center gap-1 ${filterType === 'suspicious_hours' ? 'bg-amber-600 text-white' : 'text-slate-500 hover:text-white'}`}
            >
          ...
          <button
            ...
            className="p-2 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white rounded-lg border border-slate-700 transition-colors disabled:opacity-50"
          >
        ...
        <div className="p-3 border-b border-slate-800 bg-[#0f172a] flex justify-between items-center">
          ...
          {activeFiltersCount > 0 && (
            <span className="text-[9px] text-indigo-400 font-bold uppercase flex items-center gap-1">
              <Filter className="w-3 h-3" /> Filtro Attivo
            </span>
          )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-2">
          {loading ? (
            ...
            filteredItems.map((poi) => {
              const anomalyRecord = anomalyById.get(poi.id);
              ...
              return (
                <button
                  type="button"
                  key={poi.id}
                  className="bg-slate-950 p-3 rounded-xl border border-slate-800 hover:border-indigo-500/50 transition-all group flex gap-3 cursor-pointer relative overflow-hidden w-full text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/50"
                  onClick={() => handleEdit(poi)}
                  aria-label={`Modifica ${poi.name}`}
                >
                  <div className="w-16 h-16 rounded-lg overflow-hidden border border-slate-700 shrink-0 bg-black relative">
                    ...
                    {anomalyType === 'missing_image' && (
                      <div className="absolute inset-0 flex items-center justify-center bg-black/60">
                        <ImageOff className="w-6 h-6 text-orange-500" aria-hidden />
                      </div>
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex justify-between items-center mb-2">
                      <p className="text-[10px] text-slate-500 truncate max-w-[150px]">
                        {poi.address || 'Indirizzo mancante'}
                      </p>
                      <span className="text-[9px] font-bold text-indigo-400 bg-indigo-900/10 px-1.5 rounded border border-indigo-500/20">
                        {cityName}
                      </span>
                    </div>
