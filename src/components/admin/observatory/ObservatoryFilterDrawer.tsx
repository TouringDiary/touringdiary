  ...
  return (
    <>
      ...
      <button
        ...
        tabIndex={-1}
        aria-hidden="true"
        className={`fixed inset-0 bg-black/80 backdrop-blur-sm z-admin-modal transition-opacity duration-300 border-0 p-0 cursor-default ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
        onClick={onClose}
      />

      {/* DRAWER */}
      <div
        className={`fixed inset-y-0 right-0 w-80 md:w-96 bg-slate-900 border-l border-slate-800 shadow-2xl z-admin-modal transform transition-transform duration-300 flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        {/* HEADER */}
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    type="button"
                    onClick={() => setCityStatus('all')}
                    className={`px-3 py-2 rounded-lg text-[10px] font-bold uppercase border transition-all ${cityStatus === 'all' ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-slate-950 text-slate-500 border-slate-800 hover:text-white'}`}
                  >
                    Tutte
                  </button>
                  <button
                    type="button"
                    onClick={() => setCityStatus('published')}
                    className={`px-3 py-2 rounded-lg text-[10px] font-bold uppercase border transition-all flex items-center justify-center gap-1 ${cityStatus === 'published' ? 'bg-emerald-600 text-white border-emerald-500' : 'bg-slate-950 text-slate-500 border-slate-800 hover:text-white'}`}
                  >
                    <Eye className="w-3 h-3" /> Online
                  </button>
                  <button
                    type="button"
                    onClick={() => setCityStatus('draft')}
                    className={`px-3 py-2 rounded-lg text-[10px] font-bold uppercase border transition-all flex items-center justify-center gap-1 ${cityStatus === 'draft' ? 'bg-amber-600 text-white border-amber-500' : 'bg-slate-950 text-slate-500 border-slate-800 hover:text-white'}`}
                  >
                    <Layers className="w-3 h-3" /> Bozza
                  </button>
                  <button
                    type="button"
                    onClick={() => setCityStatus('missing')}
                    className={`px-3 py-2 rounded-lg text-[10px] font-bold uppercase border transition-all flex items-center justify-center gap-1 ${cityStatus === 'missing' ? 'bg-red-600 text-white border-red-500' : 'bg-slate-950 text-slate-500 border-slate-800 hover:text-white'}`}
                  >
              ...
              {activeTab === 'overview' && (
                <div className="space-y-2 pt-2">
                  <div className="flex justify-between items-end">
                    <label
                      htmlFor="fld-admin-observatory-observatoryfilterdrawer-tsx-l198"
                      className="text-[9px] font-bold text-amber-400 uppercase tracking-wide flex items-center gap-1.5"
                    >
                  ...
                  <input
                    id="fld-admin-observatory-observatoryfilterdrawer-tsx-l198"
                    type="range"
                    min="0"
                    max="100"
                    value={minQuality}
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    type="button"
                    onClick={() => setPoiStatus('all')}
                    className={`px-3 py-2 rounded-lg text-[10px] font-bold uppercase border transition-all ${poiStatus === 'all' ? 'bg-amber-600 text-white border-amber-500' : 'bg-slate-950 text-slate-500 border-slate-800 hover:text-white'}`}
                  >
                    Tutti
                  </button>
                  <button
                    type="button"
                    onClick={() => setPoiStatus('published')}
                    className={`px-3 py-2 rounded-lg text-[10px] font-bold uppercase border transition-all flex items-center justify-center gap-1 ${poiStatus === 'published' ? 'bg-emerald-600 text-white border-emerald-500' : 'bg-slate-950 text-slate-500 border-slate-800 hover:text-white'}`}
                  >
                    <Eye className="w-3 h-3" /> Online
                  </button>
                  <button
                    type="button"
                    onClick={() => setPoiStatus('draft')}
                    className={`px-3 py-2 rounded-lg text-[10px] font-bold uppercase border transition-all flex items-center justify-center gap-1 ${poiStatus === 'draft' ? 'bg-slate-700 text-white border-slate-600' : 'bg-slate-950 text-slate-500 border-slate-800 hover:text-white'}`}
                  >
                    <EyeOff className="w-3 h-3" /> Bozza
                  </button>
                  <button
                    type="button"
                    onClick={() => setPoiStatus('needs_check')}
                    className={`px-3 py-2 rounded-lg text-[10px] font-bold uppercase border transition-all flex items-center justify-center gap-1 ${poiStatus === 'needs_check' ? 'bg-red-600 text-white border-red-500' : 'bg-slate-950 text-slate-500 border-slate-800 hover:text-white'}`}
                  >
              ...
              <div className="space-y-2 pt-2">
                ...
                <p className="text-[9px] text-slate-500 italic mb-2">
                  Filtra per tipo di incongruenza oraria.
                </p>
                    ...
                    return (
                      ...
                      >
                        <span className="flex items-center gap-2">
                          <AlertTriangle
                            className={`w-3 h-3 ${isSelected ? 'text-red-500' : 'text-slate-600'}`}
                          />
                          {type.label}
                        </span>
