import { AlertOctagon, BarChart3, Copy, Loader2, Microscope, RefreshCw } from 'lucide-react';
import { useEffect, useState } from 'react';
import { useAdminData } from '../../../hooks/useAdminData';
import { getCityQualityMetrics, getObservatoryStats } from '../../../services/observatoryService';
import type { CityQualityStats, ObservatoryStats } from '../../../types/index';
  ...
  return (
    <div className="flex flex-col h-full space-y-6 animate-in fade-in relative">
      ...
      <ObservatoryFilterDrawer
        ...
        setCityStatus={setCityStatusFilter}
        poiStatus={poiStatusFilter}
        setPoiStatus={setPoiStatusFilter}
        minQuality={minQualityFilter}
        setMinQuality={setMinQualityFilter}
        suspicionTypes={suspicionTypeFilter}
        setSuspicionTypes={setSuspicionTypeFilter}
        activeTab={activeTab}
      />
      ...
      <div className="flex bg-slate-900 p-1.5 rounded-xl border border-slate-800 w-fit shrink-0 gap-1 overflow-x-auto">
        ...
        >
          ...
          {stats && stats.anomalies_total > 0 && (
            <span className="bg-white text-red-600 px-1.5 rounded text-[9px]">
                ...
                {stats && (
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4 animate-in fade-in slide-in-from-bottom-4 shrink-0">
                    ...
                    <div className="bg-red-900/10 p-4 rounded-xl border border-red-500/20 flex flex-col justify-center gap-1">
                      <span className="text-[9px] font-black text-red-400 uppercase tracking-widest truncate">
                        Problemi GPS (0,0)
                      </span>
                      <span className="text-3xl font-display font-bold text-white">
                        {stats.missing_gps}
                    ...
                    <div className="bg-orange-900/10 p-4 rounded-xl border border-orange-500/20 flex flex-col justify-center gap-1">
                      <span className="text-[9px] font-black text-orange-400 uppercase tracking-widest truncate">
                        Foto Mancanti
                      </span>
                      <span className="text-3xl font-display font-bold text-white">
