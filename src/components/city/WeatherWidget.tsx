import { CloudLightning, CloudRain, CloudSun, Droplets, Sun, Wind } from 'lucide-react';
...
export const WeatherWidget = ({ city, startDate, endDate }: Props) => {
  ...
  const getWeather = (date: Date) => {
    ...
    if (r > 0.2)
      return {
        ...
        temp: '18°',
        color: 'text-blue-400',
        bg: 'from-blue-600/20 to-slate-700/20',
      };
    return {
      icon: CloudLightning,
      label: 'Temp.',
      ...
      bg: 'from-purple-600/20 to-slate-800/20',
    };
  };
  ...
  return (
    ...
    <div className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden shadow-lg flex flex-col no-print flex-shrink-0 h-[13rem] relative group w-full">
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative z-local-raised px-4 pt-1 pb-1 min-h-0">
        ...
        <div className="flex items-center gap-3 transform hover:scale-105 transition-transform duration-500">
          <currentWeather.icon
            className={`w-8 h-8 lg:w-10 lg:h-10 ${currentWeather.color} drop-shadow-lg`}
          />
      ...
      <div className="bg-slate-950/60 backdrop-blur-md border-t border-slate-800 h-14 lg:h-16 flex-shrink-0 relative z-local-overlay">
        <div className="flex overflow-x-auto scrollbar-hide w-full h-full items-center px-1">
          {days.slice(1).map((d, i) => {
            ...
            return (
              <div
                key={i}
                className="flex-1 min-w-[3rem] h-full hover:bg-white/5 transition-colors flex flex-col items-center justify-center gap-0.5 border-r border-slate-800/30 last:border-0"
              >
                <span className="text-[8px] font-bold text-slate-500 uppercase">
                  {d.toLocaleDateString('it-IT', { weekday: 'short' }).substring(0, 3)}
                </span>
                <w.icon className={`w-4 h-4 ${w.color}`} />
                <span className="text-[10px] font-mono font-bold text-white">{w.temp}</span>
              </div>
            );
          })}
        </div>
