                    ...
                    {Object.entries(city.classificationExplainability)
                        ...
                        .map(([cat, weight]) => (
                            <div key={cat} className="flex justify-between items-center text-xs gap-4">
                                <span className="text-slate-300 font-medium capitalize drop-shadow-md">{cat.replace(/_/g, ' ')}</span>
                                <span className="text-amber-500 font-mono font-bold drop-shadow-md">{Math.round(Number(weight))}%</span>
                            </div>
                        ))}
                </div>
            )}
