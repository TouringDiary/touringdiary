...
const ensureFiveItems = (
  items: PointOfInterest[],
  fallbackSource: PointOfInterest[] = [],
): (PointOfInterest | null)[] => {
  const sortedItems = getSortedTop5(items);
  const source = sortedItems.length > 0 ? sortedItems : getSortedTop5(fallbackSource);

  const result: (PointOfInterest | null)[] = [...source];
...
export const CityShowcaseTab = ({
  ...
  const renderEditorialNovitaGrid = () => {
    const news = visibleAllPois.filter((p) => {
      ...
      const nameMatch = activeSponsors.some(
    ...
    if (news.length === 0)
      return (
        <div className="text-slate-500 italic p-10 text-sm bg-slate-900/30 rounded-3xl border border-slate-800 border-dashed text-center w-full col-span-2">
          Nessuna novità editoriale questo mese.
        </div>
      );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[19rem_1fr_1fr_19rem] h-full w-full overflow-hidden">
        ...
        <div className="lg:col-span-2 flex-1 md:px-6 py-4 min-w-0 h-auto lg:h-full overflow-visible lg:overflow-y-auto custom-scrollbar">
          ...
          <div className="lg:hidden md:px-0 mt-8 pb-20 border-t border-slate-800/50 pt-8">
            ...
            <div className="w-full">{renderMobileSponsorCard(mobileSilver, 'silver')}</div>
          </div>
        </div>
    ...
    if (subKey === 'sea') {
      return visibleAllPois.filter((p) => {
        ...
        return (
          ...
          sub === 'beach_club' ||
          sub === 'beach_free' ||
          (p.tags && p.tags.some((t) => t.toLowerCase().includes('mare')))
        );
      });
    }
    if (subKey === 'mountain') {
  ...
  const renderMasterDetailLayout = (
    menuItems: typeof COMMUNITY_MENU,
    context: 'community' | 'natura',
  ) => {
    ...
    if (context === 'community') {
      ...
      if (displayItems.length === 0)
        displayItems = visibleAllPois.filter((p) => p.category === 'nature');
    }
            ...
            {menuItems.map((item) => {
              ...
              return (
                <button
                  type="button"
                  key={item.id}
                  onClick={() => setActiveTopCategory(item.id)}
                  className={`flex items-center gap-3 p-3 rounded-xl border transition-all group shrink-0 ${isActive ? 'bg-slate-800 border-indigo-500 shadow-md ring-1 ring-indigo-500/20' : 'bg-slate-900 border-slate-800 hover:bg-slate-800 hover:border-slate-700'}`}
                >
                  <div
                    className={`p-1.5 rounded-lg shrink-0 ${isActive ? 'bg-indigo-600 text-white' : 'bg-slate-950 text-slate-500 group-hover:text-slate-300'}`}
                  >
                    <Icon className="w-4 h-4" />
                  </div>
                  <span
                    className={`text-xs font-bold uppercase tracking-wide truncate ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-slate-200'}`}
                  >
                    {item.label}
                  </span>
        ...
        <div className="flex-1 min-w-0 h-full flex flex-col overflow-hidden relative border-r border-slate-800/50">
            ...
            {menuItems.map((item) => {
              ...
              return (
                <button
                  ...
                  key={item.id}
                  onClick={() => setActiveTopCategory(item.id)}
                  className={`flex items-center gap-2 p-2 rounded-lg border transition-all whitespace-nowrap ${isActive ? 'bg-indigo-600 text-white border-indigo-500 shadow-md' : 'bg-slate-900 text-slate-400 border-slate-800'}`}
                >
          ...
          <div className="flex-1 h-auto lg:h-full overflow-visible lg:overflow-y-auto custom-scrollbar md:px-6 py-4 min-w-0">
            <div className="flex items-center justify-between mb-6 px-1">
              ...
              <div
                className={`${filterSectionLabel10Style} bg-slate-900 px-2 py-0.5 rounded border border-slate-800`}
              >
                TOP 5
              </div>
            </div>
            <div className="flex flex-col gap-4 pb-20">
              {top5Items.map((item, idx) => (
                <div
                  key={item?.id || `empty-slot-${activeItem.id}-${idx}`}
    ...
    <div className="flex flex-col h-auto lg:h-full pt-2 overflow-visible lg:overflow-hidden custom-scrollbar">
      <div className="flex items-center justify-between px-1 bg-[#020617] py-2 border-b border-slate-800/50 shadow-lg shrink-0 lg:sticky lg:top-0 z-local-sticky">
        <div className="flex bg-[#0f172a] border border-slate-800 rounded-lg shadow-xl overflow-hidden h-8">
          <button
            type="button"
            onClick={() => setVetrinaSubTab('novita')}
            className={`px-4 h-full text-[9px] font-bold uppercase tracking-wider transition-colors border-r border-slate-800 ${vetrinaSubTab === 'novita' ? 'bg-amber-600 text-white' : 'text-yellow-400 hover:text-orange-500 hover:bg-slate-800'}`}
          >
            NOVITÀ
          </button>
          <button
            type="button"
            onClick={() => setVetrinaSubTab('community')}
            className={`px-4 h-full text-[9px] font-bold uppercase tracking-wider transition-colors border-r border-slate-800 ${vetrinaSubTab === 'community' ? 'bg-indigo-600 text-white' : 'text-yellow-400 hover:text-orange-500 hover:bg-slate-800'}`}
          >
            TOP 5 | COMMUNITY
          </button>
          <button
            type="button"
            onClick={() => setVetrinaSubTab('natura')}
            className={`px-4 h-full text-[9px] font-bold uppercase tracking-wider transition-colors ${vetrinaSubTab === 'natura' ? 'bg-emerald-600 text-white' : 'text-yellow-400 hover:text-orange-500 hover:bg-slate-800'}`}
          >
            TOP 5 | NATURA
          </button>
        </div>
