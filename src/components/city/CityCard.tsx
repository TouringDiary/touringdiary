...
const BADGE_MAP: Record<string, { text: string; style: string }> = {
  event: {
    text: 'EVENTI IN ARRIVO',
    style: 'bg-rose-600 text-white border-b border-l border-rose-800',
  },
  trend: {
    text: 'TREND DEL MESE',
    style: 'bg-blue-600 text-white border-b border-l border-blue-800',
  },
  season: {
    text: 'STAGIONE IDEALE',
    style: 'bg-emerald-600 text-white border-b border-l border-emerald-800',
  },
  editor: {
    text: 'SCELTA EDITORIALE',
    style: 'bg-purple-600 text-white border-b border-l border-purple-800',
  },
  destination: {
    text: 'DESTINAZIONE TOP',
    style: 'bg-indigo-600 text-white border-b border-l border-indigo-800',
  },
};
...
const CityDnaIcons = ({ types }: { types: string[] }) => (
  <div className="flex items-center gap-0.5 shrink-0" role="img" aria-label="DNA della città">
    ...
    {types.length > 4 && (
      <span
        className="text-[8px] md:text-[9px] text-slate-500 font-bold leading-none"
        title={`+${types.length - 4} altre categorie`}
        aria-hidden
      >
        +{types.length - 4}
      </span>
...
const VisitorMiniBadge = ({ visitors }: { visitors: number }) => (
  ...
  >
    <span className="text-[6px] md:text-[7px] uppercase text-slate-400 font-bold tracking-wide">
      Visite
    </span>
    <span className="text-[9px] md:text-[10px] font-mono font-bold text-amber-400 tabular-nums">
...
const getBadgeConfig = (dbBadge?: string, forcedBadge?: string) => {
  if (forcedBadge) {
    ...
    if (normForced.includes('destination') || normForced.includes('destinazione'))
      return BADGE_MAP.destination;

    return {
      text: forcedBadge.toUpperCase(),
      style: 'bg-amber-500 text-slate-900 border-b border-l border-amber-600',
    };
  }
...
export const CityCard: React.FC<CityCardProps> = ({
  ...
  const distance = userLocation
    ? calculateDistance(userLocation.lat, userLocation.lng, city.coords.lat, city.coords.lng)
    : null;
  ...
  const dimensions =
    className || 'w-[150px] md:w-[165px] lg:w-[145px] xl:w-[165px] h-[200px] md:h-[240px]';

  return (
    <div className={`${baseClasses} ${dimensions}`}>
      <button
        type="button"
      ...
      <div className="h-[55%] overflow-hidden relative border-b border-slate-800 pointer-events-none z-[1]">
        ...
        {badge && (
          <div className="absolute top-0 right-0 z-home-card-overlay group-hover:scale-110 transition-transform origin-top-right">
            <div
              className={`px-2 py-1 text-[8px] md:text-[9px] font-bold uppercase leading-tight text-center shadow-md rounded-bl-lg ${badge.style}`}
            >
              {badge.text}
            </div>
          </div>
      ...
      <div className="p-2 flex flex-col flex-1 justify-between bg-slate-900 relative z-[1] pointer-events-none">
        ...
        <div className="flex items-center justify-between pt-1.5 border-t border-slate-800/50 mt-0.5 shrink-0 gap-1">
          ...
          {city.cityTypes && city.cityTypes.length > 0 && <CityDnaIcons types={city.cityTypes} />}
        </div>
      </div>
    </div>
  );
