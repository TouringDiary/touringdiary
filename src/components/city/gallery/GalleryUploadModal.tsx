import { AlertCircle, Loader2, Send, Share2 } from 'lucide-react';
...
export const GalleryUploadModal = ({
  ...
  return (
    <div
      ...
      className="absolute inset-0 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm rounded-xl h-full min-h-[500px]"
    >
      <div className="bg-slate-900 border border-slate-700 p-6 rounded-2xl w-full max-w-sm shadow-2xl animate-in zoom-in-95">
        ...
        <div className="space-y-4">
          <div>
            <label
              htmlFor="fld-city-gallery-galleryuploadmodal-tsx-l43"
              className="text-xs font-bold text-slate-500 uppercase block mb-1"
            >
              Didascalia / Luogo
            </label>
            <input
              id="fld-city-gallery-galleryuploadmodal-tsx-l43"
              autoFocus
              value={description}
          ...
          <div className="flex items-center justify-between bg-slate-800/50 p-3 rounded-xl border border-slate-700/50">
            <div className="flex items-center gap-2">
              <div
                className={`p-1.5 rounded-full ${shareToLive ? 'bg-indigo-500/20 text-indigo-400' : 'bg-slate-700 text-slate-500'}`}
              >
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
                checked={shareToLive}
                onChange={(e) => setShareToLive(e.target.checked)}
          ...
          {uploadError && (
            <div className="text-red-400 text-xs font-bold bg-red-900/20 p-2 rounded border border-red-500/30 flex items-center gap-2">
