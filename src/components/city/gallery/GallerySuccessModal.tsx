...
export const GallerySuccessModal = ({ onClose }: Props) => {
  ...
  return createPortal(
    <div
      className="td-modal-overlay pointer-events-auto flex items-center justify-center p-4 bg-black/90 backdrop-blur-md"
    ...
    >
      <div className="bg-slate-900 border border-emerald-500/50 p-8 rounded-3xl max-w-sm w-full text-center shadow-2xl animate-in zoom-in-95 relative overflow-hidden">
        ...
        <div
          className={`bg-slate-950 p-4 rounded-2xl border border-slate-800 flex items-center justify-between ${rewardsEnabled ? 'mb-6' : 'mb-4'}`}
        >
          <div className="text-left">
            <p className="text-[10px] text-slate-500 font-bold uppercase">Ricompensa</p>
            <p className="text-white text-xs">Alla pubblicazione</p>
          </div>
          <div className="text-right">
            <div className="flex items-center gap-1 text-emerald-400 font-black text-xl">
              +20 <span className="text-xs self-end mb-1">XP</span>
            </div>
        ...
        <button
          type="button"
          onClick={onClose}
          className="w-full bg-slate-800 hover:bg-slate-700 text-white font-bold py-3.5 rounded-xl transition-all border border-slate-700 hover:border-slate-600"
        >
          Torna alla Galleria
        </button>
      </div>
