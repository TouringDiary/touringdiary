...
export const GalleryLightbox = ({ data, onClose, onNext, onPrev, hasNext, hasPrev }: Props) => {
    ...
    if (!data) return null;
    console.log("LIGHTBOX DATA FULL:", JSON.stringify(data, null, 2));
    // LEGGERE I DATI DIRETTAMENTE DAL CONTEXT AD OGNI RENDER
    const { isLiked, count, isLoading } = getPhotoStatus({
        id: data.id,
        likes: data.likes,
        likedByUser: data.likedByUser // ✅ FIX MINIMO: bootstrap corretto stato like nel lightbox ranking
    });
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-3 bg-black/50 p-2 px-4 rounded-full text-white">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {isLoading ? (
                        ...
                        <Heart
                            className={`w-7 h-7 cursor-pointer transition-colors ${isLiked ? 'text-red-500 fill-current' : 'text-white hover:text-red-400'
                                }`}
                            onClick={handleLike}
                        />
                    )}
