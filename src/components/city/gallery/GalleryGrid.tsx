...
import type { LightboxData } from './GalleryLightbox';
...
const PhotoCard: React.FC<PhotoCardProps> = ({
  ...
  const photoLabel = photo.description || photo.locationName || 'foto';

  return (
    <div
      className={`${dimensionClass} relative rounded-lg overflow-hidden border border-slate-800 bg-slate-900 group shadow-md hover:shadow-xl transition-all`}
    >
      <button
        type="button"
        aria-label={`Apri foto ${photoLabel}`}
        onClick={() =>
          onOpenLightbox({
            id: photo.id,
            url: photo.url,
      ...
      <ImageWithFallback
        ...
        className={`pointer-events-none relative z-[1] w-full h-full object-cover transition-transform duration-500 group-hover:scale-105 ${photo.status === 'pending' ? 'opacity-50 grayscale' : ''}`}
        draggable={false}
      />

      <div className="absolute top-2 right-2 z-local-overlay pointer-events-auto">
        <button
          type="button"
        ...
        >
          {isLoading ? (
            ...
            <Heart
              className={`w-3.5 h-3.5 transition-colors ${isLiked ? 'fill-rose-500 text-rose-500' : 'text-white group-hover:text-rose-400'}`}
            />
          )}
          <span className={isLiked ? 'text-rose-500' : 'text-white'}>{count}</span>
        </button>
      </div>

      {photo.status === 'pending' && (
        <div className="absolute top-2 left-2 z-local-overlay pointer-events-none bg-amber-500/90 text-black text-[9px] font-black uppercase px-2 py-0.5 rounded shadow-lg border border-amber-300/50">
          In Attesa
        </div>
      )}

      <div className="absolute inset-0 z-[1] bg-gradient-to-t from-black/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-3 pointer-events-none">
        <span className="text-[10px] text-white font-bold truncate">
          {photo.description || photo.locationName}
        </span>
        <div className="flex items-center gap-1 text-[9px] text-slate-400 mt-0.5">
          <User className="w-2.5 h-2.5" /> {photo.user}
        </div>
      </div>
      <div className="absolute inset-0 z-[1] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
...
export const GalleryGrid = ({
  ...
  const currentGridPhotos = activeTab === 'official' ? officialPhotos : visiblePhotos;
      ...
      <div className="relative group/slider shrink-0 pt-2 pb-2 bg-[#020617] z-local-raised px-4 md:px-8">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div
              className={`p-1.5 rounded-lg ${activeTab === 'official' ? 'bg-amber-500/20 text-amber-500' : 'bg-blue-500/20 text-blue-500'}`}
            >
          ...
          <div className="flex gap-2">
            <button
              ...
              className="p-1 bg-slate-900 border border-slate-700 rounded-lg hover:border-amber-500 text-slate-400 hover:text-white transition-colors"
            >
        ...
        <DraggableSlider ref={sliderRef} className="gap-2">
          {currentTopSlots.length > 0
            ? currentTopSlots.map((photo) => (
                <div
                  key={photo.id}
                  className="snap-center flex-shrink-0 w-[50vw] md:w-[280px] h-[150px]"
                >
                  <PhotoCard
                    photo={photo}
            ...
            : [...Array(3)].map((_, idx) => (
                <div
                  key={`empty-${idx}`}
                  className="snap-center flex-shrink-0 w-[50vw] md:w-[280px] h-[150px] relative rounded-xl overflow-hidden border border-slate-800 bg-slate-900/30 flex flex-col items-center justify-center gap-2 group transition-all"
                >
                  <CountBadge
                    display={`#${idx + 1}`}
                    size="sm"
                    variant="dark"
                    position="overlay-tl"
                    className="z-local-overlay backdrop-blur"
                  />
      ...
      <div className="w-full px-4 md:px-8 pb-10">
        <div className="flex items-center justify-between py-2 border-b border-slate-800 mb-4">
          <div className="flex items-center gap-2">
            <div
              className={`p-1.5 rounded-lg ${activeTab === 'official' ? 'bg-amber-900/20 text-amber-500' : 'bg-blue-900/20 text-blue-500'}`}
            >
              <Grid className="w-4 h-4" />
            </div>
            <div>
              <h3 className="text-xl font-display font-bold text-white leading-none capitalize">
                {activeTab} Gallery
              </h3>
              <span className="text-[10px] font-mono text-slate-500 font-bold">
                {activeTab === 'official' ? officialPhotos.length : communityPhotos.length} foto
                totali
              </span>
            </div>
          </div>
        ...
        <div className="flex flex-nowrap gap-4 overflow-x-auto pb-6 no-scrollbar snap-x snap-mandatory w-full min-w-0 max-w-full">
          ...
          {activeTab === 'community' && (
            ...
            >
              {isUploading ? (
                ...
                <>
                  <div className="p-3 rounded-full bg-slate-800 group-hover:bg-emerald-900/30 text-slate-400 group-hover:text-emerald-400 mb-2 transition-colors">
          ...
          {currentGridPhotos.map((photo) => (
            <div
              key={photo.id}
              className="snap-start flex-shrink-0 w-[65vw] md:w-[280px] h-48 md:h-64"
            >
              <PhotoCard
                photo={photo}
                onOpenLightbox={onOpenLightbox}
          ...
          {activeTab === 'community' && pagination.currentPage < pagination.totalPages && (
            ...
            >
              ...
              <span className="text-[8px] font-mono text-slate-600 mt-1">
                {communityPhotos.length - currentGridPhotos.length} rimanenti
              </span>
            </button>
          )}
