...
import { type AiSessionState, useAiPlanner } from '@/context/AiPlannerContext';
import { useModal } from '@/context/ModalContext';
import { usePlatformControl } from '@/context/PlatformControlContext';
import { useUser } from '@/context/UserContext';
import { useMobileDetect } from '@/hooks/ui/useMobileDetect';
import { useDynamicStyles } from '../../hooks/useDynamicStyles';
import type { DailyLogistics } from '../../services/ai/aiPlanner';
  ...
  return (
    <div className="grid grid-cols-2 gap-4 bg-slate-950/40 p-4 rounded-2xl border border-slate-800">
      ...
      <div className="space-y-1">
        <label
          htmlFor="ai-planner-date-dal"
          className="text-[10px] text-slate-500 uppercase font-black ml-1"
        >
          Dal
        </label>
...
const SectionHeader = ({
  ...
  return (
    <div className="flex items-center gap-3 mb-3">
      ...
      <h4
        className={`${titleStyle || 'text-lg font-display font-bold text-[#facc15] tracking-tight uppercase drop-shadow-sm'} pt-0.5`}
      >
        {title}
      </h4>
...
export const AiPlannerForm = ({ onGenerate, isLoading, error }: Props) => {
  ...
  const travelStyles =
    getCachedSetting<TravelStyleOption[]>(SETTINGS_KEYS.TRAVEL_STYLES_CONFIG) || FALLBACK_STYLES;
  ...
  const updateDayLogistic = (index: number, field: keyof DailyLogistics, value: string) => {
    const newList = [...aiSession.dailyLogistics];
  ...
  return (
    <div className="space-y-5 pb-6">
      ...
      {aiBlocked && <AiRuntimeBanner status={aiRuntimeStatus} className="mb-1" />}
      ...
      <section>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-end mb-1 px-1">
            ...
            <label
              htmlFor="fld-ai-planner-custom-days"
              className="text-[10px] font-black text-[#f97316] uppercase tracking-[0.2em] flex items-center gap-1"
            >
          ...
          <div className="flex gap-2 items-center">
            <div className="flex gap-1 flex-1">
              {[1, 2, 3, 4, 5, 6, 7].map((d) => (
                <button
                  ...
                  key={d}
                  onClick={() => {
                    handleDaysCountChange(d);
                    setIsCustomDays(false);
                  }}
                  className={`flex-1 h-10 rounded-lg font-black text-xs border-2 transition-all ${aiSession.daysCount === d && !isCustomDays ? 'bg-indigo-600 text-white border-indigo-400 shadow-md' : 'bg-slate-950 text-slate-500 border-slate-800 hover:bg-slate-900'}`}
                >
                  {d}
                </button>
              ))}
            ...
            <div
              className={`relative w-20 h-10 rounded-lg border-2 transition-all bg-slate-900 flex ${isCustomDays ? 'border-indigo-400' : 'border-slate-800'}`}
            >
              <input
                id="fld-ai-planner-custom-days"
                type="number"
                min="1"
                ...
                onChange={(e) => {
                  ...
                  if (!Number.isNaN(val)) {
                    setIsCustomDays(true);
                    handleDaysCountChange(val);
                  } else if (e.target.value === '') {
                    setIsCustomDays(true);
                    updateAiSession({ daysCount: 0 });
                  }
                }}
              ...
              <div className="absolute right-0 top-0 bottom-0 w-6 flex flex-col border-l border-slate-700/50">
                <button
                  ...
                  className="h-1/2 flex items-center justify-center hover:bg-slate-800 text-[#facc15] active:text-white transition-colors"
                >
                ...
                <button
                  ...
                  className="h-1/2 flex items-center justify-center hover:bg-slate-800 text-[#facc15] active:text-white transition-colors border-t border-slate-700/30"
                >
      ...
      <section>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center mb-1 px-1">
            <button
              type="button"
              onClick={() => setShowDailyLogistics(!showDailyLogistics)}
              className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-colors ${showDailyLogistics ? 'text-[#facc15]' : 'text-slate-500 hover:text-slate-300'}`}
            >
              {showDailyLogistics ? (
              ...
              Configura giorni singoli (Opzionale)
            </button>
          </div>
          ...
          {!showDailyLogistics ? (
            <div className="space-y-4 animate-in fade-in slide-in-from-top-2">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label
                    htmlFor="fld-aiplanner-aiplannerform-tsx-l441"
                    className="uppercase font-black ml-1 flex items-center gap-1 leading-tight"
                  >
                    <Navigation className="w-3 h-3 text-[#f97316]" />
                    <span className="text-[#f97316] text-[10px]">PARTENZA</span>
                    <span className="text-slate-500 text-[8px] font-bold tracking-wide opacity-70">
                      (Hotel, Aeroporto, ...)
                    </span>
                  </label>
                  <input
                    id="fld-aiplanner-aiplannerform-tsx-l441"
                    value={aiSession.startLocation}
                    onChange={(e) => updateAiSession({ startLocation: e.target.value })}
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-white text-xs outline-none font-bold focus:border-emerald-500 transition-colors"
                    placeholder="Es. Stazione Centrale"
                  />
                </div>
                <div className="space-y-1">
                  <label
                    htmlFor="fld-aiplanner-aiplannerform-tsx-l456"
                    className="uppercase font-black ml-1 flex items-center gap-1 leading-tight"
                  >
                  ...
                  <input
                    id="fld-aiplanner-aiplannerform-tsx-l456"
                    value={aiSession.endLocation}
                    onChange={(e) => updateAiSession({ endLocation: e.target.value })}
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-white text-xs outline-none font-bold focus:border-red-500 transition-colors"
              ...
              <div className="grid grid-cols-2 gap-4 pt-2">
                <div className="space-y-1">
                  <label
                    htmlFor="fld-aiplanner-aiplannerform-tsx-l474"
                    className="uppercase font-black ml-1 flex items-center gap-1 text-[10px] text-slate-500"
                  >
                    <Clock className="w-3 h-3" /> INIZIO TOUR
                  </label>
                  <input
                    id="fld-aiplanner-aiplannerform-tsx-l474"
                    type="time"
                    value={aiSession.startTime}
                    onChange={(e) => updateAiSession({ startTime: e.target.value })}
                    className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-white text-xs outline-none font-bold focus:border-indigo-500 transition-colors text-center"
                  />
                </div>
                <div className="space-y-1">
                  <label
                    htmlFor="fld-aiplanner-aiplannerform-tsx-l485"
                    className="uppercase font-black ml-1 flex items-center gap-1 text-[10px] text-slate-500"
                  >
                    <Clock className="w-3 h-3" /> FINE TOUR
                  </label>
                  <input
                    id="fld-aiplanner-aiplannerform-tsx-l485"
                    type="time"
                    value={aiSession.endTime}
                    onChange={(e) => updateAiSession({ endTime: e.target.value })}
                    className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-white text-xs outline-none font-bold focus:border-indigo-500 transition-colors text-center"
                  />
                </div>
              </div>
              ...
              <div className="pt-2">
                <div className="flex justify-between items-end mb-2 px-1">
                  <label
                    htmlFor="fld-aiplanner-aiplannerform-tsx-l499"
                    className={`${filterSectionLabel10Style} flex items-center gap-2`}
                  >
                    <Navigation className="w-3.5 h-3.5" /> DISTANZA MASSIMA SPOSTAMENTI
                  </label>
                  <span className="text-sm font-black text-indigo-400">
                    {aiSession.globalMaxDistance} km
                  </span>
                </div>
                <input
                  id="fld-aiplanner-aiplannerform-tsx-l499"
                  type="range"
                  min="0"
                  ...
                  onChange={(e) =>
                    updateAiSession({
                      globalMaxDistance: DISTANCE_STEPS[parseInt(e.target.value, 10)],
                    })
                  }
            ...
            <div className="space-y-3 animate-in fade-in slide-in-from-top-2 max-h-80 overflow-y-auto custom-scrollbar pr-1">
              {Array.from({ length: aiSession.daysCount }).map((_, i) => {
                const dayLog = aiSession.dailyLogistics[i] || {
                  ...
                  start:
                    i === 0
                      ? aiSession.startLocation
                      : aiSession.dailyLogistics[i - 1]?.end || aiSession.endLocation,
                ...
                return (
                  <div
                    key={i}
                    className="p-4 bg-slate-950/60 border border-slate-800 rounded-2xl space-y-3 shadow-inner hover:border-indigo-500/30 transition-colors"
                  >
                    <div className="flex items-center gap-2 border-b border-slate-800 pb-2 mb-2">
                      <span className="bg-indigo-600 text-white text-[9px] font-black px-2 py-0.5 rounded uppercase shadow-sm">
                        GIORNO {i + 1}
                      </span>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        ...
                        <input
                          value={dayLog.start}
                          onChange={(e) => updateDayLogistic(i, 'start', e.target.value)}
                          className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-[10px] text-white font-bold focus:border-indigo-500 outline-none"
                          placeholder="Luogo Start"
                        />
                      </div>
                      <div className="space-y-1">
                        <span className="text-[8px] font-black text-slate-500 uppercase ml-1">
                        ...
                        <input
                          value={dayLog.end}
                          onChange={(e) => updateDayLogistic(i, 'end', e.target.value)}
                          className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-[10px] text-white font-bold focus:border-indigo-500 outline-none"
                          placeholder="Luogo End"
                        />
                      </div>
                      <div className="space-y-1">
                        <span className="text-[8px] font-black text-slate-500 uppercase ml-1">
                        ...
                        <input
                          type="time"
                          value={dayLog.startTime}
                          onChange={(e) => updateDayLogistic(i, 'startTime', e.target.value)}
                          className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-sm font-black text-white text-center font-mono focus:border-indigo-500 outline-none"
                        />
                      </div>
                      <div className="space-y-1">
                        <span className="text-[8px] font-black text-slate-500 uppercase ml-1">
                        ...
                        <input
                          type="time"
                          value={dayLog.endTime}
                          onChange={(e) => updateDayLogistic(i, 'endTime', e.target.value)}
                          className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-sm font-black text-white text-center font-mono focus:border-indigo-500 outline-none"
                        />
                      </div>
                    </div>
      ...
      <section>
        ...
        <div className="space-y-5">
          <div className="grid grid-cols-3 gap-3">
            {travelStyles.map((s) => (
              <button
                ...
                className={`p-3 rounded-2xl border transition-all text-left flex items-center gap-3 h-20 relative overflow-hidden group ${selectedStyles.includes(s.id) ? 'bg-indigo-600 border-indigo-400 shadow-md ring-2 ring-indigo-400' : 'bg-[#0b0f1a] border-[#1e293b] hover:border-slate-600 hover:bg-slate-900'}`}
              >
                <div className="shrink-0 text-2xl drop-shadow-md group-hover:scale-110 transition-transform duration-300">
                  {s.emoji}
                </div>
                <div className="min-w-0">
                  <div
                    className={`text-sm font-black leading-tight truncate ${selectedStyles.includes(s.id) ? 'text-white' : 'text-slate-200'}`}
                  >
                    {s.label}
                  </div>
                  <div
                    className={`text-[9px] font-bold leading-tight mt-0.5 truncate ${selectedStyles.includes(s.id) ? 'text-indigo-200' : 'text-slate-500'}`}
                  >
                    {s.desc}
                  </div>
                </div>
          ...
          <div className="pt-2">
            <div className="flex justify-between items-end mb-2 px-1">
              <label
                htmlFor="fld-aiplanner-aiplannerform-tsx-l635"
                className={`${filterSectionLabel10Style} flex items-center gap-2`}
              >
                <Coffee className="w-3.5 h-3.5" /> Pausa Relax tra le tappe
              </label>
              <span className="text-sm font-black text-amber-500">
                {aiSession.bufferMinutes} min
              </span>
            </div>
            <input
              id="fld-aiplanner-aiplannerform-tsx-l635"
              type="range"
              min="0"
              max="60"
              step="5"
              value={aiSession.bufferMinutes}
              onChange={(e) => updateAiSession({ bufferMinutes: parseInt(e.target.value, 10) })}
      ...
      {error && (
        <div className="p-4 bg-red-900/30 border border-red-500/50 rounded-xl text-red-400 text-xs font-bold flex items-center gap-2 animate-in slide-in-from-top-2">
        ...
        {!aiBlocked && (
          <div className="text-center w-full max-w-sm mx-auto mt-2">
            <div className="text-center text-xs text-slate-400">
              Desideri ottenere crediti extra gratuiti?{' '}
              <button
                type="button"
            ...
            <div className="relative py-2 mt-2">
              ...
              <div className="relative flex justify-center text-[9px] uppercase font-black text-slate-600 bg-[#020617] px-2 tracking-widest">
                Oppure
              </div>
            </div>

            <div className="flex flex-col items-center gap-1">
              <p className="text-[10px] text-indigo-300 font-bold uppercase tracking-wide">
                Vuoi crediti infiniti?
              </p>
              <button
                type="button"
