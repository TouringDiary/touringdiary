import { Lightbulb, Loader2, Terminal, Zap } from 'lucide-react';
...
export const AiLoadingScreen = ({ onCancel }: Props) => {
    ...
    const load = async () => {
      ...
      if (activeTips.length > 0) setTips(activeTips);
        ...
        setTips([
          {
            id: 'def_tip',
            text: 'Stiamo disegnando il tuo viaggio perfetto...',
            active: true,
            type: 'tip',
          },
        ]);
      ...
      if (activeStatuses.length > 0) setStatuses(activeStatuses);
        ...
        setStatuses([
          { id: 'def_status', text: 'Analisi opzioni in corso...', active: true, type: 'status' },
        ]);
    };
    load();
  }, []);
  ...
  return (
    <div
      className="absolute inset-0 bg-slate-950 flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-300"
      ...
      <div className="max-w-md w-full flex flex-col items-center gap-8 relative">
        ...
        <div className="w-full bg-gradient-to-br from-slate-900 to-slate-900/50 border border-slate-800 rounded-2xl p-6 shadow-xl relative overflow-hidden group">
          ...
          <div className="relative min-h-[4rem] flex items-center justify-center">
            <p
              key={currentTipIndex}
              className="text-slate-200 font-serif text-lg italic leading-relaxed animate-in slide-in-from-bottom-2 fade-in duration-700"
            >
              "{currentTip.text}"
            </p>
          </div>
        ...
        {onCancel && (
          <button
            type="button"
            onClick={onCancel}
            className="mt-2 text-slate-600 hover:text-red-400 text-[10px] uppercase font-bold tracking-widest transition-colors"
          >
            Annulla Operazione
          </button>
        )}
