...
export const CustomCalendar: React.FC<CustomCalendarProps> = ({
  ...
  const rangeStart =
    maxDateStr && value && value <= maxDateStr
      ? value
      : minDateStr && value && minDateStr < value
        ? minDateStr
        : null;
  const rangeEnd =
    maxDateStr && value && value <= maxDateStr
    ...
    >
      ...
      <div className="grid grid-cols-7 mb-2 relative">
        {dayNames.map((day) => (
          <div
            key={day}
            className="w-8 h-8 flex items-center justify-center text-[10px] font-semibold tracking-wide text-slate-100 relative after:absolute after:right-0 after:top-1 after:bottom-1 after:w-px after:bg-slate-600/40 last:after:hidden"
          >
            {day}
          </div>
        ))}
      ...
      <div className="grid grid-cols-7 relative">
        ...
        {days.map((day) => {
          ...
          return (
            <div
              key={day}
              className={`relative w-8 h-8 flex items-center justify-center ${hideSeparator ? '' : 'after:absolute after:right-0 after:top-1 after:bottom-1 after:w-px after:bg-slate-600/30 last:after:hidden'}`}
            >
              <button
                type="button"
                disabled={isDisabled}
