...
export const SmartFilterDrawer = ({
  ...
  // --- DYNAMIC DATA ---
  const CATEGORIES =
    getCachedSetting<PoiCategoryOption[]>(SETTINGS_KEYS.POI_CATEGORIES_CONFIG) || [];
  const POI_STRUCTURE =
    getCachedSetting<Record<string, PoiStructureGroup[]>>(SETTINGS_KEYS.POI_ADVANCED_STRUCTURE) ||
    {};
  ...
  const { visibleSubCatGroups } = useMemo(() => {
    if (mode === 'staging' || !availableItems || availableItems.length === 0)
      return { visibleSubCatGroups: [] };
    ...
    const standardGroups =
      activeCatForSubs && POI_STRUCTURE[activeCatForSubs]
        ? POI_STRUCTURE[activeCatForSubs]
            .map((group: PoiStructureGroup) => {
  ...
  return createPortal(
    <div
      className={`fixed inset-0 z-overlay ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}
    >
      <button
        type="button"
      ...
      >
        ...
        <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-[#0f172a] shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3
                className={
                  headerStyle || 'text-lg font-bold text-white font-display uppercase tracking-wide'
                }
              >
            ...
            <>
              ...
              <div className="flex-1 flex flex-col min-h-0">
                <h4
                  className={
                    sectionTitleStyle ||
                    'text-xs font-black text-slate-500 uppercase tracking-widest mb-2'
                  }
                >
                ...
                <div className="bg-slate-950 border border-slate-800 rounded-xl max-h-[300px] overflow-y-auto custom-scrollbar p-1">
                  {localFilters.rawCategories && localFilters.rawCategories.length > 0 && (
                    <div className="p-2 mb-2 border-b border-slate-800/50 flex flex-wrap gap-2">
                      <span className="w-full text-[9px] text-slate-500 uppercase font-bold">
                        Attivi ({localFilters.rawCategories.length})
                      </span>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      type="button"
                      onClick={() =>
                        setLocalFilters((prev) => ({ ...prev, category: 'all', subCategory: [] }))
                      }
                ...
                <div className="animate-in slide-in-from-right-4 fade-in">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    >
                      {hideCategory
                        ? 'Tipologia Specifica'
                        : `Tipi di ${CATEGORIES.find((c) => c.id === activeCatForSubs)?.label || activeCatForSubs}`}
                    </h4>
                  </div>
                  ...
                  <div className="space-y-4">
                    {visibleSubCatGroups.map((group: PoiStructureGroup, groupIdx: number) => (
                      <div
                        key={groupIdx}
                        className="bg-slate-950/50 p-2.5 rounded-xl border border-slate-800/50"
                      >
                        <h5 className="text-[10px] font-bold text-indigo-300 uppercase tracking-wider mb-2 ml-1">
                          {group.label}
                        </h5>
                        <div className="flex flex-wrap gap-2">
                          {group.items.map((subKey: string) => {
                            ...
                            const label =
                              subKey === '_generic_' ? 'Generico' : getSubCategoryLabel(subKey);
        ...
        <div className="p-4 border-t border-slate-800 bg-[#0f172a] flex items-center gap-3 shrink-0">
          ...
          >
            ...
            {isDirty
              ? 'Applica Filtri'
              : `Vedi ${resultCount !== undefined ? resultCount : 'Risultati'}`}
          </button>
          <button
            type="button"
            onClick={handleReset}
