    ...
    >
      ...
      >
        ...
        <div className={`${bodyShell} flex flex-col items-center text-center gap-4 min-h-0`}>
          <div
            className={`w-16 h-16 rounded-full flex items-center justify-center border-2 shadow-[0_0_15px_rgba(0,0,0,0.5)] ${config.iconBg}`}
          >
            <div className={`${config.iconColor} ${isDeleting ? 'animate-spin' : 'animate-pulse'}`}>
              {isDeleting ? (
                <Loader2 className="w-8 h-8" aria-hidden />
              ) : icon ? (
                <span className="inline-flex items-center justify-center" aria-hidden>
                  {icon}
                </span>
              ) : (
                <DefaultIcon className="w-8 h-8" aria-hidden />
              )}
            </div>
          </div>
          <div className="w-full">
            <h3 id="delete-confirmation-title" className={`${modalTitleShell} mb-2`}>
              {title}
            </h3>
            <p
              id="delete-confirmation-desc"
              className={`${modalSubtitleShell} leading-relaxed whitespace-pre-line`}
            >
              {message}
            </p>
            {children}
          ...
          <div className="flex gap-3 w-full mt-2">
            ...
            >
              {isDeleting ? (
                ...
                (confirmIcon ??
                (!icon && variant === 'danger' ? <Trash2 className="w-4 h-4" /> : null))
              )}
