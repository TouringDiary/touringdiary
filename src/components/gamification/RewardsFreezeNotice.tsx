 ...
 */
export const RewardsFreezeNotice: React.FC<Props> = ({ variant = 'banner', className = '' }) => {
  if (variant === 'compact') {
    return (
      <div
  ...
  return (
    <div
      ...
      role="status"
    >
      <p className="text-sm font-bold text-indigo-100">{REWARDS_FREEZE_XP_NOTICE.headline}</p>
      <p className="text-xs text-slate-400 mt-2 leading-relaxed">{REWARDS_FREEZE_XP_NOTICE.body}</p>
    </div>
  );
};
