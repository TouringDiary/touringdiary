...
import { deleteShopProduct, saveProduct } from '../../services/shopService';
import type { ShopPartner, ShopProduct } from '../../types';
  ...
  return (
    <div className="space-y-6 md:space-y-10 animate-in fade-in pb-32 relative">
      ...
      {isAdding && (
        <div
          ref={formScrollRef}
          className="mb-10 p-6 md:p-8 bg-slate-950 border border-indigo-500/30 rounded-3xl animate-in zoom-in-95 relative shadow-inner"
        >
          <button
            type="button"
          ...
          <h4 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
            ...
            {newProduct.id ? 'Modifica Articolo' : 'Aggiungi Articolo'}
          </h4>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-[160px_1fr] gap-6">
              <div className="space-y-1">
                ...
                <button
                  ...
                  className={`w-full aspect-square rounded-xl border-2 border-dashed flex flex-col items-center justify-center cursor-pointer transition-all hover:border-indigo-500 hover:bg-slate-800 group relative overflow-hidden ${newProduct.imageUrl ? 'border-emerald-500 bg-slate-900' : 'border-slate-700 bg-slate-900'} ${isProductUploading ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  {isProductUploading ? (
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <input
                    ...
                    className="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 text-white focus:border-indigo-500 outline-none font-bold text-sm"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <select
                    ...
                    onChange={(e) =>
                      setNewProduct({
                        ...newProduct,
                        status: e.target.value as ShopProduct['status'],
                      })
                    }
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewProduct({
                        ...newProduct,
                        shippingMode: e.target.value as ShopProduct['shippingMode'],
                      })
                    }
                    className="bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 text-white text-xs font-bold uppercase outline-none focus:border-indigo-500"
                  >
                ...
                <textarea
                  ...
                  className="w-full bg-slate-900 border border-slate-800 rounded-xl p-4 text-white resize-none focus:border-indigo-500 outline-none text-sm h-24"
                  placeholder="Descrizione prodotto * (obbligatoria)"
                  required
                />
              </div>
            </div>
            ...
            {errorMsg && (
              <div className="text-red-400 text-xs font-bold bg-red-900/20 p-3 rounded border border-red-500/30 flex items-center gap-2">
            ...
            <div className="pt-4 border-t border-slate-800/50">
              <button
                ...
                disabled={isProductUploading}
                className="w-full bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white py-3.5 rounded-xl font-black uppercase tracking-[0.2em] shadow-xl shadow-emerald-900/30 transition-all flex items-center justify-center gap-3 active:scale-95 text-xs"
              >
                <Save className="w-5 h-5" />{' '}
                {newProduct.id ? 'Salva Modifiche' : 'Pubblica in Vetrina'}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="flex justify-between items-center mb-4 px-1">
        ...
        <button
          type="button"
          onClick={openNewForm}
          className="bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-lg text-xs font-bold uppercase flex items-center gap-2"
        >
      ...
      {currentShop.products.length > 0 ? (
        <div className="grid grid-cols-1 min-[360px]:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 min-[360px]:gap-4 md:gap-6">
          {currentShop.products.map((product) => (
            <div
              key={product.id}
              className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden group flex flex-col shadow-lg hover:border-slate-600 transition-all relative"
            >
              ...
              <div className="absolute top-2 left-2 z-dropdown">
                <span
                  className={`text-[8px] font-black uppercase px-2 py-0.5 rounded shadow-sm border ${product.status === 'active' ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' : 'bg-slate-800 text-slate-500 border-slate-600'}`}
                >
                  {product.status === 'active' ? 'PUBBLICATO' : 'NASCOSTO'}
                </span>
              </div>
              ...
              <div className="aspect-square relative overflow-hidden bg-black">
                <ImageWithFallback
                  ...
                  alt={product.name}
                  className={`w-full h-full object-cover transition-transform duration-500 group-hover:scale-105 ${product.status === 'inactive' ? 'opacity-50 grayscale' : ''}`}
                />
              </div>
              ...
              <div className="p-4 flex-1 flex flex-col">
                ...
                <div className="mt-auto flex justify-between items-center border-t border-slate-800 pt-2">
                  <span className="text-xs text-slate-500 font-bold uppercase">
                    {product.shippingMode === 'pickup' ? 'Ritiro' : 'Spedizione'}
                  </span>
        ...
        <div className="bg-slate-900/20 rounded-2xl border-2 border-dashed border-slate-800/50 p-12 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-slate-600 text-xs mb-6">
            Inizia ad aggiungere i tuoi prodotti per venderli.
          </p>
          <button
            type="button"
            onClick={openNewForm}
            className="text-indigo-500 font-bold text-xs uppercase hover:text-indigo-400 underline"
          >
            Crea Primo Prodotto
          </button>
        </div>
