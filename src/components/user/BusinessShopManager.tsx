                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <select
                                        ...
                                        onChange={e => setNewProduct({ ...newProduct, status: e.target.value as ShopProduct['status'] })}
                    ...
                    {currentShop.products.map(product => (
                        <div key={product.id} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden group flex flex-col shadow-lg hover:border-slate-600 transition-all relative">
                            ...
                            <div className="absolute top-2 left-2 z-dropdown">
                                <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded shadow-sm border ${product.status === 'active' ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' : 'bg-slate-800 text-slate-500 border-slate-600'}`}>
                                    {product.status === 'active' ? 'PUBBLICATO' : 'NASCOSTO'}
                                </span>
                            </div>

                            <div className="aspect-square relative overflow-hidden bg-black">
                                <ImageWithFallback src={product.imageUrl} alt={product.name} className={`w-full h-full object-cover transition-transform duration-500 group-hover:scale-105 ${product.status === 'inactive' ? 'opacity-50 grayscale' : ''}`} />
                            </div>
