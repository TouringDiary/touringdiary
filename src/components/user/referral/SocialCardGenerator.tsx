...
export const SocialCardGenerator = ({ template, user }: Props) => {
  ...
  useEffect(() => {
    ...
    // Verifica supporto share API per FILE (non solo share generico)
    try {
      if (
  ...
  const handleAction = async (forcedAction?: 'download' | 'share') => {
    ...
    const downloadName = `touring_invite_${user.referralCode}.jpg`;
    ...
    try {
      ...
      try {
        ...
        showGlobalAlert("Impossibile completare l'azione.");
      }
    } finally {
      setIsSharing(false);
    }
  ...
  return (
    <div className="flex flex-col gap-3 group">
      ...
      <div className="relative rounded-2xl overflow-hidden shadow-lg border border-slate-700 bg-slate-900 transition-all hover:shadow-2xl hover:border-indigo-500/50 hover:-translate-y-1">
        {previewUrl ? (
          <img src={previewUrl} alt="Preview" className="w-full h-auto object-cover aspect-[3/4]" />
        ) : (
          <div className="aspect-[3/4] w-full flex items-center justify-center bg-slate-800">
            {!loadFailed && <Loader2 className="w-6 h-6 text-slate-500 animate-spin" />}
          </div>
        )}

        {/* Overlay Action: sempre visibile su touch (<lg); hover/focus-within su desktop */}
