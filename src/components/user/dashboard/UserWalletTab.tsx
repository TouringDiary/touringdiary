...
import type { RewardCategory, UserReward } from '../../../services/gamificationService';
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 h-full flex flex-col">
      ...
      {activeCoupon &&
        createPortal(
          <div
            className="td-modal-overlay !p-4 bg-black/90 backdrop-blur-md"
          ...
          >
            ...
            >
              <div
                className={`p-8 relative overflow-hidden ${getCategoryTheme(activeCoupon.category || 'general').modalHeader}`}
              >
                ...
                <h3 className="text-3xl font-display font-black text-white leading-none mb-1">
                  {activeCoupon.title}
                </h3>
              ...
              <div className="p-8 flex flex-col items-center text-center bg-white flex-1">
                <div className="w-full aspect-square bg-slate-100 rounded-2xl border-2 border-dashed border-slate-300 flex items-center justify-center mb-6 relative">
                  ...
                  <div className="absolute bottom-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                    Scan Me
                  </div>
                </div>
                <div className="text-4xl font-mono font-black text-slate-900 tracking-wider mb-2">
                  {activeCoupon.code}
                </div>
                <p className="text-xs text-slate-400 font-bold uppercase mb-8">
                  Ottenuto il: {new Date(activeCoupon.dateClaimed).toLocaleDateString('it-IT')}
                </p>
                {activeCoupon.status === 'active' ? (
                  <button
                    type="button"
                    onClick={handleUseCoupon}
                    className="w-full bg-slate-900 hover:bg-black text-white font-bold py-4 rounded-xl shadow-lg transition-all active:scale-95 text-sm uppercase tracking-widest"
                  >
                    Segna come Utilizzato
                  </button>
                ) : (
                  <p className="w-full text-center text-sm font-bold uppercase tracking-widest text-slate-500 py-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0">
        <div>
          <h3 className="text-2xl font-bold text-white mb-1 flex items-center gap-2">
            <Wallet className="w-6 h-6 text-indigo-500" /> Il mio Wallet
          </h3>
        ...
        <div className="flex bg-slate-900 p-1 rounded-lg border border-slate-800">
          <button
            ...
            className={`px-4 py-1.5 rounded-md text-[10px] font-bold uppercase transition-all ${filter === 'all' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}
          >
            Tutti
          </button>
          <button
            ...
            className={`px-4 py-1.5 rounded-md text-[10px] font-bold uppercase transition-all ${filter === 'food' ? 'bg-amber-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}
          >
            Cibo
          </button>
          <button
            ...
            className={`px-4 py-1.5 rounded-md text-[10px] font-bold uppercase transition-all ${filter === 'culture' ? 'bg-purple-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}
          >
            Cultura
          </button>
        </div>
          ...
          filteredRewards.map((reward) => {
            ...
            return (
              <div
                ...
                className={`relative h-40 bg-[#020617] rounded-xl flex overflow-hidden border border-slate-800 group hover:border-slate-600 transition-all ${reward.status === 'used' ? 'opacity-60 grayscale' : ''}`}
              >
                <div className="flex-1 p-4 relative flex flex-col justify-between z-floating-panel border-r-2 border-dashed border-slate-800">
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <span
                      className={`text-[9px] font-bold uppercase px-2 py-0.5 rounded border ${reward.status === 'active' ? 'bg-emerald-900/20 text-emerald-400 border-emerald-500/30' : 'bg-slate-800 text-slate-500 border-slate-700'}`}
                    >
                ...
                <button
                  ...
                  aria-label={
                    reward.status === 'active'
                      ? `Mostra coupon ${reward.title}`
                      : `Dettagli coupon utilizzato ${reward.title}`
                  }
                  className="w-14 bg-slate-950 flex flex-col items-center justify-center relative cursor-pointer hover:bg-slate-900 transition-colors"
                  onClick={() => setActiveCoupon(reward)}
                >
                  {reward.status === 'active' ? (
                  ...
                  {reward.status === 'active' && (
                    <ScanLine className="w-5 h-5 text-white absolute bottom-3 animate-pulse" />
                  )}
                </button>
              </div>
            );
