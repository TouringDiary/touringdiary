import { Bell, Key, Loader2, type LucideIcon, Mail, Settings, Trash2 } from 'lucide-react';
    ...
    <div className="flex gap-4 items-start">
      ...
      <div>
        <div className="flex items-center gap-2">
          ...
          {badge && (
            <span className="px-2 py-0.5 bg-indigo-900/30 text-indigo-400 text-[9px] font-black uppercase rounded border border-indigo-500/20">
              {badge}
            </span>
          )}
        </div>
...
const Toggle = ({ active, onToggle, disabled = false }: ToggleProps & { disabled?: boolean }) => (
  ...
  >
    <div
      className={`w-4 h-4 rounded-full bg-white transition-all shadow-md ${active ? 'translate-x-6' : 'translate-x-0'}`}
    ></div>
  </button>
);
...
export const UserSettingsTab = () => {
  ...
  const [collabNotifPrefs, setCollabNotifPrefs] =
    useState<CollaborationNotificationCategoryPrefs | null>(null);
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 pb-12">
      <div>
        <h3 className="text-2xl font-bold text-white flex items-center gap-3">
          <Settings className="w-6 h-6 text-indigo-500" /> Configurazione Account
        </h3>
        <p className="text-slate-500 text-sm">
          Gestisci i tuoi dati e le preferenze dell&apos;applicazione.
        </p>
      </div>
      <div className="space-y-4">
        <h5 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2">
          Profilo & Sicurezza
        </h5>
      ...
      <div className="space-y-4">
        ...
        <SettingCard
          icon={Bell}
          title="Notifiche App"
          desc="Ricevi avvisi su risposte Q&A e promozioni."
          badge="Prossimamente"
        >
        ...
        <SettingCard
          icon={Mail}
          title="Newsletter"
          desc="Ricevi approfondimenti culturali e novità via email."
          badge="Prossimamente"
        >
      ...
      <div className="pt-8 border-t border-slate-800 flex justify-center">
        <button
          type="button"
          className="flex items-center gap-2 text-red-500 hover:text-red-400 font-bold uppercase text-xs tracking-widest"
        >
