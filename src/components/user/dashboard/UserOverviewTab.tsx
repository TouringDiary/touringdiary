...
import type { getNextLevelProgress, LevelInfo } from '../../../services/gamificationService';
...
interface Props {
  ...
  currentXP: number;
  progress: ReturnType<typeof getNextLevelProgress>;
...
export const UserOverviewTab = ({
  ...
  const renderRewardCard = (reward: Reward, isUnlocked: boolean) => {
    ...
    const isActiveInWallet = myRewards.some(
      (r) => r.rewardId === reward.id && r.status === 'active',
    );
    ...
    return (
      ...
      >
        <div className="p-4 flex justify-between items-start">
          <div
            className={`p-2 rounded-lg bg-slate-800 ${canClaim ? theme.text : 'text-slate-400'}`}
          >
            <Icon className="w-6 h-6" />
          </div>
          <span
            className={`text-[9px] font-black px-2 py-1 rounded-full uppercase tracking-wide border ${canClaim ? 'bg-emerald-900/20 text-emerald-500 border-emerald-500/30' : 'bg-slate-900 text-slate-500 border-slate-700'}`}
          >
        ...
        <div className="px-4 flex-1">
          <h4
            className={`text-lg font-bold leading-tight mb-2 ${canClaim ? 'text-white' : 'text-slate-300'}`}
          >
            {reward.title}
          </h4>
          <p
            className={`text-xs line-clamp-3 leading-relaxed ${canClaim ? 'text-slate-500' : 'text-slate-400'}`}
          >
            {reward.description}
          </p>
        </div>
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 pb-10 relative">
        ...
        <div className="relative z-floating-panel w-full">
          <div className="flex justify-between items-start mb-4">
            <div className="flex items-center gap-4">
              <div className="relative">
                ...
                />
                <div className="absolute -bottom-1 -right-1 w-8 h-8 bg-slate-900 text-xl border-2 border-slate-700 rounded-full flex items-center justify-center shadow-lg">
                  {currentLevel.icon}
                </div>
              </div>
              <div>
                ...
                {user.slug && <p className="text-sm text-slate-400 mt-1">@{user.slug}</p>}
                <div className="flex flex-wrap items-center gap-2 mt-2">
                  {user.role !== 'user' && (
                    <span
                      className={`text-[9px] font-black px-2 py-0.5 rounded border uppercase flex items-center gap-1 ${user.role === 'admin_all' ? 'bg-purple-900/30 text-purple-300 border-purple-500/30' : 'bg-blue-900/30 text-blue-300 border-blue-500/30'}`}
                    >
                  ...
                  <div className="flex items-center gap-1 bg-slate-950/50 px-2 py-0.5 rounded border border-slate-600/50">
                    ...
                    <span className="text-[9px] font-black text-slate-300 uppercase tracking-wide">
                      {currentLevel.name} (Liv. {currentLevel.level})
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-col items-end">
              ...
              <span
                className={`text-2xl font-black uppercase tracking-widest ${currentLevel.color} drop-shadow-md`}
              >
                {currentXP} XP
              </span>
            </div>
          ...
          <div className="mt-2">
            <div className="relative h-5 bg-slate-950 rounded-full overflow-hidden border border-slate-800 shadow-inner">
              <div
                className="absolute top-0 left-0 h-full bg-gradient-to-r from-amber-500 to-red-600 transition-all duration-1000 ease-out shadow-[0_0_10px_rgba(245,158,11,0.5)]"
              ...
              <div className="absolute inset-0 flex items-center justify-center text-[9px] font-black text-white drop-shadow-md z-floating-panel tracking-widest uppercase">
                {progress.nextLevel
                  ? `${currentXP} / ${progress.nextLevel.minXp} XP`
                  : `LIVELLO MASSIMO`}
              </div>
            </div>
          </div>
      ...
      {!rewardsEnabled && <RewardsFreezeNotice variant="banner" />}
      ...
      <div>
        <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 mb-4 flex items-center gap-2">
          ...
          {rewardsEnabled ? 'Prossimi Obiettivi' : 'Premi in arrivo'}
        </h3>
        <DraggableSlider ref={lockedSliderRef} className="pb-4 gap-4">
          {upcomingRewards.map((reward, idx) => {
            ...
            const showSeparator =
              idx === 0 || (prevReward && prevReward.requiredLevel !== reward.requiredLevel);
                  ...
                  <div className="flex flex-col justify-center items-center h-72 mx-2">
                    <div className="h-full w-px bg-slate-800 border-l border-dashed border-slate-700/50 relative">
                      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-slate-900 border border-slate-700 px-2 py-4 rounded-full flex flex-col items-center gap-1 shadow-lg z-floating-panel">
                        ...
                        <span
                          className="text-[9px] font-black text-white uppercase vertical-rl"
                          style={{ writingMode: 'vertical-rl' }}
                        >
                          LIV {reward.requiredLevel}
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          {upcomingRewards.length === 0 && rewardsEnabled && (
            <div className="text-slate-500 italic text-xs py-10 w-full text-center border-2 border-dashed border-slate-800 rounded-xl">
              Hai sbloccato tutto! Sei una leggenda.
            </div>
          )}
        </DraggableSlider>
      </div>
      ...
      <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 relative overflow-hidden">
        ...
        <h3 className="text-xl font-bold text-white mb-2 relative z-floating-panel">
          Le Mie Segnalazioni
        </h3>
        <p className="text-slate-400 text-sm relative z-floating-panel mb-4">
          {safeArray<SuggestionRequest>(suggestions).length} contributi inviati alla community.
        </p>
        <div className="flex items-center text-indigo-400 text-xs font-bold uppercase tracking-wider relative z-floating-panel">
          Grazie per il tuo contributo <ChevronRight className="w-4 h-4" />
        </div>
      </div>
    </div>
  );
