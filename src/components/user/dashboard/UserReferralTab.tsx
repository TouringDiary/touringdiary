...
import { getAllUsers, getUserById, refreshUsersCache } from '../../../services/userService';
import type { SocialTemplate, User as UserType } from '../../../types/index';
...
const SECTION_BOX_STYLE =
  'bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col gap-4';
...
export const UserReferralTab = ({ user, onUpdateUser, onSwitchToOverview }: Props) => {
  ...
  const [isDesktop, setIsDesktop] = useState(() =>
    typeof window !== 'undefined' ? window.matchMedia(DESKTOP_MIN_QUERY).matches : false,
  );
  ...
  useEffect(() => {
    ...
    const mql = window.matchMedia(DESKTOP_MIN_QUERY);
  ...
  const handleCopy = async (textToCopy?: string) => {
    ...
    try {
      ...
      showGlobalAlert(
        'Impossibile copiare negli appunti. Prova a selezionare e copiare manualmente.',
      );
    }
  };
  ...
  const handleSocialShare = (
    ...
    switch (platform) {
      ...
      case 'linkedin':
        ...
        break;
      case 'copy':
        void handleCopy(decodeURIComponent(text));
        return;
    }
    const popup = window.open(link, '_blank', 'noopener,noreferrer');
    if (!popup) {
      showGlobalAlert(
        'Impossibile aprire la finestra di condivisione. Controlla il blocco popup del browser.',
      );
    }
  };

  const handleRedeem = async () => {
    // TODO: re-enable redeemReferralCode when the flow is restored.
    showGlobalAlert('La funzione di riscatto codice è temporaneamente disabilitata.');
  };
        ...
        <div className="relative z-floating-panel flex flex-col items-center text-center gap-6">
          <div>
            ...
            <div className="text-indigo-100 text-sm max-w-md mx-auto leading-relaxed">
              <p className="mb-2">
                Condividi il tuo codice. Per ogni amico che si iscrive, entrambi ricevete{' '}
                <strong>+20 Crediti AI Extra</strong>.
              </p>
              ...
              {onSwitchToOverview && (
                ...
                >
                  Scopri tutti i vantaggi e i premi esclusivi nel tuo Profilo{' '}
          ...
          <div className="bg-white/10 backdrop-blur-md p-6 rounded-2xl border border-white/20 flex flex-col items-center gap-4 w-full max-w-sm">
            ...
            <div className="grid grid-cols-5 gap-2 w-full mt-2">
              <button
                ...
                className="bg-[#25D366] hover:bg-[#20bd5a] text-white p-2.5 rounded-xl transition-all shadow-lg flex items-center justify-center group"
                title="WhatsApp"
              >
              ...
              <button
                ...
                className="bg-[#1877F2] hover:bg-[#166fe5] text-white p-2.5 rounded-xl transition-all shadow-lg flex items-center justify-center"
                title="Facebook"
              >
              ...
              <button
                ...
                className="bg-black hover:bg-slate-900 text-white p-2.5 rounded-xl transition-all shadow-lg flex items-center justify-center"
                title="X (Twitter)"
              >
              ...
              <button
                ...
                className="bg-[#0077b5] hover:bg-[#006fa3] text-white p-2.5 rounded-xl transition-all shadow-lg flex items-center justify-center"
                title="LinkedIn"
              >
              ...
              <button
                ...
                className="bg-slate-100 hover:bg-white text-slate-600 p-2.5 rounded-xl transition-all shadow-lg flex items-center justify-center"
                title="Copia Testo Completo"
              >
                {copied ? (
          ...
          <div className="flex items-center gap-6 bg-slate-900/50 p-2 px-4 rounded-xl border border-slate-800">
            <div className="text-right">
              ...
              <div className="text-3xl font-black text-white flex items-center justify-end gap-1">
                {totalEarnedCredits}{' '}
                <span className="text-sm font-bold text-slate-600">crediti</span>
              </div>
            </div>
            <div className="w-px h-10 bg-slate-700"></div>
            <div className="flex flex-col gap-1 text-xs min-w-[80px]">
              <div className="flex justify-between items-center w-full">
                <span className="text-slate-500 font-bold uppercase text-[9px] tracking-wide">
                  Guide:
                </span>
                <span
                  className={`font-mono font-bold ${bonusFromGuide > 0 ? 'text-indigo-400' : 'text-slate-600'}`}
                >
                  +{bonusFromGuide}
                </span>
              </div>
              <div className="flex justify-between items-center w-full">
                ...
                <span
                  className={`font-mono font-bold ${bonusFromFriends > 0 ? 'text-emerald-400' : 'text-slate-600'}`}
                >
                  +{bonusFromFriends}
                </span>
              </div>
            ...
            {!user.referredBy ? (
              <div className="bg-slate-950 border border-slate-800 rounded-2xl p-5 shadow-inner">
                <p className="text-xs text-slate-400 mb-3">
                  Hai un codice amico? Inseriscilo qui per ottenere subito il tuo bonus.
                </p>
                <div className="space-y-3">
                  <input
                    ...
                    value={codeToRedeem}
                    onChange={(e) => {
                      setCodeToRedeem(e.target.value.toUpperCase());
                ...
                {redeemResult && (
                  <div
                    className={`mt-3 p-3 rounded-xl text-xs font-bold flex items-center gap-2 ${redeemResult.success ? 'bg-emerald-900/20 text-emerald-400 border border-emerald-500/30' : 'bg-red-900/20 text-red-400 border border-red-500/30'}`}
                  >
              ...
              <div className="bg-indigo-900/10 border border-indigo-500/20 p-4 rounded-2xl flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-full bg-slate-800 border-2 border-indigo-500 flex items-center justify-center font-bold text-slate-300 overflow-hidden shadow-lg shrink-0">
                    {referrer?.avatar && !referrer.avatar.includes('ui-avatars') ? (
                      <img
                        src={referrer.avatar}
                        alt={referrer.name || 'Avatar guida'}
                  ...
                  <div className="min-w-0">
                    <h4 className="text-white font-bold text-sm truncate">
                      {referrer?.name || 'Utente Sconosciuto'}
                    </h4>
                ...
                <div className="text-center bg-indigo-900/40 px-3 py-1.5 rounded-xl border border-indigo-500/30 shrink-0">
                  <span className="block text-[8px] text-indigo-300 font-black uppercase tracking-widest">
                    BONUS
                  </span>
            ...
            <div className="max-h-[300px] overflow-y-auto custom-scrollbar pr-1 space-y-3">
              {isLoading ? (
                ...
                referrals.map((friend) => (
                  <div
                    key={friend.id}
                    className="bg-slate-950 p-3 rounded-xl border border-slate-800 flex items-center justify-between group hover:border-slate-700 transition-all"
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-9 h-9 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center font-bold text-slate-400 overflow-hidden shrink-0">
                        {friend.avatar && !friend.avatar.includes('ui-avatars') ? (
                          <img
                            src={friend.avatar}
                            alt={friend.name || 'Avatar amico'}
                            className="w-full h-full object-cover"
                          />
                        ) : (
                    ...
                    <div className="text-center bg-emerald-900/10 px-2 py-1 rounded border border-emerald-500/20 shrink-0">
                      <span className="text-emerald-400 text-xs font-black flex items-center gap-0.5">
                ...
                <div className="text-center py-10 border-2 border-dashed border-slate-800 rounded-2xl flex flex-col items-center gap-2">
                  ...
                  <p className="text-slate-500 text-xs font-medium px-4">
                    Ancora nessun invito accettato.
                  </p>
                </div>
              )}
            </div>
      ...
      <div className={SECTION_BOX_STYLE}>
        ...
        {isLoading ? (
          ...
          <div className="w-full overflow-hidden">
            {templates.length > 0 ? (
              <DraggableSlider className="pb-4 gap-4">
                {templates.map((tpl) => (
