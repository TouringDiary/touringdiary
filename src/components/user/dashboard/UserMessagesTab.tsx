...
import { markPartnerLogsAsRead, sendUserMessage } from '../../../services/sponsorService';
import type { SponsorRequest, User as UserType } from '../../../types/index';
...
export const UserMessagesTab = ({ user, requests, onRefresh }: UserMessagesTabProps) => {
  ...
  const { getText: getDisabledMsg } = useSystemMessage(
    PLATFORM_MESSAGE_TEMPLATE_KEYS.COMMS_USER_SPONSOR_DISABLED,
  );
  ...
  const renderList = () => (
    <div
      className={`w-full md:w-80 bg-slate-900 border-r border-slate-800 flex flex-col ${selectedRequestId && !isMobileList ? 'hidden md:flex' : 'flex'}`}
    >
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        {requests.length === 0 ? (
          <div className="p-8 text-center text-slate-500 text-sm">Nessuna pratica attiva.</div>
        ) : (
          requests.map((req) => {
            const unreadCount =
              req.partnerLogs?.filter((l) => l.direction === 'outbound' && l.isUnread).length || 0;
            return (
              <button
                type="button"
                key={req.id}
              ...
              >
                <div className="flex justify-between items-start mb-1">
                  ...
                  <span
                    className={`text-[9px] px-1.5 py-0.5 rounded uppercase font-bold border ${req.status === 'approved' ? 'bg-emerald-900/30 text-emerald-400 border-emerald-500/30' : 'bg-amber-900/30 text-amber-400 border-amber-500/30'}`}
                  >
  ...
  const renderChat = () => {
    if (!activeRequest)
      return (
        <div className="flex-1 hidden md:flex items-center justify-center text-slate-500 italic bg-[#020617]">
    ...
    return (
      <div
        className={`flex-1 flex flex-col h-full bg-[#020617] ${isMobileList ? 'hidden md:flex' : 'flex'}`}
      >
        {/* Chat Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 flex items-center gap-1">
                <Briefcase className="w-3 h-3" /> {activeRequest.companyName}
              </p>
            </div>
          </div>
        </div>
        ...
        {/* Messages Area */}
        <div
          className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-[#050b14]"
          ref={scrollRef}
        >
          ...
          {sortedLogs.map((log, idx) => {
            ...
            return (
              <div key={idx} className={`flex w-full ${isUser ? 'justify-end' : 'justify-start'}`}>
                <div
                  className={`max-w-[80%] md:max-w-[60%] p-3 rounded-2xl text-sm relative shadow-md ${isUser ? 'bg-blue-600 text-white rounded-br-none' : 'bg-slate-800 text-slate-200 rounded-bl-none border border-slate-700'}`}
                >
                  <p className="whitespace-pre-wrap leading-relaxed">{log.message}</p>
                  <div
                    className={`text-[9px] mt-1.5 flex items-center justify-end gap-1 ${isUser ? 'text-blue-200' : 'text-slate-500'}`}
                  >
        ...
        <div className="p-4 bg-slate-900 border-t border-slate-800">
          {!commsEnabled ? (
            <div className="text-center text-sm text-slate-400 bg-slate-950 border border-slate-800 rounded-xl p-4">
              {getDisabledMsg().body ||
                'La messaggistica con lo staff non è al momento disponibile.'}
            </div>
          ) : (
          ...
          <p className="text-[10px] text-slate-500 mt-2 text-center">
            {commsEnabled
              ? 'Lo staff risponde solitamente entro 24 ore.'
              : 'Messaggistica disattivata (fase 1).'}
          </p>
        </div>
      </div>
    );
