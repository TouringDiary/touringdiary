...
import { formatRoadbookDuration, type RoadbookSummary } from '@/domain/diary/roadbookSummary';
import type { Itinerary, RoadbookDay } from '../../types/index';
...
export const RoadbookDocument = ({
  itinerary,
  roadbook,
  activeCityName,
  logoBase64,
  summaryData,
}: RoadbookDocumentProps) => {
  const cleanLocationName = (name: string) => {
    if (!name) return '';
    return name
      .replace(/^Punto di Partenza\s*[(:-]?\s*/i, '')
      .replace(/\)$/, '')
      .trim();
  };
      ...
      <Page size="A4" style={styles.page}>
        ...
        {roadbook.map((day, dIdx) => {
          const dayItems = itinerary.items
            .filter((i) => i.dayIndex === day.dayIndex)
                  ...
                  <View key={sIdx} style={styles.segmentBox} wrap={false}>
                    <View style={styles.segmentHeader}>
                      ...
                      <View style={[styles.locationBox, { alignItems: 'flex-end' }]}>
                        <Text style={[styles.locationName, { textAlign: 'right' }]}>
                          {segment.to}
                        </Text>
          ...
          <View style={styles.summaryGrid}>
            ...
            <View style={styles.summaryCol}>
              ...
              <View style={styles.summaryRow}>
                ...
                <Text style={styles.summaryVal}>
                  {formatRoadbookDuration(summaryData.totalWalkingMinutes)}
                </Text>
              </View>
              <View style={styles.summaryRow}>
                <Text style={styles.summaryLabel}>Mezzi/Auto</Text>
                <Text style={styles.summaryVal}>
                  {formatRoadbookDuration(summaryData.totalTransitMinutes)}
                </Text>
              </View>
            ...
            <View style={styles.summaryCol}>
              ...
              <View style={styles.summaryTotal}>
                ...
                <Text style={styles.summaryTotalVal}>€ {summaryData.totalBudget.toFixed(2)}</Text>
              </View>
            </View>
          </View>
        </View>
