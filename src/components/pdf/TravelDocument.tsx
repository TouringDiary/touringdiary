...
import {
  buildHeroCoverCollagePlan,
  HERO_COVER_STACK_GAP,
  heroCoverStackImageHeight,
} from '../../utils/heroCoverCollagePlan';
import { logPdfImagePipeline } from '../../utils/pdfImagePipelineLog';
import type { PreparedItinerary, PreparedItineraryItem } from '../../utils/pdfUtils';
      ...
      <View style={styles.coverContainer}>
        ...
        {options.summary && (
          <View style={styles.coverTextBlock}>
            ...
            <Text style={styles.coverSubtitle}>{itinerary.formattedCityList}</Text>
            ...
            <View style={styles.coverPhotosFooter}>
              {images.length > 0 ? (
                ...
                <View style={styles.coverEmptyPhotosPlaceholder}>
                  <Text style={styles.coverEmptyPhotosText}>NESSUNA FOTO DISPONIBILE</Text>
                </View>
              )}
            </View>
          </View>
            ...
            <View key={item.id} style={styles.timelineItem}>
              ...
              <View style={styles.colContent}>
                <View style={styles.colContentMain}>
                  {distKm != null && distKm > 0 && (
                    <View style={styles.poiDistanceLead}>
                      <Text style={styles.poiDistanceLeadText}>--- DISTANZA {distKm} KM ---</Text>
                    </View>
                  )}
                  ...
                  {poi?.address && <Text style={styles.poiAddress}>{poi.address}</Text>}

                  {options.details && <Text style={styles.poiDescription}>{descriptionText}</Text>}
                ...
                <View style={styles.colContentSide}>
                  {poi?.visitDuration && (
                    <Text style={styles.poiDuration}>Durata visita: {poi.visitDuration}</Text>
                  )}

                  {willRenderImage && item.processedImage && (
                    <Image src={item.processedImage} style={styles.inlineImage} />
                  )}
                </View>
              </View>
            </View>
