    ...
    try {
      if (isMock) {
        ...
        if (finalizeError || !data?.success) {
          throw new Error(
            finalizeError?.message || data?.error || 'Errore durante la finalizzazione del mock',
          );
        }
      }
      ...
      const message =
        err instanceof Error ? err.message : "Errore durante la finalizzazione dell'acquisto.";
    ...
    <div className="min-h-[80vh] flex items-center justify-center p-4 md:p-8">
      <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-500">
        ...
        <div className="h-32 bg-gradient-to-br from-indigo-600/20 via-slate-900 to-slate-900 relative flex items-center justify-center">
          ...
          {status === 'error' && <AlertCircle className="w-16 h-16 text-rose-500" />}
        </div>
        ...
        <div className="p-8 md:p-12 text-center space-y-6">
          ...
          {status === 'success' && (
            <>
              ...
              <p className="text-slate-400 text-sm font-medium leading-relaxed">
                I tuoi crediti sono stati elaborati correttamente e sono ora disponibili nel tuo
                wallet.
              </p>
              ...
              <div className="pt-4">
                <div className="bg-slate-950/50 border border-slate-800 rounded-2xl p-4 flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-mono text-slate-400 truncate ml-4 max-w-[200px]">
                    {sessionId}
                  </span>
                </div>
              </div>
          ...
          {status === 'error' && (
            <>
              ...
              <p className="text-rose-400/80 text-sm font-medium leading-relaxed">
                {error ||
                  "Non è stato possibile confermare l'acquisto. Se il pagamento è andato a buon fine, i crediti verranno accreditati a breve."}
              </p>
            </>
          )}
