...
import type { ItineraryItem, PoiCategory, PointOfInterest } from '@/types';
import { openMap } from '@/utils/common';
import { TL_ARC_REACH, TL_LINE_X, TL_RAIL_W } from './timelineLayout';
...
export const ItineraryItemCard: React.FC<ItineraryItemCardProps> = ({
  ...
  const hasValidCoords =
    item.poi.coords && (item.poi.coords.lat !== 0 || item.poi.coords.lng !== 0);

  let userDist = null;
  if (userLocation && hasValidCoords && item.poi.coords) {
    userDist = calculateDistance(
      userLocation.lat,
      userLocation.lng,
      item.poi.coords.lat,
      item.poi.coords.lng,
  ...
  const handleDragStart = (e: React.DragEvent) => {
    ...
    e.dataTransfer.setData('text/plain', JSON.stringify({ type: 'MOVE_ITEM', id: item.id }));
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    e.stopPropagation();
    setIsDragOver(false);
    onItemDrop(e, dayIndex, item.timeSlotStr || '09:00');
  };
  ...
  const renderInfoBox = () => {
    return (
      <span
        className={`${durationStyle} leading-[1.75rem] text-center flex items-center justify-center gap-0.5 h-full`}
      >
  ...
  return (
    <SwipeToDelete
      ...
      label={item.isCustom ? 'Elimina' : 'Elimina tappa'}
      inlineLabel={item.isCustom}
      ...
      >
          ...
          {hasMovement && (
            <>
              ...
              <button
                ...
                style={{
                  left: `${LINE_X - TL_ARC_REACH}px`,
                  top: `calc(${movementMidRem}rem - 12px)`,
                }}
            ...
            {item.isCustom ? (
              <div ref={iconAnchorRef}>
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    onIconClick(iconPickerOpen === item.id ? null : item.id);
                  }}
                  ...
                  <div className="grid grid-cols-5 gap-2">
                    {Object.entries(CUSTOM_ICONS).map(([key, Icon]) => (
                      <button
                        type="button"
                        key={key}
                        onClick={() => onIconSelect(item.id, key)}
                        className="p-2 hover:bg-stone-200 rounded text-stone-700 flex justify-center transition-colors items-center"
                        title={key}
                      >
              ...
              <div className="grid grid-cols-5 gap-1">
                {TRANSPORT_MODES.map((m) => (
                  <button
                    ...
                    key={m.key}
                    onClick={() => {
                      onTransportSelect(item.id, m.key);
                      setTransportOpen(false);
                    }}
        ...
        <div className="flex-1 min-w-0 flex bg-transparent pointer-events-none z-floating-panel">
          ...
          <div className="flex-1 min-w-0 flex flex-col border-r border-stone-300/40">
            <div className={`${rowHeightClass} flex items-center relative z-floating-panel w-full`}>
              ...
              <div className="w-12 flex flex-col items-center justify-center shrink-0 h-full z-floating-panel relative pointer-events-auto leading-[1.75rem] border-r border-stone-300/30">
                {editingTimeId === item.id ? (
                  <select
                    autoFocus
                    className={`w-full bg-transparent text-stone-900 border-b border-stone-500 focus:outline-none p-0 h-full leading-[1.75rem] appearance-none text-center ${timeSlotStyle}`}
                    value={item.timeSlotStr}
                    onChange={(e) => {
                      onTimeChange(item.id, e.target.value, dayIndex);
                  ...
                  <button
                    ...
                    className={`${timeSlotStyle} leading-[1.75rem] flex items-center justify-center w-full h-full hover:text-amber-700`}
                  >
                    {item.timeSlotStr || (
                  ...
                  isEditingNote ? (
                    ...
                    >
                      {item.poi.description || (
                        <span className="text-stone-400 italic">Clicca per scrivere...</span>
                      )}
                    </div>
                  )
                ) : (
                  <h4
                    onClick={() => onViewDetail(item.poi)}
                    className={`${poiNameStyle} truncate leading-[1.75rem] h-full flex items-center cursor-pointer hover:text-amber-700 transition-colors w-full`}
                  >
                    {item.poi.name}
                  </h4>
                )}
              ...
              <div
                className={`flex items-center justify-end z-dropdown gap-1 shrink-0 pl-1 pointer-events-auto ${isMobile ? 'pr-11' : 'pr-1'}`}
              >
                {userDist && (
                  <div className="flex items-center gap-0.5 text-[8px] md:text-[10px] font-bold text-emerald-600 bg-emerald-100/50 px-1 rounded border border-emerald-500/30 whitespace-nowrap shrink-0">
            ...
            {showSecondRow && (
              <div
                className={`${rowHeightClass} flex items-center border-b border-stone-300/30 relative z-floating-panel w-full h-[1.75rem]`}
              >
                ...
                <div
                  className={`flex-1 flex items-center min-w-0 h-full overflow-hidden pl-2 pointer-events-auto ${isMobile ? 'pr-11' : 'pr-9'}`}
                >
                  {item.poi.address ? (
                    <button
                      type="button"
                      onClick={() => {
                        if (!item.poi.coords) return;
                        openMap(item.poi.coords.lat, item.poi.coords.lng);
                      }}
                    ...
                    <span className="text-[9px] text-stone-400 italic leading-[1.75rem] h-full flex items-center">
                      Nessun indirizzo
                    </span>
                  )}
                </div>
