...
import type { Itinerary, ItineraryItem } from '../../../types/index';
        ...
        <MobileMoveModal
          ...
          onConfirm={(nd, nt, forceSwap) => {
            if (state.itemToMove) {
              onDayDrop(
                nd,
                JSON.stringify({ type: 'MOVE_ITEM', id: state.itemToMove.id, forceSwap }),
                nt,
              );
            }
      ...
      {state.memoTargetItem && (
        <AddToItineraryModal
          ...
          onRemove={(itemId) => {
            ...
            if (state.memoTargetItem?.id === itemId) {
              setters.setMemoTargetItem(null);
            }
          }}
          ...
          onDateSet={(s, e) =>
            setters.setItinerary((prev) => ({ ...prev, startDate: s, endDate: e }))
          }
        />
      )}
        ...
        createPortal(
          ...
          >
            ...
            >
              ...
              />
              <div
                className="absolute -top-10 -right-10 w-32 h-32 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none"
                aria-hidden
              />
              <div className={`${bodyShell} flex flex-col items-center text-center gap-4 min-h-0`}>
                ...
                <h2
                  id="diary-xp-success-title"
                  className={`${modalTitleShell} uppercase tracking-wider`}
                >
                  {state.toastMessage.title || getSuccessMsg().title || 'Ottimo Lavoro!'}
                </h2>
                <p
                  id="diary-xp-success-desc"
                  className={`${modalSubtitleShell} whitespace-pre-line`}
                >
                  {state.toastMessage.xp > 0 ? (
                    <>
                      Hai guadagnato punti esperienza per questa attività.
                      <strong className="text-white block mt-2 text-lg">
                        +{state.toastMessage.xp} XP
                      </strong>
                      <span className="text-xs font-normal opacity-70">
                        Aggiunti al tuo profilo
                      </span>
                    </>
                  ) : (
