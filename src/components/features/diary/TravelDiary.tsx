...
import type { CitySummary, PointOfInterest, User } from '@/types';
...
const TravelDiaryContent = ({
  ...
  const { itinerary, savedProjects, highlightDates, highlightedItemId, state, setters, actions } =
  ...
  return (
    <div
      ref={containerRef}
      className="isolate h-full flex flex-col rounded-sm shadow-xl overflow-hidden border border-slate-600 relative group/diary bg-[#e7e5e4] select-none"
    >
      <style>{`
      ...
      {state.toastMessage && state.toastMessage.xp > 0 && (
          ...
          <div className="bg-slate-900 border border-emerald-500 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 max-w-md">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Hai guadagnato{' '}
                <span className="text-amber-400 font-bold">+{state.toastMessage.xp} XP</span>
              </p>
              {!rewardsEnabled && (
          ...
          <>
            ...
            {!isNotesActive && (
              <DiaryTimeline
                ...
                onColorSelect={(idx, cls) => {
                  actions.updateDayStyle(idx, cls);
                  setters.setColorPickerOpen(null);
                }}
                ...
                onEditResource={actions.toggleItemType}
