...
import type { CitySummary, ItineraryItem, PointOfInterest } from '@/types';
...
export const DiaryDay: React.FC<DiaryDayProps> = ({
  ...
  const dayCityStr = uniqueCityIds
    .map((id) => getCityDisplayName(id, cityManifest))
    .filter(Boolean)
    .join(', ');
  ...
  const timelineItems = items
    ...
    .sort((a, b) => a.timeSlotStr.localeCompare(b.timeSlotStr));
  ...
  return (
    <div
      ref={(el) => {
        if (dayRefs.current) dayRefs.current[dayIndex] = el;
      }}
      className="relative pb-0 mb-0 transition-all rounded-xl overflow-hidden"
    >
      {/* HEADER GIORNO: Altezza fissa h-7 (1 RIGA esatta) */}
      <div
        className={`flex items-center justify-center gap-2 ${ROW_H} border-b w-full box-border rounded-t-xl transition-colors border-stone-300 bg-[#e7e5e4]`}
      >
        {/* Micro-toolbar compatta Giorno */}
        <div
          ref={colorAnchorRef}
          className={`px-4 h-[24px] rounded-full shadow-sm flex items-center justify-center relative ${dayStyle.class}`}
        >
      ...
      >
        ...
        {!isOverDropZone && isMobile ? (
          <span className="text-[10px] font-semibold leading-snug text-center text-balance px-3 pointer-events-none select-none">
            Qui compariranno le tappe del tuo viaggio.
            <br />
            Usa il pulsante <PlusCircle className="inline-block align-middle w-3.5 h-3.5 mx-0.5" />{' '}
            ADD della scheda del luogo desiderato!
          </span>
        ) : (
          <div className="flex items-center gap-2 pointer-events-none select-none">
            <PlusCircle
              className={`w-3.5 h-3.5 shrink-0 ${isOverDropZone ? 'animate-bounce' : ''}`}
            />
      ...
      {resourceItems.length > 0 && (
        <div className="w-full relative z-floating-panel bg-transparent">
          ...
          <div
            className={`flex items-center justify-center gap-2 ${ROW_H} border-t border-dashed border-black/40`}
          >
            <Briefcase className="w-3.5 h-3.5 text-stone-600" />
            <span className="text-[9px] font-black uppercase text-stone-700 tracking-widest pt-0.5">
              CONTATTI
            </span>
          </div>
