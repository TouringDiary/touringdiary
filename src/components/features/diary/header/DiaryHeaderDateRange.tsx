    ...
    <div className="flex items-center w-full min-w-0 gap-3">
      <div className="flex flex-1 min-w-0 items-center gap-3">
        <div className="flex-1 min-w-0" ref={startAnchorRef}>
          <div
            className={`flex items-center bg-slate-800 rounded border h-9 overflow-hidden ${highlightDates ? 'border-red-500 ring-2 ring-red-500 animate-pulse' : 'border-slate-700'}`}
          >
            <div className="h-full w-10 flex items-center justify-center border-r border-slate-600/50 bg-slate-900/30 shrink-0">
              <span className="text-[10px] font-bold text-amber-500 uppercase leading-none">
                Dal
              </span>
            </div>
      ...
      <div className="flex shrink-0 items-center">
        <DiaryHeaderUndoRedo onUndo={onUndo} onRedo={onRedo} canUndo={canUndo} canRedo={canRedo} />
      </div>
    </div>
  );
};
