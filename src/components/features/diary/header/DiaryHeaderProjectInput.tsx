        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <div ref={loadMenuRef}>
            <button
              ...
              className={`text-slate-400 hover:text-white hover:bg-slate-800 p-1.5 rounded-full transition-colors ${loadMenuOpen ? 'bg-slate-800 text-white' : ''}`}
              title="Apri/Carica"
              aria-label="Apri/Carica"
            >
            ...
            >
              <DiaryToolbarPopoverHeader
                ...
                trailing={
                  isSyncing ? <RefreshCw className="w-3 h-3 animate-spin" aria-hidden /> : undefined
                }
              />
          ...
          <button
            ...
            className="text-slate-400 hover:text-blue-400 hover:bg-slate-800 p-1.5 rounded-full transition-colors"
            title="Esporta / Stampa"
            aria-label="Esporta / Stampa"
          >
          ...
          <div ref={shareMenuRef}>
            <button
              ...
              className={`text-slate-400 hover:text-white hover:bg-slate-800 p-1.5 rounded-full transition-colors ${shareMenuOpen ? 'bg-slate-800 text-white' : ''}`}
              title="Condividi"
            ...
            >
              ...
              <button type="button" onClick={handleShareCollaborative} className={SHARE_ITEM_CLASS}>
              ...
              {isAlreadyPublished ? (
                ...
                <button
                  ...
                  title={
                    !canPublish && !isGuest ? 'Aggiungi tappe e un nome al viaggio' : undefined
                  }
                >
          ...
          <button
            type="button"
            onClick={onClear}
            className="text-slate-400 hover:text-red-400 hover:bg-slate-800 p-1.5 rounded-full transition-colors"
            aria-label="Svuota diario"
          >
            <Trash2 className="w-[16.5px] h-[16.5px]" />
          </button>
        </div>
      </div>
      {isGuest && <p className="text-[10px] text-slate-500 pl-1">{GUEST_SAVE_MESSAGE}</p>}
    </div>
  );
};
