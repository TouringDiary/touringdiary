...
const DiaryHeaderSaveBadge: React.FC<{
  ...
  return (
    ...
    >
      ...
      {showLabel && <span className="tabular-nums whitespace-nowrap">{badgeLabel}</span>}
    </div>
  );
};
...
export const DiaryHeader: React.FC<DiaryHeaderProps> = ({
  ...
  const isDiaryShared = useSharedResourceIndicator(itinerary.id ? 'diary' : null, itinerary.id);
  ...
  const [displayStartDate, setDisplayStartDate] = useState(
    formatDateForDisplay(itinerary.startDate),
  );
  ...
  const endMinDateStr =
    itinerary.startDate && itinerary.startDate > minDateStr ? itinerary.startDate : minDateStr;
  ...
  const guestSaveRelevant =
    ...
    !!itinerary.startDate ||
    !!itinerary.endDate;
  const canPublish = itinerary.items.length > 0 && itinerary.name.trim().length > 0 && !isGuest;
  ...
  const targetProjectName =
    savedProjects.find((p) => p.id === deleteTargetId)?.name || 'questo diario';

  return (
    <div
      className="p-3 border-b border-stone-300 bg-slate-900 shadow-sm relative no-print-bg flex-shrink-0 transition-all"
    ...
    >
      ...
      <DeleteConfirmationModal
        ...
        message={
          getDeleteMsg({ name: targetProjectName }).body ||
          `Stai per cancellare definitivamente "${targetProjectName}". L'azione è irreversibile.`
        }
      ...
      <DeleteConfirmationModal
        ...
        message={
          getClearMsg().body ||
          `Le tappe del diario verranno eliminate.\nLe modifiche non salvate andranno perse.\n\nVuoi procedere?`
        }
        isDeleting={false}
        variant="danger"
