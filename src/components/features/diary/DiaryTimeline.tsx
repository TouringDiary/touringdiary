...
import type { CitySummary, Itinerary, ItineraryItem, PointOfInterest } from '@/types';
...
export const DiaryTimeline: React.FC<DiaryTimelineProps> = (props) => {
  ...
  return (
    <div className="min-h-full flex flex-col">
      ...
      <div
        className="sticky top-0 h-7 w-full px-3 md:px-4 flex items-center justify-between bg-[#e7e5e4] border-b border-stone-300 shadow-sm flex-shrink-0 box-border"
      ...
      >
        ...
        <div ref={containerRef} className="flex-1 min-w-0 mr-2 flex items-center relative h-full">
          ...
          <div
            ref={ghostRef}
            className="absolute invisible opacity-0 pointer-events-none flex whitespace-nowrap font-handwriting text-sm md:text-xl font-bold"
          >
            {uniqueCityIds.map((id, idx) => (
              <span key={id} className="ghost-city-item px-1">
                {cityName(id)}
                {idx < uniqueCityIds.length - 1 ? ',' : ''}
              </span>
            ))}
          </div>
          ...
          <div
            ref={dotsGhostRef}
            className="absolute invisible opacity-0 pointer-events-none font-handwriting text-sm md:text-xl font-bold whitespace-nowrap"
          >
            , + ...
          </div>
            ...
            {uniqueCityIds.length > 0 ? (
              <>
                <div className="flex items-center gap-1 overflow-hidden whitespace-nowrap">
                  {uniqueCityIds.slice(0, visibleCitiesCount).map((id, idx) => (
                    <span key={id} className="flex items-center flex-shrink-0">
                      ...
                      {idx < Math.min(uniqueCityIds.length, visibleCitiesCount) - 1 && (
                ...
                {visibleCitiesCount < uniqueCityIds.length && (
                  ...
                  >
                    <span className="cursor-help text-stone-400 hover:text-amber-600 ml-1">
                      , + ...
                    </span>
                    <AnchoredPopover
                    ...
                    >
                      <div className="flex flex-col gap-1">
                        <p className="text-[10px] uppercase tracking-wider text-slate-500 font-sans mb-2">
                          Tutte le tappe
                        </p>
            ...
            onClick={() => {
              if (isDayTab(props.activeTab)) {
                props.onColorPickerToggle(
                  props.colorPickerOpen === props.activeTab ? null : props.activeTab,
                );
              } else {
...
const DaySelectorPopover: React.FC<DaySelectorPopoverProps> = ({
  isOpen,
  onClose,
  anchorRef,
  title,
