...
import {
  removeRejectionAsync,
  removeRejectionByNameAsync,
...
/** Inizializza un UndoAction con timestamp; il payload resta tipizzato dal generic. */
function createUndoAction<T>(
  init: Omit<UndoAction<T>, 'timestamp'> & { timestamp?: number },
): UndoAction<T> {
  return {
    timestamp: Date.now(),
    ...init,
...
function createRuntimeSuitcaseItem(
  ...
  const itemId =
    metadata.id && isEphemeralItemId(metadata.id) ? metadata.id : `draft-item-${randomUUID()}`;
  return {
    id: itemId,
...
export const useSuitcaseItemActions = ({
  ...
  const applyDocumentMutation = useCallback(
    async (action: UndoAction, persistFn?: () => Promise<void>) => {
  ...
  const persistCategoryDelete = useCallback(
    async (suitcaseId: string, suitcase: Suitcase, target: CategoryDeleteSnapshot['target']) => {
      ...
      await Promise.all(
        removedItems
          .filter((item) => item.id && !isEphemeralItemId(item.id) && !item.id.startsWith('temp-'))
  ...
  const handleUpdateItemConfirmed = useCallback(
    async (itemId: string, updates: UpdateSuitcaseItemDto, currentItem: SuitcaseItem) => {
      try {
        if (updates.is_checked !== undefined) {
          ...
          if (isDuplicate) {
            showToast(
              `"${currentItem.name}" è già presente in ${newCategory}`,
              undefined,
              'destructive',
            );
            return;
    ...
    [
      ...
      checkDuplicateItem,
      getActiveSuitcase,
      persistItemDisplayOrder,
      onDocumentDirty,
      onSuitcaseLocalUpdate,
    ],
  );

  const handleDeleteItemConfirmed = useCallback(
    async (itemToDelete: SuitcaseItem) => {
      try {
        const id = itemToDelete.id;
        const suitcase = getActiveSuitcase();

        const action = createUndoAction<SuitcaseItem>({
          id,
          type: 'delete',
        ...
        if (onDocumentDirty) {
          ...
          if (itemToDelete.is_ai_suggestion && activeTabId) {
            ...
            showToast(
              'Oggetto spostato nei rifiutati',
              "L'oggetto non verrà più suggerito per questa valigia.",
              'success',
            );
          }
          return;
  ...
  const handleAddItemConfirmed = useCallback(
    async (
      suitcaseId: string,
      name: string,
      category: string,
  ...
  const handleRestoreFromBlacklist = useCallback(
    async (rejection: SuitcaseRejection) => {
    ...
    async (rejectionId: string, name: string) => {
      try {
        ...
        showToast(
          ...
          "L'oggetto potrà essere nuovamente proposto dai suggerimenti AI.",
          'success',
        );
      } catch (err) {
  ...
  const handleDeleteCategoryConfirmed = useCallback(
    async (target: CategoryDeleteModalTarget) => {
      ...
      try {
        ...
        const action = createUndoAction<CategoryDeleteSnapshot>({
          ...
          inverse: async (payload) => {
            ...
            await fetchUserSuitcases();
            showToast(`Categoria ripristinata: ${payload.target.name}`, undefined, 'success');
          },
          apply: async (payload) => {
            ...
            await fetchUserSuitcases();
            showToast(`Categoria eliminata: ${payload.target.name}`, undefined, 'destructive');
          },
        });
