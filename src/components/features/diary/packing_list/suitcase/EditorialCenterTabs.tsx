...
export const getRelevantProducts = (item: SuitcaseItem, products: SuggestionProduct[]) => {
  ...
  return products.filter((p) => {
    ...
    const nameMatch =
      productName.includes(normalizedItemName) || normalizedItemName.includes(productName);

    // 2. Match Categoria Diretta o Affine
    const categoryMatch =
      productCategories.includes(itemCategory) ||
      productCategories.some((c) => itemAffinities.includes(c));

    // 3. Match Tags
    const tagMatch =
      itemTags.some((it) => productTags.includes(it)) ||
