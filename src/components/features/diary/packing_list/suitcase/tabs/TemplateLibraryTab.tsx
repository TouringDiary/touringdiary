import { Copy, Edit3, Layout, Loader2, Plus, Trash2 } from 'lucide-react';
        ...
        <div className="flex-1 space-y-1">
          {masters.map((m) => (
            <button
              ...
              className={`w-full px-3 py-3 rounded-xl flex items-center gap-3 transition-all ${selectedMasterId === m.id ? 'bg-slate-800 text-white shadow-lg border border-white/5' : 'text-slate-400 hover:bg-slate-800/40 hover:text-slate-200'}`}
            >
      ...
      <main className="flex-1 lg:overflow-y-auto bg-slate-950 p-8">
        {currentMaster ? (
              ...
              <div className="relative z-floating-panel space-y-6">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-6">
                    <div className="text-5xl bg-slate-950 w-20 h-20 rounded-2xl flex items-center justify-center border border-white/5 shadow-inner">
                      {currentMaster.icon}
                    </div>
                    <div>
                      <div className="flex items-center gap-3 mb-1">
                        ...
                        <button
                          type="button"
                          onClick={async () => {
                            const t = prompt('Nuovo Titolo:', currentMaster.title);
                            if (t) {
                              await updateMasterTemplateTitleAsync(currentMaster.id, t);
                              fetchData();
                            }
                          }}
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 text-white text-xs font-bold rounded-xl border border-white/5 transition-all"
                    >
                ...
                <p className="text-sm text-slate-400">
                  Gli oggetti del template sono gestiti in{' '}
                  <strong className="text-indigo-400">Standard Items</strong> (core condiviso) e{' '}
                  <strong className="text-indigo-400">Template Specifici</strong> (tab dedicata).
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="h-full flex flex-col items-center justify-center text-slate-700 gap-4">
            ...
            <p className="text-sm font-medium">Seleziona un template.</p>
          </div>
        )}
      </main>
    </div>
