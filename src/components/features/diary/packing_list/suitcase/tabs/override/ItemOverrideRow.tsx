...
import type { ItemOverride, SuggestionProduct, SuitcaseItem } from '@/types/suitcase';
...
export const ItemOverrideRow: React.FC<ItemOverrideRowProps> = ({
  ...
  return (
    <div
      className={`p-5 rounded-2xl border transition-all ${override?.is_saved ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-slate-900 border-slate-700/50 hover:border-slate-600 shadow-xl'}`}
    >
      <div className="flex items-center gap-6">
        <div className="w-1/3">
          ...
          <div className="flex items-center gap-1.5 mt-1">
            <div
              className={`w-1.5 h-1.5 rounded-full ${override?.is_saved ? 'bg-emerald-500' : 'bg-slate-600'}`}
            />
        ...
        <div className="flex-1 flex items-center gap-3">
          ...
          {currentProduct && (
            <div className="w-12 h-12 rounded-lg bg-white overflow-hidden border border-white/10 shrink-0 shadow-sm p-1">
              <img
                src={currentProduct.image_url ?? undefined}
                alt=""
