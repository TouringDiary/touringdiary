import { Globe, Layout, ListTree, Package, ShoppingBag, Sliders, Sparkles } from 'lucide-react';
...
import {
  ...
  StandardItemsTab,
  TemplateLibraryTab,
  TemplateSpecificItemsTab,
} from './EditorialCenterTabs';

type AffiliateEditorialCenterProps = {};
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300 pb-20 relative flex flex-col h-full">
      ...
      <div className="flex-1 overflow-hidden relative">
        {activeTab === 'overrides' && (
          <OverrideTab selectedMasterId={selectedMasterId} onSelectMaster={setSelectedMasterId} />
        )}
        {activeTab === 'global' && <GlobalSuggestionsTab />}
        {activeTab === 'library' && (
          <TemplateLibraryTab
      ...
      <footer className="mt-6 py-4 border-t border-slate-900 flex items-center justify-between pointer-events-none text-slate-500 shrink-0">
        <div className="flex items-center gap-4 text-[9px] uppercase tracking-widest font-black">
          ...
          <code className="text-slate-500 bg-white/5 px-2 py-0.5 rounded border border-white/5">
            {activeTab === 'overrides'
              ? 'Override Mode (Template specific)'
              : activeTab === 'global'
                ? 'Global Mode (Partner-driven)'
                : activeTab === 'library'
                  ? 'Library Mode (Content Masters)'
                  : activeTab === 'standard'
                    ? 'Standard Items (DB)'
                    : activeTab === 'template_items'
                      ? 'Template Specifics (DB)'
                      : 'AI Catalog (DB)'}
          </code>
        </div>
