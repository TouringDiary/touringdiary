    ...
    <div className="p-4 rounded-2xl bg-amber-500/5 border border-amber-500/10 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h5 className="text-[10px] font-black uppercase text-amber-600 tracking-wider">
            Suggerimenti AI
          </h5>
        </div>
        <button
          type="button"
          onClick={onClear}
          className="text-[10px] text-amber-700 hover:text-amber-500 font-bold"
        >
          Nascondi
        </button>
      </div>
          ...
          suggestions.map((item, idx) => (
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="text-xs font-bold text-slate-300 truncate group-hover:text-amber-200">
                  {item.name}
                </div>
                <div className="text-[9px] text-slate-600 uppercase tracking-tight">
                  {item.category}
                </div>
              </div>
