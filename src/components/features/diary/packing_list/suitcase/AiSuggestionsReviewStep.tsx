...
const REVIEW_LOADING_CLASS = 'text-xs text-slate-400 font-bold uppercase tracking-widest';
const REVIEW_SHOW_MORE_CLASS = 'text-[10px] font-black uppercase tracking-widest';
...
export const AiSuggestionsReviewStep: React.FC<AiSuggestionsReviewStepProps> = ({
  ...
  const groupedData = useMemo(() => {
    ...
    const categorySource = quotaFeedback?.selectedCategories.length
    ...
    <div className="space-y-6">
      ...
      {isGenerating && suggestions.length === 0 ? (
        ...
        <div className="py-20 text-center space-y-3">
          ...
          <p className={`${helperStyle || 'text-sm text-slate-300 font-bold'}`}>
            Nessun suggerimento richiesto.
          </p>
        ...
        <div className="space-y-8">
          {hasAnyActiveContent ? (
            <>
              {groupedData.activeCategories.map((category) => {
                ...
                return (
                  <div
                    key={category}
                    className="bg-white/[0.03] border border-white/5 rounded-[2rem] p-5 md:p-6 space-y-5"
                  >
                    ...
                    {quotaNote && (
                      <p
                        className={`${helperStyle || 'text-[11px] text-slate-500 font-medium'} px-1`}
                      >
                        {quotaNote}
                      </p>
                    )}
                    ...
                    <div className="space-y-2">
                      {items.map((s, idx) => {
                        const key =
                          suggestionKey?.(s.name, s.category) ?? `${s.category}::${s.name}`;
                        return (
            ...
            <div className="py-20 text-center space-y-3">
              ...
              <p className={`${helperStyle || 'text-sm text-slate-400'}`}>
                Nessun nuovo suggerimento trovato per queste categorie.
              </p>
            </div>
          ) : null}
          ...
          {groupedData.exhaustedCategories.length > 0 && (
            <div className="space-y-4 pt-2">
              <div className="space-y-1 px-1">
                ...
                <p className={`${helperStyle || 'text-[11px] text-slate-500 font-medium'}`}>
                  Queste categorie erano selezionate, ma il catalogo non ha altri candidati
                  disponibili.
                </p>
              </div>
                ...
                {groupedData.exhaustedCategories.map((category) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 space-y-1">
                      <h5 className={REVIEW_EXHAUSTED_GROUP_LABEL_CLASS}>{category}</h5>
                      <p className={`${helperStyle || 'text-[11px] text-slate-500 font-medium'}`}>
