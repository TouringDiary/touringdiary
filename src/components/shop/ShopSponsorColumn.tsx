...
import { UniversalCard } from '../city/ShowcaseCards'; 

interface ShopSponsorColumnProps {
    side: 'left' | 'right';
...
export const ShopSponsorColumn: React.FC<ShopSponsorColumnProps> = ({ 
    side, offsetMultiplier, goldSponsors, silverSponsors, onAddToItinerary, onOpenSponsor 
}) => {
    ...
    const renderSlot = (sponsor: PointOfInterest | undefined, tier: 'gold' | 'silver', slotKey: string) => (
        <div key={slotKey} className="flex-1 w-full min-h-0 relative">
            {sponsor ? (
                <UniversalCard 
                    poi={sponsor} 
                    onOpenDetail={() => {}} // Nello shop sponsor column il click apre dettaglio o nulla? Di solito dettaglio.
