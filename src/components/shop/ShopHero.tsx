  ...
  return (
    <div className="flex flex-col lg:flex-row w-full border-b border-slate-800 bg-[#020617]">
      ...
      <div className="w-full lg:w-[75%] h-64 md:h-72 lg:h-[40vh] relative group overflow-hidden border-b lg:border-b-0 lg:border-r border-slate-800">
        <DraggableSlider
          ref={galleryRef}
          className="h-full w-full [&::-webkit-scrollbar]:hidden scrollbar-hide"
        >
          {galleryImages.map((img, idx) => (
            <button
              key={idx}
              type="button"
              aria-label={`Apri foto ${shop.name} ${idx + 1}`}
              onClick={() => onOpenLightbox(img)}
              className="snap-center shrink-0 w-full h-full relative cursor-zoom-in border-0 bg-transparent p-0"
            >
              <ImageWithFallback
                src={img}
                alt={`${shop.name} ${idx}`}
        ...
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-dropdown">
          {galleryImages.map((_, dotIdx) => (
            <div
              key={dotIdx}
              className={`h-1.5 rounded-full transition-all duration-500 ${dotIdx === activePhotoIndex ? 'w-8 bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.6)]' : 'w-4 bg-slate-500/30 border border-white/5'}`}
            />
          ))}
        </div>
        ...
        <div className="relative z-floating-panel flex flex-col h-full justify-between">
          <div>
            ...
            <p className="text-slate-300 text-sm md:text-base leading-relaxed italic font-serif opacity-90 line-clamp-4 lg:line-clamp-none">
              "
              {shop.shortBio ||
                "La nostra attività nasce dalla volontà di preservare le tradizioni campane e offrire prodotti d'eccellenza."}
              "
            </p>
          </div>
          ...
          >
            <BookOpen className="w-4 h-4 group-hover:scale-110 transition-transform" /> LEGGI LA
            STORIA
          </button>
        </div>
      </div>
