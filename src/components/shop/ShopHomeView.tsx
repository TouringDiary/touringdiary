import { ArrowRight, Award, type LucideIcon, MapPin, Sparkles } from 'lucide-react';
import type React from 'react';
import { useEffect, useState } from 'react';
import { useSystemMessage } from '../../hooks/useSystemMessage';
import type { ShopCategory, ShopPartner } from '../../types/index';
import { openMap } from '../../utils/common';
import { ImageWithFallback } from '../common/ImageWithFallback';

interface ShopHomeViewProps {
  premiumShops: ShopPartner[];
  categories: { id: ShopCategory; label: string; icon: LucideIcon; color: string; desc: string }[];
...
export const ShopHomeView: React.FC<ShopHomeViewProps> = ({
  ...
  // Se la lista premium si riduce, riallinea l'indice (evita slide vuota).
  useEffect(() => {
    setActiveIndex((prev) => {
      if (premiumShops.length === 0) return 0;
      return prev >= premiumShops.length ? 0 : prev;
    });
  }, [premiumShops.length]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(
      () => setActiveIndex((prev) => (prev + 1) % premiumShops.length),
      5000,
    );
      ...
      <div className="h-64 md:h-[55%] flex flex-col min-h-0 min-w-0">
        ...
        <div className="flex-1 relative rounded-[3rem] overflow-hidden border border-slate-800 shadow-2xl ring-1 ring-white/5 bg-[#020617]">
          {premiumShops.length > 0 ? (
                ...
                <div className="absolute bottom-4 left-4 right-4 md:bottom-10 md:left-10 md:right-10 flex justify-between items-end">
                  <div className="max-w-2xl min-w-0">
                    <div className="flex items-center gap-3 mb-2 md:mb-4">
                      <span className="bg-amber-500 text-black text-[8px] md:text-[10px] font-black px-2 md:px-4 py-1 md:py-1.5 rounded-full shadow-lg uppercase tracking-tighter border border-amber-400">
                        SPONSOR
                      </span>
                      <span className="bg-white/10 backdrop-blur-xl text-white text-[7px] md:text-[9px] font-bold px-2 md:px-3 py-1 md:py-1.5 rounded-full border border-white/20 uppercase">
                        {shop.category}
                      </span>
                    </div>
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 md:gap-4 bg-indigo-600 hover:bg-indigo-500 text-white font-black uppercase text-[9px] md:text-xs tracking-[0.15em] md:tracking-[0.2em] min-h-11 px-4 py-3 md:min-h-0 md:px-10 md:py-6 rounded-2xl md:rounded-[2rem] transition-all shadow-2xl transform active:scale-95 shrink-0 touch-manipulation focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-300"
                  >
                    Entra in Bottega <ArrowRight className="w-4 h-4 md:w-8 md:h-8" />
                  </button>
                </div>
              </div>
            ))
            ...
            <button
              type="button"
              className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center w-full h-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/50"
              onClick={onOpenShopSponsor}
              aria-label={`${goldPromo.title}. ${goldPromo.body}`}
            >
              <div className="w-full h-full border-2 border-dashed border-slate-800 rounded-[2.5rem] flex flex-col items-center justify-center bg-slate-900/20 hover:bg-slate-900/40 transition-colors group cursor-pointer">
                <div className="p-5 bg-amber-500/10 rounded-full mb-6 group-hover:scale-110 transition-transform border border-amber-500/20">
                  <Award className="w-16 h-16 text-amber-500" aria-hidden />
                </div>
                ...
                <span className="px-8 py-3 bg-amber-600 hover:bg-amber-500 text-white rounded-2xl text-xs font-black uppercase tracking-[0.2em] shadow-lg transition-all transform active:scale-95 flex items-center gap-2">
                  {goldPromo.body} <ArrowRight className="w-4 h-4" aria-hidden />
                </span>
              </div>
            </button>
          )}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 md:gap-6 h-full pb-2">
          {categories.map((cat, idx) => (
            <button
              ...
              className="group relative bg-slate-900 border border-slate-800 rounded-[2rem] flex items-stretch transition-all hover:border-indigo-500/50 hover:bg-slate-800 hover:-translate-y-1 shadow-lg h-full overflow-hidden text-left"
            >
              <div
                className={`w-10 md:w-24 bg-[#020617] border-r border-slate-800 flex items-center justify-center p-0 shrink-0 ${cat.color}`}
              >
                <cat.icon
                  className="w-full h-full p-3 md:p-7 transition-transform group-hover:scale-110"
                  strokeWidth={1.5}
                />
              </div>
              <div className="flex flex-col justify-center px-2 md:px-8 items-start min-w-0">
                <h5
                  className={`text-xs ${isSidebarOpen ? 'md:text-xl' : 'md:text-4xl'} font-bold text-white font-display leading-none truncate w-full`}
                >
                  {cat.label}
                </h5>
                <p className="text-[7px] md:text-[11px] text-slate-300 uppercase font-black tracking-widest leading-tight mt-1 md:mt-3 opacity-90 line-clamp-1">
                  {cat.desc}
                </p>
              </div>
            </button>
