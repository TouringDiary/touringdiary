import { Hammer, Loader2, type LucideIcon, Map, Scissors, Utensils, Wine } from 'lucide-react';
...
import type { PointOfInterest, ShopCategory } from '../../types/index';
import { TravelDiary } from '../features/diary/TravelDiary';
import { AddToItineraryModal } from '../modals/AddToItineraryModal';
import { ShopHeader } from './ShopHeader';

// --- LAZY IMPORTS ---
const ShopDetailView = React.lazy(() =>
  import('./ShopDetailView').then((m) => ({ default: m.ShopDetailView })),
);
const ShopHomeView = React.lazy(() =>
  import('./ShopHomeView').then((m) => ({ default: m.ShopHomeView })),
);
const ShopCategoryView = React.lazy(() =>
  import('./ShopCategoryView').then((m) => ({ default: m.ShopCategoryView })),
);
...
export const ShopPage = ({
  ...
  // RECUPERO LOGICA DIARIO (add + undo centralizzati)
  const { handleSmartDrop, confirmAddToItinerary } = useDiaryInteractionsContext();
  ...
  const confirmShopAddToItinerary = (dayIndex: number, timeSlotStr: string) => {
    if (!pendingPoi) return;
    // cityId pagina solo se il POI non ha già il proprio (Around Me / sponsor).
    const poi = {
      ...pendingPoi,
      cityId: pendingPoi.cityId || cityId,
    };
    const added = confirmAddToItinerary(poi, dayIndex, timeSlotStr);
    if (added) {
      handleConfirmAdd();
    } else {
      // Duplicate/conflict: chiude solo il modal Shop; il modal globale gestisce il seguito.
      setPendingPoi(null);
    }
  };
    ...
    <div className="relative w-full h-full flex flex-col bg-[#020617] animate-in fade-in duration-500 overflow-hidden">
      {lightboxImage && (
        ...
        >
          ...
          <img
            src={lightboxImage}
            alt="Fullscreen"
            style={{ zIndex: Z_LIGHTBOX_CONTENT }}
            className="relative max-w-[95vw] max-h-[90vh] rounded shadow-2xl object-contain pointer-events-none"
          />
          <CloseButton
            onClose={() => setLightboxImage(null)}
            variant="primary"
            position="absolute"
            style={{ zIndex: Z_LIGHTBOX_CLOSE }}
      ...
      {pendingPoi && (
        <AddToItineraryModal
          ...
          onConfirm={confirmShopAddToItinerary}
          onRemove={(itemId) => {
            removeItem(itemId);
            setPendingPoi(null);
          }}
      ...
      <div
        ...
        className={`absolute top-0 bottom-0 right-0 w-full md:w-[400px] bg-slate-950 border-l border-slate-800 shadow-2xl transform transition-transform duration-500 ease-in-out ${showPlanner ? 'translate-x-0' : 'translate-x-full'} flex flex-col`}
      >
        <div className="flex justify-between items-center p-4 border-b border-slate-800 bg-[#0f172a]">
          <div className="flex items-center gap-2">
            <Map className="w-5 h-5 text-amber-500" />
            <h3 className="font-bold text-white text-lg">Il Tuo Viaggio</h3>
          </div>
      ...
      {showPlanner && (
        <button
          type="button"
          tabIndex={-1}
          aria-label="Chiudi planner"
          style={{ zIndex: Z_OVERLAY_BACKDROP }}
          className="absolute inset-0 h-full w-full cursor-default border-0 bg-black/50 p-0 md:hidden"
      ...
      <div className="relative w-full h-full max-w-[1920px] mx-auto flex flex-col overflow-hidden bg-[#020617]">
        ...
        >
          <Suspense fallback={<ShopLoader />}>{renderMainContent()}</Suspense>
        </div>
      </div>
    </div>
  );
