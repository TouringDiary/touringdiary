...
import type { ShopProduct } from '../../types/index';
        ...
        <div className="w-full md:w-1/2 p-6 md:p-10 flex flex-col justify-center overflow-y-auto custom-scrollbar bg-slate-900">
          <div className="space-y-6">
            <div>
              ...
              <h3 className="text-2xl md:text-4xl font-display font-bold text-white leading-tight tracking-tight">
                {product.name}
              </h3>
            </div>
            ...
            <div className="flex items-center gap-4 py-4 border-y border-slate-800">
              ...
              <div
                className={`px-3 py-1 rounded-full border text-[10px] font-black uppercase tracking-widest flex items-center gap-2 ${product.shippingMode === 'pickup' ? 'bg-amber-900/20 border-amber-500/30 text-amber-500' : 'bg-indigo-900/20 border-indigo-500/30 text-indigo-400'}`}
              >
                {product.shippingMode === 'pickup' ? (
            ...
            <div className="pt-4">
              <button
                type="button"
                className="w-full bg-indigo-600 hover:bg-indigo-500 text-white py-4 rounded-xl font-black uppercase tracking-[0.2em] text-xs shadow-xl transition-all transform active:scale-95 flex items-center justify-center gap-3"
              >
                <MessageCircle className="w-5 h-5" /> Contatta Bottega
              </button>
              <p className="text-center text-[10px] text-slate-500 mt-3">
                Non vincolante. Apre una chat diretta col venditore.
              </p>
            </div>
          </div>
        </div>
