...
import type { ShopPartner, ShopProduct } from '../../types/index';
    ...
    <div className="flex flex-col h-full p-6 md:p-8">
      <div className="flex items-center justify-between mb-6 shrink-0">
        ...
        <div className="flex gap-2">
          <button
            type="button"
            onClick={() => productsRef.current?.scroll('left')}
            aria-label="Scorri prodotti a sinistra"
            className="p-2 bg-slate-900 border border-slate-700 hover:border-slate-500 rounded-xl text-slate-400 hover:text-white transition-all"
          >
            <ChevronLeft className="w-4 h-4" />
          </button>
          <button
            type="button"
            onClick={() => productsRef.current?.scroll('right')}
          ...
          <DraggableSlider ref={productsRef} className="h-full gap-5 pb-4 pt-4">
            {shop.products.map((product, idx) => (
              <button
                ...
                className="snap-start shrink-0 w-52 md:w-64 h-64 md:h-80 flex flex-col group cursor-pointer relative transition-all hover:-translate-y-2 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/50"
              >
                <div className="flex-1 rounded-3xl overflow-hidden border border-slate-800 relative bg-slate-900 shadow-xl group-hover:shadow-2xl group-hover:border-indigo-500/50 transition-all flex flex-col">
                  <div className="h-[60%] overflow-hidden relative">
                    ...
                    <div
                      aria-hidden
                      className="absolute top-3 right-3 bg-white text-black p-2 rounded-full shadow-lg transform scale-0 group-hover:scale-100 transition-transform duration-300"
                    >
                      <Plus className="w-4 h-4 font-bold" aria-hidden />
                    </div>
                  </div>
                  ...
                  <div className="flex-1 p-5 flex flex-col justify-between bg-slate-900 relative z-floating-panel">
                    <div>
                      ...
                      <h5 className="text-xl font-bold text-white font-display leading-tight mb-2 group-hover:text-amber-400 transition-colors line-clamp-2">
                        {product.name}
                      </h5>
