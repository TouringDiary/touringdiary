...
export const BottegaSponsorCard: React.FC<BottegaSponsorCardProps> = ({
  poi,
  onAddToItinerary,
}) => {
  ...
  return (
    <div
      className={`group relative w-full h-28 rounded-xl border overflow-hidden transition-all bg-slate-900 shadow-md ${isGold ? 'border-amber-500/60 hover:border-amber-400' : 'border-slate-200 hover:border-white shadow-white/5'}`}
    >
      <ImageWithFallback
        src={poi.imageUrl}
        alt={poi.name}
      ...
      <div className="absolute bottom-2 left-2 right-2 flex justify-between items-end pointer-events-none">
        <div className="min-w-0 flex-1 pr-10">
          <h5 className="text-white font-bold text-[9px] leading-tight truncate drop-shadow-md mb-0.5">
            {poi.name}
          </h5>
