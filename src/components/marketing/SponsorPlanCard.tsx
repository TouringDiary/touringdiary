...
const SponsorPlanCard: React.FC<SponsorPlanCardProps> = ({
  planKey,
  planType,
  config,
  isRecommended,
  ...
  const borderColorClass = planType
    ? tierBorderColors[planType] || 'bg-slate-600'
    : 'bg-transparent';
        ...
        <div className="text-center mb-6">
          {hasPromo ? (
            <div className="flex flex-col items-center">
              ...
              {promoLabel && (
                <span className="text-green-400 text-sm font-semibold mt-1">{promoLabel}</span>
              )}
            </div>
          ) : (
