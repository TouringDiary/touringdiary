...
export const getServicesConfig = (): ServiceCategoryDef[] => {
  const config = getCachedSetting<ServiceConfigRow[]>(SETTINGS_KEYS.SERVICES_CONFIG);
...
export const getServiceTypeMapping = () => {
  return (
    getCachedSetting<Record<string, { label: string; types: { val: string; label: string }[] }>>(
      SETTINGS_KEYS.SERVICES_TYPE_MAPPING,
    ) || {}
  );
};
...
export const getBoxIdForType = (type: string): string => {
  ...
  if (t.includes('banca') || t.includes('bank') || t.includes('atm') || t.includes('bancomat'))
    return 'bank';
