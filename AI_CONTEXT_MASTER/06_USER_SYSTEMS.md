...
*   **Visione casa:** `AI_CONTEXT/35_MYSPACE_PRODUCT_VISION.md` (v2.4.3+) — catalogo = solo Viaggi; Valigia = Diari·Valigie·Template (apertura diretta); create modale; Salva con nome esteso; copia su conflitto associazione; Preferiti=vista (filtro geo + 6 box); Esploratore=archivio. Implementazione decisioni 1–8: **WF-13 Completato**.
*   **Packing:** `AI_CONTEXT/31_PACKING_SUITCASE_SYSTEM.md` (v2.1.1+) — Valigia-viaggio vs root Valigia; una Valigia ↔ un Viaggio; Valigia⇄Diario invariata; copia su conflitto; UNIQUE `suitcase_id` (migrazione WF-13).
